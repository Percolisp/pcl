(in-package :pcl)
;; Generated from cl/pack-impl.pl via ./pl2cl, then hand-patched:
;;   - Float subs (pl-_pack_float32/64, pl-_unpack_float32/64) replaced with
;;     SBCL sb-kernel implementations (pack-impl.pl has stubs that return ""/0.0)
;; To regenerate: ./pl2cl cl/pack-impl.pl | sed '2,15d' > cl/pcl-pack.lisp
;;   then re-apply the float implementations from lines 722-822.

(p-declare-sub pl-p_unpack)
(p-declare-sub pl-_unpack_tmpl)
(p-declare-sub pl-_unpack_str)
(p-declare-sub pl-_unpack_utf8_char)
(p-declare-sub pl-p_pack)
(p-declare-sub pl-_pack_tmpl)
(p-declare-sub pl-_pack_utf8_char)
(p-declare-sub pl-_pack_str_one)
(p-declare-sub pl-_unpack_float64)
(p-declare-sub pl-_unpack_float32)
(p-declare-sub pl-_pack_float64)
(p-declare-sub pl-_pack_float32)
(p-declare-sub pl-_unpack_read_int)
(p-declare-sub pl-_pack_emit_int)
(p-declare-sub pl-_pack_parse_count)
(p-declare-sub pl-_pack_parse_mods)
(p-declare-sub pl-_pack_find_group_end)
(p-declare-sub pl-_pack_skip_ws)
(p-declare-sub pl-_pack_type_info)
;; my $CAN_ENDIAN = 'sSiIlLqQjJfFdDpP'
(p-eval-always
 (defvar $CAN_ENDIAN (make-p-box nil)))
;; my $CAN_SHRIEK = 'sSiIlLnNvV'
(p-eval-always
 (defvar $CAN_SHRIEK (make-p-box nil)))
;; sub _pack_type_info { ... }
(p-sub pl-_pack_type_info (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($ch (make-p-box nil)) ($bang (make-p-box nil)))
             ;; my ($ch, $bang) = @_
             (p-list-= (vector $ch $bang) @_)
             
             ;; if    ($ch eq 'c') { return (1, 1, 0) }     elsif ($ch eq 'C') { return (1, 0, 0) }     elsif ($ch eq 's') { return (2, 1, 0) }     elsif ($ch eq 'S') { return (2, 0, 0) }     elsif ($ch eq 'n') { return (2, ($bang ? 1 : 0), 1) }     elsif ($ch eq 'v') { return (2, ($bang ? 1 : 0), 0) }     elsif ($ch eq 'i') { return (4, 1, 0) }     elsif ($ch eq 'I') { return (4, 0, 0) }     elsif ($ch eq 'l') { return (($bang ? 8 : 4), 1, 0) }     elsif ($ch eq 'L') { return (($bang ? 8 : 4), 0, 0) }     elsif ($ch eq 'N') { return (4, ($bang ? 1 : 0), 1) }     elsif ($ch eq 'V') { return (4, ($bang ? 1 : 0), 0) }     elsif ($ch eq 'q') { return (8, 1, 0) }     elsif ($ch eq 'Q') { return (8, 0, 0) }     elsif ($ch eq 'j') { return (8, 1, 0) }     elsif ($ch eq 'J') { return (8, 0, 0) }
             ;; if ($ch eq 'c')
             (p-if         (p-str-eq $ch "c")
                           (progn
                             ;; return (1, 1, 0)
                             (p-return 1 1 0)
                             
                             )
                           ;; elsif ($ch eq 'C')
                           (p-if           (p-str-eq $ch "C")
                                           (progn
                                             ;; return (1, 0, 0)
                                             (p-return 1 0 0)
                                             
                                             )
                                           ;; elsif ($ch eq 's')
                                           (p-if             (p-str-eq $ch "s")
                                                             (progn
                                                               ;; return (2, 1, 0)
                                                               (p-return 2 1 0)
                                                               
                                                               )
                                                             ;; elsif ($ch eq 'S')
                                                             (p-if               (p-str-eq $ch "S")
                                                                                 (progn
                                                                                   ;; return (2, 0, 0)
                                                                                   (p-return 2 0 0)
                                                                                   
                                                                                   )
                                                                                 ;; elsif ($ch eq 'n')
                                                                                 (p-if                 (p-str-eq $ch "n")
                                                                                                       (progn
                                                                                                         ;; return (2, ($bang ? 1 : 0), 1)
                                                                                                         (p-return 2 (p-if $bang 1 0) 1)
                                                                                                         
                                                                                                         )
                                                                                                       ;; elsif ($ch eq 'v')
                                                                                                       (p-if                   (p-str-eq $ch "v")
                                                                                                                               (progn
                                                                                                                                 ;; return (2, ($bang ? 1 : 0), 0)
                                                                                                                                 (p-return 2 (p-if $bang 1 0) 0)
                                                                                                                                 
                                                                                                                                 )
                                                                                                                               ;; elsif ($ch eq 'i')
                                                                                                                               (p-if                     (p-str-eq $ch "i")
                                                                                                                                                         (progn
                                                                                                                                                           ;; return (4, 1, 0)
                                                                                                                                                           (p-return 4 1 0)
                                                                                                                                                           
                                                                                                                                                           )
                                                                                                                                                         ;; elsif ($ch eq 'I')
                                                                                                                                                         (p-if                       (p-str-eq $ch "I")
                                                                                                                                                                                     (progn
                                                                                                                                                                                       ;; return (4, 0, 0)
                                                                                                                                                                                       (p-return 4 0 0)
                                                                                                                                                                                       
                                                                                                                                                                                       )
                                                                                                                                                                                     ;; elsif ($ch eq 'l')
                                                                                                                                                                                     (p-if                         (p-str-eq $ch "l")
                                                                                                                                                                                                                   (progn
                                                                                                                                                                                                                     ;; return (($bang ? 8 : 4), 1, 0)
                                                                                                                                                                                                                     (p-return (p-if $bang 8 4) 1 0)
                                                                                                                                                                                                                     
                                                                                                                                                                                                                     )
                                                                                                                                                                                                                   ;; elsif ($ch eq 'L')
                                                                                                                                                                                                                   (p-if                           (p-str-eq $ch "L")
                                                                                                                                                                                                                                                   (progn
                                                                                                                                                                                                                                                     ;; return (($bang ? 8 : 4), 0, 0)
                                                                                                                                                                                                                                                     (p-return (p-if $bang 8 4) 0 0)
                                                                                                                                                                                                                                                     
                                                                                                                                                                                                                                                     )
                                                                                                                                                                                                                                                   ;; elsif ($ch eq 'N')
                                                                                                                                                                                                                                                   (p-if                             (p-str-eq $ch "N")
                                                                                                                                                                                                                                                                                     (progn
                                                                                                                                                                                                                                                                                       ;; return (4, ($bang ? 1 : 0), 1)
                                                                                                                                                                                                                                                                                       (p-return 4 (p-if $bang 1 0) 1)
                                                                                                                                                                                                                                                                                       
                                                                                                                                                                                                                                                                                       )
                                                                                                                                                                                                                                                                                     ;; elsif ($ch eq 'V')
                                                                                                                                                                                                                                                                                     (p-if                               (p-str-eq $ch "V")
                                                                                                                                                                                                                                                                                                                         (progn
                                                                                                                                                                                                                                                                                                                           ;; return (4, ($bang ? 1 : 0), 0)
                                                                                                                                                                                                                                                                                                                           (p-return 4 (p-if $bang 1 0) 0)
                                                                                                                                                                                                                                                                                                                           
                                                                                                                                                                                                                                                                                                                           )
                                                                                                                                                                                                                                                                                                                         ;; elsif ($ch eq 'q')
                                                                                                                                                                                                                                                                                                                         (p-if                                 (p-str-eq $ch "q")
                                                                                                                                                                                                                                                                                                                                                               (progn
                                                                                                                                                                                                                                                                                                                                                                 ;; return (8, 1, 0)
                                                                                                                                                                                                                                                                                                                                                                 (p-return 8 1 0)
                                                                                                                                                                                                                                                                                                                                                                 
                                                                                                                                                                                                                                                                                                                                                                 )
                                                                                                                                                                                                                                                                                                                                                               ;; elsif ($ch eq 'Q')
                                                                                                                                                                                                                                                                                                                                                               (p-if                                   (p-str-eq $ch "Q")
                                                                                                                                                                                                                                                                                                                                                                                                       (progn
                                                                                                                                                                                                                                                                                                                                                                                                         ;; return (8, 0, 0)
                                                                                                                                                                                                                                                                                                                                                                                                         (p-return 8 0 0)
                                                                                                                                                                                                                                                                                                                                                                                                         
                                                                                                                                                                                                                                                                                                                                                                                                         )
                                                                                                                                                                                                                                                                                                                                                                                                       ;; elsif ($ch eq 'j')
                                                                                                                                                                                                                                                                                                                                                                                                       (p-if                                     (p-str-eq $ch "j")
                                                                                                                                                                                                                                                                                                                                                                                                                                                 (progn
                                                                                                                                                                                                                                                                                                                                                                                                                                                   ;; return (8, 1, 0)
                                                                                                                                                                                                                                                                                                                                                                                                                                                   (p-return 8 1 0)
                                                                                                                                                                                                                                                                                                                                                                                                                                                   
                                                                                                                                                                                                                                                                                                                                                                                                                                                   )
                                                                                                                                                                                                                                                                                                                                                                                                                                                 ;; elsif ($ch eq 'J')
                                                                                                                                                                                                                                                                                                                                                                                                                                                 (p-if                                       (p-str-eq $ch "J")
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             (progn
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               ;; return (8, 0, 0)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               (p-return 8 0 0)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               )
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             nil
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             )
                                                                                                                                                                                                                                                                                                                                                                                                                                                 )
                                                                                                                                                                                                                                                                                                                                                                                                       )
                                                                                                                                                                                                                                                                                                                                                               )
                                                                                                                                                                                                                                                                                                                         )
                                                                                                                                                                                                                                                                                     )
                                                                                                                                                                                                                                                   )
                                                                                                                                                                                                                   )
                                                                                                                                                                                     )
                                                                                                                                                         )
                                                                                                                               )
                                                                                                       )
                                                                                 )
                                                             )
                                           )
                           )
             
             ;; return ()
             (p-return)
             
             )
           )
         )
       )

;; sub _pack_skip_ws { ... }
(p-sub pl-_pack_skip_ws (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($s (make-p-box nil)) ($ti (make-p-box nil)))
             ;; my ($s, $ti) = @_
             (p-list-= (vector $s $ti) @_)
             
             (let (($tlen (make-p-box nil)))
               ;; my $tlen = length($s)
               (p-my-= $tlen (p-length $s))
               
               (let (($ch (make-p-box nil)))
                 ;; while ($ti < $tlen) {         my $ch = substr($s, $ti, 1);         if ($ch eq ' ' || $ch eq "\t" || $ch eq "\n" || $ch eq "\r" || $ch eq "\f") {             $ti++;         } elsif ($ch eq '#') {             $ti++;             while ($ti < $tlen && substr($s, $ti, 1) ne "\n") { $ti++ }             $ti++ if $ti < $tlen;         } else {             last;         }     }
                 (p-while (p-< $ti $tlen)
                          (let (($ch (make-p-box nil)))
                            ;; my $ch = substr($s, $ti, 1)
                            (p-my-= $ch (p-substr $s $ti 1))
                            
                            ;; if ($ch eq ' ' || $ch eq "\t" || $ch eq "\n" || $ch eq "\r" || $ch eq "\f") {             $ti++;         } elsif ($ch eq '#') {             $ti++;             while ($ti < $tlen && substr($s, $ti, 1) ne "\n") { $ti++ }             $ti++ if $ti < $tlen;         } else {             last;         }
                            ;; if ($ch eq ' ' || $ch eq "\t" || $ch eq "\n" || $ch eq "\r" || $ch eq "\f")
                            (p-if                 (p-|| (p-|| (p-|| (p-|| (p-str-eq $ch " ") (p-str-eq $ch "	")) (p-str-eq $ch "
")) (p-str-eq $ch "")) (p-str-eq $ch ""))
                                                  (progn
                                                    ;; $ti++
                                                    (p-post++ $ti)
                                                    
                                                    )
                                                  ;; elsif ($ch eq '#')
                                                  (p-if                   (p-str-eq $ch "#")
                                                                          (progn
                                                                            (let ((--pcl-if-ret--0 nil))
                                                                              ;; $ti++
                                                                              (let ((*wantarray* :void))                         (p-post++ $ti))
                                                                              
                                                                              ;; while ($ti < $tlen && substr($s, $ti, 1) ne "\n") { $ti++ }
                                                                              (p-while (p-&& (p-< $ti $tlen) (p-str-ne (p-substr $s $ti 1) "
"))
                                                                                       ;; $ti++
                                                                                       (p-post++ $ti)
                                                                                       
                                                                                       )
                                                                              
                                                                              ;; $ti++ if $ti < $tlen
                                                                              (p-if (setf --pcl-if-ret--0                         (p-< $ti $tlen))
                                                                                    (setf --pcl-if-ret--0                         (p-post++ $ti))
                                                                                    nil)
                                                                              
                                                                              --pcl-if-ret--0)
                                                                            )
                                                                          ;; else
                                                                          (progn
                                                                            ;; last
                                                                            (p-last)
                                                                            
                                                                            )
                                                                          )
                                                  )
                            
                            )
                          )
                 
                 ;; return $ti
                 (p-return $ti)
                 
                 )
               )
             )
           )
         )
       )

;; sub _pack_find_group_end { ... }
(p-sub pl-_pack_find_group_end (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($s (make-p-box nil)) ($ti (make-p-box nil)))
             ;; my ($s, $ti) = @_
             (p-list-= (vector $s $ti) @_)
             
             (let (($tlen (make-p-box nil)))
               ;; my $tlen = length($s)
               (p-my-= $tlen (p-length $s))
               
               (let (($depth (make-p-box nil)))
                 ;; my $depth = 1
                 (p-my-= $depth 1)
                 
                 (let (($ch (make-p-box nil)))
                   ;; while ($ti < $tlen && $depth > 0) {         my $ch = substr($s, $ti, 1);         if ($ch eq '#') {             $ti++;             while ($ti < $tlen && substr($s, $ti, 1) ne "\n") { $ti++ }         } elsif ($ch eq '(') {             $depth++; $ti++;         } elsif ($ch eq ')') {             $depth--;             $ti++ if $depth > 0;         } else {             $ti++;         }     }
                   (p-while (p-&& (p-< $ti $tlen) (p-> $depth 0))
                            (let (($ch (make-p-box nil)))
                              ;; my $ch = substr($s, $ti, 1)
                              (p-my-= $ch (p-substr $s $ti 1))
                              
                              ;; if ($ch eq '#') {             $ti++;             while ($ti < $tlen && substr($s, $ti, 1) ne "\n") { $ti++ }         } elsif ($ch eq '(') {             $depth++; $ti++;         } elsif ($ch eq ')') {             $depth--;             $ti++ if $depth > 0;         } else {             $ti++;         }
                              ;; if ($ch eq '#')
                              (p-if                   (p-str-eq $ch "#")
                                                      (progn
                                                        ;; $ti++
                                                        (p-post++ $ti)
                                                        
                                                        ;; while ($ti < $tlen && substr($s, $ti, 1) ne "\n") { $ti++ }
                                                        (p-while (p-&& (p-< $ti $tlen) (p-str-ne (p-substr $s $ti 1) "
"))
                                                                 ;; $ti++
                                                                 (p-post++ $ti)
                                                                 
                                                                 )
                                                        
                                                        )
                                                      ;; elsif ($ch eq '(')
                                                      (p-if                     (p-str-eq $ch "(")
                                                                                (progn
                                                                                  ;; $depth++
                                                                                  (let ((*wantarray* :void))                         (p-post++ $depth))
                                                                                  
                                                                                  ;; $ti++
                                                                                  (p-post++ $ti)
                                                                                  
                                                                                  )
                                                                                ;; elsif ($ch eq ')')
                                                                                (p-if                       (p-str-eq $ch ")")
                                                                                                            (progn
                                                                                                              (let ((--pcl-if-ret--1 nil))
                                                                                                                ;; $depth--
                                                                                                                (let ((*wantarray* :void))                             (p-post-- $depth))
                                                                                                                
                                                                                                                ;; $ti++ if $depth > 0
                                                                                                                (p-if (setf --pcl-if-ret--1                             (p-> $depth 0))
                                                                                                                      (setf --pcl-if-ret--1                             (p-post++ $ti))
                                                                                                                      nil)
                                                                                                                
                                                                                                                --pcl-if-ret--1)
                                                                                                              )
                                                                                                            ;; else
                                                                                                            (progn
                                                                                                              ;; $ti++
                                                                                                              (p-post++ $ti)
                                                                                                              
                                                                                                              )
                                                                                                            )
                                                                                )
                                                      )
                              
                              )
                            )
                   
                   ;; return $ti
                   (p-return $ti)
                   
                   )
                 )
               )
             )
           )
         )
       )

;; sub _pack_parse_mods { ... }
(p-sub pl-_pack_parse_mods (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($tmpl (make-p-box nil)) ($ti_ref (make-p-box nil)) ($inh_be (make-p-box nil)) ($inh_le (make-p-box nil)) ($ch (make-p-box nil)) ($ctx (make-p-box nil)))
             ;; my ($tmpl, $ti_ref, $inh_be, $inh_le, $ch, $ctx) = @_
             (p-list-= (vector $tmpl $ti_ref $inh_be $inh_le $ch $ctx) @_)
             
             (let (($tlen (make-p-box nil)))
               ;; my $tlen = length($tmpl)
               (p-my-= $tlen (p-length $tmpl))
               
               (let (($bang (make-p-box nil)) ($be (make-p-box nil)) ($le (make-p-box nil)))
                 ;; my ($bang, $be, $le) = (0, $inh_be, $inh_le)
                 (p-list-= (vector $bang $be $le) (vector 0 $inh_be $inh_le))
                 
                 (let (($got_be (make-p-box nil)) ($got_le (make-p-box nil)))
                   ;; my ($got_be, $got_le) = (0, 0)
                   (p-list-= (vector $got_be $got_le) (vector 0 0))
                   
                   (let (($m (make-p-box nil)))
                     ;; while ($$ti_ref < $tlen) {         my $m = substr($tmpl, $$ti_ref, 1);         if ($m eq '!') {             die "'!' allowed only after types $CAN_SHRIEK in $ctx\n"                 unless index($CAN_SHRIEK, $ch) >= 0;             $bang = 1; $$ti_ref++;         } elsif ($m eq '>') {             die "'>' allowed only after types $CAN_ENDIAN in $ctx\n"                 unless index($CAN_ENDIAN, $ch) >= 0 || $ch eq '(';             die "Can't use both '<' and '>' after type '$ch' in $ctx\n" if $got_le;             $be = 1; $le = 0; $got_be = 1; $$ti_ref++;         } elsif ($m eq '<') {             die "'<' allowed only after types $CAN_ENDIAN in $ctx\n"                 unless index($CAN_ENDIAN, $ch) >= 0 || $ch eq '(';             die "Can't use both '<' and '>' after type '$ch' in $ctx\n" if $got_be;             $le = 1; $be = 0; $got_le = 1; $$ti_ref++;         } else {             last;         }     }
                     (p-while (p-< (p-cast-$ $ti_ref) $tlen)
                              (let (($m (make-p-box nil)))
                                ;; my $m = substr($tmpl, $$ti_ref, 1)
                                (p-my-= $m (p-substr $tmpl (p-cast-$ $ti_ref) 1))
                                
                                ;; if ($m eq '!') {             die "'!' allowed only after types $CAN_SHRIEK in $ctx\n"                 unless index($CAN_SHRIEK, $ch) >= 0;             $bang = 1; $$ti_ref++;         } elsif ($m eq '>') {             die "'>' allowed only after types $CAN_ENDIAN in $ctx\n"                 unless index($CAN_ENDIAN, $ch) >= 0 || $ch eq '(';             die "Can't use both '<' and '>' after type '$ch' in $ctx\n" if $got_le;             $be = 1; $le = 0; $got_be = 1; $$ti_ref++;         } elsif ($m eq '<') {             die "'<' allowed only after types $CAN_ENDIAN in $ctx\n"                 unless index($CAN_ENDIAN, $ch) >= 0 || $ch eq '(';             die "Can't use both '<' and '>' after type '$ch' in $ctx\n" if $got_be;             $le = 1; $be = 0; $got_le = 1; $$ti_ref++;         } else {             last;         }
                                ;; if ($m eq '!')
                                (p-if                     (p-str-eq $m "!")
                                                          (progn
                                                            ;; die "'!' allowed only after types $CAN_SHRIEK in $ctx\n"                 unless index($CAN_SHRIEK, $ch) >= 0
                                                            (p-unless                         (p->= (p-index $CAN_SHRIEK $ch) 0)                         (p-die (p-string-concat "'!' allowed only after types " $CAN_SHRIEK " in " $ctx "
")))
                                                            
                                                            ;; $bang = 1
                                                            (p-my-= $bang 1)
                                                            
                                                            ;; $$ti_ref++
                                                            (p-post++ (p-cast-$ $ti_ref))
                                                            
                                                            )
                                                          ;; elsif ($m eq '>')
                                                          (p-if                       (p-str-eq $m ">")
                                                                                      (progn
                                                                                        ;; die "'>' allowed only after types $CAN_ENDIAN in $ctx\n"                 unless index($CAN_ENDIAN, $ch) >= 0 || $ch eq '('
                                                                                        (let ((*wantarray* :void)) (p-unless                           (p-|| (p->= (p-index $CAN_ENDIAN $ch) 0) (p-str-eq $ch "("))                           (p-die (p-string-concat "'>' allowed only after types " $CAN_ENDIAN " in " $ctx "
"))))
                                                                                        
                                                                                        ;; die "Can't use both '<' and '>' after type '$ch' in $ctx\n" if $got_le
                                                                                        (let ((*wantarray* :void)) (p-if                           $got_le                           (p-die (p-string-concat "Can't use both '<' and '>' after type '" $ch "' in " $ctx "
"))))
                                                                                        
                                                                                        ;; $be = 1
                                                                                        (let ((*wantarray* :void))                           (p-my-= $be 1))
                                                                                        
                                                                                        ;; $le = 0
                                                                                        (let ((*wantarray* :void))                           (p-my-= $le 0))
                                                                                        
                                                                                        ;; $got_be = 1
                                                                                        (let ((*wantarray* :void))                           (p-my-= $got_be 1))
                                                                                        
                                                                                        ;; $$ti_ref++
                                                                                        (p-post++ (p-cast-$ $ti_ref))
                                                                                        
                                                                                        )
                                                                                      ;; elsif ($m eq '<')
                                                                                      (p-if                         (p-str-eq $m "<")
                                                                                                                    (progn
                                                                                                                      ;; die "'<' allowed only after types $CAN_ENDIAN in $ctx\n"                 unless index($CAN_ENDIAN, $ch) >= 0 || $ch eq '('
                                                                                                                      (let ((*wantarray* :void)) (p-unless                             (p-|| (p->= (p-index $CAN_ENDIAN $ch) 0) (p-str-eq $ch "("))                             (p-die (p-string-concat "'<' allowed only after types " $CAN_ENDIAN " in " $ctx "
"))))
                                                                                                                      
                                                                                                                      ;; die "Can't use both '<' and '>' after type '$ch' in $ctx\n" if $got_be
                                                                                                                      (let ((*wantarray* :void)) (p-if                             $got_be                             (p-die (p-string-concat "Can't use both '<' and '>' after type '" $ch "' in " $ctx "
"))))
                                                                                                                      
                                                                                                                      ;; $le = 1
                                                                                                                      (let ((*wantarray* :void))                             (p-my-= $le 1))
                                                                                                                      
                                                                                                                      ;; $be = 0
                                                                                                                      (let ((*wantarray* :void))                             (p-my-= $be 0))
                                                                                                                      
                                                                                                                      ;; $got_le = 1
                                                                                                                      (let ((*wantarray* :void))                             (p-my-= $got_le 1))
                                                                                                                      
                                                                                                                      ;; $$ti_ref++
                                                                                                                      (p-post++ (p-cast-$ $ti_ref))
                                                                                                                      
                                                                                                                      )
                                                                                                                    ;; else
                                                                                                                    (progn
                                                                                                                      ;; last
                                                                                                                      (p-last)
                                                                                                                      
                                                                                                                      )
                                                                                                                    )
                                                                                      )
                                                          )
                                
                                )
                              )
                     
                     ;; return ($bang, $be, $le)
                     (p-return $bang $be $le)
                     
                     )
                   )
                 )
               )
             )
           )
         )
       )

;; sub _pack_parse_count { ... }
(p-sub pl-_pack_parse_count (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($tmpl (make-p-box nil)) ($ti_ref (make-p-box nil)))
             ;; my ($tmpl, $ti_ref) = @_
             (p-list-= (vector $tmpl $ti_ref) @_)
             
             (let (($tlen (make-p-box nil)))
               ;; my $tlen = length($tmpl)
               (p-my-= $tlen (p-length $tmpl))
               
               ;; $$ti_ref = _pack_skip_ws($tmpl, $$ti_ref)
               (let ((*wantarray* :void))           (p-setf (p-cast-$ $ti_ref) (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl (p-cast-$ $ti_ref)))))
               
               ;; if ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) eq '*') {         $$ti_ref++; return (1, undef, 1);     }
               ;; if ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) eq '*')
               (p-if           (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-str-eq (p-substr $tmpl (p-cast-$ $ti_ref) 1) "*"))
                               (progn
                                 ;; $$ti_ref++
                                 (let ((*wantarray* :void))               (p-post++ (p-cast-$ $ti_ref)))
                                 
                                 ;; return (1, undef, 1)
                                 (p-return 1 (p-undef) 1)
                                 
                                 )
                               nil
                               )
               
               (let (($n (make-p-box nil)))
                 ;; if ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) eq '[') {         $$ti_ref++;         my $n = 0;         while ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) =~ /\d/) {             $n = $n * 10 + substr($tmpl, $$ti_ref, 1); $$ti_ref++;         }         $$ti_ref++ if $$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) eq ']';         return (0, $n, $n);     }
                 ;; if ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) eq '[')
                 (p-if             (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-str-eq (p-substr $tmpl (p-cast-$ $ti_ref) 1) "["))
                                   (progn
                                     (let (($n (make-p-box nil)))
                                       ;; $$ti_ref++
                                       (let ((*wantarray* :void))                   (p-post++ (p-cast-$ $ti_ref)))
                                       
                                       ;; my $n = 0
                                       (p-my-= $n 0)
                                       
                                       ;; while ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) =~ /\d/) {             $n = $n * 10 + substr($tmpl, $$ti_ref, 1); $$ti_ref++;         }
                                       (p-while (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-=~ (p-substr $tmpl (p-cast-$ $ti_ref) 1) (p-regex "/\\d/")))
                                                ;; $n = $n * 10 + substr($tmpl, $$ti_ref, 1)
                                                (let ((*wantarray* :void))                     (p-my-= $n (p-+ (p-* $n 10) (p-substr $tmpl (p-cast-$ $ti_ref) 1))))
                                                
                                                ;; $$ti_ref++
                                                (p-post++ (p-cast-$ $ti_ref))
                                                
                                                )
                                       
                                       ;; $$ti_ref++ if $$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) eq ']'
                                       (let ((*wantarray* :void)) (p-if                   (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-str-eq (p-substr $tmpl (p-cast-$ $ti_ref) 1) "]"))                   (p-post++ (p-cast-$ $ti_ref))))
                                       
                                       ;; return (0, $n, $n)
                                       (p-return 0 $n $n)
                                       
                                       )
                                     )
                                   nil
                                   )
                 
                 (let (($n (make-p-box nil)))
                   ;; if ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) =~ /\d/) {         my $n = 0;         while ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) =~ /\d/) {             $n = $n * 10 + substr($tmpl, $$ti_ref, 1); $$ti_ref++;         }         return (0, $n, $n);     }
                   ;; if ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) =~ /\d/)
                   (p-if               (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-=~ (p-substr $tmpl (p-cast-$ $ti_ref) 1) (p-regex "/\\d/")))
                                       (progn
                                         (let (($n (make-p-box nil)))
                                           ;; my $n = 0
                                           (p-my-= $n 0)
                                           
                                           ;; while ($$ti_ref < $tlen && substr($tmpl, $$ti_ref, 1) =~ /\d/) {             $n = $n * 10 + substr($tmpl, $$ti_ref, 1); $$ti_ref++;         }
                                           (p-while (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-=~ (p-substr $tmpl (p-cast-$ $ti_ref) 1) (p-regex "/\\d/")))
                                                    ;; $n = $n * 10 + substr($tmpl, $$ti_ref, 1)
                                                    (let ((*wantarray* :void))                       (p-my-= $n (p-+ (p-* $n 10) (p-substr $tmpl (p-cast-$ $ti_ref) 1))))
                                                    
                                                    ;; $$ti_ref++
                                                    (p-post++ (p-cast-$ $ti_ref))
                                                    
                                                    )
                                           
                                           ;; return (0, $n, $n)
                                           (p-return 0 $n $n)
                                           
                                           )
                                         )
                                       nil
                                       )
                   
                   ;; return (0, 1, 1)
                   (p-return 0 1 1)
                   
                   )
                 )
               )
             )
           )
         )
       )

;; sub _pack_emit_int { ... }
(p-sub pl-_pack_emit_int (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($k (make-p-box nil)))
             (let (($val (make-p-box nil)) ($nbytes (make-p-box nil)) ($signed (make-p-box nil)) ($be (make-p-box nil)))
               ;; my ($val, $nbytes, $signed, $be) = @_
               (p-list-= (vector $val $nbytes $signed $be) @_)
               
               ;; $val = int($val + 0)
               (let ((*wantarray* :void))           (p-my-= $val (p-int (p-+ $val 0))))
               
               (let (($result (make-p-box nil)))
                 ;; my $result = ''
                 (p-my-= $result "")
                 
                 ;; if ($be) {         for (my $k = $nbytes - 1; $k >= 0; $k--) {             $result .= chr(($val >> (8 * $k)) & 0xFF);         }     } else {         for (my $k = 0; $k < $nbytes; $k++) {             $result .= chr(($val >> (8 * $k)) & 0xFF);         }     }
                 ;; if ($be)
                 (p-if             $be
                                   (progn
                                     (let (($k (make-p-box nil)))
                                       ;; for (my $k = $nbytes - 1; $k >= 0; $k--) {             $result .= chr(($val >> (8 * $k)) & 0xFF);         }
                                       (let (($k (make-p-box nil)))
                                         (p-for (                  (p-my-= $k (p-- $nbytes 1)))
                                                (                  (p->= $k 0))
                                                (                  (p-post-- $k))
                                                ;; $result .= chr(($val >> (8 * $k)) & 0xFF)
                                                (p-.= $result (p-chr (p-bit-and (p->> $val (p-* 8 $k)) #xFF)))
                                                
                                                )
                                         )
                                       
                                       )
                                     )
                                   ;; else
                                   (progn
                                     (let (($k (make-p-box nil)))
                                       ;; for (my $k = 0; $k < $nbytes; $k++) {             $result .= chr(($val >> (8 * $k)) & 0xFF);         }
                                       (let (($k (make-p-box nil)))
                                         (p-for (                  (p-my-= $k 0))
                                                (                  (p-< $k $nbytes))
                                                (                  (p-post++ $k))
                                                ;; $result .= chr(($val >> (8 * $k)) & 0xFF)
                                                (p-.= $result (p-chr (p-bit-and (p->> $val (p-* 8 $k)) #xFF)))
                                                
                                                )
                                         )
                                       
                                       )
                                     )
                                   )
                 
                 ;; return $result
                 (p-return $result)
                 
                 )
               )
             )
           )
         )
       )

;; sub _unpack_read_int { ... }
(p-sub pl-_unpack_read_int (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($k (make-p-box nil)))
             (let (($s (make-p-box nil)) ($si (make-p-box nil)) ($nbytes (make-p-box nil)) ($be (make-p-box nil)) ($signed (make-p-box nil)))
               ;; my ($s, $si, $nbytes, $be, $signed) = @_
               (p-list-= (vector $s $si $nbytes $be $signed) @_)
               
               (let (($slen (make-p-box nil)))
                 ;; my $slen = length($s)
                 (p-my-= $slen (p-length $s))
                 
                 (let (($v (make-p-box nil)))
                   ;; my $v = 0
                   (p-my-= $v 0)
                   
                   ;; if ($be) {         for (my $k = 0; $k < $nbytes; $k++) {             $v = ($v << 8) | (($si+$k < $slen) ? ord(substr($s,$si+$k,1)) : 0);         }     } else {         for (my $k = $nbytes - 1; $k >= 0; $k--) {             $v = ($v << 8) | (($si+$k < $slen) ? ord(substr($s,$si+$k,1)) : 0);         }     }
                   ;; if ($be)
                   (p-if               $be
                                       (progn
                                         (let (($k (make-p-box nil)))
                                           ;; for (my $k = 0; $k < $nbytes; $k++) {             $v = ($v << 8) | (($si+$k < $slen) ? ord(substr($s,$si+$k,1)) : 0);         }
                                           (let (($k (make-p-box nil)))
                                             (p-for (                    (p-my-= $k 0))
                                                    (                    (p-< $k $nbytes))
                                                    (                    (p-post++ $k))
                                                    ;; $v = ($v << 8) | (($si+$k < $slen) ? ord(substr($s,$si+$k,1)) : 0)
                                                    (p-my-= $v (p-bit-or (p-<< $v 8) (p-if (p-< (p-+ $si $k) $slen) (p-ord (p-substr $s (p-+ $si $k) 1)) 0)))
                                                    
                                                    )
                                             )
                                           
                                           )
                                         )
                                       ;; else
                                       (progn
                                         (let (($k (make-p-box nil)))
                                           ;; for (my $k = $nbytes - 1; $k >= 0; $k--) {             $v = ($v << 8) | (($si+$k < $slen) ? ord(substr($s,$si+$k,1)) : 0);         }
                                           (let (($k (make-p-box nil)))
                                             (p-for (                    (p-my-= $k (p-- $nbytes 1)))
                                                    (                    (p->= $k 0))
                                                    (                    (p-post-- $k))
                                                    ;; $v = ($v << 8) | (($si+$k < $slen) ? ord(substr($s,$si+$k,1)) : 0)
                                                    (p-my-= $v (p-bit-or (p-<< $v 8) (p-if (p-< (p-+ $si $k) $slen) (p-ord (p-substr $s (p-+ $si $k) 1)) 0)))
                                                    
                                                    )
                                             )
                                           
                                           )
                                         )
                                       )
                   
                   (let (($max (make-p-box nil)))
                     ;; if ($signed) {         if ($nbytes == 8) {             # Avoid NV precision issues for 64-bit: use UV bitwise shift to detect sign bit             if ($v >> 63) { $v = -(~$v + 1) }  # two's-complement negation via UV bitwise ops         } else {             my $max = 2 ** ($nbytes * 8);             $v -= $max if $v >= $max / 2;         }     }
                     ;; if ($signed)
                     (p-if                 $signed
                                           (progn
                                             (let (($max (make-p-box nil)))
                                               ;; if ($nbytes == 8) {             # Avoid NV precision issues for 64-bit: use UV bitwise shift to detect sign bit             if ($v >> 63) { $v = -(~$v + 1) }  # two's-complement negation via UV bitwise ops         } else {             my $max = 2 ** ($nbytes * 8);             $v -= $max if $v >= $max / 2;         }
                                               ;; if ($nbytes == 8)
                                               (p-if                       (p-== $nbytes 8)
                                                                           (progn
                                                                             (let ((--pcl-if-ret--2 nil))
                                                                               ;; if ($v >> 63) { $v = -(~$v + 1) }
                                                                               ;; if ($v >> 63)
                                                                               (p-if (setf --pcl-if-ret--2                             (p->> $v 63))
                                                                                     (progn
                                                                                       ;; $v = -(~$v + 1)
                                                                                       (setf --pcl-if-ret--2                                 (p-my-= $v (p-- (p-+ (p-bit-not $v) 1))))
                                                                                       
                                                                                       )
                                                                                     nil
                                                                                     )
                                                                               
                                                                               --pcl-if-ret--2)
                                                                             )
                                                                           ;; else
                                                                           (progn
                                                                             (let (($max (make-p-box nil)))
                                                                               (let ((--pcl-if-ret--3 nil))
                                                                                 ;; my $max = 2 ** ($nbytes * 8)
                                                                                 (p-my-= $max (p-** 2 (p-* $nbytes 8)))
                                                                                 
                                                                                 ;; $v -= $max if $v >= $max / 2
                                                                                 (p-if (setf --pcl-if-ret--3                               (p->= $v (p-/ $max 2)))
                                                                                       (setf --pcl-if-ret--3                               (p-decf $v $max))
                                                                                       nil)
                                                                                 
                                                                                 --pcl-if-ret--3)
                                                                               )
                                                                             )
                                                                           )
                                               
                                               )
                                             )
                                           nil
                                           )
                     
                     ;; return $v
                     (p-return $v)
                     
                     )
                   )
                 )
               )
             )
           )
         )
       )

;; sub _pack_float32   { ... }
(p-sub pl-_pack_float32 (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($val (make-p-box nil)) ($be (make-p-box nil)))
             ;; my ($val, $be) = @_
             (p-list-= (vector $val $be) @_)
             
             ;; return ""
             (let* ((v (coerce (to-number (unbox $val)) (quote single-float)))
                    (bits (sb-kernel:single-float-bits v))
                    (be-p (and (p-box-value $be) t)))
               (if be-p
                   (p-return (map (quote string) (function code-char)
                                  (list (ldb (byte 8 24) bits) (ldb (byte 8 16) bits)
                                        (ldb (byte 8 8) bits)  (ldb (byte 8 0) bits))))
                   (p-return (map (quote string) (function code-char)
                                  (list (ldb (byte 8 0) bits)  (ldb (byte 8 8) bits)
                                        (ldb (byte 8 16) bits) (ldb (byte 8 24) bits))))))
             
             )
           )
         )
       )

;; sub _pack_float64   { ... }
(p-sub pl-_pack_float64 (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($val (make-p-box nil)) ($be (make-p-box nil)))
             ;; my ($val, $be) = @_
             (p-list-= (vector $val $be) @_)
             
             ;; return ""
             (let* ((v (coerce (to-number (unbox $val)) (quote double-float)))
                    (bits (sb-kernel:double-float-bits v))
                    (be-p (and (p-box-value $be) t)))
               (if be-p
                   (p-return (map (quote string) (function code-char)
                                  (loop for k from 7 downto 0 collect (ldb (byte 8 (* 8 k)) bits))))
                   (p-return (map (quote string) (function code-char)
                                  (loop for k from 0 to 7 collect (ldb (byte 8 (* 8 k)) bits))))))
             
             )
           )
         )
       )

;; sub _unpack_float32 { ... }
(p-sub pl-_unpack_float32 (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($s (make-p-box nil)) ($si (make-p-box nil)) ($be (make-p-box nil)))
             ;; my ($s, $si, $be) = @_
             (p-list-= (vector $s $si $be) @_)
             
             ;; return 0.0
             (let* ((s-str (to-string (unbox $s)))
                    (si-val (truncate (to-number (unbox $si))))
                    (be-p (and (p-box-value $be) t))
                    (bits (if be-p
                              (logior (ash (char-code (char s-str (+ si-val 0))) 24)
                                      (ash (char-code (char s-str (+ si-val 1))) 16)
                                      (ash (char-code (char s-str (+ si-val 2))) 8)
                                      (char-code (char s-str (+ si-val 3))))
                              (logior (char-code (char s-str (+ si-val 0)))
                                      (ash (char-code (char s-str (+ si-val 1))) 8)
                                      (ash (char-code (char s-str (+ si-val 2))) 16)
                                      (ash (char-code (char s-str (+ si-val 3))) 24)))))
               (p-return (sb-kernel:make-single-float bits)))
             
             )
           )
         )
       )

;; sub _unpack_float64 { ... }
(p-sub pl-_unpack_float64 (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($s (make-p-box nil)) ($si (make-p-box nil)) ($be (make-p-box nil)))
             ;; my ($s, $si, $be) = @_
             (p-list-= (vector $s $si $be) @_)
             
             ;; return 0.0
             (let* ((s-str (to-string (unbox $s)))
                    (si-val (truncate (to-number (unbox $si))))
                    (be-p (and (p-box-value $be) t))
                    (bits (if be-p
                              (loop for k from 0 to 7 summing
                                    (ash (char-code (char s-str (+ si-val k))) (* 8 (- 7 k))))
                              (loop for k from 0 to 7 summing
                                    (ash (char-code (char s-str (+ si-val k))) (* 8 k))))))
               (p-return (sb-kernel:make-double-float
                          (ldb (byte 32 32) bits)
                          (ldb (byte 32 0) bits))))
             
             )
           )
         )
       )

;; sub _pack_str_one { ... }
(p-sub pl-_pack_str_one (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($k (make-p-box nil)) ($bs (make-p-box nil)) ($bit (make-p-box nil)) ($cs (make-p-box nil)))
             (let ((--pcl-if-ret--4 nil))
               (let (($ch (make-p-box nil)) ($arg (make-p-box nil)) ($nrep (make-p-box nil)) ($star (make-p-box nil)) ($result_ref (make-p-box nil)))
                 ;; my ($ch, $arg, $nrep, $star, $result_ref) = @_
                 (p-list-= (vector $ch $arg $nrep $star $result_ref) @_)
                 
                 ;; $arg = '' unless defined $arg
                 (let ((*wantarray* :void)) (p-unless             (p-defined $arg)             (p-my-= $arg "")))
                 
                 (let (($slen (make-p-box nil)))
                   ;; my $slen = length($arg)
                   (p-my-= $slen (p-length $arg))
                   
                   (let (($len (make-p-box nil)) ($body (make-p-box nil)) ($nbits (make-p-box nil)) ($byte (make-p-box nil)) ($idx (make-p-box nil)) ($nyb (make-p-box nil)) ($hi (make-p-box nil)) ($lo (make-p-box nil)) ($ce (make-p-box nil)) ($chunk (make-p-box nil)) ($clen (make-p-box nil)) ($b0 (make-p-box nil)) ($b1 (make-p-box nil)) ($b2 (make-p-box nil)) ($cm (make-p-box nil)) ($uu (make-p-box nil)))
                     ;; if ($ch eq 'a') {         my $len = $star ? $slen : $nrep;         for (my $k = 0; $k < $len; $k++) {             $$result_ref .= $k < $slen ? substr($arg,$k,1) : chr(0);         }     } elsif ($ch eq 'A') {         my $len = $star ? $slen : $nrep;         for (my $k = 0; $k < $len; $k++) {             $$result_ref .= $k < $slen ? substr($arg,$k,1) : ' ';         }     } elsif ($ch eq 'Z') {         my $len = $star ? $slen + 1 : $nrep;         my $body = $len > 1 ? $len - 1 : 0;         for (my $k = 0; $k < $body; $k++) {             $$result_ref .= $k < $slen ? substr($arg,$k,1) : chr(0);         }         $$result_ref .= chr(0);     } elsif ($ch eq 'b') {         my $nbits = $star ? $slen : $nrep;         for (my $bs = 0; $bs < $nbits; $bs += 8) {             my $byte = 0;             for (my $bit = 0; $bit < 8 && $bs+$bit < $nbits; $bit++) {                 my $idx = $bs + $bit;                 $byte |= (1 << $bit) if $idx < $slen && substr($arg,$idx,1) eq '1';             }             $$result_ref .= chr($byte);         }     } elsif ($ch eq 'B') {         my $nbits = $star ? $slen : $nrep;         for (my $bs = 0; $bs < $nbits; $bs += 8) {             my $byte = 0;             for (my $bit = 0; $bit < 8 && $bs+$bit < $nbits; $bit++) {                 my $idx = $bs + $bit;                 $byte |= (1 << (7 - $bit)) if $idx < $slen && substr($arg,$idx,1) eq '1';             }             $$result_ref .= chr($byte);         }     } elsif ($ch eq 'H') {         my $nyb = $star ? $slen : $nrep;  # count is number of nybbles         for (my $k = 0; $k < $nyb; $k += 2) {             my $hi = $k   < $slen ? hex(substr($arg,$k,  1)) : 0;             my $lo = $k+1 < $slen ? hex(substr($arg,$k+1,1)) : 0;             $$result_ref .= chr(($hi << 4) | $lo);         }     } elsif ($ch eq 'h') {         my $nyb = $star ? $slen : $nrep;  # count is number of nybbles         for (my $k = 0; $k < $nyb; $k += 2) {             my $lo = $k   < $slen ? hex(substr($arg,$k,  1)) : 0;             my $hi = $k+1 < $slen ? hex(substr($arg,$k+1,1)) : 0;             $$result_ref .= chr(($hi << 4) | $lo);         }     } elsif ($ch eq 'u') {         for (my $cs = 0; $cs < $slen; $cs += 45) {             my $ce = $cs + 45 < $slen ? $cs + 45 : $slen;             my $chunk = substr($arg, $cs, $ce - $cs);             my $clen = length($chunk);             $$result_ref .= chr(32 + $clen);             for (my $k = 0; $k < $clen; $k += 3) {                 my $b0 = ord(substr($chunk,$k,1));                 my $b1 = $k+1 < $clen ? ord(substr($chunk,$k+1,1)) : 0;                 my $b2 = $k+2 < $clen ? ord(substr($chunk,$k+2,1)) : 0;                 my $cm = ($b0 << 16) | ($b1 << 8) | $b2;                 my $uu = sub { my $c = 32 + ($_[0] & 63); $c == 32 ? 96 : $c };                 $$result_ref .= chr($uu->(($cm>>18)&63)) . chr($uu->(($cm>>12)&63))                               . chr($uu->(($cm>> 6)&63)) . chr($uu->( $cm    &63));             }             $$result_ref .= "\n";         }     }
                     ;; if ($ch eq 'a')
                     (p-if (setf --pcl-if-ret--4                 (p-str-eq $ch "a"))
                           (progn
                             (let (($len (make-p-box nil)) ($k (make-p-box nil)))
                               ;; my $len = $star ? $slen : $nrep
                               (p-my-= $len (p-if $star $slen $nrep))
                               
                               ;; for (my $k = 0; $k < $len; $k++) {             $$result_ref .= $k < $slen ? substr($arg,$k,1) : chr(0);         }
                               (let (($k (make-p-box nil)))
                                 (p-for (                      (p-my-= $k 0))
                                        (                      (p-< $k $len))
                                        (                      (p-post++ $k))
                                        ;; $$result_ref .= $k < $slen ? substr($arg,$k,1) : chr(0)
                                        (p-.= (p-cast-$ $result_ref) (p-if (p-< $k $slen) (p-substr $arg $k 1) (p-chr 0)))
                                        
                                        )
                                 )
                               
                               )
                             )
                           ;; elsif ($ch eq 'A')
                           (p-if (setf --pcl-if-ret--4                   (p-str-eq $ch "A"))
                                 (progn
                                   (let (($len (make-p-box nil)) ($k (make-p-box nil)))
                                     ;; my $len = $star ? $slen : $nrep
                                     (p-my-= $len (p-if $star $slen $nrep))
                                     
                                     ;; for (my $k = 0; $k < $len; $k++) {             $$result_ref .= $k < $slen ? substr($arg,$k,1) : ' ';         }
                                     (let (($k (make-p-box nil)))
                                       (p-for (                        (p-my-= $k 0))
                                              (                        (p-< $k $len))
                                              (                        (p-post++ $k))
                                              ;; $$result_ref .= $k < $slen ? substr($arg,$k,1) : ' '
                                              (p-.= (p-cast-$ $result_ref) (p-if (p-< $k $slen) (p-substr $arg $k 1) " "))
                                              
                                              )
                                       )
                                     
                                     )
                                   )
                                 ;; elsif ($ch eq 'Z')
                                 (p-if (setf --pcl-if-ret--4                     (p-str-eq $ch "Z"))
                                       (progn
                                         (let (($len (make-p-box nil)) ($body (make-p-box nil)) ($k (make-p-box nil)))
                                           ;; my $len = $star ? $slen + 1 : $nrep
                                           (p-my-= $len (p-if $star (p-+ $slen 1) $nrep))
                                           
                                           ;; my $body = $len > 1 ? $len - 1 : 0
                                           (p-my-= $body (p-if (p-> $len 1) (p-- $len 1) 0))
                                           
                                           ;; for (my $k = 0; $k < $body; $k++) {             $$result_ref .= $k < $slen ? substr($arg,$k,1) : chr(0);         }
                                           (let (($k (make-p-box nil)))
                                             (p-for (                          (p-my-= $k 0))
                                                    (                          (p-< $k $body))
                                                    (                          (p-post++ $k))
                                                    ;; $$result_ref .= $k < $slen ? substr($arg,$k,1) : chr(0)
                                                    (p-.= (p-cast-$ $result_ref) (p-if (p-< $k $slen) (p-substr $arg $k 1) (p-chr 0)))
                                                    
                                                    )
                                             )
                                           
                                           ;; $$result_ref .= chr(0)
                                           (setf --pcl-if-ret--4                           (p-.= (p-cast-$ $result_ref) (p-chr 0)))
                                           
                                           )
                                         )
                                       ;; elsif ($ch eq 'b')
                                       (p-if (setf --pcl-if-ret--4                       (p-str-eq $ch "b"))
                                             (progn
                                               (let (($nbits (make-p-box nil)) ($bs (make-p-box nil)) ($byte (make-p-box nil)) ($bit (make-p-box nil)) ($idx (make-p-box nil)))
                                                 ;; my $nbits = $star ? $slen : $nrep
                                                 (p-my-= $nbits (p-if $star $slen $nrep))
                                                 
                                                 ;; for (my $bs = 0; $bs < $nbits; $bs += 8) {             my $byte = 0;             for (my $bit = 0; $bit < 8 && $bs+$bit < $nbits; $bit++) {                 my $idx = $bs + $bit;                 $byte |= (1 << $bit) if $idx < $slen && substr($arg,$idx,1) eq '1';             }             $$result_ref .= chr($byte);         }
                                                 (let (($bs (make-p-box nil)))
                                                   (p-for (                            (p-my-= $bs 0))
                                                          (                            (p-< $bs $nbits))
                                                          (                            (p-incf $bs 8))
                                                          (let (($byte (make-p-box nil)) ($bit (make-p-box nil)) ($idx (make-p-box nil)))
                                                            ;; my $byte = 0
                                                            (p-my-= $byte 0)
                                                            
                                                            ;; for (my $bit = 0; $bit < 8 && $bs+$bit < $nbits; $bit++) {                 my $idx = $bs + $bit;                 $byte |= (1 << $bit) if $idx < $slen && substr($arg,$idx,1) eq '1';             }
                                                            (let (($bit (make-p-box nil)))
                                                              (p-for (                                  (p-my-= $bit 0))
                                                                     (                                  (p-&& (p-< $bit 8) (p-< (p-+ $bs $bit) $nbits)))
                                                                     (                                  (p-post++ $bit))
                                                                     (let (($idx (make-p-box nil)))
                                                                       (let ((--pcl-if-ret--5 nil))
                                                                         ;; my $idx = $bs + $bit
                                                                         (p-my-= $idx (p-+ $bs $bit))
                                                                         
                                                                         ;; $byte |= (1 << $bit) if $idx < $slen && substr($arg,$idx,1) eq '1'
                                                                         (p-if (setf --pcl-if-ret--5                                           (p-&& (p-< $idx $slen) (p-str-eq (p-substr $arg $idx 1) "1")))
                                                                               (setf --pcl-if-ret--5                                           (p-bit-or= $byte (p-<< 1 $bit)))
                                                                               nil)
                                                                         
                                                                         --pcl-if-ret--5)
                                                                       )
                                                                     )
                                                              )
                                                            
                                                            ;; $$result_ref .= chr($byte)
                                                            (p-.= (p-cast-$ $result_ref) (p-chr $byte))
                                                            
                                                            )
                                                          )
                                                   )
                                                 
                                                 )
                                               )
                                             ;; elsif ($ch eq 'B')
                                             (p-if (setf --pcl-if-ret--4                         (p-str-eq $ch "B"))
                                                   (progn
                                                     (let (($nbits (make-p-box nil)) ($bs (make-p-box nil)) ($byte (make-p-box nil)) ($bit (make-p-box nil)) ($idx (make-p-box nil)))
                                                       ;; my $nbits = $star ? $slen : $nrep
                                                       (p-my-= $nbits (p-if $star $slen $nrep))
                                                       
                                                       ;; for (my $bs = 0; $bs < $nbits; $bs += 8) {             my $byte = 0;             for (my $bit = 0; $bit < 8 && $bs+$bit < $nbits; $bit++) {                 my $idx = $bs + $bit;                 $byte |= (1 << (7 - $bit)) if $idx < $slen && substr($arg,$idx,1) eq '1';             }             $$result_ref .= chr($byte);         }
                                                       (let (($bs (make-p-box nil)))
                                                         (p-for (                              (p-my-= $bs 0))
                                                                (                              (p-< $bs $nbits))
                                                                (                              (p-incf $bs 8))
                                                                (let (($byte (make-p-box nil)) ($bit (make-p-box nil)) ($idx (make-p-box nil)))
                                                                  ;; my $byte = 0
                                                                  (p-my-= $byte 0)
                                                                  
                                                                  ;; for (my $bit = 0; $bit < 8 && $bs+$bit < $nbits; $bit++) {                 my $idx = $bs + $bit;                 $byte |= (1 << (7 - $bit)) if $idx < $slen && substr($arg,$idx,1) eq '1';             }
                                                                  (let (($bit (make-p-box nil)))
                                                                    (p-for (                                    (p-my-= $bit 0))
                                                                           (                                    (p-&& (p-< $bit 8) (p-< (p-+ $bs $bit) $nbits)))
                                                                           (                                    (p-post++ $bit))
                                                                           (let (($idx (make-p-box nil)))
                                                                             (let ((--pcl-if-ret--6 nil))
                                                                               ;; my $idx = $bs + $bit
                                                                               (p-my-= $idx (p-+ $bs $bit))
                                                                               
                                                                               ;; $byte |= (1 << (7 - $bit)) if $idx < $slen && substr($arg,$idx,1) eq '1'
                                                                               (p-if (setf --pcl-if-ret--6                                             (p-&& (p-< $idx $slen) (p-str-eq (p-substr $arg $idx 1) "1")))
                                                                                     (setf --pcl-if-ret--6                                             (p-bit-or= $byte (p-<< 1 (p-- 7 $bit))))
                                                                                     nil)
                                                                               
                                                                               --pcl-if-ret--6)
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; $$result_ref .= chr($byte)
                                                                  (p-.= (p-cast-$ $result_ref) (p-chr $byte))
                                                                  
                                                                  )
                                                                )
                                                         )
                                                       
                                                       )
                                                     )
                                                   ;; elsif ($ch eq 'H')
                                                   (p-if (setf --pcl-if-ret--4                           (p-str-eq $ch "H"))
                                                         (progn
                                                           (let (($nyb (make-p-box nil)) ($k (make-p-box nil)) ($hi (make-p-box nil)) ($lo (make-p-box nil)))
                                                             ;; my $nyb = $star ? $slen : $nrep
                                                             (p-my-= $nyb (p-if $star $slen $nrep))
                                                             
                                                             ;; for (my $k = 0; $k < $nyb; $k += 2) {             my $hi = $k   < $slen ? hex(substr($arg,$k,  1)) : 0;             my $lo = $k+1 < $slen ? hex(substr($arg,$k+1,1)) : 0;             $$result_ref .= chr(($hi << 4) | $lo);         }
                                                             (let (($k (make-p-box nil)))
                                                               (p-for (                                (p-my-= $k 0))
                                                                      (                                (p-< $k $nyb))
                                                                      (                                (p-incf $k 2))
                                                                      (let (($hi (make-p-box nil)) ($lo (make-p-box nil)))
                                                                        ;; my $hi = $k   < $slen ? hex(substr($arg,$k,  1)) : 0
                                                                        (p-my-= $hi (p-if (p-< $k $slen) (p-hex (p-substr $arg $k 1)) 0))
                                                                        
                                                                        ;; my $lo = $k+1 < $slen ? hex(substr($arg,$k+1,1)) : 0
                                                                        (p-my-= $lo (p-if (p-< (p-+ $k 1) $slen) (p-hex (p-substr $arg (p-+ $k 1) 1)) 0))
                                                                        
                                                                        ;; $$result_ref .= chr(($hi << 4) | $lo)
                                                                        (p-.= (p-cast-$ $result_ref) (p-chr (p-bit-or (p-<< $hi 4) $lo)))
                                                                        
                                                                        )
                                                                      )
                                                               )
                                                             
                                                             )
                                                           )
                                                         ;; elsif ($ch eq 'h')
                                                         (p-if (setf --pcl-if-ret--4                             (p-str-eq $ch "h"))
                                                               (progn
                                                                 (let (($nyb (make-p-box nil)) ($k (make-p-box nil)) ($lo (make-p-box nil)) ($hi (make-p-box nil)))
                                                                   ;; my $nyb = $star ? $slen : $nrep
                                                                   (p-my-= $nyb (p-if $star $slen $nrep))
                                                                   
                                                                   ;; for (my $k = 0; $k < $nyb; $k += 2) {             my $lo = $k   < $slen ? hex(substr($arg,$k,  1)) : 0;             my $hi = $k+1 < $slen ? hex(substr($arg,$k+1,1)) : 0;             $$result_ref .= chr(($hi << 4) | $lo);         }
                                                                   (let (($k (make-p-box nil)))
                                                                     (p-for (                                  (p-my-= $k 0))
                                                                            (                                  (p-< $k $nyb))
                                                                            (                                  (p-incf $k 2))
                                                                            (let (($lo (make-p-box nil)) ($hi (make-p-box nil)))
                                                                              ;; my $lo = $k   < $slen ? hex(substr($arg,$k,  1)) : 0
                                                                              (p-my-= $lo (p-if (p-< $k $slen) (p-hex (p-substr $arg $k 1)) 0))
                                                                              
                                                                              ;; my $hi = $k+1 < $slen ? hex(substr($arg,$k+1,1)) : 0
                                                                              (p-my-= $hi (p-if (p-< (p-+ $k 1) $slen) (p-hex (p-substr $arg (p-+ $k 1) 1)) 0))
                                                                              
                                                                              ;; $$result_ref .= chr(($hi << 4) | $lo)
                                                                              (p-.= (p-cast-$ $result_ref) (p-chr (p-bit-or (p-<< $hi 4) $lo)))
                                                                              
                                                                              )
                                                                            )
                                                                     )
                                                                   
                                                                   )
                                                                 )
                                                               ;; elsif ($ch eq 'u')
                                                               (p-if (setf --pcl-if-ret--4                               (p-str-eq $ch "u"))
                                                                     (progn
                                                                       (let (($cs (make-p-box nil)) ($ce (make-p-box nil)) ($chunk (make-p-box nil)) ($clen (make-p-box nil)) ($k (make-p-box nil)) ($b0 (make-p-box nil)) ($b1 (make-p-box nil)) ($b2 (make-p-box nil)) ($cm (make-p-box nil)) ($uu (make-p-box nil)))
                                                                         ;; for (my $cs = 0; $cs < $slen; $cs += 45) {             my $ce = $cs + 45 < $slen ? $cs + 45 : $slen;             my $chunk = substr($arg, $cs, $ce - $cs);             my $clen = length($chunk);             $$result_ref .= chr(32 + $clen);             for (my $k = 0; $k < $clen; $k += 3) {                 my $b0 = ord(substr($chunk,$k,1));                 my $b1 = $k+1 < $clen ? ord(substr($chunk,$k+1,1)) : 0;                 my $b2 = $k+2 < $clen ? ord(substr($chunk,$k+2,1)) : 0;                 my $cm = ($b0 << 16) | ($b1 << 8) | $b2;                 my $uu = sub { my $c = 32 + ($_[0] & 63); $c == 32 ? 96 : $c };                 $$result_ref .= chr($uu->(($cm>>18)&63)) . chr($uu->(($cm>>12)&63))                               . chr($uu->(($cm>> 6)&63)) . chr($uu->( $cm    &63));             }             $$result_ref .= "\n";         }
                                                                         (let (($cs (make-p-box nil)))
                                                                           (p-for (                                    (p-my-= $cs 0))
                                                                                  (                                    (p-< $cs $slen))
                                                                                  (                                    (p-incf $cs 45))
                                                                                  (let (($ce (make-p-box nil)) ($chunk (make-p-box nil)) ($clen (make-p-box nil)) ($k (make-p-box nil)) ($b0 (make-p-box nil)) ($b1 (make-p-box nil)) ($b2 (make-p-box nil)) ($cm (make-p-box nil)) ($uu (make-p-box nil)))
                                                                                    ;; my $ce = $cs + 45 < $slen ? $cs + 45 : $slen
                                                                                    (p-my-= $ce (p-if (p-< (p-+ $cs 45) $slen) (p-+ $cs 45) $slen))
                                                                                    
                                                                                    ;; my $chunk = substr($arg, $cs, $ce - $cs)
                                                                                    (p-my-= $chunk (p-substr $arg $cs (p-- $ce $cs)))
                                                                                    
                                                                                    ;; my $clen = length($chunk)
                                                                                    (p-my-= $clen (p-length $chunk))
                                                                                    
                                                                                    ;; $$result_ref .= chr(32 + $clen)
                                                                                    (let ((*wantarray* :void))                                           (p-.= (p-cast-$ $result_ref) (p-chr (p-+ 32 $clen))))
                                                                                    
                                                                                    ;; for (my $k = 0; $k < $clen; $k += 3) {                 my $b0 = ord(substr($chunk,$k,1));                 my $b1 = $k+1 < $clen ? ord(substr($chunk,$k+1,1)) : 0;                 my $b2 = $k+2 < $clen ? ord(substr($chunk,$k+2,1)) : 0;                 my $cm = ($b0 << 16) | ($b1 << 8) | $b2;                 my $uu = sub { my $c = 32 + ($_[0] & 63); $c == 32 ? 96 : $c };                 $$result_ref .= chr($uu->(($cm>>18)&63)) . chr($uu->(($cm>>12)&63))                               . chr($uu->(($cm>> 6)&63)) . chr($uu->( $cm    &63));             }
                                                                                    (let (($k (make-p-box nil)))
                                                                                      (p-for (                                          (p-my-= $k 0))
                                                                                             (                                          (p-< $k $clen))
                                                                                             (                                          (p-incf $k 3))
                                                                                             (let (($b0 (make-p-box nil)) ($b1 (make-p-box nil)) ($b2 (make-p-box nil)) ($cm (make-p-box nil)) ($uu (make-p-box nil)))
                                                                                               ;; my $b0 = ord(substr($chunk,$k,1))
                                                                                               (p-my-= $b0 (p-ord (p-substr $chunk $k 1)))
                                                                                               
                                                                                               ;; my $b1 = $k+1 < $clen ? ord(substr($chunk,$k+1,1)) : 0
                                                                                               (p-my-= $b1 (p-if (p-< (p-+ $k 1) $clen) (p-ord (p-substr $chunk (p-+ $k 1) 1)) 0))
                                                                                               
                                                                                               ;; my $b2 = $k+2 < $clen ? ord(substr($chunk,$k+2,1)) : 0
                                                                                               (p-my-= $b2 (p-if (p-< (p-+ $k 2) $clen) (p-ord (p-substr $chunk (p-+ $k 2) 1)) 0))
                                                                                               
                                                                                               ;; my $cm = ($b0 << 16) | ($b1 << 8) | $b2
                                                                                               (p-my-= $cm (p-bit-or (p-bit-or (p-<< $b0 16) (p-<< $b1 8)) $b2))
                                                                                               
                                                                                               ;; my $uu = sub { my $c = 32 + ($_[0] & 63); $c == 32 ? 96 : $c }
                                                                                               (p-my-= $uu (lambda (&rest %_args)
                                                                                                             (let ((@_ (p-flatten-args %_args))
                                                                                                                   (*pcl-caller-wantarray* *wantarray*))
                                                                                                               (catch :p-return
                                                                                                                 (block nil
                                                                                                                   (let (($c (make-p-box nil)))
                                                                                                                     ;; my $c = 32 + ($_[0] & 63)
                                                                                                                     (p-my-= $c (p-+ 32 (p-bit-and (p-aref @_ 0) 63)))
                                                                                                                     
                                                                                                                     ;; $c == 32 ? 96 : $c
                                                                                                                     (p-if (p-== $c 32) 96 $c)
                                                                                                                     
                                                                                                                     )
                                                                                                                   )
                                                                                                                 )
                                                                                                               )
                                                                                                             ))
                                                                                               
                                                                                               ;; $$result_ref .= chr($uu->(($cm>>18)&63)) . chr($uu->(($cm>>12)&63))                               . chr($uu->(($cm>> 6)&63)) . chr($uu->( $cm    &63))
                                                                                               (p-.= (p-cast-$ $result_ref) (p-. (p-. (p-. (p-chr (p-funcall-ref $uu (p-bit-and (p->> $cm 18) 63))) (p-chr (p-funcall-ref $uu (p-bit-and (p->> $cm 12) 63)))) (p-chr (p-funcall-ref $uu (p-bit-and (p->> $cm 6) 63)))) (p-chr (p-funcall-ref $uu (p-bit-and $cm 63)))))
                                                                                               
                                                                                               )
                                                                                             )
                                                                                      )
                                                                                    
                                                                                    ;; $$result_ref .= "\n"
                                                                                    (p-.= (p-cast-$ $result_ref) "
")
                                                                                    
                                                                                    )
                                                                                  )
                                                                           )
                                                                         
                                                                         )
                                                                       )
                                                                     nil
                                                                     )
                                                               )
                                                         )
                                                   )
                                             )
                                       )
                                 )
                           )
                     
                     )
                   )
                 )
               --pcl-if-ret--4)
             )
           )
         )
       )

;; sub _pack_utf8_char { ... }
(p-sub pl-_pack_utf8_char (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($code (make-p-box nil)) ($r (make-p-box nil)))
             ;; my ($code, $r) = @_
             (p-list-= (vector $code $r) @_)
             
             ;; if    ($code < 0x80)    { $$r .= chr($code) }     elsif ($code < 0x800)   { $$r .= chr(0xC0|($code>>6)) . chr(0x80|($code&0x3F)) }     elsif ($code < 0x10000) { $$r .= chr(0xE0|($code>>12)) . chr(0x80|(($code>>6)&0x3F)) . chr(0x80|($code&0x3F)) }     else  { $$r .= chr(0xF0|($code>>18)) . chr(0x80|(($code>>12)&0x3F)) . chr(0x80|(($code>>6)&0x3F)) . chr(0x80|($code&0x3F)) }
             ;; if ($code < 0x80)
             (p-if         (p-< $code #x80)
                           (progn
                             ;; $$r .= chr($code)
                             (p-.= (p-cast-$ $r) (p-chr $code))
                             
                             )
                           ;; elsif ($code < 0x800)
                           (p-if           (p-< $code #x800)
                                           (progn
                                             ;; $$r .= chr(0xC0|($code>>6)) . chr(0x80|($code&0x3F))
                                             (p-.= (p-cast-$ $r) (p-. (p-chr (p-bit-or #xC0 (p->> $code 6))) (p-chr (p-bit-or #x80 (p-bit-and $code #x3F)))))
                                             
                                             )
                                           ;; elsif ($code < 0x10000)
                                           (p-if             (p-< $code #x10000)
                                                             (progn
                                                               ;; $$r .= chr(0xE0|($code>>12)) . chr(0x80|(($code>>6)&0x3F)) . chr(0x80|($code&0x3F))
                                                               (p-.= (p-cast-$ $r) (p-. (p-. (p-chr (p-bit-or #xE0 (p->> $code 12))) (p-chr (p-bit-or #x80 (p-bit-and (p->> $code 6) #x3F)))) (p-chr (p-bit-or #x80 (p-bit-and $code #x3F)))))
                                                               
                                                               )
                                                             ;; else
                                                             (progn
                                                               ;; $$r .= chr(0xF0|($code>>18)) . chr(0x80|(($code>>12)&0x3F)) . chr(0x80|(($code>>6)&0x3F)) . chr(0x80|($code&0x3F))
                                                               (p-.= (p-cast-$ $r) (p-. (p-. (p-. (p-chr (p-bit-or #xF0 (p->> $code 18))) (p-chr (p-bit-or #x80 (p-bit-and (p->> $code 12) #x3F)))) (p-chr (p-bit-or #x80 (p-bit-and (p->> $code 6) #x3F)))) (p-chr (p-bit-or #x80 (p-bit-and $code #x3F)))))
                                                               
                                                               )
                                                             )
                                           )
                           )
             
             )
           )
         )
       )

;; sub _pack_tmpl { ... }
(p-sub pl-_pack_tmpl (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($r (make-p-box nil)) ($k (make-p-box nil)))
             (let (($tmpl (make-p-box nil)) ($ai_ref (make-p-box nil)) ($args_ref (make-p-box nil)) ($result_ref (make-p-box nil)) ($inh_be (make-p-box nil)) ($inh_le (make-p-box nil)))
               ;; my ($tmpl, $ai_ref, $args_ref, $result_ref, $inh_be, $inh_le) = @_
               (p-list-= (vector $tmpl $ai_ref $args_ref $result_ref $inh_be $inh_le) @_)
               
               (let (($nargs (make-p-box nil)))
                 ;; my $nargs = scalar(@$args_ref)
                 (p-my-= $nargs (p-scalar (p-cast-@ $args_ref)))
                 
                 (let (($ti (make-p-box nil)))
                   ;; my $ti = 0
                   (p-my-= $ti 0)
                   
                   (let (($tlen (make-p-box nil)))
                     ;; my $tlen = length($tmpl)
                     (p-my-= $tlen (p-length $tmpl))
                     
                     (let (($ch (make-p-box nil)) ($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)) ($bang (make-p-box nil)) ($be (make-p-box nil)) ($le (make-p-box nil)) ($star (make-p-box nil)) ($count (make-p-box nil)) ($nrep (make-p-box nil)) ($darg (make-p-box nil)) ($dlen (make-p-box nil)) ($nb (make-p-box nil)) ($sig (make-p-box nil)) ($dbe (make-p-box nil)) ($dfmt (make-p-box nil)) ($dbang (make-p-box nil)) ($dbe2 (make-p-box nil)) ($dle2 (make-p-box nil)) ($dstar2 (make-p-box nil)) ($dcnt2 (make-p-box nil)) ($dnrep2 (make-p-box nil)) ($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)) ($inner (make-p-box nil)) ($fp (make-p-box nil)) ($t (make-p-box nil)) ($be2 (make-p-box nil)) ($v (make-p-box nil)) ($arg (make-p-box nil)) (@bytes (make-array 0 :adjustable t :fill-pointer 0)))
                       ;; while (1) {         $ti = _pack_skip_ws($tmpl, $ti);         last if $ti >= $tlen;         my $ch = substr($tmpl, $ti, 1); $ti++;         my ($grpbeg, $grpend) = (undef, undef);         if ($ch eq '(') {             $grpend = _pack_find_group_end($tmpl, $ti);             $grpbeg = $ti; $ti = $grpend + 1; $ch = '(';         }         my ($bang, $be, $le) = _pack_parse_mods($tmpl, \$ti, $inh_be, $inh_le, $ch, 'pack');         $ti = _pack_skip_ws($tmpl, $ti);         my ($star, $count, $nrep) = _pack_parse_count($tmpl, \$ti);         $ti = _pack_skip_ws($tmpl, $ti);          # Slash â count prefix: ch encodes length, next char is data format         if ($ti < $tlen && substr($tmpl, $ti, 1) eq '/') {             $ti++;             my $darg = ($$ai_ref < $nargs) ? $args_ref->[$$ai_ref++] : '';             $darg = '' unless defined $darg;             my $dlen = length($darg);             my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang);             if ($nb) {                 $$result_ref .= _pack_emit_int($dlen, $nb, $sig, $be ? 1 : ($le ? 0 : $dbe));             }             $ti = _pack_skip_ws($tmpl, $ti); last if $ti >= $tlen;             my $dfmt = substr($tmpl, $ti, 1); $ti++;             my ($dbang, $dbe2, $dle2) = _pack_parse_mods($tmpl, \$ti, $be, $le, $dfmt, 'pack');             $ti = _pack_skip_ws($tmpl, $ti);             my ($dstar2, $dcnt2, $dnrep2) = _pack_parse_count($tmpl, \$ti);             _pack_str_one($dfmt, $darg, $dlen, 0, $result_ref)                 if $dfmt eq 'a' || $dfmt eq 'A' || $dfmt eq 'Z';             if ($dfmt ne 'a' && $dfmt ne 'A' && $dfmt ne 'Z') {                 my ($dnb, $dsig, $ddbe) = _pack_type_info($dfmt, $dbang);                 if ($dnb) {                     $$result_ref .= _pack_emit_int($dlen, $dnb, $dsig, $dbe2 ? 1 : ($dle2 ? 0 : $ddbe));                 }             }             next;         }          # Group         if (defined $grpbeg) {             my $inner = substr($tmpl, $grpbeg, $grpend - $grpbeg);             if ($star) {                 while ($$ai_ref < $nargs) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }             } else {                 for (my $r = 0; $r < $nrep; $r++) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }             }             next;         }          # Position ops (no arg)         if ($ch eq 'x') { for (my $k=0;$k<$nrep;$k++) { $$result_ref .= chr(0) } next }         if ($ch eq 'X') {             my $fp = length($$result_ref) - $nrep;             $$result_ref = substr($$result_ref, 0, $fp < 0 ? 0 : $fp); next;         }         if ($ch eq '@') {             my $t = defined($count) ? $count : 0;             if (length($$result_ref) < $t) { $$result_ref .= chr(0) x ($t - length($$result_ref)) }             else { $$result_ref = substr($$result_ref, 0, $t) }             next;         }         if ($ch eq 'p' || $ch eq 'P' || $ch eq 'D') {             $$ai_ref++ if $$ai_ref < $nargs; next;         }          # Integer types         my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang);         if ($nb) {             my $be2 = $be ? 1 : ($le ? 0 : $dbe);             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_emit_int(int($v+0), $nb, $sig, $be2);             }             next;         }          # Float types         if ($ch eq 'f') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_float32($v+0.0, $be2);             }             next;         }         if ($ch eq 'd' || $ch eq 'F') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_float64($v+0.0, $be2);             }             next;         }          # Single-arg string types         if ($ch eq 'a'||$ch eq 'A'||$ch eq 'Z'||$ch eq 'b'||$ch eq 'B'||             $ch eq 'H'||$ch eq 'h'||$ch eq 'u') {             my $arg = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // '') : '';             _pack_str_one($ch, $arg, $nrep, $star, $result_ref);             next;         }          # Per-rep types: U W w         if ($ch eq 'U') {             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 _pack_utf8_char(int($v+0), $result_ref);             }             next;         }         if ($ch eq 'W') {             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= chr(int($v+0));             }             next;         }         if ($ch eq 'w') {             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $v = $v + 0;                 die "Cannot compress negative numbers in pack\n" if $v < 0;                 die "Cannot compress Inf in pack\n" if $v != 0 && $v == $v * 2;                 die "Can only compress unsigned integers in pack\n" if $v != int($v);                 $v = int($v);                 if ($v == 0) { $$result_ref .= chr(0); next }                 my @bytes;                 while ($v > 0) { unshift @bytes, ($v & 0x7F); $v >>= 7 }                 for (my $k = 0; $k < $#bytes; $k++) { $$result_ref .= chr($bytes[$k] | 0x80) }                 $$result_ref .= chr($bytes[-1]);             }             next;         }          die "Invalid type '$ch' in pack\n";     }
                       (p-while 1
                                (let (($ch (make-p-box nil)) ($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)) ($bang (make-p-box nil)) ($be (make-p-box nil)) ($le (make-p-box nil)) ($star (make-p-box nil)) ($count (make-p-box nil)) ($nrep (make-p-box nil)) ($darg (make-p-box nil)) ($dlen (make-p-box nil)) ($nb (make-p-box nil)) ($sig (make-p-box nil)) ($dbe (make-p-box nil)) ($dfmt (make-p-box nil)) ($dbang (make-p-box nil)) ($dbe2 (make-p-box nil)) ($dle2 (make-p-box nil)) ($dstar2 (make-p-box nil)) ($dcnt2 (make-p-box nil)) ($dnrep2 (make-p-box nil)) ($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)) ($inner (make-p-box nil)) ($r (make-p-box nil)) ($k (make-p-box nil)) ($fp (make-p-box nil)) ($t (make-p-box nil)) ($be2 (make-p-box nil)) ($v (make-p-box nil)) ($arg (make-p-box nil)) (@bytes (make-array 0 :adjustable t :fill-pointer 0)))
                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                  (p-my-= $ti (pl-_pack_skip_ws $tmpl $ti))
                                  
                                  ;; last if $ti >= $tlen
                                  (p-if                       (p->= $ti $tlen)                       (p-last))
                                  
                                  ;; my $ch = substr($tmpl, $ti, 1)
                                  (p-my-= $ch (p-substr $tmpl $ti 1))
                                  
                                  ;; $ti++
                                  (p-post++ $ti)
                                  
                                  ;; my ($grpbeg, $grpend) = (undef, undef)
                                  (p-list-= (vector $grpbeg $grpend) (vector (p-undef) (p-undef)))
                                  
                                  ;; if ($ch eq '(') {             $grpend = _pack_find_group_end($tmpl, $ti);             $grpbeg = $ti; $ti = $grpend + 1; $ch = '(';         }
                                  ;; if ($ch eq '(')
                                  (p-if                       (p-str-eq $ch "(")
                                                              (progn
                                                                ;; $grpend = _pack_find_group_end($tmpl, $ti)
                                                                (p-my-= $grpend (pl-_pack_find_group_end $tmpl $ti))
                                                                
                                                                ;; $grpbeg = $ti
                                                                (p-my-= $grpbeg $ti)
                                                                
                                                                ;; $ti = $grpend + 1
                                                                (p-my-= $ti (p-+ $grpend 1))
                                                                
                                                                ;; $ch = '('
                                                                (p-my-= $ch "(")
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; my ($bang, $be, $le) = _pack_parse_mods($tmpl, \$ti, $inh_be, $inh_le, $ch, 'pack')
                                  (p-list-= (vector $bang $be $le) (let ((*wantarray* t)) (pl-_pack_parse_mods $tmpl (p-backslash $ti) $inh_be $inh_le $ch "pack")))
                                  
                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                  (let ((*wantarray* :void))                       (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                  
                                  ;; my ($star, $count, $nrep) = _pack_parse_count($tmpl, \$ti)
                                  (p-list-= (vector $star $count $nrep) (let ((*wantarray* t)) (pl-_pack_parse_count $tmpl (p-backslash $ti))))
                                  
                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                  (let ((*wantarray* :void))                       (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                  
                                  ;; if ($ti < $tlen && substr($tmpl, $ti, 1) eq '/') {             $ti++;             my $darg = ($$ai_ref < $nargs) ? $args_ref->[$$ai_ref++] : '';             $darg = '' unless defined $darg;             my $dlen = length($darg);             my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang);             if ($nb) {                 $$result_ref .= _pack_emit_int($dlen, $nb, $sig, $be ? 1 : ($le ? 0 : $dbe));             }             $ti = _pack_skip_ws($tmpl, $ti); last if $ti >= $tlen;             my $dfmt = substr($tmpl, $ti, 1); $ti++;             my ($dbang, $dbe2, $dle2) = _pack_parse_mods($tmpl, \$ti, $be, $le, $dfmt, 'pack');             $ti = _pack_skip_ws($tmpl, $ti);             my ($dstar2, $dcnt2, $dnrep2) = _pack_parse_count($tmpl, \$ti);             _pack_str_one($dfmt, $darg, $dlen, 0, $result_ref)                 if $dfmt eq 'a' || $dfmt eq 'A' || $dfmt eq 'Z';             if ($dfmt ne 'a' && $dfmt ne 'A' && $dfmt ne 'Z') {                 my ($dnb, $dsig, $ddbe) = _pack_type_info($dfmt, $dbang);                 if ($dnb) {                     $$result_ref .= _pack_emit_int($dlen, $dnb, $dsig, $dbe2 ? 1 : ($dle2 ? 0 : $ddbe));                 }             }             next;         }
                                  ;; if ($ti < $tlen && substr($tmpl, $ti, 1) eq '/')
                                  (p-if                       (p-&& (p-< $ti $tlen) (p-str-eq (p-substr $tmpl $ti 1) "/"))
                                                              (progn
                                                                (let (($darg (make-p-box nil)) ($dlen (make-p-box nil)) ($nb (make-p-box nil)) ($sig (make-p-box nil)) ($dbe (make-p-box nil)) ($dfmt (make-p-box nil)) ($dbang (make-p-box nil)) ($dbe2 (make-p-box nil)) ($dle2 (make-p-box nil)) ($dstar2 (make-p-box nil)) ($dcnt2 (make-p-box nil)) ($dnrep2 (make-p-box nil)) ($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)))
                                                                  ;; $ti++
                                                                  (let ((*wantarray* :void))                             (p-post++ $ti))
                                                                  
                                                                  ;; my $darg = ($$ai_ref < $nargs) ? $args_ref->[$$ai_ref++] : ''
                                                                  (p-my-= $darg (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) ""))
                                                                  
                                                                  ;; $darg = '' unless defined $darg
                                                                  (let ((*wantarray* :void)) (p-unless                             (p-defined $darg)                             (p-my-= $darg "")))
                                                                  
                                                                  ;; my $dlen = length($darg)
                                                                  (p-my-= $dlen (p-length $darg))
                                                                  
                                                                  ;; my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang)
                                                                  (p-list-= (vector $nb $sig $dbe) (let ((*wantarray* t)) (pl-_pack_type_info $ch $bang)))
                                                                  
                                                                  ;; if ($nb) {                 $$result_ref .= _pack_emit_int($dlen, $nb, $sig, $be ? 1 : ($le ? 0 : $dbe));             }
                                                                  ;; if ($nb)
                                                                  (p-if                             $nb
                                                                                                    (progn
                                                                                                      ;; $$result_ref .= _pack_emit_int($dlen, $nb, $sig, $be ? 1 : ($le ? 0 : $dbe))
                                                                                                      (p-.= (p-cast-$ $result_ref) (pl-_pack_emit_int $dlen $nb $sig (p-if $be 1 (p-if $le 0 $dbe))))
                                                                                                      
                                                                                                      )
                                                                                                    nil
                                                                                                    )
                                                                  
                                                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                                                  (let ((*wantarray* :void))                             (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                                                  
                                                                  ;; last if $ti >= $tlen
                                                                  (let ((*wantarray* :void)) (p-if                             (p->= $ti $tlen)                             (p-last)))
                                                                  
                                                                  ;; my $dfmt = substr($tmpl, $ti, 1)
                                                                  (p-my-= $dfmt (p-substr $tmpl $ti 1))
                                                                  
                                                                  ;; $ti++
                                                                  (let ((*wantarray* :void))                             (p-post++ $ti))
                                                                  
                                                                  ;; my ($dbang, $dbe2, $dle2) = _pack_parse_mods($tmpl, \$ti, $be, $le, $dfmt, 'pack')
                                                                  (p-list-= (vector $dbang $dbe2 $dle2) (let ((*wantarray* t)) (pl-_pack_parse_mods $tmpl (p-backslash $ti) $be $le $dfmt "pack")))
                                                                  
                                                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                                                  (let ((*wantarray* :void))                             (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                                                  
                                                                  ;; my ($dstar2, $dcnt2, $dnrep2) = _pack_parse_count($tmpl, \$ti)
                                                                  (p-list-= (vector $dstar2 $dcnt2 $dnrep2) (let ((*wantarray* t)) (pl-_pack_parse_count $tmpl (p-backslash $ti))))
                                                                  
                                                                  ;; _pack_str_one($dfmt, $darg, $dlen, 0, $result_ref)                 if $dfmt eq 'a' || $dfmt eq 'A' || $dfmt eq 'Z'
                                                                  (let ((*wantarray* :void)) (p-if                             (p-|| (p-|| (p-str-eq $dfmt "a") (p-str-eq $dfmt "A")) (p-str-eq $dfmt "Z"))                             (let ((*wantarray* :void)) (pl-_pack_str_one $dfmt $darg $dlen 0 $result_ref))))
                                                                  
                                                                  ;; if ($dfmt ne 'a' && $dfmt ne 'A' && $dfmt ne 'Z') {                 my ($dnb, $dsig, $ddbe) = _pack_type_info($dfmt, $dbang);                 if ($dnb) {                     $$result_ref .= _pack_emit_int($dlen, $dnb, $dsig, $dbe2 ? 1 : ($dle2 ? 0 : $ddbe));                 }             }
                                                                  ;; if ($dfmt ne 'a' && $dfmt ne 'A' && $dfmt ne 'Z')
                                                                  (p-if                             (p-&& (p-&& (p-str-ne $dfmt "a") (p-str-ne $dfmt "A")) (p-str-ne $dfmt "Z"))
                                                                                                    (progn
                                                                                                      (let (($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)))
                                                                                                        (let ((--pcl-if-ret--7 nil))
                                                                                                          ;; my ($dnb, $dsig, $ddbe) = _pack_type_info($dfmt, $dbang)
                                                                                                          (p-list-= (vector $dnb $dsig $ddbe) (let ((*wantarray* t)) (pl-_pack_type_info $dfmt $dbang)))
                                                                                                          
                                                                                                          ;; if ($dnb) {                     $$result_ref .= _pack_emit_int($dlen, $dnb, $dsig, $dbe2 ? 1 : ($dle2 ? 0 : $ddbe));                 }
                                                                                                          ;; if ($dnb)
                                                                                                          (p-if (setf --pcl-if-ret--7                                     $dnb)
                                                                                                                (progn
                                                                                                                  ;; $$result_ref .= _pack_emit_int($dlen, $dnb, $dsig, $dbe2 ? 1 : ($dle2 ? 0 : $ddbe))
                                                                                                                  (setf --pcl-if-ret--7                                         (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_emit_int $dlen $dnb $dsig (p-if $dbe2 1 (p-if $dle2 0 $ddbe))))))
                                                                                                                  
                                                                                                                  )
                                                                                                                nil
                                                                                                                )
                                                                                                          
                                                                                                          --pcl-if-ret--7)
                                                                                                        )
                                                                                                      )
                                                                                                    nil
                                                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if (defined $grpbeg) {             my $inner = substr($tmpl, $grpbeg, $grpend - $grpbeg);             if ($star) {                 while ($$ai_ref < $nargs) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }             } else {                 for (my $r = 0; $r < $nrep; $r++) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }             }             next;         }
                                  ;; if (defined $grpbeg)
                                  (p-if                       (p-defined $grpbeg)
                                                              (progn
                                                                (let (($inner (make-p-box nil)) ($r (make-p-box nil)))
                                                                  ;; my $inner = substr($tmpl, $grpbeg, $grpend - $grpbeg)
                                                                  (p-my-= $inner (p-substr $tmpl $grpbeg (p-- $grpend $grpbeg)))
                                                                  
                                                                  ;; if ($star) {                 while ($$ai_ref < $nargs) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }             } else {                 for (my $r = 0; $r < $nrep; $r++) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }             }
                                                                  ;; if ($star)
                                                                  (p-if                             $star
                                                                                                    (progn
                                                                                                      ;; while ($$ai_ref < $nargs) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }
                                                                                                      (p-while (p-< (p-cast-$ $ai_ref) $nargs)
                                                                                                               ;; _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le)
                                                                                                               (pl-_pack_tmpl $inner $ai_ref $args_ref $result_ref $be $le)
                                                                                                               )
                                                                                                      
                                                                                                      )
                                                                                                    ;; else
                                                                                                    (progn
                                                                                                      (let (($r (make-p-box nil)))
                                                                                                        ;; for (my $r = 0; $r < $nrep; $r++) {                     _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le);                 }
                                                                                                        (let (($r (make-p-box nil)))
                                                                                                          (p-for (                                  (p-my-= $r 0))
                                                                                                                 (                                  (p-< $r $nrep))
                                                                                                                 (                                  (p-post++ $r))
                                                                                                                 ;; _pack_tmpl($inner, $ai_ref, $args_ref, $result_ref, $be, $le)
                                                                                                                 (pl-_pack_tmpl $inner $ai_ref $args_ref $result_ref $be $le)
                                                                                                                 
                                                                                                                 )
                                                                                                          )
                                                                                                        
                                                                                                        )
                                                                                                      )
                                                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'x') { for (my $k=0;$k<$nrep;$k++) { $$result_ref .= chr(0) } next }
                                  ;; if ($ch eq 'x')
                                  (p-if                       (p-str-eq $ch "x")
                                                              (progn
                                                                (let (($k (make-p-box nil)))
                                                                  ;; for (my $k=0;$k<$nrep;$k++) { $$result_ref .= chr(0) }
                                                                  (let (($k (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $k 0))
                                                                           (                            (p-< $k $nrep))
                                                                           (                            (p-post++ $k))
                                                                           ;; $$result_ref .= chr(0)
                                                                           (p-.= (p-cast-$ $result_ref) (p-chr 0))
                                                                           
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'X') {             my $fp = length($$result_ref) - $nrep;             $$result_ref = substr($$result_ref, 0, $fp < 0 ? 0 : $fp); next;         }
                                  ;; if ($ch eq 'X')
                                  (p-if                       (p-str-eq $ch "X")
                                                              (progn
                                                                (let (($fp (make-p-box nil)))
                                                                  ;; my $fp = length($$result_ref) - $nrep
                                                                  (p-my-= $fp (p-- (p-length (p-cast-$ $result_ref)) $nrep))
                                                                  
                                                                  ;; $$result_ref = substr($$result_ref, 0, $fp < 0 ? 0 : $fp)
                                                                  (let ((*wantarray* :void))                             (p-setf (p-cast-$ $result_ref) (p-substr (p-cast-$ $result_ref) 0 (p-if (p-< $fp 0) 0 $fp))))
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq '@') {             my $t = defined($count) ? $count : 0;             if (length($$result_ref) < $t) { $$result_ref .= chr(0) x ($t - length($$result_ref)) }             else { $$result_ref = substr($$result_ref, 0, $t) }             next;         }
                                  ;; if ($ch eq '@')
                                  (p-if                       (p-str-eq $ch "@")
                                                              (progn
                                                                (let (($t (make-p-box nil)))
                                                                  ;; my $t = defined($count) ? $count : 0
                                                                  (p-my-= $t (p-if (p-defined $count) $count 0))
                                                                  
                                                                  ;; if (length($$result_ref) < $t) { $$result_ref .= chr(0) x ($t - length($$result_ref)) }             else { $$result_ref = substr($$result_ref, 0, $t) }
                                                                  ;; if (length($$result_ref) < $t)
                                                                  (p-if                             (p-< (p-length (p-cast-$ $result_ref)) $t)
                                                                                                    (progn
                                                                                                      ;; $$result_ref .= chr(0) x ($t - length($$result_ref))
                                                                                                      (p-.= (p-cast-$ $result_ref) (p-str-x (p-chr 0) (p-- $t (p-length (p-cast-$ $result_ref)))))
                                                                                                      
                                                                                                      )
                                                                                                    ;; else
                                                                                                    (progn
                                                                                                      ;; $$result_ref = substr($$result_ref, 0, $t)
                                                                                                      (p-setf (p-cast-$ $result_ref) (p-substr (p-cast-$ $result_ref) 0 $t))
                                                                                                      
                                                                                                      )
                                                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'p' || $ch eq 'P' || $ch eq 'D') {             $$ai_ref++ if $$ai_ref < $nargs; next;         }
                                  ;; if ($ch eq 'p' || $ch eq 'P' || $ch eq 'D')
                                  (p-if                       (p-|| (p-|| (p-str-eq $ch "p") (p-str-eq $ch "P")) (p-str-eq $ch "D"))
                                                              (progn
                                                                ;; $$ai_ref++ if $$ai_ref < $nargs
                                                                (let ((*wantarray* :void)) (p-if                           (p-< (p-cast-$ $ai_ref) $nargs)                           (p-post++ (p-cast-$ $ai_ref))))
                                                                
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; $nrep = $nargs - $$ai_ref if $star
                                  (when (p-true-p $star)
                                    (p-my-= $nrep (p-- $nargs (p-cast-$ $ai_ref))))

                                  ;; my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang)
                                  (p-list-= (vector $nb $sig $dbe) (let ((*wantarray* t)) (pl-_pack_type_info $ch $bang)))

                                  ;; if ($nb) {             my $be2 = $be ? 1 : ($le ? 0 : $dbe);             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_emit_int(int($v+0), $nb, $sig, $be2);             }             next;         }
                                  ;; if ($nb)
                                  (p-if                       $nb
                                                              (progn
                                                                (let (($be2 (make-p-box nil)) ($r (make-p-box nil)) ($v (make-p-box nil)))
                                                                  ;; my $be2 = $be ? 1 : ($le ? 0 : $dbe)
                                                                  (p-my-= $be2 (p-if $be 1 (p-if $le 0 $dbe)))
                                                                  
                                                                  ;; for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_emit_int(int($v+0), $nb, $sig, $be2);             }
                                                                  (let (($r (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $r 0))
                                                                           (                            (p-< $r $nrep))
                                                                           (                            (p-post++ $r))
                                                                           (let (($v (make-p-box nil)))
                                                                             ;; my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0
                                                                             (p-my-= $v (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                                                             
                                                                             ;; $$result_ref .= _pack_emit_int(int($v+0), $nb, $sig, $be2)
                                                                             (p-.= (p-cast-$ $result_ref) (pl-_pack_emit_int (p-int (p-+ $v 0)) $nb $sig $be2))
                                                                             
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'f') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_float32($v+0.0, $be2);             }             next;         }
                                  ;; if ($ch eq 'f')
                                  (p-if                       (p-str-eq $ch "f")
                                                              (progn
                                                                (let (($be2 (make-p-box nil)) ($r (make-p-box nil)) ($v (make-p-box nil)))
                                                                  ;; my $be2 = $be ? 1 : ($le ? 0 : 0)
                                                                  (p-my-= $be2 (p-if $be 1 (p-if $le 0 0)))
                                                                  
                                                                  ;; for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_float32($v+0.0, $be2);             }
                                                                  (let (($r (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $r 0))
                                                                           (                            (p-< $r $nrep))
                                                                           (                            (p-post++ $r))
                                                                           (let (($v (make-p-box nil)))
                                                                             ;; my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0
                                                                             (p-my-= $v (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                                                             
                                                                             ;; $$result_ref .= _pack_float32($v+0.0, $be2)
                                                                             (p-.= (p-cast-$ $result_ref) (pl-_pack_float32 (p-+ $v 0.0) $be2))
                                                                             
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'd' || $ch eq 'F') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_float64($v+0.0, $be2);             }             next;         }
                                  ;; if ($ch eq 'd' || $ch eq 'F')
                                  (p-if                       (p-|| (p-str-eq $ch "d") (p-str-eq $ch "F"))
                                                              (progn
                                                                (let (($be2 (make-p-box nil)) ($r (make-p-box nil)) ($v (make-p-box nil)))
                                                                  ;; my $be2 = $be ? 1 : ($le ? 0 : 0)
                                                                  (p-my-= $be2 (p-if $be 1 (p-if $le 0 0)))
                                                                  
                                                                  ;; for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= _pack_float64($v+0.0, $be2);             }
                                                                  (let (($r (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $r 0))
                                                                           (                            (p-< $r $nrep))
                                                                           (                            (p-post++ $r))
                                                                           (let (($v (make-p-box nil)))
                                                                             ;; my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0
                                                                             (p-my-= $v (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                                                             
                                                                             ;; $$result_ref .= _pack_float64($v+0.0, $be2)
                                                                             (p-.= (p-cast-$ $result_ref) (pl-_pack_float64 (p-+ $v 0.0) $be2))
                                                                             
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'a'||$ch eq 'A'||$ch eq 'Z'||$ch eq 'b'||$ch eq 'B'||             $ch eq 'H'||$ch eq 'h'||$ch eq 'u') {             my $arg = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // '') : '';             _pack_str_one($ch, $arg, $nrep, $star, $result_ref);             next;         }
                                  ;; if ($ch eq 'a'||$ch eq 'A'||$ch eq 'Z'||$ch eq 'b'||$ch eq 'B'||
                                  ;;             $ch eq 'H'||$ch eq 'h'||$ch eq 'u')
                                  (p-if                       (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-str-eq $ch "a") (p-str-eq $ch "A")) (p-str-eq $ch "Z")) (p-str-eq $ch "b")) (p-str-eq $ch "B")) (p-str-eq $ch "H")) (p-str-eq $ch "h")) (p-str-eq $ch "u"))
                                                              (progn
                                                                (let (($arg (make-p-box nil)))
                                                                  ;; my $arg = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // '') : ''
                                                                  (p-my-= $arg (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) "") ""))
                                                                  
                                                                  ;; _pack_str_one($ch, $arg, $nrep, $star, $result_ref)
                                                                  (let ((*wantarray* :void))                             (let ((*wantarray* :void)) (pl-_pack_str_one $ch $arg $nrep $star $result_ref)))
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'U') {             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 _pack_utf8_char(int($v+0), $result_ref);             }             next;         }
                                  ;; if ($ch eq 'U')
                                  (p-if                       (p-str-eq $ch "U")
                                                              (progn
                                                                (let (($r (make-p-box nil)) ($v (make-p-box nil)))
                                                                  ;; for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 _pack_utf8_char(int($v+0), $result_ref);             }
                                                                  (let (($r (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $r 0))
                                                                           (                            (p-< $r $nrep))
                                                                           (                            (p-post++ $r))
                                                                           (let (($v (make-p-box nil)))
                                                                             ;; my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0
                                                                             (p-my-= $v (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                                                             
                                                                             ;; _pack_utf8_char(int($v+0), $result_ref)
                                                                             (pl-_pack_utf8_char (p-int (p-+ $v 0)) $result_ref)
                                                                             
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'W') {             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= chr(int($v+0));             }             next;         }
                                  ;; if ($ch eq 'W')
                                  (p-if                       (p-str-eq $ch "W")
                                                              (progn
                                                                (let (($r (make-p-box nil)) ($v (make-p-box nil)))
                                                                  ;; for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $$result_ref .= chr(int($v+0));             }
                                                                  (let (($r (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $r 0))
                                                                           (                            (p-< $r $nrep))
                                                                           (                            (p-post++ $r))
                                                                           (let (($v (make-p-box nil)))
                                                                             ;; my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0
                                                                             (p-my-= $v (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                                                             
                                                                             ;; $$result_ref .= chr(int($v+0))
                                                                             (p-.= (p-cast-$ $result_ref) (p-chr (p-int (p-+ $v 0))))
                                                                             
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'w') {             for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $v = $v + 0;                 die "Cannot compress negative numbers in pack\n" if $v < 0;                 die "Cannot compress Inf in pack\n" if $v != 0 && $v == $v * 2;                 die "Can only compress unsigned integers in pack\n" if $v != int($v);                 $v = int($v);                 if ($v == 0) { $$result_ref .= chr(0); next }                 my @bytes;                 while ($v > 0) { unshift @bytes, ($v & 0x7F); $v >>= 7 }                 for (my $k = 0; $k < $#bytes; $k++) { $$result_ref .= chr($bytes[$k] | 0x80) }                 $$result_ref .= chr($bytes[-1]);             }             next;         }
                                  ;; if ($ch eq 'w')
                                  (p-if                       (p-str-eq $ch "w")
                                                              (progn
                                                                (let (($r (make-p-box nil)) ($v (make-p-box nil)) (@bytes (make-array 0 :adjustable t :fill-pointer 0)) ($k (make-p-box nil)))
                                                                  ;; for (my $r = 0; $r < $nrep; $r++) {                 my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0;                 $v = $v + 0;                 die "Cannot compress negative numbers in pack\n" if $v < 0;                 die "Cannot compress Inf in pack\n" if $v != 0 && $v == $v * 2;                 die "Can only compress unsigned integers in pack\n" if $v != int($v);                 $v = int($v);                 if ($v == 0) { $$result_ref .= chr(0); next }                 my @bytes;                 while ($v > 0) { unshift @bytes, ($v & 0x7F); $v >>= 7 }                 for (my $k = 0; $k < $#bytes; $k++) { $$result_ref .= chr($bytes[$k] | 0x80) }                 $$result_ref .= chr($bytes[-1]);             }
                                                                  (let (($r (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $r 0))
                                                                           (                            (p-< $r $nrep))
                                                                           (                            (p-post++ $r))
                                                                           (let (($v (make-p-box nil)) (@bytes (make-array 0 :adjustable t :fill-pointer 0)) ($k (make-p-box nil)))
                                                                             ;; my $v = ($$ai_ref < $nargs) ? ($args_ref->[$$ai_ref++] // 0) : 0
                                                                             (p-my-= $v (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                                                             
                                                                             ;; $v = $v + 0
                                                                             (let ((*wantarray* :void))                                   (p-my-= $v (p-+ $v 0)))
                                                                             
                                                                             ;; die "Cannot compress negative numbers in pack\n" if $v < 0
                                                                             (let ((*wantarray* :void)) (p-if                                   (p-< $v 0)                                   (p-die "Cannot compress negative numbers in pack
")))
                                                                             
                                                                             ;; die "Cannot compress Inf in pack\n" if $v != 0 && $v == $v * 2
                                                                             (let ((*wantarray* :void)) (p-if                                   (p-&& (p-!= $v 0) (p-== $v (p-* $v 2)))                                   (p-die "Cannot compress Inf in pack
")))
                                                                             
                                                                             ;; die "Can only compress unsigned integers in pack\n" if $v != int($v)
                                                                             (let ((*wantarray* :void)) (p-if                                   (p-!= $v (p-int $v))                                   (p-die "Can only compress unsigned integers in pack
")))
                                                                             
                                                                             ;; $v = int($v)
                                                                             (let ((*wantarray* :void))                                   (p-my-= $v (p-int $v)))
                                                                             
                                                                             ;; if ($v == 0) { $$result_ref .= chr(0); next }
                                                                             ;; if ($v == 0)
                                                                             (p-if                                   (p-== $v 0)
                                                                                                                     (progn
                                                                                                                       ;; $$result_ref .= chr(0)
                                                                                                                       (let ((*wantarray* :void))                                       (p-.= (p-cast-$ $result_ref) (p-chr 0)))
                                                                                                                       
                                                                                                                       ;; next
                                                                                                                       (p-next)
                                                                                                                       
                                                                                                                       )
                                                                                                                     nil
                                                                                                                     )
                                                                             
                                                                             ;; my @bytes (bare declaration)
                                                                             
                                                                             ;; while ($v > 0) { unshift @bytes, ($v & 0x7F); $v >>= 7 }
                                                                             (p-while (p-> $v 0)
                                                                                      ;; unshift @bytes, ($v & 0x7F)
                                                                                      (let ((*wantarray* :void))                                     (p-unshift @bytes (p-bit-and $v #x7F)))
                                                                                      
                                                                                      ;; $v >>= 7
                                                                                      (p->>= $v 7)
                                                                                      
                                                                                      )
                                                                             
                                                                             ;; for (my $k = 0; $k < $#bytes; $k++) { $$result_ref .= chr($bytes[$k] | 0x80) }
                                                                             (let (($k (make-p-box nil)))
                                                                               (p-for (                                  (p-my-= $k 0))
                                                                                      (                                  (p-< $k (p-array-last-index @bytes)))
                                                                                      (                                  (p-post++ $k))
                                                                                      ;; $$result_ref .= chr($bytes[$k] | 0x80)
                                                                                      (p-.= (p-cast-$ $result_ref) (p-chr (p-bit-or (p-aref @bytes $k) #x80)))
                                                                                      
                                                                                      )
                                                                               )
                                                                             
                                                                             ;; $$result_ref .= chr($bytes[-1])
                                                                             (p-.= (p-cast-$ $result_ref) (p-chr (p-aref @bytes -1)))
                                                                             
                                                                             )
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; die "Invalid type '$ch' in pack\n"
                                  (p-die (p-string-concat "Invalid type '" $ch "' in pack
"))
                                  
                                  )
                                )
                       
                       )
                     )
                   )
                 )
               )
             )
           )
         )
       )

;; sub p_pack { ... }
(p-sub pl-p_pack (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($tmpl (make-p-box nil)) (@args (make-array 0 :adjustable t :fill-pointer 0)))
             ;; my ($tmpl, @args) = @_
             (p-list-= (vector $tmpl @args) @_)
             
             (let (($result (make-p-box nil)))
               ;; my $result = ''
               (p-my-= $result "")
               
               (let (($ai (make-p-box nil)))
                 ;; my $ai = 0
                 (p-my-= $ai 0)
                 
                 ;; _pack_tmpl($tmpl, \$ai, \@args, \$result, 0, 0)
                 (let ((*wantarray* :void))             (let ((*wantarray* :void)) (pl-_pack_tmpl $tmpl (p-backslash $ai) (p-backslash @args) (p-backslash $result) 0 0)))
                 
                 ;; return $result
                 (p-return $result)
                 
                 )
               )
             )
           )
         )
       )

;; sub _unpack_utf8_char { ... }
(p-sub pl-_unpack_utf8_char (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($k (make-p-box nil)))
             (let (($s (make-p-box nil)) ($si_ref (make-p-box nil)))
               ;; my ($s, $si_ref) = @_
               (p-list-= (vector $s $si_ref) @_)
               
               (let (($slen (make-p-box nil)))
                 ;; my $slen = length($s)
                 (p-my-= $slen (p-length $s))
                 
                 ;; return 0 if $$si_ref >= $slen
                 (let ((*wantarray* :void)) (p-if             (p->= (p-cast-$ $si_ref) $slen)             (p-return 0)))
                 
                 (let (($b0 (make-p-box nil)))
                   ;; my $b0 = ord(substr($s, $$si_ref, 1))
                   (p-my-= $b0 (p-ord (p-substr $s (p-cast-$ $si_ref) 1)))
                   
                   (let (($nb (make-p-box nil)) ($code (make-p-box nil)))
                     ;; my ($nb, $code) (bare declaration)
                     
                     ;; if    ($b0 < 0x80) { $nb=1; $code=$b0 }     elsif ($b0 < 0xE0) { $nb=2; $code=$b0&0x1F }     elsif ($b0 < 0xF0) { $nb=3; $code=$b0&0x0F }     else               { $nb=4; $code=$b0&0x07 }
                     ;; if ($b0 < 0x80)
                     (p-if                 (p-< $b0 #x80)
                                           (progn
                                             ;; $nb=1
                                             (let ((*wantarray* :void))                     (p-my-= $nb 1))
                                             
                                             ;; $code=$b0
                                             (p-my-= $code $b0)
                                             
                                             )
                                           ;; elsif ($b0 < 0xE0)
                                           (p-if                   (p-< $b0 #xE0)
                                                                   (progn
                                                                     ;; $nb=2
                                                                     (let ((*wantarray* :void))                       (p-my-= $nb 2))
                                                                     
                                                                     ;; $code=$b0&0x1F
                                                                     (p-my-= $code (p-bit-and $b0 #x1F))
                                                                     
                                                                     )
                                                                   ;; elsif ($b0 < 0xF0)
                                                                   (p-if                     (p-< $b0 #xF0)
                                                                                             (progn
                                                                                               ;; $nb=3
                                                                                               (let ((*wantarray* :void))                         (p-my-= $nb 3))
                                                                                               
                                                                                               ;; $code=$b0&0x0F
                                                                                               (p-my-= $code (p-bit-and $b0 #x0F))
                                                                                               
                                                                                               )
                                                                                             ;; else
                                                                                             (progn
                                                                                               ;; $nb=4
                                                                                               (let ((*wantarray* :void))                         (p-my-= $nb 4))
                                                                                               
                                                                                               ;; $code=$b0&0x07
                                                                                               (p-my-= $code (p-bit-and $b0 #x07))
                                                                                               
                                                                                               )
                                                                                             )
                                                                   )
                                           )
                     
                     ;; for (my $k=1; $k<$nb; $k++) {         $code = ($code<<6)|(ord(substr($s, $$si_ref+$k, 1))&0x3F) if $$si_ref+$k<$slen;     }
                     (let (($k (make-p-box nil)))
                       (p-for (                (p-my-= $k 1))
                              (                (p-< $k $nb))
                              (                (p-post++ $k))
                              (let ((--pcl-if-ret--8 nil))
                                ;; $code = ($code<<6)|(ord(substr($s, $$si_ref+$k, 1))&0x3F) if $$si_ref+$k<$slen
                                (p-if (setf --pcl-if-ret--8                       (p-< (p-+ (p-cast-$ $si_ref) $k) $slen))
                                      (setf --pcl-if-ret--8                       (p-my-= $code (p-bit-or (p-<< $code 6) (p-bit-and (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) $k) 1)) #x3F))))
                                      nil)
                                
                                --pcl-if-ret--8)
                              )
                       )
                     
                     ;; $$si_ref += $nb
                     (let ((*wantarray* :void))                 (p-incf (p-cast-$ $si_ref) $nb))
                     
                     ;; return $code
                     (p-return $code)
                     
                     )
                   )
                 )
               )
             )
           )
         )
       )

;; sub _unpack_str { ... }
(p-sub pl-_unpack_str (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($i (make-p-box nil)) ($k (make-p-box nil)))
             (let ((--pcl-if-ret--9 nil))
               (let (($ch (make-p-box nil)) ($nrep (make-p-box nil)) ($all (make-p-box nil)) ($s__lex__1 (make-p-box nil)) ($si_ref__lex__2 (make-p-box nil)) ($push_val (make-p-box nil)) ($checksum_p (make-p-box nil)))
                 ;; my ($ch, $nrep, $all, $s, $si_ref, $push_val, $checksum_p) = @_
                 (p-list-= (vector $ch $nrep $all $s__lex__1 $si_ref__lex__2 $push_val $checksum_p) @_)
                 
                 (let (($slen__lex__3 (make-p-box nil)))
                   ;; my $slen = length($s)
                   (p-my-= $slen__lex__3               (p-length $s__lex__1))
                   
                   (let (($n (make-p-box nil)) ($raw (make-p-box nil)) ($hex (make-p-box nil)) ($b (make-p-box nil)) ($nbits (make-p-box nil)) ($bits (make-p-box nil)) ($decoded (make-p-box nil)) ($lc (make-p-box nil)) ($nb (make-p-box nil)) ($ng (make-p-box nil)) ($get (make-p-box nil)) ($cm (make-p-box nil)) ($done (make-p-box nil)) ($v (make-p-box nil)) ($more (make-p-box nil)))
                     ;; if ($ch eq 'A' || $ch eq 'a' || $ch eq 'Z') {         my $n = $all ? ($slen - $$si_ref) : $nrep;         $n = 0 if $n < 0;         my $raw = $$si_ref < $slen ? substr($s, $$si_ref, $n) : '';         $$si_ref += $n;         $raw =~ s/[ \x00]+$// if $ch eq 'A';         $raw =~ s/\x00.*//s   if $ch eq 'Z';         $push_val->($raw);     } elsif ($ch eq 'H') {         my $n = $all ? (2 * ($slen - $$si_ref)) : $nrep;  # n = number of nybbles         my $hex = '';         for (my $i = 0; $i < int($n/2); $i++) {             my $b = $$si_ref+$i < $slen ? ord(substr($s,$$si_ref+$i,1)) : 0;             $hex .= sprintf('%02x', $b);         }         $hex = substr($hex, 0, $n);         $$si_ref += int(($n+1)/2);         $push_val->($hex);     } elsif ($ch eq 'h') {         my $n = $all ? (2 * ($slen - $$si_ref)) : $nrep;  # n = number of nybbles         my $hex = '';         for (my $i = 0; $i < int($n/2); $i++) {             my $b = $$si_ref+$i < $slen ? ord(substr($s,$$si_ref+$i,1)) : 0;             $hex .= sprintf('%x%x', $b&0xF, ($b>>4)&0xF);         }         $hex = substr($hex, 0, $n);         $$si_ref += int(($n+1)/2);         $push_val->($hex);     } elsif ($ch eq 'B') {         my $nbits = $all ? (8*($slen-$$si_ref)) : $nrep;         if ($checksum_p) {             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $push_val->( ($b>>(7-($i%8))) & 1 );             }         } else {             my $bits = '';             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $bits .= (($b>>(7-($i%8)))&1) ? '1' : '0';             }             $push_val->($bits);         }         $$si_ref += int(($nbits+7)/8);     } elsif ($ch eq 'b') {         my $nbits = $all ? (8*($slen-$$si_ref)) : $nrep;         if ($checksum_p) {             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $push_val->( ($b>>($i%8)) & 1 );             }         } else {             my $bits = '';             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $bits .= (($b>>($i%8))&1) ? '1' : '0';             }             $push_val->($bits);         }         $$si_ref += int(($nbits+7)/8);     } elsif ($ch eq 'u') {         my $decoded = '';         while ($$si_ref < $slen) {             my $lc = ord(substr($s, $$si_ref, 1));             my $nb = ($lc - 32) & 63; $$si_ref++;             last if $nb == 0;             my $ng = int(($nb+2)/3);             for (my $k=0; $k<$ng; $k++) {                 my $get = sub { my $i=$$si_ref+$_[0]; $i<$slen?(ord(substr($s,$i,1))-32)&63:0 };                 my $cm = ($get->(4*$k)<<18)|($get->(4*$k+1)<<12)|($get->(4*$k+2)<<6)|$get->(4*$k+3);                 $decoded .= chr(($cm>>16)&0xFF) if $k*3   < $nb;                 $decoded .= chr(($cm>> 8)&0xFF) if $k*3+1 < $nb;                 $decoded .= chr( $cm     &0xFF) if $k*3+2 < $nb;             }             $$si_ref += $ng*4;             $$si_ref++ if $$si_ref < $slen && substr($s,$$si_ref,1) eq "\n";         }         $push_val->($decoded);     } elsif ($ch eq 'U') {         my $n = $all ? 9**9 : $nrep;         my $done = 0;         while ($done < $n && $$si_ref < $slen) {             $push_val->(_unpack_utf8_char($s, $si_ref)); $done++;         }     } elsif ($ch eq 'W') {         my $n = $all ? ($slen-$$si_ref) : $nrep;         for (my $i=0; $i<$n && $$si_ref<$slen; $i++) {             $push_val->(ord(substr($s, $$si_ref++, 1)));         }     } elsif ($ch eq 'w') {         my $done = 0;         while (($all || $done < $nrep) && $$si_ref < $slen) {             my ($v, $more) = (0, 1);             while ($more) {                 die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                 my $b = ord(substr($s, $$si_ref++, 1));                 $more = $b & 0x80; $v = ($v<<7)|($b&0x7F);             }             $push_val->($v); $done++;         }     }
                     ;; if ($ch eq 'A' || $ch eq 'a' || $ch eq 'Z')
                     (p-if (setf --pcl-if-ret--9                 (p-|| (p-|| (p-str-eq $ch "A") (p-str-eq $ch "a")) (p-str-eq $ch "Z")))
                           (progn
                             (let (($n (make-p-box nil)) ($raw (make-p-box nil)))
                               ;; my $n = $all ? ($slen - $$si_ref) : $nrep
                               (p-my-= $n (p-if $all (p-- $slen__lex__3 (p-cast-$ $si_ref__lex__2)) $nrep))
                               
                               ;; $n = 0 if $n < 0
                               (let ((*wantarray* :void)) (p-if                       (p-< $n 0)                       (p-my-= $n 0)))
                               
                               ;; my $raw = $$si_ref < $slen ? substr($s, $$si_ref, $n) : ''
                               (p-my-= $raw (p-if (p-< (p-cast-$ $si_ref__lex__2) $slen__lex__3) (p-substr $s__lex__1 (p-cast-$ $si_ref__lex__2) $n) ""))
                               
                               ;; $$si_ref += $n
                               (let ((*wantarray* :void))                       (p-incf (p-cast-$ $si_ref__lex__2) $n))
                               
                               ;; $raw =~ s/[ \x00]+$// if $ch eq 'A'
                               (let ((*wantarray* :void)) (p-if                       (p-str-eq $ch "A")                       (p-=~ $raw (p-subst "[ \\x00]+$" ""))))
                               
                               ;; $raw =~ s/\x00.*//s   if $ch eq 'Z'
                               (let ((*wantarray* :void)) (p-if                       (p-str-eq $ch "Z")                       (p-=~ $raw (p-subst "\\x00.*" "" :s))))
                               
                               ;; $push_val->($raw)
                               (setf --pcl-if-ret--9                       (let ((*wantarray* nil)) (p-funcall-ref $push_val $raw)))
                               
                               )
                             )
                           ;; elsif ($ch eq 'H')
                           (p-if (setf --pcl-if-ret--9                   (p-str-eq $ch "H"))
                                 (progn
                                   (let (($n (make-p-box nil)) ($hex (make-p-box nil)) ($i (make-p-box nil)) ($b (make-p-box nil)))
                                     ;; my $n = $all ? (2 * ($slen - $$si_ref)) : $nrep
                                     (p-my-= $n (p-if $all (p-* 2 (p-- $slen__lex__3 (p-cast-$ $si_ref__lex__2))) $nrep))
                                     
                                     ;; my $hex = ''
                                     (p-my-= $hex "")
                                     
                                     ;; for (my $i = 0; $i < int($n/2); $i++) {             my $b = $$si_ref+$i < $slen ? ord(substr($s,$$si_ref+$i,1)) : 0;             $hex .= sprintf('%02x', $b);         }
                                     (let (($i (make-p-box nil)))
                                       (p-for (                        (p-my-= $i 0))
                                              (                        (p-< $i (p-int (p-/ $n 2))))
                                              (                        (p-post++ $i))
                                              (let (($b (make-p-box nil)))
                                                ;; my $b = $$si_ref+$i < $slen ? ord(substr($s,$$si_ref+$i,1)) : 0
                                                (p-my-= $b (p-if (p-< (p-+ (p-cast-$ $si_ref__lex__2) $i) $slen__lex__3) (p-ord (p-substr $s__lex__1 (p-+ (p-cast-$ $si_ref__lex__2) $i) 1)) 0))
                                                
                                                ;; $hex .= sprintf('%02x', $b)
                                                (p-.= $hex (p-sprintf "%02x" $b))
                                                
                                                )
                                              )
                                       )
                                     
                                     ;; $hex = substr($hex, 0, $n)
                                     (let ((*wantarray* :void))                         (p-my-= $hex (p-substr $hex 0 $n)))
                                     
                                     ;; $$si_ref += int(($n+1)/2)
                                     (let ((*wantarray* :void))                         (p-incf (p-cast-$ $si_ref__lex__2) (p-int (p-/ (p-+ $n 1) 2))))
                                     
                                     ;; $push_val->($hex)
                                     (setf --pcl-if-ret--9                         (let ((*wantarray* nil)) (p-funcall-ref $push_val $hex)))
                                     
                                     )
                                   )
                                 ;; elsif ($ch eq 'h')
                                 (p-if (setf --pcl-if-ret--9                     (p-str-eq $ch "h"))
                                       (progn
                                         (let (($n (make-p-box nil)) ($hex (make-p-box nil)) ($i (make-p-box nil)) ($b (make-p-box nil)))
                                           ;; my $n = $all ? (2 * ($slen - $$si_ref)) : $nrep
                                           (p-my-= $n (p-if $all (p-* 2 (p-- $slen__lex__3 (p-cast-$ $si_ref__lex__2))) $nrep))
                                           
                                           ;; my $hex = ''
                                           (p-my-= $hex "")
                                           
                                           ;; for (my $i = 0; $i < int($n/2); $i++) {             my $b = $$si_ref+$i < $slen ? ord(substr($s,$$si_ref+$i,1)) : 0;             $hex .= sprintf('%x%x', $b&0xF, ($b>>4)&0xF);         }
                                           (let (($i (make-p-box nil)))
                                             (p-for (                          (p-my-= $i 0))
                                                    (                          (p-< $i (p-int (p-/ $n 2))))
                                                    (                          (p-post++ $i))
                                                    (let (($b (make-p-box nil)))
                                                      ;; my $b = $$si_ref+$i < $slen ? ord(substr($s,$$si_ref+$i,1)) : 0
                                                      (p-my-= $b (p-if (p-< (p-+ (p-cast-$ $si_ref__lex__2) $i) $slen__lex__3) (p-ord (p-substr $s__lex__1 (p-+ (p-cast-$ $si_ref__lex__2) $i) 1)) 0))
                                                      
                                                      ;; $hex .= sprintf('%x%x', $b&0xF, ($b>>4)&0xF)
                                                      (p-.= $hex (p-sprintf "%x%x" (p-bit-and $b #xF) (p-bit-and (p->> $b 4) #xF)))
                                                      
                                                      )
                                                    )
                                             )
                                           
                                           ;; $hex = substr($hex, 0, $n)
                                           (let ((*wantarray* :void))                           (p-my-= $hex (p-substr $hex 0 $n)))
                                           
                                           ;; $$si_ref += int(($n+1)/2)
                                           (let ((*wantarray* :void))                           (p-incf (p-cast-$ $si_ref__lex__2) (p-int (p-/ (p-+ $n 1) 2))))
                                           
                                           ;; $push_val->($hex)
                                           (setf --pcl-if-ret--9                           (let ((*wantarray* nil)) (p-funcall-ref $push_val $hex)))
                                           
                                           )
                                         )
                                       ;; elsif ($ch eq 'B')
                                       (p-if (setf --pcl-if-ret--9                       (p-str-eq $ch "B"))
                                             (progn
                                               (let (($nbits (make-p-box nil)) ($i (make-p-box nil)) ($b (make-p-box nil)) ($bits (make-p-box nil)))
                                                 ;; my $nbits = $all ? (8*($slen-$$si_ref)) : $nrep
                                                 (p-my-= $nbits (p-if $all (p-* 8 (p-- $slen__lex__3 (p-cast-$ $si_ref__lex__2))) $nrep))
                                                 
                                                 ;; if ($checksum_p) {             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $push_val->( ($b>>(7-($i%8))) & 1 );             }         } else {             my $bits = '';             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $bits .= (($b>>(7-($i%8)))&1) ? '1' : '0';             }             $push_val->($bits);         }
                                                 ;; if ($checksum_p)
                                                 (p-if                             $checksum_p
                                                                                   (progn
                                                                                     (let (($i (make-p-box nil)) ($b (make-p-box nil)))
                                                                                       ;; for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $push_val->( ($b>>(7-($i%8))) & 1 );             }
                                                                                       (let (($i (make-p-box nil)))
                                                                                         (p-for (                                  (p-my-= $i 0))
                                                                                                (                                  (p-< $i $nbits))
                                                                                                (                                  (p-post++ $i))
                                                                                                (let (($b (make-p-box nil)))
                                                                                                  ;; my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0
                                                                                                  (p-my-= $b (p-if (p-< (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) $slen__lex__3) (p-ord (p-substr $s__lex__1 (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) 1)) 0))
                                                                                                  
                                                                                                  ;; $push_val->( ($b>>(7-($i%8))) & 1 )
                                                                                                  (p-funcall-ref $push_val (p-bit-and (p->> $b (p-- 7 (p-% $i 8))) 1))
                                                                                                  
                                                                                                  )
                                                                                                )
                                                                                         )
                                                                                       
                                                                                       )
                                                                                     )
                                                                                   ;; else
                                                                                   (progn
                                                                                     (let (($bits (make-p-box nil)) ($i (make-p-box nil)) ($b (make-p-box nil)))
                                                                                       ;; my $bits = ''
                                                                                       (p-my-= $bits "")
                                                                                       
                                                                                       ;; for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $bits .= (($b>>(7-($i%8)))&1) ? '1' : '0';             }
                                                                                       (let (($i (make-p-box nil)))
                                                                                         (p-for (                                  (p-my-= $i 0))
                                                                                                (                                  (p-< $i $nbits))
                                                                                                (                                  (p-post++ $i))
                                                                                                (let (($b (make-p-box nil)))
                                                                                                  ;; my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0
                                                                                                  (p-my-= $b (p-if (p-< (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) $slen__lex__3) (p-ord (p-substr $s__lex__1 (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) 1)) 0))
                                                                                                  
                                                                                                  ;; $bits .= (($b>>(7-($i%8)))&1) ? '1' : '0'
                                                                                                  (p-.= $bits (p-if (p-bit-and (p->> $b (p-- 7 (p-% $i 8))) 1) "1" "0"))
                                                                                                  
                                                                                                  )
                                                                                                )
                                                                                         )
                                                                                       
                                                                                       ;; $push_val->($bits)
                                                                                       (p-funcall-ref $push_val $bits)
                                                                                       
                                                                                       )
                                                                                     )
                                                                                   )
                                                 
                                                 ;; $$si_ref += int(($nbits+7)/8)
                                                 (setf --pcl-if-ret--9                             (p-incf (p-cast-$ $si_ref__lex__2) (p-int (p-/ (p-+ $nbits 7) 8))))
                                                 
                                                 )
                                               )
                                             ;; elsif ($ch eq 'b')
                                             (p-if (setf --pcl-if-ret--9                         (p-str-eq $ch "b"))
                                                   (progn
                                                     (let (($nbits (make-p-box nil)) ($i (make-p-box nil)) ($b (make-p-box nil)) ($bits (make-p-box nil)))
                                                       ;; my $nbits = $all ? (8*($slen-$$si_ref)) : $nrep
                                                       (p-my-= $nbits (p-if $all (p-* 8 (p-- $slen__lex__3 (p-cast-$ $si_ref__lex__2))) $nrep))
                                                       
                                                       ;; if ($checksum_p) {             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $push_val->( ($b>>($i%8)) & 1 );             }         } else {             my $bits = '';             for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $bits .= (($b>>($i%8))&1) ? '1' : '0';             }             $push_val->($bits);         }
                                                       ;; if ($checksum_p)
                                                       (p-if                               $checksum_p
                                                                                           (progn
                                                                                             (let (($i (make-p-box nil)) ($b (make-p-box nil)))
                                                                                               ;; for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $push_val->( ($b>>($i%8)) & 1 );             }
                                                                                               (let (($i (make-p-box nil)))
                                                                                                 (p-for (                                    (p-my-= $i 0))
                                                                                                        (                                    (p-< $i $nbits))
                                                                                                        (                                    (p-post++ $i))
                                                                                                        (let (($b (make-p-box nil)))
                                                                                                          ;; my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0
                                                                                                          (p-my-= $b (p-if (p-< (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) $slen__lex__3) (p-ord (p-substr $s__lex__1 (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) 1)) 0))
                                                                                                          
                                                                                                          ;; $push_val->( ($b>>($i%8)) & 1 )
                                                                                                          (p-funcall-ref $push_val (p-bit-and (p->> $b (p-% $i 8)) 1))
                                                                                                          
                                                                                                          )
                                                                                                        )
                                                                                                 )
                                                                                               
                                                                                               )
                                                                                             )
                                                                                           ;; else
                                                                                           (progn
                                                                                             (let (($bits (make-p-box nil)) ($i (make-p-box nil)) ($b (make-p-box nil)))
                                                                                               ;; my $bits = ''
                                                                                               (p-my-= $bits "")
                                                                                               
                                                                                               ;; for (my $i=0; $i<$nbits; $i++) {                 my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0;                 $bits .= (($b>>($i%8))&1) ? '1' : '0';             }
                                                                                               (let (($i (make-p-box nil)))
                                                                                                 (p-for (                                    (p-my-= $i 0))
                                                                                                        (                                    (p-< $i $nbits))
                                                                                                        (                                    (p-post++ $i))
                                                                                                        (let (($b (make-p-box nil)))
                                                                                                          ;; my $b = $$si_ref+int($i/8)<$slen ? ord(substr($s,$$si_ref+int($i/8),1)) : 0
                                                                                                          (p-my-= $b (p-if (p-< (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) $slen__lex__3) (p-ord (p-substr $s__lex__1 (p-+ (p-cast-$ $si_ref__lex__2) (p-int (p-/ $i 8))) 1)) 0))
                                                                                                          
                                                                                                          ;; $bits .= (($b>>($i%8))&1) ? '1' : '0'
                                                                                                          (p-.= $bits (p-if (p-bit-and (p->> $b (p-% $i 8)) 1) "1" "0"))
                                                                                                          
                                                                                                          )
                                                                                                        )
                                                                                                 )
                                                                                               
                                                                                               ;; $push_val->($bits)
                                                                                               (p-funcall-ref $push_val $bits)
                                                                                               
                                                                                               )
                                                                                             )
                                                                                           )
                                                       
                                                       ;; $$si_ref += int(($nbits+7)/8)
                                                       (setf --pcl-if-ret--9                               (p-incf (p-cast-$ $si_ref__lex__2) (p-int (p-/ (p-+ $nbits 7) 8))))
                                                       
                                                       )
                                                     )
                                                   ;; elsif ($ch eq 'u')
                                                   (p-if (setf --pcl-if-ret--9                           (p-str-eq $ch "u"))
                                                         (progn
                                                           (let (($decoded (make-p-box nil)) ($lc (make-p-box nil)) ($nb (make-p-box nil)) ($ng (make-p-box nil)) ($k (make-p-box nil)) ($get (make-p-box nil)) ($cm (make-p-box nil)))
                                                             ;; my $decoded = ''
                                                             (p-my-= $decoded "")
                                                             
                                                             ;; while ($$si_ref < $slen) {             my $lc = ord(substr($s, $$si_ref, 1));             my $nb = ($lc - 32) & 63; $$si_ref++;             last if $nb == 0;             my $ng = int(($nb+2)/3);             for (my $k=0; $k<$ng; $k++) {                 my $get = sub { my $i=$$si_ref+$_[0]; $i<$slen?(ord(substr($s,$i,1))-32)&63:0 };                 my $cm = ($get->(4*$k)<<18)|($get->(4*$k+1)<<12)|($get->(4*$k+2)<<6)|$get->(4*$k+3);                 $decoded .= chr(($cm>>16)&0xFF) if $k*3   < $nb;                 $decoded .= chr(($cm>> 8)&0xFF) if $k*3+1 < $nb;                 $decoded .= chr( $cm     &0xFF) if $k*3+2 < $nb;             }             $$si_ref += $ng*4;             $$si_ref++ if $$si_ref < $slen && substr($s,$$si_ref,1) eq "\n";         }
                                                             (p-while (p-< (p-cast-$ $si_ref__lex__2) $slen__lex__3)
                                                                      (let (($lc (make-p-box nil)) ($nb (make-p-box nil)) ($ng (make-p-box nil)) ($k (make-p-box nil)) ($get (make-p-box nil)) ($cm (make-p-box nil)))
                                                                        (let ((--pcl-if-ret--10 nil))
                                                                          ;; my $lc = ord(substr($s, $$si_ref, 1))
                                                                          (p-my-= $lc (p-ord (p-substr $s__lex__1 (p-cast-$ $si_ref__lex__2) 1)))
                                                                          
                                                                          ;; my $nb = ($lc - 32) & 63
                                                                          (p-my-= $nb (p-bit-and (p-- $lc 32) 63))
                                                                          
                                                                          ;; $$si_ref++
                                                                          (let ((*wantarray* :void))                                       (p-post++ (p-cast-$ $si_ref__lex__2)))
                                                                          
                                                                          ;; last if $nb == 0
                                                                          (let ((*wantarray* :void)) (p-if                                       (p-== $nb 0)                                       (p-last)))
                                                                          
                                                                          ;; my $ng = int(($nb+2)/3)
                                                                          (p-my-= $ng (p-int (p-/ (p-+ $nb 2) 3)))
                                                                          
                                                                          ;; for (my $k=0; $k<$ng; $k++) {                 my $get = sub { my $i=$$si_ref+$_[0]; $i<$slen?(ord(substr($s,$i,1))-32)&63:0 };                 my $cm = ($get->(4*$k)<<18)|($get->(4*$k+1)<<12)|($get->(4*$k+2)<<6)|$get->(4*$k+3);                 $decoded .= chr(($cm>>16)&0xFF) if $k*3   < $nb;                 $decoded .= chr(($cm>> 8)&0xFF) if $k*3+1 < $nb;                 $decoded .= chr( $cm     &0xFF) if $k*3+2 < $nb;             }
                                                                          (let (($k (make-p-box nil)))
                                                                            (p-for (                                      (p-my-= $k 0))
                                                                                   (                                      (p-< $k $ng))
                                                                                   (                                      (p-post++ $k))
                                                                                   (let (($get (make-p-box nil)) ($cm (make-p-box nil)))
                                                                                     (let ((--pcl-if-ret--11 nil))
                                                                                       ;; my $get = sub { my $i=$$si_ref+$_[0]; $i<$slen?(ord(substr($s,$i,1))-32)&63:0 }
                                                                                       (p-my-= $get (lambda (&rest %_args)
                                                                                                      (let ((@_ (p-flatten-args %_args))
                                                                                                            (*pcl-caller-wantarray* *wantarray*))
                                                                                                        (catch :p-return
                                                                                                          (block nil
                                                                                                            ;; my $i=$$si_ref+$_[0]
                                                                                                            (p-my-= $i (p-+ (p-cast-$ $si_ref__lex__2) (p-aref @_ 0)))
                                                                                                            
                                                                                                            ;; $i<$slen?(ord(substr($s,$i,1))-32)&63:0
                                                                                                            (p-if (p-< $i $slen__lex__3) (p-bit-and (p-- (p-ord (p-substr $s__lex__1 $i 1)) 32) 63) 0)
                                                                                                            
                                                                                                            )
                                                                                                          )
                                                                                                        )
                                                                                                      ))
                                                                                       
                                                                                       ;; my $cm = ($get->(4*$k)<<18)|($get->(4*$k+1)<<12)|($get->(4*$k+2)<<6)|$get->(4*$k+3)
                                                                                       (p-my-= $cm (p-bit-or (p-bit-or (p-bit-or (p-<< (let ((*wantarray* nil)) (p-funcall-ref $get (p-* 4 $k))) 18) (p-<< (let ((*wantarray* nil)) (p-funcall-ref $get (p-+ (p-* 4 $k) 1))) 12)) (p-<< (let ((*wantarray* nil)) (p-funcall-ref $get (p-+ (p-* 4 $k) 2))) 6)) (let ((*wantarray* nil)) (p-funcall-ref $get (p-+ (p-* 4 $k) 3)))))
                                                                                       
                                                                                       ;; $decoded .= chr(($cm>>16)&0xFF) if $k*3   < $nb
                                                                                       (let ((*wantarray* :void)) (p-if                                               (p-< (p-* $k 3) $nb)                                               (p-.= $decoded (p-chr (p-bit-and (p->> $cm 16) #xFF)))))
                                                                                       
                                                                                       ;; $decoded .= chr(($cm>> 8)&0xFF) if $k*3+1 < $nb
                                                                                       (let ((*wantarray* :void)) (p-if                                               (p-< (p-+ (p-* $k 3) 1) $nb)                                               (p-.= $decoded (p-chr (p-bit-and (p->> $cm 8) #xFF)))))
                                                                                       
                                                                                       ;; $decoded .= chr( $cm     &0xFF) if $k*3+2 < $nb
                                                                                       (p-if (setf --pcl-if-ret--11                                               (p-< (p-+ (p-* $k 3) 2) $nb))
                                                                                             (setf --pcl-if-ret--11                                               (p-.= $decoded (p-chr (p-bit-and $cm #xFF))))
                                                                                             nil)
                                                                                       
                                                                                       --pcl-if-ret--11)
                                                                                     )
                                                                                   )
                                                                            )
                                                                          
                                                                          ;; $$si_ref += $ng*4
                                                                          (let ((*wantarray* :void))                                       (p-incf (p-cast-$ $si_ref__lex__2) (p-* $ng 4)))
                                                                          
                                                                          ;; $$si_ref++ if $$si_ref < $slen && substr($s,$$si_ref,1) eq "\n"
                                                                          (p-if (setf --pcl-if-ret--10                                       (p-&& (p-< (p-cast-$ $si_ref__lex__2) $slen__lex__3) (p-str-eq (p-substr $s__lex__1 (p-cast-$ $si_ref__lex__2) 1) "
")))
                                                                                (setf --pcl-if-ret--10                                       (p-post++ (p-cast-$ $si_ref__lex__2)))
                                                                                nil)
                                                                          
                                                                          --pcl-if-ret--10)
                                                                        )
                                                                      )
                                                             
                                                             ;; $push_val->($decoded)
                                                             (setf --pcl-if-ret--9                                 (let ((*wantarray* nil)) (p-funcall-ref $push_val $decoded)))
                                                             
                                                             )
                                                           )
                                                         ;; elsif ($ch eq 'U')
                                                         (p-if (setf --pcl-if-ret--9                             (p-str-eq $ch "U"))
                                                               (progn
                                                                 (let (($n (make-p-box nil)) ($done (make-p-box nil)))
                                                                   ;; my $n = $all ? 9**9 : $nrep
                                                                   (p-my-= $n (p-if $all (p-** 9 9) $nrep))
                                                                   
                                                                   ;; my $done = 0
                                                                   (p-my-= $done 0)
                                                                   
                                                                   ;; while ($done < $n && $$si_ref < $slen) {             $push_val->(_unpack_utf8_char($s, $si_ref)); $done++;         }
                                                                   (p-while (p-&& (p-< $done $n) (p-< (p-cast-$ $si_ref__lex__2) $slen__lex__3))
                                                                            ;; $push_val->(_unpack_utf8_char($s, $si_ref))
                                                                            (let ((*wantarray* :void))                                     (let ((*wantarray* :void)) (p-funcall-ref $push_val (let ((*wantarray* :void)) (pl-_unpack_utf8_char $s__lex__1 $si_ref__lex__2)))))
                                                                            
                                                                            ;; $done++
                                                                            (p-post++ $done)
                                                                            
                                                                            )
                                                                   
                                                                   )
                                                                 )
                                                               ;; elsif ($ch eq 'W')
                                                               (p-if (setf --pcl-if-ret--9                               (p-str-eq $ch "W"))
                                                                     (progn
                                                                       (let (($n (make-p-box nil)) ($i (make-p-box nil)))
                                                                         ;; my $n = $all ? ($slen-$$si_ref) : $nrep
                                                                         (p-my-= $n (p-if $all (p-- $slen__lex__3 (p-cast-$ $si_ref__lex__2)) $nrep))
                                                                         
                                                                         ;; for (my $i=0; $i<$n && $$si_ref<$slen; $i++) {             $push_val->(ord(substr($s, $$si_ref++, 1)));         }
                                                                         (let (($i (make-p-box nil)))
                                                                           (p-for (                                    (p-my-= $i 0))
                                                                                  (                                    (p-&& (p-< $i $n) (p-< (p-cast-$ $si_ref__lex__2) $slen__lex__3)))
                                                                                  (                                    (p-post++ $i))
                                                                                  ;; $push_val->(ord(substr($s, $$si_ref++, 1)))
                                                                                  (p-funcall-ref $push_val (p-ord (p-substr $s__lex__1 (p-post++ (p-cast-$ $si_ref__lex__2)) 1)))
                                                                                  
                                                                                  )
                                                                           )
                                                                         
                                                                         )
                                                                       )
                                                                     ;; elsif ($ch eq 'w')
                                                                     (p-if (setf --pcl-if-ret--9                                 (p-str-eq $ch "w"))
                                                                           (progn
                                                                             (let (($done (make-p-box nil)) ($v (make-p-box nil)) ($more (make-p-box nil)) ($b (make-p-box nil)))
                                                                               ;; my $done = 0
                                                                               (p-my-= $done 0)
                                                                               
                                                                               ;; while (($all || $done < $nrep) && $$si_ref < $slen) {             my ($v, $more) = (0, 1);             while ($more) {                 die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                 my $b = ord(substr($s, $$si_ref++, 1));                 $more = $b & 0x80; $v = ($v<<7)|($b&0x7F);             }             $push_val->($v); $done++;         }
                                                                               (p-while (p-&& (p-|| $all (p-< $done $nrep)) (p-< (p-cast-$ $si_ref__lex__2) $slen__lex__3))
                                                                                        (let (($v (make-p-box nil)) ($more (make-p-box nil)) ($b (make-p-box nil)))
                                                                                          ;; my ($v, $more) = (0, 1)
                                                                                          (p-list-= (vector $v $more) (vector 0 1))
                                                                                          
                                                                                          ;; while ($more) {                 die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                 my $b = ord(substr($s, $$si_ref++, 1));                 $more = $b & 0x80; $v = ($v<<7)|($b&0x7F);             }
                                                                                          (p-while $more
                                                                                                   (let (($b (make-p-box nil)))
                                                                                                     ;; die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen
                                                                                                     (let ((*wantarray* :void)) (p-if                                               (p->= (p-cast-$ $si_ref__lex__2) $slen__lex__3)                                               (p-die "Unterminated compressed integer in unpack
")))
                                                                                                     
                                                                                                     ;; my $b = ord(substr($s, $$si_ref++, 1))
                                                                                                     (p-my-= $b (p-ord (p-substr $s__lex__1 (p-post++ (p-cast-$ $si_ref__lex__2)) 1)))
                                                                                                     
                                                                                                     ;; $more = $b & 0x80
                                                                                                     (let ((*wantarray* :void))                                               (p-my-= $more (p-bit-and $b #x80)))
                                                                                                     
                                                                                                     ;; $v = ($v<<7)|($b&0x7F)
                                                                                                     (p-my-= $v (p-bit-or (p-<< $v 7) (p-bit-and $b #x7F)))
                                                                                                     
                                                                                                     )
                                                                                                   )
                                                                                          
                                                                                          ;; $push_val->($v)
                                                                                          (let ((*wantarray* :void))                                           (let ((*wantarray* :void)) (p-funcall-ref $push_val $v)))
                                                                                          
                                                                                          ;; $done++
                                                                                          (p-post++ $done)
                                                                                          
                                                                                          )
                                                                                        )
                                                                               
                                                                               )
                                                                             )
                                                                           nil
                                                                           )
                                                                     )
                                                               )
                                                         )
                                                   )
                                             )
                                       )
                                 )
                           )
                     
                     )
                   )
                 )
               --pcl-if-ret--9)
             )
           )
         )
       )

;; sub _unpack_tmpl { ... }
(p-sub pl-_unpack_tmpl (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($i (make-p-box nil)) ($r (make-p-box nil)))
             (let (($tmpl (make-p-box nil)) ($s (make-p-box nil)) ($si_ref (make-p-box nil)) ($push_val (make-p-box nil)) ($inh_be (make-p-box nil)) ($inh_le (make-p-box nil)) ($checksum_p (make-p-box nil)))
               ;; my ($tmpl, $s, $si_ref, $push_val, $inh_be, $inh_le, $checksum_p) = @_
               (p-list-= (vector $tmpl $s $si_ref $push_val $inh_be $inh_le $checksum_p) @_)
               
               (let (($slen (make-p-box nil)))
                 ;; my $slen = length($s)
                 (p-my-= $slen (p-length $s))
                 
                 (let (($ti (make-p-box nil)))
                   ;; my $ti = 0
                   (p-my-= $ti 0)
                   
                   (let (($tlen (make-p-box nil)))
                     ;; my $tlen = length($tmpl)
                     (p-my-= $tlen (p-length $tmpl))
                     
                     (let (($ch (make-p-box nil)) ($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)) ($bang (make-p-box nil)) ($be (make-p-box nil)) ($le (make-p-box nil)) ($all (make-p-box nil)) ($count (make-p-box nil)) ($nrep (make-p-box nil)) ($nb (make-p-box nil)) ($sig (make-p-box nil)) ($dbe (make-p-box nil)) ($slash_n (make-p-box nil)) ($be2 (make-p-box nil)) ($more (make-p-box nil)) ($b (make-p-box nil)) ($n (make-p-box nil)) ($raw (make-p-box nil)) ($dch (make-p-box nil)) ($dbang (make-p-box nil)) ($dbe2 (make-p-box nil)) ($dle2 (make-p-box nil)) ($dall (make-p-box nil)) ($dcnt (make-p-box nil)) ($dnrep (make-p-box nil)) ($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)) ($dbe3 (make-p-box nil)) ($ge (make-p-box nil)) ($inner (make-p-box nil)))
                       ;; while (1) {         $ti = _pack_skip_ws($tmpl, $ti);         last if $ti >= $tlen;         my $ch = substr($tmpl, $ti, 1); $ti++;         my ($grpbeg, $grpend) = (undef, undef);         if ($ch eq '(') {             $grpend = _pack_find_group_end($tmpl, $ti);             $grpbeg = $ti; $ti = $grpend + 1; $ch = '(';         }         my ($bang, $be, $le) = _pack_parse_mods($tmpl, \$ti, $inh_be, $inh_le, $ch, 'unpack');         $ti = _pack_skip_ws($tmpl, $ti);         my ($all, $count, $nrep) = _pack_parse_count($tmpl, \$ti);         $ti = _pack_skip_ws($tmpl, $ti);          # Slash mode         if ($ti < $tlen && substr($tmpl, $ti, 1) eq '/') {             $ti++;             my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang);             my $slash_n = 0;             if ($nb) {                 my $be2 = $be ? 1 : ($le ? 0 : $dbe);                 $slash_n = _unpack_read_int($s, $$si_ref, $nb, $be2, $sig);                 $$si_ref += $nb;             } elsif ($ch eq 'w') {                 my $more = 1;                 while ($more) {                     die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                     my $b = ord(substr($s, $$si_ref++, 1));                     $more = $b & 0x80; $slash_n = ($slash_n<<7)|($b&0x7F);                 }             } else {                 my $n = $all ? ($slen-$$si_ref) : $nrep;                 my $raw = substr($s, $$si_ref, $n); $$si_ref += $n;                 $raw =~ s/[ \x00]+$// if $ch eq 'A';                 $raw =~ s/\x00.*//s   if $ch eq 'Z';                 $slash_n = length($raw);             }             $ti = _pack_skip_ws($tmpl, $ti); last if $ti >= $tlen;             my $dch = substr($tmpl, $ti, 1); $ti++;             my ($dbang, $dbe2, $dle2) = _pack_parse_mods($tmpl, \$ti, $be, $le, $dch, 'unpack');             $ti = _pack_skip_ws($tmpl, $ti);             my ($dall, $dcnt, $dnrep) = _pack_parse_count($tmpl, \$ti);             my ($dnb, $dsig, $ddbe) = _pack_type_info($dch, $dbang);             if ($dnb) {                 my $dbe3 = $dbe2 ? 1 : ($dle2 ? 0 : $ddbe);                 for (my $i=0; $i<$slash_n && $$si_ref+$dnb<=$slen; $i++) {                     $push_val->(_unpack_read_int($s, $$si_ref, $dnb, $dbe3, $dsig));                     $$si_ref += $dnb;                 }             } elsif ($dch eq 'A'||$dch eq 'a'||$dch eq 'Z') {                 _unpack_str($dch, $slash_n, 0, $s, $si_ref, $push_val, $checksum_p);             } elsif ($dch eq '(') {                 my $ge = _pack_find_group_end($tmpl, $ti);                 my $inner = substr($tmpl, $ti, $ge - $ti); $ti = $ge + 1;                 for (my $r=0; $r<$slash_n; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             }             next;         }          # Group         if (defined $grpbeg) {             my $inner = substr($tmpl, $grpbeg, $grpend - $grpbeg);             if ($all) {                 while ($$si_ref < $slen) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             } else {                 for (my $r=0; $r<$nrep; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             }             next;         }          # Position/skip (no push)         if ($ch eq 'x') { $$si_ref += $all ? ($slen-$$si_ref) : $nrep; next }         if ($ch eq 'X') { $$si_ref -= $nrep; $$si_ref = 0 if $$si_ref<0; next }         if ($ch eq '@') { $$si_ref = defined($count) ? $count : 0; next }         if ($ch eq '%' || $ch eq '!' ) { next }         if ($ch eq 'p'||$ch eq 'P'||$ch eq 'D') { next }          # Integer types         my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang);         if ($nb) {             my $be2 = $be ? 1 : ($le ? 0 : $dbe);             my $n = $all ? int(($slen-$$si_ref)/$nb) : $nrep;             for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_read_int($s, $$si_ref, $nb, $be2, $sig));                 $$si_ref += $nb;             }             next;         }          # Float types         if ($ch eq 'f') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             my $n = $all ? int(($slen-$$si_ref)/4) : $nrep;             for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_float32($s, $$si_ref, $be2)); $$si_ref += 4;             }             next;         }         if ($ch eq 'd'||$ch eq 'F') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             my $n = $all ? int(($slen-$$si_ref)/8) : $nrep;             for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_float64($s, $$si_ref, $be2)); $$si_ref += 8;             }             next;         }          # String/bit/etc types         if ($ch eq 'A'||$ch eq 'a'||$ch eq 'Z'||$ch eq 'H'||$ch eq 'h'||             $ch eq 'B'||$ch eq 'b'||$ch eq 'u'||$ch eq 'U'||$ch eq 'W'||$ch eq 'w') {             _unpack_str($ch, $nrep, $all, $s, $si_ref, $push_val, $checksum_p);             next;         }          die "Invalid type '$ch' in unpack\n";     }
                       (p-while 1
                                (let (($ch (make-p-box nil)) ($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)) ($bang (make-p-box nil)) ($be (make-p-box nil)) ($le (make-p-box nil)) ($all (make-p-box nil)) ($count (make-p-box nil)) ($nrep (make-p-box nil)) ($nb (make-p-box nil)) ($sig (make-p-box nil)) ($dbe (make-p-box nil)) ($slash_n (make-p-box nil)) ($be2 (make-p-box nil)) ($more (make-p-box nil)) ($b (make-p-box nil)) ($n (make-p-box nil)) ($raw (make-p-box nil)) ($dch (make-p-box nil)) ($dbang (make-p-box nil)) ($dbe2 (make-p-box nil)) ($dle2 (make-p-box nil)) ($dall (make-p-box nil)) ($dcnt (make-p-box nil)) ($dnrep (make-p-box nil)) ($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)) ($dbe3 (make-p-box nil)) ($i (make-p-box nil)) ($ge (make-p-box nil)) ($inner (make-p-box nil)) ($r (make-p-box nil)))
                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                  (p-my-= $ti (pl-_pack_skip_ws $tmpl $ti))
                                  
                                  ;; last if $ti >= $tlen
                                  (p-if                       (p->= $ti $tlen)                       (p-last))
                                  
                                  ;; my $ch = substr($tmpl, $ti, 1)
                                  (p-my-= $ch (p-substr $tmpl $ti 1))
                                  
                                  ;; $ti++
                                  (p-post++ $ti)
                                  
                                  ;; my ($grpbeg, $grpend) = (undef, undef)
                                  (p-list-= (vector $grpbeg $grpend) (vector (p-undef) (p-undef)))
                                  
                                  ;; if ($ch eq '(') {             $grpend = _pack_find_group_end($tmpl, $ti);             $grpbeg = $ti; $ti = $grpend + 1; $ch = '(';         }
                                  ;; if ($ch eq '(')
                                  (p-if                       (p-str-eq $ch "(")
                                                              (progn
                                                                ;; $grpend = _pack_find_group_end($tmpl, $ti)
                                                                (p-my-= $grpend (pl-_pack_find_group_end $tmpl $ti))
                                                                
                                                                ;; $grpbeg = $ti
                                                                (p-my-= $grpbeg $ti)
                                                                
                                                                ;; $ti = $grpend + 1
                                                                (p-my-= $ti (p-+ $grpend 1))
                                                                
                                                                ;; $ch = '('
                                                                (p-my-= $ch "(")
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; my ($bang, $be, $le) = _pack_parse_mods($tmpl, \$ti, $inh_be, $inh_le, $ch, 'unpack')
                                  (p-list-= (vector $bang $be $le) (let ((*wantarray* t)) (pl-_pack_parse_mods $tmpl (p-backslash $ti) $inh_be $inh_le $ch "unpack")))
                                  
                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                  (let ((*wantarray* :void))                       (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                  
                                  ;; my ($all, $count, $nrep) = _pack_parse_count($tmpl, \$ti)
                                  (p-list-= (vector $all $count $nrep) (let ((*wantarray* t)) (pl-_pack_parse_count $tmpl (p-backslash $ti))))
                                  
                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                  (let ((*wantarray* :void))                       (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                  
                                  ;; if ($ti < $tlen && substr($tmpl, $ti, 1) eq '/') {             $ti++;             my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang);             my $slash_n = 0;             if ($nb) {                 my $be2 = $be ? 1 : ($le ? 0 : $dbe);                 $slash_n = _unpack_read_int($s, $$si_ref, $nb, $be2, $sig);                 $$si_ref += $nb;             } elsif ($ch eq 'w') {                 my $more = 1;                 while ($more) {                     die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                     my $b = ord(substr($s, $$si_ref++, 1));                     $more = $b & 0x80; $slash_n = ($slash_n<<7)|($b&0x7F);                 }             } else {                 my $n = $all ? ($slen-$$si_ref) : $nrep;                 my $raw = substr($s, $$si_ref, $n); $$si_ref += $n;                 $raw =~ s/[ \x00]+$// if $ch eq 'A';                 $raw =~ s/\x00.*//s   if $ch eq 'Z';                 $slash_n = length($raw);             }             $ti = _pack_skip_ws($tmpl, $ti); last if $ti >= $tlen;             my $dch = substr($tmpl, $ti, 1); $ti++;             my ($dbang, $dbe2, $dle2) = _pack_parse_mods($tmpl, \$ti, $be, $le, $dch, 'unpack');             $ti = _pack_skip_ws($tmpl, $ti);             my ($dall, $dcnt, $dnrep) = _pack_parse_count($tmpl, \$ti);             my ($dnb, $dsig, $ddbe) = _pack_type_info($dch, $dbang);             if ($dnb) {                 my $dbe3 = $dbe2 ? 1 : ($dle2 ? 0 : $ddbe);                 for (my $i=0; $i<$slash_n && $$si_ref+$dnb<=$slen; $i++) {                     $push_val->(_unpack_read_int($s, $$si_ref, $dnb, $dbe3, $dsig));                     $$si_ref += $dnb;                 }             } elsif ($dch eq 'A'||$dch eq 'a'||$dch eq 'Z') {                 _unpack_str($dch, $slash_n, 0, $s, $si_ref, $push_val, $checksum_p);             } elsif ($dch eq '(') {                 my $ge = _pack_find_group_end($tmpl, $ti);                 my $inner = substr($tmpl, $ti, $ge - $ti); $ti = $ge + 1;                 for (my $r=0; $r<$slash_n; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             }             next;         }
                                  ;; if ($ti < $tlen && substr($tmpl, $ti, 1) eq '/')
                                  (p-if                       (p-&& (p-< $ti $tlen) (p-str-eq (p-substr $tmpl $ti 1) "/"))
                                                              (progn
                                                                (let (($nb (make-p-box nil)) ($sig (make-p-box nil)) ($dbe (make-p-box nil)) ($slash_n (make-p-box nil)) ($be2 (make-p-box nil)) ($more (make-p-box nil)) ($b (make-p-box nil)) ($n (make-p-box nil)) ($raw (make-p-box nil)) ($dch (make-p-box nil)) ($dbang (make-p-box nil)) ($dbe2 (make-p-box nil)) ($dle2 (make-p-box nil)) ($dall (make-p-box nil)) ($dcnt (make-p-box nil)) ($dnrep (make-p-box nil)) ($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)) ($dbe3 (make-p-box nil)) ($i (make-p-box nil)) ($ge (make-p-box nil)) ($inner (make-p-box nil)) ($r (make-p-box nil)))
                                                                  ;; $ti++
                                                                  (let ((*wantarray* :void))                             (p-post++ $ti))
                                                                  
                                                                  ;; my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang)
                                                                  (p-list-= (vector $nb $sig $dbe) (let ((*wantarray* t)) (pl-_pack_type_info $ch $bang)))
                                                                  
                                                                  ;; my $slash_n = 0
                                                                  (p-my-= $slash_n 0)
                                                                  
                                                                  ;; if ($nb) {                 my $be2 = $be ? 1 : ($le ? 0 : $dbe);                 $slash_n = _unpack_read_int($s, $$si_ref, $nb, $be2, $sig);                 $$si_ref += $nb;             } elsif ($ch eq 'w') {                 my $more = 1;                 while ($more) {                     die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                     my $b = ord(substr($s, $$si_ref++, 1));                     $more = $b & 0x80; $slash_n = ($slash_n<<7)|($b&0x7F);                 }             } else {                 my $n = $all ? ($slen-$$si_ref) : $nrep;                 my $raw = substr($s, $$si_ref, $n); $$si_ref += $n;                 $raw =~ s/[ \x00]+$// if $ch eq 'A';                 $raw =~ s/\x00.*//s   if $ch eq 'Z';                 $slash_n = length($raw);             }
                                                                  ;; if ($nb)
                                                                  (p-if                             $nb
                                                                                                    (progn
                                                                                                      (let (($be2 (make-p-box nil)))
                                                                                                        ;; my $be2 = $be ? 1 : ($le ? 0 : $dbe)
                                                                                                        (p-my-= $be2 (p-if $be 1 (p-if $le 0 $dbe)))
                                                                                                        
                                                                                                        ;; $slash_n = _unpack_read_int($s, $$si_ref, $nb, $be2, $sig)
                                                                                                        (let ((*wantarray* :void))                                   (p-my-= $slash_n (let ((*wantarray* nil)) (pl-_unpack_read_int $s (p-cast-$ $si_ref) $nb $be2 $sig))))
                                                                                                        
                                                                                                        ;; $$si_ref += $nb
                                                                                                        (p-incf (p-cast-$ $si_ref) $nb)
                                                                                                        
                                                                                                        )
                                                                                                      )
                                                                                                    ;; elsif ($ch eq 'w')
                                                                                                    (p-if                               (p-str-eq $ch "w")
                                                                                                                                        (progn
                                                                                                                                          (let (($more (make-p-box nil)) ($b (make-p-box nil)))
                                                                                                                                            ;; my $more = 1
                                                                                                                                            (p-my-= $more 1)
                                                                                                                                            
                                                                                                                                            ;; while ($more) {                     die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen;                     my $b = ord(substr($s, $$si_ref++, 1));                     $more = $b & 0x80; $slash_n = ($slash_n<<7)|($b&0x7F);                 }
                                                                                                                                            (p-while $more
                                                                                                                                                     (let (($b (make-p-box nil)))
                                                                                                                                                       ;; die "Unterminated compressed integer in unpack\n" if $$si_ref >= $slen
                                                                                                                                                       (p-if                                         (p->= (p-cast-$ $si_ref) $slen)                                         (p-die "Unterminated compressed integer in unpack
"))
                                                                                                                                                       
                                                                                                                                                       ;; my $b = ord(substr($s, $$si_ref++, 1))
                                                                                                                                                       (p-my-= $b (p-ord (p-substr $s (p-post++ (p-cast-$ $si_ref)) 1)))
                                                                                                                                                       
                                                                                                                                                       ;; $more = $b & 0x80
                                                                                                                                                       (p-my-= $more (p-bit-and $b #x80))
                                                                                                                                                       
                                                                                                                                                       ;; $slash_n = ($slash_n<<7)|($b&0x7F)
                                                                                                                                                       (p-my-= $slash_n (p-bit-or (p-<< $slash_n 7) (p-bit-and $b #x7F)))
                                                                                                                                                       
                                                                                                                                                       )
                                                                                                                                                     )
                                                                                                                                            
                                                                                                                                            )
                                                                                                                                          )
                                                                                                                                        ;; else
                                                                                                                                        (progn
                                                                                                                                          (let (($n (make-p-box nil)) ($raw (make-p-box nil)))
                                                                                                                                            ;; my $n = $all ? ($slen-$$si_ref) : $nrep
                                                                                                                                            (p-my-= $n (p-if $all (p-- $slen (p-cast-$ $si_ref)) $nrep))
                                                                                                                                            
                                                                                                                                            ;; my $raw = substr($s, $$si_ref, $n)
                                                                                                                                            (p-my-= $raw (p-substr $s (p-cast-$ $si_ref) $n))
                                                                                                                                            
                                                                                                                                            ;; $$si_ref += $n
                                                                                                                                            (let ((*wantarray* :void))                                     (p-incf (p-cast-$ $si_ref) $n))
                                                                                                                                            
                                                                                                                                            ;; $raw =~ s/[ \x00]+$// if $ch eq 'A'
                                                                                                                                            (let ((*wantarray* :void)) (p-if                                     (p-str-eq $ch "A")                                     (p-=~ $raw (p-subst "[ \\x00]+$" ""))))
                                                                                                                                            
                                                                                                                                            ;; $raw =~ s/\x00.*//s   if $ch eq 'Z'
                                                                                                                                            (let ((*wantarray* :void)) (p-if                                     (p-str-eq $ch "Z")                                     (p-=~ $raw (p-subst "\\x00.*" "" :s))))
                                                                                                                                            
                                                                                                                                            ;; $slash_n = length($raw)
                                                                                                                                            (p-my-= $slash_n (p-length $raw))
                                                                                                                                            
                                                                                                                                            )
                                                                                                                                          )
                                                                                                                                        )
                                                                                                    )
                                                                  
                                                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                                                  (let ((*wantarray* :void))                             (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                                                  
                                                                  ;; last if $ti >= $tlen
                                                                  (let ((*wantarray* :void)) (p-if                             (p->= $ti $tlen)                             (p-last)))
                                                                  
                                                                  ;; my $dch = substr($tmpl, $ti, 1)
                                                                  (p-my-= $dch (p-substr $tmpl $ti 1))
                                                                  
                                                                  ;; $ti++
                                                                  (let ((*wantarray* :void))                             (p-post++ $ti))
                                                                  
                                                                  ;; my ($dbang, $dbe2, $dle2) = _pack_parse_mods($tmpl, \$ti, $be, $le, $dch, 'unpack')
                                                                  (p-list-= (vector $dbang $dbe2 $dle2) (let ((*wantarray* t)) (pl-_pack_parse_mods $tmpl (p-backslash $ti) $be $le $dch "unpack")))
                                                                  
                                                                  ;; $ti = _pack_skip_ws($tmpl, $ti)
                                                                  (let ((*wantarray* :void))                             (p-my-= $ti (let ((*wantarray* nil)) (pl-_pack_skip_ws $tmpl $ti))))
                                                                  
                                                                  ;; my ($dall, $dcnt, $dnrep) = _pack_parse_count($tmpl, \$ti)
                                                                  (p-list-= (vector $dall $dcnt $dnrep) (let ((*wantarray* t)) (pl-_pack_parse_count $tmpl (p-backslash $ti))))
                                                                  
                                                                  ;; my ($dnb, $dsig, $ddbe) = _pack_type_info($dch, $dbang)
                                                                  (p-list-= (vector $dnb $dsig $ddbe) (let ((*wantarray* t)) (pl-_pack_type_info $dch $dbang)))
                                                                  
                                                                  ;; if ($dnb) {                 my $dbe3 = $dbe2 ? 1 : ($dle2 ? 0 : $ddbe);                 for (my $i=0; $i<$slash_n && $$si_ref+$dnb<=$slen; $i++) {                     $push_val->(_unpack_read_int($s, $$si_ref, $dnb, $dbe3, $dsig));                     $$si_ref += $dnb;                 }             } elsif ($dch eq 'A'||$dch eq 'a'||$dch eq 'Z') {                 _unpack_str($dch, $slash_n, 0, $s, $si_ref, $push_val, $checksum_p);             } elsif ($dch eq '(') {                 my $ge = _pack_find_group_end($tmpl, $ti);                 my $inner = substr($tmpl, $ti, $ge - $ti); $ti = $ge + 1;                 for (my $r=0; $r<$slash_n; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             }
                                                                  ;; if ($dnb)
                                                                  (p-if                             $dnb
                                                                                                    (progn
                                                                                                      (let (($dbe3 (make-p-box nil)) ($i (make-p-box nil)))
                                                                                                        ;; my $dbe3 = $dbe2 ? 1 : ($dle2 ? 0 : $ddbe)
                                                                                                        (p-my-= $dbe3 (p-if $dbe2 1 (p-if $dle2 0 $ddbe)))
                                                                                                        
                                                                                                        ;; for (my $i=0; $i<$slash_n && $$si_ref+$dnb<=$slen; $i++) {                     $push_val->(_unpack_read_int($s, $$si_ref, $dnb, $dbe3, $dsig));                     $$si_ref += $dnb;                 }
                                                                                                        (let (($i (make-p-box nil)))
                                                                                                          (p-for (                                  (p-my-= $i 0))
                                                                                                                 (                                  (p-&& (p-< $i $slash_n) (p-<= (p-+ (p-cast-$ $si_ref) $dnb) $slen)))
                                                                                                                 (                                  (p-post++ $i))
                                                                                                                 ;; $push_val->(_unpack_read_int($s, $$si_ref, $dnb, $dbe3, $dsig))
                                                                                                                 (p-funcall-ref $push_val (pl-_unpack_read_int $s (p-cast-$ $si_ref) $dnb $dbe3 $dsig))
                                                                                                                 
                                                                                                                 ;; $$si_ref += $dnb
                                                                                                                 (p-incf (p-cast-$ $si_ref) $dnb)
                                                                                                                 
                                                                                                                 )
                                                                                                          )
                                                                                                        
                                                                                                        )
                                                                                                      )
                                                                                                    ;; elsif ($dch eq 'A'||$dch eq 'a'||$dch eq 'Z')
                                                                                                    (p-if                               (p-|| (p-|| (p-str-eq $dch "A") (p-str-eq $dch "a")) (p-str-eq $dch "Z"))
                                                                                                                                        (progn
                                                                                                                                          ;; _unpack_str($dch, $slash_n, 0, $s, $si_ref, $push_val, $checksum_p)
                                                                                                                                          (pl-_unpack_str $dch $slash_n 0 $s $si_ref $push_val $checksum_p)
                                                                                                                                          
                                                                                                                                          )
                                                                                                                                        ;; elsif ($dch eq '(')
                                                                                                                                        (p-if                                 (p-str-eq $dch "(")
                                                                                                                                                                              (progn
                                                                                                                                                                                (let (($ge (make-p-box nil)) ($inner (make-p-box nil)) ($r (make-p-box nil)))
                                                                                                                                                                                  ;; my $ge = _pack_find_group_end($tmpl, $ti)
                                                                                                                                                                                  (p-my-= $ge (let ((*wantarray* nil)) (pl-_pack_find_group_end $tmpl $ti)))
                                                                                                                                                                                  
                                                                                                                                                                                  ;; my $inner = substr($tmpl, $ti, $ge - $ti)
                                                                                                                                                                                  (p-my-= $inner (p-substr $tmpl $ti (p-- $ge $ti)))
                                                                                                                                                                                  
                                                                                                                                                                                  ;; $ti = $ge + 1
                                                                                                                                                                                  (let ((*wantarray* :void))                                       (p-my-= $ti (p-+ $ge 1)))
                                                                                                                                                                                  
                                                                                                                                                                                  ;; for (my $r=0; $r<$slash_n; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }
                                                                                                                                                                                  (let (($r (make-p-box nil)))
                                                                                                                                                                                    (p-for (                                      (p-my-= $r 0))
                                                                                                                                                                                           (                                      (p-< $r $slash_n))
                                                                                                                                                                                           (                                      (p-post++ $r))
                                                                                                                                                                                           ;; _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p)
                                                                                                                                                                                           (pl-_unpack_tmpl $inner $s $si_ref $push_val $be $le $checksum_p)
                                                                                                                                                                                           
                                                                                                                                                                                           )
                                                                                                                                                                                    )
                                                                                                                                                                                  
                                                                                                                                                                                  )
                                                                                                                                                                                )
                                                                                                                                                                              nil
                                                                                                                                                                              )
                                                                                                                                        )
                                                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if (defined $grpbeg) {             my $inner = substr($tmpl, $grpbeg, $grpend - $grpbeg);             if ($all) {                 while ($$si_ref < $slen) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             } else {                 for (my $r=0; $r<$nrep; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             }             next;         }
                                  ;; if (defined $grpbeg)
                                  (p-if                       (p-defined $grpbeg)
                                                              (progn
                                                                (let (($inner (make-p-box nil)) ($r (make-p-box nil)))
                                                                  ;; my $inner = substr($tmpl, $grpbeg, $grpend - $grpbeg)
                                                                  (p-my-= $inner (p-substr $tmpl $grpbeg (p-- $grpend $grpbeg)))
                                                                  
                                                                  ;; if ($all) {                 while ($$si_ref < $slen) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             } else {                 for (my $r=0; $r<$nrep; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }             }
                                                                  ;; if ($all)
                                                                  (p-if                             $all
                                                                                                    (progn
                                                                                                      ;; while ($$si_ref < $slen) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }
                                                                                                      (p-while (p-< (p-cast-$ $si_ref) $slen)
                                                                                                               ;; _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p)
                                                                                                               (pl-_unpack_tmpl $inner $s $si_ref $push_val $be $le $checksum_p)
                                                                                                               
                                                                                                               )
                                                                                                      
                                                                                                      )
                                                                                                    ;; else
                                                                                                    (progn
                                                                                                      (let (($r (make-p-box nil)))
                                                                                                        ;; for (my $r=0; $r<$nrep; $r++) {                     _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p);                 }
                                                                                                        (let (($r (make-p-box nil)))
                                                                                                          (p-for (                                  (p-my-= $r 0))
                                                                                                                 (                                  (p-< $r $nrep))
                                                                                                                 (                                  (p-post++ $r))
                                                                                                                 ;; _unpack_tmpl($inner, $s, $si_ref, $push_val, $be, $le, $checksum_p)
                                                                                                                 (pl-_unpack_tmpl $inner $s $si_ref $push_val $be $le $checksum_p)
                                                                                                                 
                                                                                                                 )
                                                                                                          )
                                                                                                        
                                                                                                        )
                                                                                                      )
                                                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'x') { $$si_ref += $all ? ($slen-$$si_ref) : $nrep; next }
                                  ;; if ($ch eq 'x')
                                  (p-if                       (p-str-eq $ch "x")
                                                              (progn
                                                                ;; $$si_ref += $all ? ($slen-$$si_ref) : $nrep
                                                                (let ((*wantarray* :void))                           (p-incf (p-cast-$ $si_ref) (p-if $all (p-- $slen (p-cast-$ $si_ref)) $nrep)))
                                                                
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'X') { $$si_ref -= $nrep; $$si_ref = 0 if $$si_ref<0; next }
                                  ;; if ($ch eq 'X')
                                  (p-if                       (p-str-eq $ch "X")
                                                              (progn
                                                                ;; $$si_ref -= $nrep
                                                                (let ((*wantarray* :void))                           (p-decf (p-cast-$ $si_ref) $nrep))
                                                                
                                                                ;; $$si_ref = 0 if $$si_ref<0
                                                                (let ((*wantarray* :void)) (p-if                           (p-< (p-cast-$ $si_ref) 0)                           (p-setf (p-cast-$ $si_ref) 0)))
                                                                
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq '@') { $$si_ref = defined($count) ? $count : 0; next }
                                  ;; if ($ch eq '@')
                                  (p-if                       (p-str-eq $ch "@")
                                                              (progn
                                                                ;; $$si_ref = defined($count) ? $count : 0
                                                                (let ((*wantarray* :void))                           (p-setf (p-cast-$ $si_ref) (p-if (p-defined $count) $count 0)))
                                                                
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq '%' || $ch eq '!' ) { next }
                                  ;; if ($ch eq '%' || $ch eq '!')
                                  (p-if                       (p-|| (p-str-eq $ch "%") (p-str-eq $ch "!"))
                                                              (progn
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'p'||$ch eq 'P'||$ch eq 'D') { next }
                                  ;; if ($ch eq 'p'||$ch eq 'P'||$ch eq 'D')
                                  (p-if                       (p-|| (p-|| (p-str-eq $ch "p") (p-str-eq $ch "P")) (p-str-eq $ch "D"))
                                                              (progn
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; my ($nb, $sig, $dbe) = _pack_type_info($ch, $bang)
                                  (p-list-= (vector $nb $sig $dbe) (let ((*wantarray* t)) (pl-_pack_type_info $ch $bang)))
                                  
                                  ;; if ($nb) {             my $be2 = $be ? 1 : ($le ? 0 : $dbe);             my $n = $all ? int(($slen-$$si_ref)/$nb) : $nrep;             for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_read_int($s, $$si_ref, $nb, $be2, $sig));                 $$si_ref += $nb;             }             next;         }
                                  ;; if ($nb)
                                  (p-if                       $nb
                                                              (progn
                                                                (let (($be2 (make-p-box nil)) ($n (make-p-box nil)) ($i (make-p-box nil)))
                                                                  ;; my $be2 = $be ? 1 : ($le ? 0 : $dbe)
                                                                  (p-my-= $be2 (p-if $be 1 (p-if $le 0 $dbe)))
                                                                  
                                                                  ;; my $n = $all ? int(($slen-$$si_ref)/$nb) : $nrep
                                                                  (p-my-= $n (p-if $all (p-int (p-/ (p-- $slen (p-cast-$ $si_ref)) $nb)) $nrep))
                                                                  
                                                                  ;; for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_read_int($s, $$si_ref, $nb, $be2, $sig));                 $$si_ref += $nb;             }
                                                                  (let (($i (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $i 0))
                                                                           (                            (p-< $i $n))
                                                                           (                            (p-post++ $i))
                                                                           ;; $push_val->(_unpack_read_int($s, $$si_ref, $nb, $be2, $sig))
                                                                           (let ((*wantarray* :void))                                 (let ((*wantarray* :void)) (p-funcall-ref $push_val (let ((*wantarray* :void)) (pl-_unpack_read_int $s (p-cast-$ $si_ref) $nb $be2 $sig)))))
                                                                           
                                                                           ;; $$si_ref += $nb
                                                                           (p-incf (p-cast-$ $si_ref) $nb)
                                                                           
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'f') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             my $n = $all ? int(($slen-$$si_ref)/4) : $nrep;             for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_float32($s, $$si_ref, $be2)); $$si_ref += 4;             }             next;         }
                                  ;; if ($ch eq 'f')
                                  (p-if                       (p-str-eq $ch "f")
                                                              (progn
                                                                (let (($be2 (make-p-box nil)) ($n (make-p-box nil)) ($i (make-p-box nil)))
                                                                  ;; my $be2 = $be ? 1 : ($le ? 0 : 0)
                                                                  (p-my-= $be2 (p-if $be 1 (p-if $le 0 0)))
                                                                  
                                                                  ;; my $n = $all ? int(($slen-$$si_ref)/4) : $nrep
                                                                  (p-my-= $n (p-if $all (p-int (p-/ (p-- $slen (p-cast-$ $si_ref)) 4)) $nrep))
                                                                  
                                                                  ;; for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_float32($s, $$si_ref, $be2)); $$si_ref += 4;             }
                                                                  (let (($i (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $i 0))
                                                                           (                            (p-< $i $n))
                                                                           (                            (p-post++ $i))
                                                                           ;; $push_val->(_unpack_float32($s, $$si_ref, $be2))
                                                                           (let ((*wantarray* :void))                                 (let ((*wantarray* :void)) (p-funcall-ref $push_val (let ((*wantarray* :void)) (pl-_unpack_float32 $s (p-cast-$ $si_ref) $be2)))))
                                                                           
                                                                           ;; $$si_ref += 4
                                                                           (p-incf (p-cast-$ $si_ref) 4)
                                                                           
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'd'||$ch eq 'F') {             my $be2 = $be ? 1 : ($le ? 0 : 0);             my $n = $all ? int(($slen-$$si_ref)/8) : $nrep;             for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_float64($s, $$si_ref, $be2)); $$si_ref += 8;             }             next;         }
                                  ;; if ($ch eq 'd'||$ch eq 'F')
                                  (p-if                       (p-|| (p-str-eq $ch "d") (p-str-eq $ch "F"))
                                                              (progn
                                                                (let (($be2 (make-p-box nil)) ($n (make-p-box nil)) ($i (make-p-box nil)))
                                                                  ;; my $be2 = $be ? 1 : ($le ? 0 : 0)
                                                                  (p-my-= $be2 (p-if $be 1 (p-if $le 0 0)))
                                                                  
                                                                  ;; my $n = $all ? int(($slen-$$si_ref)/8) : $nrep
                                                                  (p-my-= $n (p-if $all (p-int (p-/ (p-- $slen (p-cast-$ $si_ref)) 8)) $nrep))
                                                                  
                                                                  ;; for (my $i=0; $i<$n; $i++) {                 $push_val->(_unpack_float64($s, $$si_ref, $be2)); $$si_ref += 8;             }
                                                                  (let (($i (make-p-box nil)))
                                                                    (p-for (                            (p-my-= $i 0))
                                                                           (                            (p-< $i $n))
                                                                           (                            (p-post++ $i))
                                                                           ;; $push_val->(_unpack_float64($s, $$si_ref, $be2))
                                                                           (let ((*wantarray* :void))                                 (let ((*wantarray* :void)) (p-funcall-ref $push_val (let ((*wantarray* :void)) (pl-_unpack_float64 $s (p-cast-$ $si_ref) $be2)))))
                                                                           
                                                                           ;; $$si_ref += 8
                                                                           (p-incf (p-cast-$ $si_ref) 8)
                                                                           
                                                                           )
                                                                    )
                                                                  
                                                                  ;; next
                                                                  (p-next)
                                                                  
                                                                  )
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; if ($ch eq 'A'||$ch eq 'a'||$ch eq 'Z'||$ch eq 'H'||$ch eq 'h'||             $ch eq 'B'||$ch eq 'b'||$ch eq 'u'||$ch eq 'U'||$ch eq 'W'||$ch eq 'w') {             _unpack_str($ch, $nrep, $all, $s, $si_ref, $push_val, $checksum_p);             next;         }
                                  ;; if ($ch eq 'A'||$ch eq 'a'||$ch eq 'Z'||$ch eq 'H'||$ch eq 'h'||
                                  ;;             $ch eq 'B'||$ch eq 'b'||$ch eq 'u'||$ch eq 'U'||$ch eq 'W'||$ch eq 'w')
                                  (p-if                       (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-str-eq $ch "A") (p-str-eq $ch "a")) (p-str-eq $ch "Z")) (p-str-eq $ch "H")) (p-str-eq $ch "h")) (p-str-eq $ch "B")) (p-str-eq $ch "b")) (p-str-eq $ch "u")) (p-str-eq $ch "U")) (p-str-eq $ch "W")) (p-str-eq $ch "w"))
                                                              (progn
                                                                ;; _unpack_str($ch, $nrep, $all, $s, $si_ref, $push_val, $checksum_p)
                                                                (let ((*wantarray* :void))                           (let ((*wantarray* :void)) (pl-_unpack_str $ch $nrep $all $s $si_ref $push_val $checksum_p)))
                                                                
                                                                ;; next
                                                                (p-next)
                                                                
                                                                )
                                                              nil
                                                              )
                                  
                                  ;; die "Invalid type '$ch' in unpack\n"
                                  (p-die (p-string-concat "Invalid type '" $ch "' in unpack
"))
                                  
                                  )
                                )
                       
                       )
                     )
                   )
                 )
               )
             )
           )
         )
       )

;; sub p_unpack { ... }
(p-sub pl-p_unpack (&rest %_args)
       (let ((@_ (p-flatten-args %_args)))
         (block nil
           (let (($c (make-p-box nil)))
             (let (($tmpl (make-p-box nil)) ($s (make-p-box nil)))
               ;; my ($tmpl, $s) = @_
               (p-list-= (vector $tmpl $s) @_)
               
               ;; $s = '' unless defined $s
               (let ((*wantarray* :void)) (p-unless           (p-defined $s)           (p-my-= $s "")))
               
               (let (($checksum_width (make-p-box nil)))
                 ;; my $checksum_width = 0
                 (p-my-= $checksum_width 0)
                 
                 ;; if ($tmpl =~ s/^%(\d*)//) {         $checksum_width = length($1) ? int($1) : 16;     }
                 ;; if ($tmpl =~ s/^%(\d*)//)
                 (p-if             (p-=~ $tmpl (p-subst "^%(\\d*)" ""))
                                   (progn
                                     ;; $checksum_width = length($1) ? int($1) : 16
                                     (p-my-= $checksum_width (p-if (p-length $1) (p-int $1) 16))
                                     
                                     )
                                   nil
                                   )
                 
                 (let (($utf8_mode (make-p-box nil)))
                   ;; my $utf8_mode = ($tmpl =~ s/^U0//)
                   (p-my-= $utf8_mode (p-=~ $tmpl (p-subst "^U0" "")))
                   
                   (let (($bytes (make-p-box nil)) ($code (make-p-box nil)))
                     ;; if ($utf8_mode) {         my $bytes = '';         for my $c (split //, $s) {             my $code = ord($c);             if    ($code < 0x80)    { $bytes .= chr($code) }             elsif ($code < 0x800)   { $bytes .= chr(0xC0|($code>>6)) . chr(0x80|($code&0x3F)) }             elsif ($code < 0x10000) { $bytes .= chr(0xE0|($code>>12)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F)) }             else { $bytes .= chr(0xF0|($code>>18)).chr(0x80|(($code>>12)&0x3F)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F)) }         }         $s = $bytes;     }
                     ;; if ($utf8_mode)
                     (p-if                 $utf8_mode
                                           (progn
                                             (let (($bytes (make-p-box nil)) ($c (make-p-box nil)) ($code (make-p-box nil)))
                                               ;; my $bytes = ''
                                               (p-my-= $bytes "")
                                               
                                               ;; for my $c (split //, $s) {             my $code = ord($c);             if    ($code < 0x80)    { $bytes .= chr($code) }             elsif ($code < 0x800)   { $bytes .= chr(0xC0|($code>>6)) . chr(0x80|($code&0x3F)) }             elsif ($code < 0x10000) { $bytes .= chr(0xE0|($code>>12)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F)) }             else { $bytes .= chr(0xF0|($code>>18)).chr(0x80|(($code>>12)&0x3F)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F)) }         }
                                               (p-foreach ($c                       (p-split (p-regex "//") $s))
                                                          (let (($code (make-p-box nil)))
                                                            ;; my $code = ord($c)
                                                            (p-my-= $code (p-ord $c))
                                                            
                                                            ;; if    ($code < 0x80)    { $bytes .= chr($code) }             elsif ($code < 0x800)   { $bytes .= chr(0xC0|($code>>6)) . chr(0x80|($code&0x3F)) }             elsif ($code < 0x10000) { $bytes .= chr(0xE0|($code>>12)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F)) }             else { $bytes .= chr(0xF0|($code>>18)).chr(0x80|(($code>>12)&0x3F)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F)) }
                                                            ;; if ($code < 0x80)
                                                            (p-if                           (p-< $code #x80)
                                                                                            (progn
                                                                                              ;; $bytes .= chr($code)
                                                                                              (p-.= $bytes (p-chr $code))
                                                                                              
                                                                                              )
                                                                                            ;; elsif ($code < 0x800)
                                                                                            (p-if                             (p-< $code #x800)
                                                                                                                              (progn
                                                                                                                                ;; $bytes .= chr(0xC0|($code>>6)) . chr(0x80|($code&0x3F))
                                                                                                                                (p-.= $bytes (p-. (p-chr (p-bit-or #xC0 (p->> $code 6))) (p-chr (p-bit-or #x80 (p-bit-and $code #x3F)))))
                                                                                                                                
                                                                                                                                )
                                                                                                                              ;; elsif ($code < 0x10000)
                                                                                                                              (p-if                               (p-< $code #x10000)
                                                                                                                                                                  (progn
                                                                                                                                                                    ;; $bytes .= chr(0xE0|($code>>12)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F))
                                                                                                                                                                    (p-.= $bytes (p-. (p-. (p-chr (p-bit-or #xE0 (p->> $code 12))) (p-chr (p-bit-or #x80 (p-bit-and (p->> $code 6) #x3F)))) (p-chr (p-bit-or #x80 (p-bit-and $code #x3F)))))
                                                                                                                                                                    
                                                                                                                                                                    )
                                                                                                                                                                  ;; else
                                                                                                                                                                  (progn
                                                                                                                                                                    ;; $bytes .= chr(0xF0|($code>>18)).chr(0x80|(($code>>12)&0x3F)).chr(0x80|(($code>>6)&0x3F)).chr(0x80|($code&0x3F))
                                                                                                                                                                    (p-.= $bytes (p-. (p-. (p-. (p-chr (p-bit-or #xF0 (p->> $code 18))) (p-chr (p-bit-or #x80 (p-bit-and (p->> $code 12) #x3F)))) (p-chr (p-bit-or #x80 (p-bit-and (p->> $code 6) #x3F)))) (p-chr (p-bit-or #x80 (p-bit-and $code #x3F)))))
                                                                                                                                                                    
                                                                                                                                                                    )
                                                                                                                                                                  )
                                                                                                                              )
                                                                                            )
                                                            
                                                            )
                                                          )
                                               
                                               ;; $s = $bytes
                                               (p-my-= $s $bytes)
                                               
                                               )
                                             )
                                           nil
                                           )
                     
                     (let ((@result__lex__4 (make-array 0 :adjustable t :fill-pointer 0)))
                       ;; my @result (bare declaration)
                       
                       (let (($checksum__lex__5 (make-p-box nil)))
                         ;; my $checksum = 0
                         (p-my-= $checksum__lex__5                     0)
                         
                         (let (($push_val (make-p-box nil)))
                           ;; my $push_val = $checksum_width         ? sub { $checksum += $_[0] }         : sub { push @result, $_[0] }
                           (p-my-= $push_val (p-if $checksum_width (lambda (&rest %_args)
                                                                     (let ((@_ (p-flatten-args %_args))
                                                                           (*pcl-caller-wantarray* *wantarray*))
                                                                       (catch :p-return
                                                                         (block nil
                                                                           ;; $checksum += $_[0]
                                                                           (p-incf $checksum__lex__5 (p-aref @_ 0))
                                                                           
                                                                           )
                                                                         )
                                                                       )
                                                                     ) (lambda (&rest %_args)
                                                                     (let ((@_ (p-flatten-args %_args))
                                                                           (*pcl-caller-wantarray* *wantarray*))
                                                                       (catch :p-return
                                                                         (block nil
                                                                           ;; push @result, $_[0]
                                                                           (p-push @result__lex__4 (p-aref @_ 0))
                                                                           
                                                                           )
                                                                         )
                                                                       )
                                                                     )))
                           
                           (let (($si (make-p-box nil)))
                             ;; my $si = 0
                             (p-my-= $si 0)
                             
                             ;; _unpack_tmpl($tmpl, $s, \$si, $push_val, 0, 0, $checksum_width ? 1 : 0)
                             (let ((*wantarray* :void))                         (let ((*wantarray* :void)) (pl-_unpack_tmpl $tmpl $s (p-backslash $si) $push_val 0 0 (p-if $checksum_width 1 0))))
                             
                             ;; if ($checksum_width) {         return $checksum % (2 ** $checksum_width);     }
                             ;; if ($checksum_width)
                             (p-if                         $checksum_width
                                                           (progn
                                                             ;; return $checksum % (2 ** $checksum_width)
                                                             (p-return (p-% $checksum__lex__5 (p-** 2 $checksum_width)))
                                                             
                                                             )
                                                           nil
                                                           )
                             
                             ;; return @result
                             (p-return @result__lex__4)
                             
                             )
                           )
                         )
                       )
                     )
                   )
                 )
               )
             )
           )
         )
       )

(defvar $a (make-p-box nil))
(defvar $b (make-p-box nil))

;; use strict (pragma)

;; use warnings (pragma)

(box-set $CAN_ENDIAN "sSiIlLqQjJfFdDpP")

(box-set $CAN_SHRIEK "sSiIlLnNvV")

;; # Returns (nbytes, signed_flag, big_endian_default) for integer types; () otherwise.
;; # Returns index of matching ')'. Starts just after the opening '('.
;; # Parse !, >, < modifiers. Updates $$ti_ref. Dies on invalid modifier.
;; # Parse count: *, [N], digits. Returns ($all, $count, $nrep).
;; # Pack integer value as nbytes in specified byte order.
;; # Read nbytes from $s at $si as integer.
;; # Float stubs â bodies replaced with SBCL sb-kernel calls after ./pl2cl.
;; # Pack string/bit/uuencode types. Each consumes ONE arg from $args_ref.
;; # Encode one UTF-8 codepoint, append to $$r.
;; # Main pack loop. Reads from $args_ref via $$ai_ref, appends to $$result_ref.
;; # Decode one UTF-8 sequence from $s at $$si_ref. Returns codepoint, advances si.
;; # Unpack string/bit/uu types. $checksum_p: for B/b push individual bits.
;; # Core unpack loop. Reads from $s using $$si_ref, pushes items via $push_val.
;; 1
1


(defun p-pack (template &rest args)
  (apply #'pl-p_pack template args))

(defun p-unpack (template &optional (str $_))
  (pl-p_unpack template str))
