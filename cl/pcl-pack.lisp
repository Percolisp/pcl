                ...
                (p-while 1
                  (let (($ch (make-p-box nil)) ($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)) ($bang (make-p-box nil)) ($all (make-p-box nil)) ($count (make-p-box nil)) ($nrep (make-p-box nil)) ($inner (make-p-box nil)) ($n (make-p-box nil)) ($nb (make-p-box nil)))
                    (let ((--pcl-if-ret--2 nil))
                      ...
                      (p-while (p-&& (p-< $ti $tlen) (p-=~ (p-substr $tmpl $ti 1) (p-regex "/[!<>]/")))
            ...
            (p-if             (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-str-eq (p-substr $tmpl (p-cast-$ $ti_ref) 1) "["))
              (progn
                (let (($start (make-p-box nil)) ($depth (make-p-box nil)) ($c (make-p-box nil)) ($inner (make-p-box nil)) ($n (make-p-box nil)))
                  ...
                  (let ((*wantarray* :void)) (p-if                   (p-> $depth 0)                   (p-die "No group ending character ']' found in template
")))
                              ...
                              (progn
                                (let (($dnb (make-p-box nil)) ($dsig (make-p-box nil)) ($ddbe (make-p-box nil)))
                                  (let ((--pcl-if-ret--10 nil))
                                    ...
                                    ;; if ($dnb)
                                    (p-if (setf --pcl-if-ret--10                                     $dnb)
                                      (progn
                                        ;; $$result_ref .= _pack_emit_int($dlen, $dnb, $dsig, $dbe2 ? 1 : ($dle2 ? 0 : $ddbe))
                                        (setf --pcl-if-ret--10                                         (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_emit_int $dlen $dnb $dsig (p-if $dbe2 1 (p-if $dle2 0 $ddbe))))))
...
")))
                      ...
                      ;; die "'/' must follow a numeric type in pack\n" if $ch eq '/'
                      (let ((*wantarray* :void)) (p-if                       (p-str-eq $ch "/")                       (p-die "'/' must follow a numeric type in pack
")))
          ...
          (let (($n_open (make-p-box nil)) ($n_close (make-p-box nil)))
            ...
            (let (($tlen (make-p-box nil)))
              ...
              (let (($c (make-p-box nil)))
                ...
                ;; die "No group ending character ']' found in template\n" if $n_open > $n_close
                (let ((*wantarray* :void)) (p-if                 (p-> $n_open $n_close)                 (p-die "No group ending character ']' found in template
")))
...
;; 1
1
;; Override float stubs with real SBCL IEEE 754 implementations.
;; The transpiled stubs return "" / 0.0; these use sb-kernel bit accessors.
