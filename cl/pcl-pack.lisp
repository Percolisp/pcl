...
(defvar $i (make-p-box nil))
(defvar $pcl_pack_comma_warned (make-p-box nil))
(defvar %x (make-hash-table :test 'equal))
...
(p-sub pl-_pack_type_info
  (&optional ($ch (p-undef)) ($bang (p-undef)) &rest %_args)
  (declare (ignore %_args) (dynamic-extent %_args))
  (block nil
    (let ((*wantarray* :void))
      (p-if (p-str-eq $ch "c")
        (progn (p-return (if *wantarray* (vector 1 1 0) (progn 1 1 0))))
        (p-if (p-str-eq $ch "C")
          (progn (p-return (if *wantarray* (vector 1 0 0) (progn 1 0 0))))
          (p-if (p-str-eq $ch "s")
            (progn (p-return (if *wantarray* (vector 2 1 0) (progn 2 1 0))))
            (p-if (p-str-eq $ch "S")
              (progn (p-return (if *wantarray* (vector 2 0 0) (progn 2 0 0))))
              (p-if (p-str-eq $ch "n")
                (progn
                  (p-return
                    (if *wantarray* (vector 2 (p-if $bang 1 0) 1) (progn 2 (p-if $bang 1 0) 1))))
                (p-if (p-str-eq $ch "v")
                  (progn
                    (p-return
                      (if *wantarray* (vector 2 (p-if $bang 1 0) 0) (progn 2 (p-if $bang 1 0) 0))))
                  (p-if (p-str-eq $ch "i")
                    (progn (p-return (if *wantarray* (vector 4 1 0) (progn 4 1 0))))
                    (p-if (p-str-eq $ch "I")
                      (progn (p-return (if *wantarray* (vector 4 0 0) (progn 4 0 0))))
                      (p-if (p-str-eq $ch "l")
                        (progn
                          (p-return
                            (if *wantarray* (vector (p-if $bang 8 4) 1 0) (progn (p-if $bang 8 4) 1 0))))
                        (p-if (p-str-eq $ch "L")
                          (progn
                            (p-return
                              (if *wantarray* (vector (p-if $bang 8 4) 0 0) (progn (p-if $bang 8 4) 0 0))))
                          (p-if (p-str-eq $ch "N")
                            (progn
                              (p-return
                                (if *wantarray* (vector 4 (p-if $bang 1 0) 1) (progn 4 (p-if $bang 1 0) 1))))
                            (p-if (p-str-eq $ch "V")
                              (progn
                                (p-return
                                  (if *wantarray* (vector 4 (p-if $bang 1 0) 0) (progn 4 (p-if $bang 1 0) 0))))
                              (p-if (p-str-eq $ch "q")
                                (progn (p-return (if *wantarray* (vector 8 1 0) (progn 8 1 0))))
                                (p-if (p-str-eq $ch "Q")
                                  (progn
                                    (p-return (if *wantarray* (vector 8 0 0) (progn 8 0 0))))
                                  (p-if (p-str-eq $ch "j")
                                    (progn
                                      (p-return (if *wantarray* (vector 8 1 0) (progn 8 1 0))))
                                    (p-if (p-str-eq $ch "J")
                                      (progn
                                        (p-return (if *wantarray* (vector 8 0 0) (progn 8 0 0))))))))))))))))))))
      (p-return (progn )))))
          ...
          (let (($tlen (make-p-box nil)))
            ...
            (p-while (p-< $ti $tlen)
              (let (($ch (make-p-box nil)))
                ...
                (p-if
                  (p-|| (p-|| (p-|| (p-|| (p-str-eq $ch " ") (p-str-eq $ch "	")) (p-str-eq $ch "
")) (p-str-eq $ch "")) (p-str-eq $ch ""))
                  (progn (p-post++ $ti))
                  (p-if (p-str-eq $ch ",")
                    (progn
                      (p-if (p-! $pcl_pack_comma_warned)
                        (progn
                          (p-warn :loc "cl/pack-impl.pl line 92" "Invalid type ',' in pack
")
                    ...
                    (p-if (p-str-eq $ch "#")
                      (progn (p-post++ $ti)
                        (p-while
                          (p-&& (p-< $ti $tlen) (p-str-ne (p-substr $s $ti 1) "
"))
                          (p-post++ $ti))
...
"))
                        (p-post++ $ti)))
                    (p-if (p-str-eq $ch "(")
                      (progn (p-post++ $depth) (p-post++ $ti))
                      (p-if (p-str-eq $ch ")")
                        (progn (p-post-- $depth) (p-if (p-> $depth 0) (p-post++ $ti)))
                        (progn (p-post++ $ti)))))))
              (p-return $ti))))))))
                    ...
                    (p-if (p-str-eq $m "!")
                      (progn
                        (p-if (p-! (p->= (p-index $CAN_SHRIEK $ch) 0))
                          (p-die :loc "cl/pack-impl.pl line 138" (p-string-concat "'!' allowed only after types " $CAN_SHRIEK " in " $ctx "
")))
                        (p-if $got_bang
                          (p-warn :loc "cl/pack-impl.pl line 140" (p-string-concat "Duplicate modifier '!' after '" $ch "' in " $ctx "
")))
                        (p-my-= $bang 1)
                        (p-my-= $got_bang 1)
                      ...
                      (p-if (p-str-eq $m ">")
                        (progn
                          (p-if
                            ...
                            (p-die :loc "cl/pack-impl.pl line 143" (p-string-concat "'>' allowed only after types " $CAN_ENDIAN " in " $ctx "
")))
                          (p-if $got_le
                            (p-die :loc "cl/pack-impl.pl line 145" (p-string-concat "Can't use both '<' and '>' after type '" $ch "' in " $ctx "
")))
                          (p-if $inh_le
                            (p-die :loc "cl/pack-impl.pl line 146" (p-string-concat "Can't use '>' in a group with different byte-order in " $ctx "
")))
                          (p-if $got_be
                            (p-warn :loc "cl/pack-impl.pl line 147" (p-string-concat "Duplicate modifier '>' after '" $ch "' in " $ctx "
")))
                          (p-my-= $be 1)
                          (p-my-= $le 0)
                          (p-my-= $got_be 1)
                        ...
                        (p-if (p-str-eq $m "<")
                          (progn
                            (p-if
                              (p-! (p-|| (p->= (p-index $CAN_ENDIAN $ch) 0) (p-str-eq $ch "(")))
                              (p-die :loc "cl/pack-impl.pl line 150" (p-string-concat "'<' allowed only after types " $CAN_ENDIAN " in " $ctx "
")))
                            (p-if $got_be
                              (p-die :loc "cl/pack-impl.pl line 152" (p-string-concat "Can't use both '<' and '>' after type '" $ch "' in " $ctx "
")))
                            (p-if $inh_be
                              (p-die :loc "cl/pack-impl.pl line 153" (p-string-concat "Can't use '<' in a group with different byte-order in " $ctx "
")))
                            (p-if $got_le
                              (p-warn :loc "cl/pack-impl.pl line 154" (p-string-concat "Duplicate modifier '<' after '" $ch "' in " $ctx "
")))
                            (p-my-= $le 1)
                            (p-my-= $be 0)
                            (p-my-= $got_le 1)
                            (p-post++ (p-cast-$ $ti_ref)))
                          (progn (p-last)))))))
                (p-return (if *wantarray* (vector $bang $be $le) (progn $bang $be $le)))))))))))
                  ...
                  (let (($ch (make-p-box nil)))
                    ...
                    (let (($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)))
                      ...
                      (let (($bang (make-p-box nil)))
                        (p-my-= $bang 0)
                        (p-while
                          (p-&& (p-< $ti $tlen) (let ((*wantarray* nil)) (p-=~ (p-substr $tmpl $ti 1) (p-regex "/[!<>]/"))))
                        ...
                        (let
                          ...
                          (p-if (p-defined $grpbeg)
                            (progn
                              (let (($inner (make-p-box nil)))
                                ...
                                (p-incf $pos (p-* (let ((*wantarray* nil)) (pl-_pack_template_size $inner)) $nrep))
                                (p-next))))
                          (p-if (p-str-eq $ch "@")
                            (progn
                              (p-my-= $pos
                                (p-if $bang (p-if (p-defined $count) $count 0) (p-+ 0 (p-if (p-defined $count) $count 0))))
                              (p-next)))
                          (p-if (p-str-eq $ch "x")
                            (progn
                              (p-if $bang
                          ...
                          (let (($nb (make-p-box nil)))
                            ...
                            (p-if (p-|| (p-str-eq $ch "p") (p-str-eq $ch "P"))
                              (progn (p-incf $pos (p-* 8 $nrep)) (p-next)))
                            (p-if
                              (p-|| (p-|| (p-str-eq $ch "W") (p-str-eq $ch "U")) (p-str-eq $ch "w"))
                              (progn (p-incf $pos $nrep) (p-next)))
      ...
      (let (($tmpl (make-p-box nil)) ($ti_ref (make-p-box nil)))
        ...
        (let ((*wantarray* :void))
          (let (($tlen (make-p-box nil)))
            (p-my-= $tlen (p-length $tmpl))
            (p-if
              (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-str-eq (p-substr $tmpl (p-cast-$ $ti_ref) 1) "*"))
              (progn (p-post++ (p-cast-$ $ti_ref))
                (p-return
                  (if *wantarray* (vector 1 (let ((*wantarray* t)) (p-undef)) 1) (progn 1 (let ((*wantarray* t)) (p-undef)) 1)))))
            (p-if
              (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-str-eq (p-substr $tmpl (p-cast-$ $ti_ref) 1) "["))
              (progn (p-post++ (p-cast-$ $ti_ref))
                (let (($start (make-p-box nil)))
                  ...
                  (let (($depth (make-p-box nil)))
                    ...
                    (p-while (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (p-> $depth 0))
                      (let (($c (make-p-box nil)))
                        ...
                        (p-if (p-str-eq $c "[")
                          (progn (p-post++ $depth))
                          (p-if (p-str-eq $c "]") (progn (p-post-- $depth))))))
                    (p-if (p-> $depth 0)
                      (p-die :loc "cl/pack-impl.pl line 241" "No group ending character ']' found in template
"))
                    (let (($inner (make-p-box nil)))
                      (p-my-= $inner
                      ...
                      (p-if (let ((*wantarray* nil)) (p-=~ $inner (p-regex "/^\\d+$/")))
                        (progn
                          (let (($n (make-p-box nil)))
                            (p-my-= $n (p-+ $inner 0))
                            (p-return (if *wantarray* (vector 0 $n $n) (progn 0 $n $n))))))
                      (p-if (p->= (p-index $inner "@") 0)
                        (p-die :loc "cl/pack-impl.pl line 248" "Within []-length '@' not allowed
"))
                      (p-if
                        (p-&& (let ((*wantarray* nil)) (p-=~ $inner (p-regex "/^\\d/"))) (let ((*wantarray* nil)) (p-!~ $inner (p-regex "/^\\d+$/"))))
                        (p-die :loc "cl/pack-impl.pl line 249" "Malformed integer in []
"))
                      (let (($n (make-p-box nil)))
                        (p-my-= $n (pl-_pack_template_size $inner))
                        (p-return (if *wantarray* (vector 0 $n $n) (progn 0 $n $n)))))))))
            (p-if
              (p-&& (p-< (p-cast-$ $ti_ref) $tlen) (let ((*wantarray* nil)) (p-=~ (p-substr $tmpl (p-cast-$ $ti_ref) 1) (p-regex "/\\d/"))))
              (progn
                (let (($n (make-p-box nil)))
                  (p-my-= $n 0)
                  (p-while
                    ...
                    (p-post++ (p-cast-$ $ti_ref)))
                  (p-return (if *wantarray* (vector 0 $n $n) (progn 0 $n $n))))))
            (p-return (if *wantarray* (vector 0 1 1) (progn 0 1 1)))))))))
            ...
            (progn
              (let (($k 0))
                (p-for ()
                  ...
                  ((setf $k (p-+ $k 1)))
                  (p-my-= $v
                    (p-bit-or (p-<< $v 8) (p-if (p-< (p-+ $si $k) $slen) (p-ord (p-substr $s (p-+ $si $k) 1)) 0))))))
            (progn
              (let (($k (p-- $nbytes 1)))
                (p-for ()
                  ((p->= $k 0))
...
(p-sub pl-_pack_float32
  (&optional ($val (p-undef)) ($be (p-undef)) &rest %_args)
  (declare (ignore %_args) (dynamic-extent %_args))
  (block nil (p-return "")))
...
(p-sub pl-_unpack_float32
  (&optional ($s (p-undef)) ($si (p-undef)) ($be (p-undef)) &rest %_args)
  (declare (ignore %_args) (dynamic-extent %_args))
  (block nil (p-return 0.0)))
                                  ...
                                  (let (($bit 0))
                                    (p-for ()
                                      ...
                                      (let (($idx (make-p-box nil)))
                                        (p-my-= $idx (p-+ $bs $bit))
                                        (p-if
                                          (p-&& (p-< $idx $slen) (p-str-eq (p-substr $arg $idx 1) "1"))
                        ...
                        (p-if (setf --pcl-if-ret--0 (p-str-eq $ch "H"))
                                  ...
                                  (p-for ((p-my-= $k 0))
                                    ...
                                    (let (($hi (make-p-box nil)))
                                      ...
                                      (let (($lo (make-p-box nil)))
                                        (p-my-= $lo
                                          (p-if (p-< (p-+ $k 1) $slen) (p-hex (p-substr $arg (p-+ $k 1) 1)) 0))
                          ...
                          (p-if (setf --pcl-if-ret--0 (p-str-eq $ch "h"))
                            ...
                            (p-if (setf --pcl-if-ret--0 (p-str-eq $ch "u"))
                              (setf --pcl-if-ret--0
                                (progn
                                  (let (($line_len (make-p-box nil)))
                                    (p-my-= $line_len 45)
                                    (p-if (p-&& (p-! $star) (p-> $nrep 45))
                                      (progn
                                        (p-if (p-> $nrep 63)
                                          (progn
                                            (p-warn :loc "cl/pack-impl.pl line 371" "Field too wide in 'u' format in pack")
                                            (p-my-= $line_len 63))
                                          (progn (p-my-= $line_len $nrep)))))
                                    (let (($cs (make-p-box nil)))
                                      (p-for ((p-my-= $cs 0))
                                        ((p-< $cs $slen))
                                        ((p-incf $cs $line_len))
                                        (let (($ce (make-p-box nil)))
                                          (p-my-= $ce
                                            (p-if (p-< (p-+ $cs $line_len) $slen) (p-+ $cs $line_len) $slen))
                                          (let (($chunk (make-p-box nil)))
                                            (p-my-= $chunk (p-substr $arg $cs (p-- $ce $cs)))
                                            (let (($clen (make-p-box nil)))
                                              (p-my-= $clen (p-length $chunk))
                                              (p-.= (p-cast-$ $result_ref) (p-chr (p-+ 32 $clen)))
                                              (let (($k (make-p-box nil)))
                                                (p-for ((p-my-= $k 0))
                                                  ((p-< $k $clen))
                                                  ((p-incf $k 3))
                                                  (let (($b0 (make-p-box nil)))
                                                    ...
                                                    (let (($b1 (make-p-box nil)))
                                                      (p-my-= $b1
                                                        (p-if (p-< (p-+ $k 1) $clen) (p-ord (p-substr $chunk (p-+ $k 1) 1)) 0))
                                                      (let (($b2 (make-p-box nil)))
                                                        (p-my-= $b2
                                                          (p-if (p-< (p-+ $k 2) $clen) (p-ord (p-substr $chunk (p-+ $k 2) 1)) 0))
                                                        (let (($cm (make-p-box nil)))
                                                          (p-my-= $cm
                                                            (p-bit-or (p-bit-or (p-<< $b0 16) (p-<< $b1 8)) $b2))
      ...
      (block nil
        (let ((*wantarray* :void))
          (let (($c (make-p-box nil)))
            ...
            ;; $c == 32 ? 96 : $c
            (let ((*wantarray* *pcl-caller-wantarray*)) (p-if (p-== $c 32) 96 $c))
      ...
      (let
        ...
        (let ((*wantarray* :void))
          ...
          (p-if (p-> $depth $MAX_GROUP_DEPTH)
            (p-die :loc "cl/pack-impl.pl line 412" "Too deeply nested ()-groups in pack
"))
                  ...
                  (let (($ch (make-p-box nil)))
                    ...
                    (let (($grpbeg (make-p-box nil)) ($grpend (make-p-box nil)))
                      ...
                      (let
                        ...
                        (let (($ti_before_count (make-p-box nil)))
                                    ...
                                    (block nil
                                      (tagbody :redo
                                        (let (($c (make-p-box nil)))
                                          ...
                                          (p-if
                                            (p-|| (p-|| (p-str-eq $c "*") (p-str-eq $c "[")) (let ((*wantarray* nil)) (p-=~ $c (p-regex "/\\d/"))))
                                            (p-die :loc "cl/pack-impl.pl line 439" "'/' does not take a repeat count in pack
")))
                                        :next)))
                                  (let (($dfmt (make-p-box nil)))
                                    ...
                                    (let
                                      ...
                                      (p-my-= $ti (pl-_pack_skip_ws $tmpl $ti))
                                      (let (($ti_before_dcount (make-p-box nil)))
                                          ...
                                          (let (($had_dcount (make-p-box nil)))
                                            ...
                                            (let
                                              ...
                                              (let (($actual_count (make-p-box nil)))
                                                (p-if
                                                  (p-|| (p-|| (p-str-eq $dfmt "a") (p-str-eq $dfmt "A")) (p-str-eq $dfmt "Z"))
                                                  (progn
                                                    (let (($darg (make-p-box nil)))
                                                      (p-my-= $darg
                                                        (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) ""))
                                                      ...
                                                      (let (($dlen (make-p-box nil)))
                                                        ...
                                                        (p-if (p-|| (p-! $had_dcount) $dstar2)
                                                          (progn
                                                            (p-my-= $actual_count
                                                              (p-if (p-str-eq $dfmt "Z") (p-+ $dlen 1) $dlen)))
                                                        ...
                                                        (let
                                                          ...
                                                          (p-if $nb
                                                            (progn
                                                              (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_emit_int $actual_count $nb $sig (p-if $be 1 (vector (p-if $le 0 $dbe)))))))
                                                            (p-if
                                                              (p-|| (p-str-eq $ch "A") (p-str-eq $ch "a"))
                                                              (progn
                                                    ...
                                                    (progn
                                                      (let (($remaining (make-p-box nil)))
                                                        ...
                                                        (p-if (p-|| (p-! $had_dcount) $dstar2)
                                                          ...
                                                          (progn
                                                            (p-my-= $actual_count
                                                              (p-if (p-< $dnrep2 $remaining) $dnrep2 $remaining))))
                                                        (let
                                                          (($nb (make-p-box nil))
                                                            ($sig (make-p-box nil))
                                                            ($dbe (make-p-box nil)))
                                                          ...
                                                          (let (($dbe_eff (make-p-box nil)))
                                                            (p-my-= $dbe_eff
                                                              (p-if $dbe2 1 (p-if $dle2 0 $ddbe)))
                                                            (let (($i__cond__0 0))
                                                              (p-for ()
                                                                ((p-&& (p-< $i__cond__0 $actual_count) (p-< (p-cast-$ $ai_ref) $nargs)))
                                                                ((setf $i__cond__0
                                                                    (p-+ $i__cond__0 1)))
                                                                (let (($val (make-p-box nil)))
                                                                  (p-my-= $val
                                                                    (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))))
                                                                  (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_emit_int $val $dnb $dsig $dbe_eff))))))))))
                                                      ...
                                                      (progn
                                                        (let (($remaining (make-p-box nil)))
                                                          (p-my-= $remaining
                                                            (p-- $nargs (p-cast-$ $ai_ref)))
                                                          (p-my-= $actual_count
                                                            (p-if (p-|| (p-! $had_dcount) $dstar2) $remaining (p-if (p-< $dnrep2 $remaining) $dnrep2 $remaining)))
                                                          (let
                                                            (($nb (make-p-box nil))
                                                              ($sig (make-p-box nil))
                                                              ($dbe (make-p-box nil)))
                                                            ...
                                                            (let (($i__cond__1 0))
                                                              (p-for ()
                                                                ((p-&& (p-< $i__cond__1 $actual_count) (p-< (p-cast-$ $ai_ref) $nargs)))
                                                                ((setf $i__cond__1
                                                                    (p-+ $i__cond__1 1)))
                                                                (let (($v (make-p-box nil)))
                                                                  (p-my-= $v
                                                                    (p-+ (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0))
                                                                  (p-if (p-== $v 0)
                                                                    (progn
                                                                      (p-.= (p-cast-$ $result_ref) (p-chr 0)))
                                                                    (progn
                              ...
                              (p-if (p-defined $grpbeg)
                                (progn
                                  (let (($inner (make-p-box nil)))
                                    ...
                                    (let (($gti (make-p-box nil)))
                                      (p-my-= $gti (pl-_pack_skip_ws $inner 0))
                                      (p-if (p-< $gti (p-length $inner))
                                        (progn
                                          (let (($fc (make-p-box nil)))
                                            (p-my-= $fc (p-substr $inner $gti 1))
                                            (p-if
                                              (let ((*wantarray* nil)) (p-=~ $fc (p-regex "/^[\\d\\*\\[]/")))
                                              (p-die :loc "cl/pack-impl.pl line 556" "()-group starts with a count in pack
")))))
                                      (p-if $star
                                        (progn
                                          (p-while (p-< (p-cast-$ $ai_ref) $nargs)
                                            (let (($ai_before (make-p-box nil)))
                                        ...
                                        (progn
                                          (let (($r__cond__15 0))
                                            (p-for ()
                                              ((p-< $r__cond__15 $nrep))
                                              ((setf $r__cond__15 (p-+ $r__cond__15 1)))
                                              (let (($iter_base (make-p-box nil)))
                                                (p-my-= $iter_base
                                                  (p-length (p-cast-$ $result_ref)))
                                                (pl-_pack_tmpl $inner
                              ...
                              (p-if (p-str-eq $ch ".")
                                (progn
                                  (let (($tgt (make-p-box nil)))
                                    (p-my-= $tgt
                                      (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-int (p-+ (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0)) 0))
                                    (let (($abs_tgt (make-p-box nil)))
                                      ...
                                      (let (($cur (make-p-box nil)))
                                        ...
                                        (p-next))))))
                              (p-if
                                (p-|| (p-|| (p-str-eq $ch "p") (p-str-eq $ch "P")) (p-str-eq $ch "D"))
                                (progn
                                  (p-die :loc "cl/pack-impl.pl line 618" (p-string-concat "Invalid type '" $ch "' in pack
"))))
                                  ...
                                  (progn
                                    (let (($be2 (make-p-box nil)))
                                      ...
                                      (let (($r__cond__16 0))
                                        (p-for ()
                                          ((p-< $r__cond__16 $nrep))
                                          ((setf $r__cond__16 (p-+ $r__cond__16 1)))
                                          (let (($v (make-p-box nil)))
                                            (p-my-= $v
                                              (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                            (let (($nv (make-p-box nil)))
                                              (p-my-= $nv (p-+ $v 0))
                                              (p-if (p-!= $nv $nv)
                                                (p-die :loc "cl/pack-impl.pl line 631" "Cannot pack NaN in pack
"))
                                              (p-if (p-&& (p-!= $nv 0) (p-== $nv (p-* $nv 2)))
                                                (p-die :loc "cl/pack-impl.pl line 632" (p-. (p-. "Cannot pack " (p-if (p-< $nv 0) "-Inf" "Inf")) " in pack
")))
                                              (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_emit_int (let ((*wantarray* t)) (p-int $nv)) $nb $sig $be2)))))))
                                      (p-next))))
                                (p-if (p-str-eq $ch "f")
                                  (progn
                                    (let (($be2 (make-p-box nil)))
                                      ...
                                      (let (($r__cond__17 0))
                                        (p-for ()
                                          ((p-< $r__cond__17 $nrep))
                                          ((setf $r__cond__17 (p-+ $r__cond__17 1)))
                                          (let (($v (make-p-box nil)))
                                            (p-my-= $v
                                              (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                            (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_float32 $v $be2))))))
                                      (p-next))))
                                  ...
                                  (progn
                                    (let (($be2 (make-p-box nil)))
                                      ...
                                      (let (($r__cond__18 0))
                                        (p-for ()
                                          ((p-< $r__cond__18 $nrep))
                                          ((setf $r__cond__18 (p-+ $r__cond__18 1)))
                                          (let (($v (make-p-box nil)))
                                            (p-my-= $v
                                              (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                            (p-.= (p-cast-$ $result_ref) (let ((*wantarray* nil)) (pl-_pack_float64 $v $be2))))))
                                      (p-next))))
                                (p-if
                                  (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-str-eq $ch "a") (p-str-eq $ch "A")) (p-str-eq $ch "Z")) (p-str-eq $ch "b")) (p-str-eq $ch "B")) (p-str-eq $ch "H")) (p-str-eq $ch "h")) (p-str-eq $ch "u"))
                                  (progn
                                    (let (($arg (make-p-box nil)))
                                      (p-my-= $arg
                                        (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) "") ""))
                                ...
                                (p-if (p-str-eq $ch "U")
                                  (progn
                                    (let (($r__cond__19 0))
                                      (p-for ()
                                        ((p-< $r__cond__19 $nrep))
                                        ((setf $r__cond__19 (p-+ $r__cond__19 1)))
                                        (let (($v (make-p-box nil)))
                                          (p-my-= $v
                                            (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                          (let (($nv (make-p-box nil)))
                                            (p-my-= $nv (p-+ $v 0))
                                            (p-if (p-!= $nv $nv)
                                              (p-die :loc "cl/pack-impl.pl line 669" "Cannot pack NaN in pack
"))
                                            (p-if (p-&& (p-!= $nv 0) (p-== $nv (p-* $nv 2)))
                                              (p-die :loc "cl/pack-impl.pl line 670" (p-. (p-. "Cannot pack " (p-if (p-< $nv 0) "-Inf" "Inf")) " in pack
")))
                                ...
                                (p-if (p-str-eq $ch "W")
                                  (progn
                                    (let (($r__cond__20 0))
                                      (p-for ()
                                        ((p-< $r__cond__20 $nrep))
                                        ((setf $r__cond__20 (p-+ $r__cond__20 1)))
                                        (let (($v (make-p-box nil)))
                                          (p-my-= $v
                                            (p-if (p-< (p-cast-$ $ai_ref) $nargs) (p-// (p-aref-deref $args_ref (p-post++ (p-cast-$ $ai_ref))) 0) 0))
                                          (let (($nv (make-p-box nil)))
                                            (p-my-= $nv (p-+ $v 0))
                                            (p-if (p-!= $nv $nv)
                                              (p-die :loc "cl/pack-impl.pl line 679" "Cannot pack NaN in pack
"))
                                            (p-if (p-&& (p-!= $nv 0) (p-== $nv (p-* $nv 2)))
                                              (p-die :loc "cl/pack-impl.pl line 680" (p-. (p-. "Cannot pack " (p-if (p-< $nv 0) "-Inf" "Inf")) " in pack
")))
                                ...
                                (p-if (p-str-eq $ch "w")
                                  (progn
                                    (let (($r__cond__21 0))
                                      (p-for ()
                                        ((p-< $r__cond__21 $nrep))
                                        ((setf $r__cond__21 (p-+ $r__cond__21 1)))
                                        (let (($raw (make-p-box nil)))
                                          ...
                                          (let (($orig_s (make-p-box nil)))
                                            ...
                                            (let (($v (make-p-box nil)))
                                              (p-my-= $v (p-+ $raw 0))
                                              (p-if (p-!= $v $v)
                                                (p-die :loc "cl/pack-impl.pl line 690" "Cannot compress NaN in pack
"))
                                              (p-if (p-&& (p-< $v 0) (p-== $v (p-* $v 2)))
                                                (p-die :loc "cl/pack-impl.pl line 691" "Cannot compress -Inf in pack
"))
                                              (p-if (p-< $v 0)
                                                (p-die :loc "cl/pack-impl.pl line 692" "Cannot compress negative numbers in pack
"))
                                              (p-if (p-&& (p-!= $v 0) (p-== $v (p-* $v 2)))
                                                (p-die :loc "cl/pack-impl.pl line 693" "Cannot compress Inf in pack
"))
                                              (p-if (p-!= $v (p-int $v))
                                                (p-die :loc "cl/pack-impl.pl line 694" "Can only compress unsigned integers in pack
"))
                                              (p-if
                                                (p-&& (let ((*wantarray* nil)) (p-=~ $orig_s (p-regex "/[eE]/"))) (p->= $v (p-** 2 64)))
                                                (p-die :loc "cl/pack-impl.pl line 700" "Can only compress unsigned integers in pack
"))
                                              (p-my-= $v (p-int $v))
                                              (p-if (p-== $v 0)
                                    ...
                                    (p-next)))
                                (p-if (p-str-eq $ch "/")
                                  (p-die :loc "cl/pack-impl.pl line 712" "Invalid type '/' in pack
"))
                                (p-die :loc "cl/pack-impl.pl line 713" (p-string-concat "Invalid type '" $ch "' in pack
"))))))))))))))))))

(p-sub pl-_pack_check_brackets
  (&optional ($tmpl (p-undef)) &rest %_args)
  (declare (ignore %_args) (dynamic-extent %_args))
  (block nil
    (let ((*wantarray* :void))
      (let (($n_open (make-p-box nil)) ($n_close (make-p-box nil)))
        ...
        (let (($tlen (make-p-box nil)))
          ...
          (let (($i__cond__2 0))
            (p-for ()
              ((p-< $i__cond__2 $tlen))
              ((setf $i__cond__2 (p-+ $i__cond__2 1)))
              (let (($c (make-p-box nil)))
          ...
          (p-if (p-> $n_open $n_close)
            (p-die :loc "cl/pack-impl.pl line 726" "No group ending character ']' found in template
"))
          ...
          (let ((@stk (make-array 0 :adjustable t :fill-pointer 0)))
            ...
            (let (($i__cond__3 0))
              (p-for ()
                ((p-< $i__cond__3 $tlen))
                ((setf $i__cond__3 (p-+ $i__cond__3 1)))
                (let (($c (make-p-box nil)))
                      ...
                      (p-if (p-str-eq $c "]")
                        (progn
                          (p-if (p-|| (p-! @stk) (p-str-ne (p-aref @stk -1) "["))
                            (p-die :loc "cl/pack-impl.pl line 735" "Mismatched brackets in template
"))
                          (p-pop @stk))
                        (p-if (p-str-eq $c ")")
                          (progn (p-if (p-&& @stk (p-str-eq (p-aref @stk -1) "(")) (p-pop @stk))))))))))))))))
                ...
                (let (($k 1))
                  (p-for ()
                    ...
                    (p-if (p-< (p-+ (p-cast-$ $si_ref) $k) $slen)
                      (p-my-= $code (p-bit-or (p-<< $code 6) (p-bit-and (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) $k) 1)) #x3F))))))
  ...
  (p-args-body
    (block nil
      (let
        ...
        (let ((*wantarray* nil)) (p-list-= (vector $ch $nrep $all $s $si_ref $push_val $checksum_p) @_))
        (let ((*wantarray* :void))
          (let (($slen (make-p-box nil)))
            (p-my-= $slen (p-length $s))
                ...
                (setf --pcl-if-ret--2
                  (progn
                    (let (($n (make-p-box nil)))
                      ...
                      (let (($raw (make-p-box nil)))
                        (p-my-= $raw
                          (p-if (p-< (p-cast-$ $si_ref) $slen) (p-substr $s (p-cast-$ $si_ref) $n) ""))
                        ...
                        (let (($hex (make-p-box nil)))
                          ...
                          (let (($i__cond__4 0))
                            (p-for ()
                              ...
                              ((setf $i__cond__4 (p-+ $i__cond__4 1)))
                              (let (($b (make-p-box nil)))
                                (p-my-= $b
                                  (p-if (p-< (p-+ (p-cast-$ $si_ref) $i__cond__4) $slen) (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) $i__cond__4) 1)) 0))
                          ...
                          (let (($hex (make-p-box nil)))
                            ...
                            (let (($i__cond__5 0))
                              (p-for ()
                                ...
                                ((setf $i__cond__5 (p-+ $i__cond__5 1)))
                                (let (($b (make-p-box nil)))
                                  (p-my-= $b
                                    (p-if (p-< (p-+ (p-cast-$ $si_ref) $i__cond__5) $slen) (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) $i__cond__5) 1)) 0))
                            ...
                            (p-if $checksum_p
                              (progn
                                (let (($i__cond__6 0))
                                  (p-for ()
                                    ((p-< $i__cond__6 $nbits))
                                    ((setf $i__cond__6 (p-+ $i__cond__6 1)))
                                    (let (($b (make-p-box nil)))
                                      (p-my-= $b
                                        (p-if (p-< (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__6 8))) $slen) (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__6 8))) 1)) 0))
                                      (p-funcall-ref $push_val (p-bit-and (p->> $b (p-- 7 (p-% $i__cond__6 8))) 1))))))
                              (progn
                                (let (($bits (make-p-box nil)))
                                  ...
                                  (let (($i__cond__7 0))
                                    (p-for ()
                                      ((p-< $i__cond__7 $nbits))
                                      ((setf $i__cond__7 (p-+ $i__cond__7 1)))
                                      (let (($b (make-p-box nil)))
                                        (p-my-= $b
                                          (p-if (p-< (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__7 8))) $slen) (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__7 8))) 1)) 0))
                                        (p-.= $bits (p-if (p-bit-and (p->> $b (p-- 7 (p-% $i__cond__7 8))) 1) "1" "0")))))
                              ...
                              (p-if $checksum_p
                                (progn
                                  (let (($i__cond__8 0))
                                    (p-for ()
                                      ((p-< $i__cond__8 $nbits))
                                      ((setf $i__cond__8 (p-+ $i__cond__8 1)))
                                      (let (($b (make-p-box nil)))
                                        (p-my-= $b
                                          (p-if (p-< (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__8 8))) $slen) (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__8 8))) 1)) 0))
                                        (p-funcall-ref $push_val (p-bit-and (p->> $b (p-% $i__cond__8 8)) 1))))))
                                (progn
                                  (let (($bits (make-p-box nil)))
                                    (p-my-= $bits "")
                                    (let (($i__cond__9 0))
                                      (p-for ()
                                        ((p-< $i__cond__9 $nbits))
                                        ((setf $i__cond__9 (p-+ $i__cond__9 1)))
                                        (let (($b (make-p-box nil)))
                                          (p-my-= $b
                                            (p-if (p-< (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__9 8))) $slen) (p-ord (p-substr $s (p-+ (p-cast-$ $si_ref) (p-int (p-/ $i__cond__9 8))) 1)) 0))
                                          (p-.= $bits (p-if (p-bit-and (p->> $b (p-% $i__cond__9 8)) 1) "1" "0")))))
                                    (p-funcall-ref $push_val $bits))))
                              (let ((*wantarray* *pcl-caller-wantarray*))
                                (p-incf (p-cast-$ $si_ref) (p-int (p-/ (p-+ $nbits 7) 8)))))))
                            ...
                            (progn
                              (let (($decoded (make-p-box nil)))
                                ...
                                (p-while (p-< (p-cast-$ $si_ref) $slen)
                                  (let (($lc (make-p-box nil)))
                                    (p-my-= $lc (p-ord (p-substr $s (p-cast-$ $si_ref) 1)))
                                    (let (($nb (make-p-box nil)))
                                      (p-my-= $nb (p-bit-and (p-- $lc 32) 63))
                                      (p-post++ (p-cast-$ $si_ref))
                                      (p-if (p-== $nb 0) (p-last))
                                      (let (($ng (make-p-box nil)))
      ...
      (block nil
        (let ((*wantarray* :void))
          (let (($i (make-p-box nil)))
            ;; my $i=$$si_ref+$_[0]
            (p-my-= $i (p-+ (p-cast-$ $si_ref) (p-aref @_ 0)))
            
            ;; $i<$slen?(ord(substr($s,$i,1))-32)&63:0
            (let ((*wantarray* *pcl-caller-wantarray*)) (p-if (p-< $i $slen) (p-bit-and (p-- (p-ord (p-substr $s $i 1)) 32) 63) 0))
                                              ...
                                              (let (($cm (make-p-box nil)))
                                                (p-my-= $cm
                                                  (p-bit-or (p-bit-or (p-bit-or (p-<< (let ((*wantarray* nil)) (p-funcall-ref $get (p-* 4 $k))) 18) (p-<< (let ((*wantarray* nil)) (p-funcall-ref $get (p-+ (p-* 4 $k) 1))) 12)) (p-<< (let ((*wantarray* nil)) (p-funcall-ref $get (p-+ (p-* 4 $k) 2))) 6)) (let ((*wantarray* nil)) (p-funcall-ref $get (p-+ (p-* 4 $k) 3)))))
                                                (p-if (p-< (p-* $k 3) $nb)
                                                  (p-.= $decoded (p-chr (p-bit-and (p->> $cm 16) #xFF))))
                                                (p-if (p-< (p-+ (p-* $k 3) 1) $nb)
                                                  (p-.= $decoded (p-chr (p-bit-and (p->> $cm 8) #xFF))))
                                                (p-if (p-< (p-+ (p-* $k 3) 2) $nb)
                                                  (p-.= $decoded (p-chr (p-bit-and $cm #xFF))))))))
                                        (p-incf (p-cast-$ $si_ref) (p-* $ng 4))
                                        (p-if
                                          (p-&& (p-< (p-cast-$ $si_ref) $slen) (p-str-eq (p-substr $s (p-cast-$ $si_ref) 1) "
"))
                                          (p-post++ (p-cast-$ $si_ref)))))))
                                (let ((*wantarray* *pcl-caller-wantarray*))
                                ...
                                (let (($n (make-p-box nil)))
                                  ...
                                  (let (($done (make-p-box nil)))
                                    ...
                                    (p-while
                                      ...
                                      (p-funcall-ref $push_val (pl-_unpack_utf8_char $s $si_ref))
                                      (p-post++ $done))))))
                              ...
                              (setf --pcl-if-ret--2
                                (progn
                                  (let (($n (make-p-box nil)))
                                    ...
                                    (let (($i__cond__10 0))
                                      (p-for ()
                                        ((p-&& (p-< $i__cond__10 $n) (p-< (p-cast-$ $si_ref) $slen)))
                                        ((setf $i__cond__10 (p-+ $i__cond__10 1)))
                                        (p-funcall-ref $push_val (p-ord (p-substr $s (p-post++ (p-cast-$ $si_ref)) 1))))))))
                              (p-if (setf --pcl-if-ret--2 (p-str-eq $ch "w"))
                                (setf --pcl-if-ret--2
                                  (progn
                                    (let (($done (make-p-box nil)))
                                      (p-my-= $done 0)
                                      (p-while
                                        (p-&& (p-|| $all (p-< $done $nrep)) (p-< (p-cast-$ $si_ref) $slen))
                                        (let (($v (make-p-box nil)) ($more (make-p-box nil)))
                                          (let ((*wantarray* nil)) (p-list-= (vector $v $more) (vector 0 1)))
                                          (p-while $more
                                            (p-if (p->= (p-cast-$ $si_ref) $slen)
                                              (p-die :loc "cl/pack-impl.pl line 877" "Unterminated compressed integer in unpack
"))
                                            (let (($b (make-p-box nil)))
                                              (p-my-= $b
    ...
    (block nil
      (let
        ...
        (let ((*wantarray* :void))
          ...
          (p-if (p-! (p-defined $depth)) (p-my-= $depth 0))
          (p-if (p-> $depth $MAX_GROUP_DEPTH)
            (p-die :loc "cl/pack-impl.pl line 893" "Too deeply nested ()-groups in unpack
"))
          (let (($slen (make-p-box nil)))
            (p-my-= $slen (p-length $s))
            (let (($ti (make-p-box nil)))
                            ...
                            (let (($had_count (make-p-box nil)))
                              ...
                              (p-if (p-&& (p-< $ti $tlen) (p-str-eq (p-substr $tmpl $ti 1) "/"))
                                (progn (p-post++ $ti)
                                  (p-my-= $ti (pl-_pack_skip_ws $tmpl $ti))
                                  (p-if (p->= $ti $tlen)
                                    (p-die :loc "cl/pack-impl.pl line 917" "Code missing after '/' in unpack
"))
                                  (let ((*package* *package*))
                                    (block nil
                                      (tagbody :redo
                                        (let (($c (make-p-box nil)))
                                          ...
                                          (p-if
                                            (p-|| (p-|| (p-str-eq $c "*") (p-str-eq $c "[")) (let ((*wantarray* nil)) (p-=~ $c (p-regex "/\\d/"))))
                                            (p-die :loc "cl/pack-impl.pl line 921" "'/' does not take a repeat count in unpack
")))
                                        :next)))
                                  (let
                                    (($nb (make-p-box nil))
                                          ...
                                          (let (($be2 (make-p-box nil)))
                                            ...
                                            (p-if (p-> (p-+ (p-cast-$ $si_ref) $nb) $slen)
                                              (progn (p-if (p-! (p-> $depth 0)) (p-last))
                                                (p-die :loc "cl/pack-impl.pl line 929" "length/code after end of string in unpack
")))
                                            (p-my-= $slash_n
                                              (let ((*wantarray* nil)) (pl-_unpack_read_int $s (p-cast-$ $si_ref) $nb $be2 $sig)))
                                            (p-incf (p-cast-$ $si_ref) $nb)))
                                        (p-if (p-str-eq $ch "w")
                                          (progn
                                            (let (($more (make-p-box nil)))
                                              ...
                                              (p-while $more
                                                (p-if (p->= (p-cast-$ $si_ref) $slen)
                                                  (p-die :loc "cl/pack-impl.pl line 936" "Unterminated compressed integer in unpack
"))
                                                (let (($b (make-p-box nil)))
                                                  (p-my-= $b
                                                    (p-ord (p-substr $s (p-post++ (p-cast-$ $si_ref)) 1)))
                                                  (p-my-= $more (p-bit-and $b #x80))
                                                  (p-my-= $slash_n
                                                    (p-bit-or (p-<< $slash_n 7) (p-bit-and $b #x7F)))))))
                                          (p-if (p-str-eq $ch "Z")
                                            (progn
                                              (let (($end (make-p-box nil)))
                                                (p-my-= $end
                                                  (p-index $s " " (p-cast-$ $si_ref)))
                                                (p-if (p-< $end 0) (progn (p-my-= $end $slen)))
                                                (let (($raw (make-p-box nil)))
                                                  (p-my-= $raw
                                                    (p-substr $s (p-cast-$ $si_ref) (p-- $end (p-cast-$ $si_ref))))
                                                        ...
                                                        (progn
                                                          (let (($dbe3 (make-p-box nil)))
                                                            (p-my-= $dbe3
                                                              (p-if $dbe2 1 (p-if $dle2 0 $ddbe)))
                                                            (p-if
                                                              (p-> (p-+ (p-cast-$ $si_ref) $dnb) $slen)
                                                              (p-die :loc "cl/pack-impl.pl line 973" "length/code after end of string in unpack
"))
                                                            (p-my-= $slash_n
                                                              (let ((*wantarray* nil)) (pl-_unpack_read_int $s (p-cast-$ $si_ref) $dnb $dbe3 $dsig)))
                                                        ...
                                                        (p-if (p-str-eq $dch "w")
                                                          ...
                                                          (progn
                                                            (let (($raw2 (make-p-box nil)))
                                                              (p-my-= $raw2
                                                                (p-if (p-< (p-cast-$ $si_ref) $slen) (p-substr $s (p-cast-$ $si_ref) $slash_n) ""))
                                                              (p-incf (p-cast-$ $si_ref) $slash_n)
                                                              (p-if (p-str-eq $dch "A")
                                                                (p-=~ $raw2 (p-subst "[ \\x00]+$" "")))
                                                              (p-if (p-str-eq $dch "Z")
                                                                (p-=~ $raw2 (p-subst "\\x00.*" "" :s)))
                                                      ...
                                                      (p-if $dnb
                                                        (progn
                                                          (let (($dbe3 (make-p-box nil)))
                                                            ...
                                                            (let (($i__cond__11 0))
                                                              (p-for ()
                                                                ((p-&& (p-< $i__cond__11 $slash_n) (p-<= (p-+ (p-cast-$ $si_ref) $dnb) $slen)))
                                                                ((setf $i__cond__11
                                                                    (p-+ $i__cond__11 1)))
                                                                (p-funcall-ref $push_val (pl-_unpack_read_int $s (p-cast-$ $si_ref) $dnb $dbe3 $dsig))
                                                                (p-incf (p-cast-$ $si_ref) $dnb)))))
                                                        (p-if
                                                          (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-str-eq $dch "A") (p-str-eq $dch "a")) (p-str-eq $dch "Z")) (p-str-eq $dch "B")) (p-str-eq $dch "b")) (p-str-eq $dch "H")) (p-str-eq $dch "h")) (p-str-eq $dch "u")) (p-str-eq $dch "U"))
                                                          (progn
                                                            (pl-_unpack_str $dch
                                                              $slash_n
                                                              0
                                                          ...
                                                          (p-if (p-str-eq $dch "(")
                                                            (progn
                                                              (let (($ge (make-p-box nil)))
                                                                (p-my-= $ge
                                                                  (pl-_pack_find_group_end $tmpl
                                                                    $ti))
                                                                (let (($inner (make-p-box nil)))
                                                                  (p-my-= $inner
                                                                    (p-substr $tmpl $ti (p-- $ge $ti)))
                                                                  (p-my-= $ti (p-+ $ge 1))
                                                                  (let (($r__cond__22 0))
                                                                    (p-for ()
                                                                      ((p-< $r__cond__22
                                      ...
                                      (p-if (p-< $gti (p-length $inner))
                                        (progn
                                          (let (($fc (make-p-box nil)))
                                            ...
                                            (p-if
                                              (let ((*wantarray* nil)) (p-=~ $fc (p-regex "/^[\\d\\*\\[]/")))
                                              (p-die :loc "cl/pack-impl.pl line 1026" "()-group starts with a count in unpack
")))))
                                      (p-if $all
                                        (progn
                                          (p-while (p-< (p-cast-$ $si_ref) $slen)
                                            (let (($si_before (make-p-box nil)))
                                        ...
                                        (progn
                                          (let (($r__cond__23 0))
                                            (p-for ()
                                              ((p-< $r__cond__23 $nrep))
                                              ((setf $r__cond__23 (p-+ $r__cond__23 1)))
                                              (let (($iter_base (make-p-box nil)))
                                                (p-my-= $iter_base (p-cast-$ $si_ref))
                                                (pl-_unpack_tmpl $inner
                                                  $s
                              ...
                              (p-if (p-str-eq $ch "@")
                                (progn
                                  (let (($n (make-p-box nil)))
                                    (p-my-= $n (p-if (p-defined $count) $count 0))
                                    (p-setf (p-cast-$ $si_ref) (p-if $bang $n (p-+ $group_base $n)))
                                    (p-next))))
                              ...
                              (p-if
                                (p-|| (p-|| (p-str-eq $ch "p") (p-str-eq $ch "P")) (p-str-eq $ch "D"))
                                (progn
                                  (p-die :loc "cl/pack-impl.pl line 1069" (p-string-concat "Invalid type '" $ch "' in unpack
"))))
                              (p-if (p-str-eq $ch ".")
                                (progn
                                  (p-if $all
                                ...
                                (p-if $nb
                                  (progn
                                    (let (($be2 (make-p-box nil)))
                                      ...
                                      (let (($n (make-p-box nil)))
                                        (p-my-= $n
                                          (p-if $all (p-int (p-/ (p-- $slen (p-cast-$ $si_ref)) $nb)) $nrep))
                                        (let (($i__cond__12 0))
                                          (p-for ()
                                            ((p-< $i__cond__12 $n))
                                            ((setf $i__cond__12 (p-+ $i__cond__12 1)))
                                            (p-if (p-> (p-+ (p-cast-$ $si_ref) $nb) $slen)
                                              (p-last))
                                ...
                                (p-if (p-str-eq $ch "f")
                                  (progn
                                    (let (($be2 (make-p-box nil)))
                                      ...
                                      (let (($n (make-p-box nil)))
                                        (p-my-= $n
                                          (p-if $all (p-int (p-/ (p-- $slen (p-cast-$ $si_ref)) 4)) $nrep))
                                        (let (($i__cond__13 0))
                                          (p-for ()
                                            ((p-< $i__cond__13 $n))
                                            ((setf $i__cond__13 (p-+ $i__cond__13 1)))
                                ...
                                (p-if (p-|| (p-str-eq $ch "d") (p-str-eq $ch "F"))
                                  (progn
                                    (let (($be2 (make-p-box nil)))
                                      ...
                                      (let (($n (make-p-box nil)))
                                        (p-my-= $n
                                          (p-if $all (p-int (p-/ (p-- $slen (p-cast-$ $si_ref)) 8)) $nrep))
                                        (let (($i__cond__14 0))
                                          (p-for ()
                                            ((p-< $i__cond__14 $n))
                                            ((setf $i__cond__14 (p-+ $i__cond__14 1)))
                                        ...
                                        (p-next)))))
                                (p-if
                                  (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-|| (p-str-eq $ch "A") (p-str-eq $ch "a")) (p-str-eq $ch "Z")) (p-str-eq $ch "H")) (p-str-eq $ch "h")) (p-str-eq $ch "B")) (p-str-eq $ch "b")) (p-str-eq $ch "u")) (p-str-eq $ch "U")) (p-str-eq $ch "W")) (p-str-eq $ch "w"))
                                  (progn
                                    (pl-_unpack_str $ch
                                      $nrep
                                      $all
                                    ...
                                    (p-next)))
                                (p-if (p-str-eq $ch "/")
                                  (p-die :loc "cl/pack-impl.pl line 1128" "'/' must follow a numeric type in unpack
"))
                                (p-die :loc "cl/pack-impl.pl line 1129" (p-string-concat "Invalid type '" $ch "' in unpack
"))))))))))))))))))
        ...
        (let ((*wantarray* :void))
          (let (($tlen (make-p-box nil)))
            ...
            (let (($ti (make-p-box nil)))
              (p-my-= $ti 0)
              (p-my-= $ti (pl-_pack_skip_ws $tmpl $ti))
              (p-if (p->= $ti $tlen) (p-return (if *wantarray* (vector "" "") (progn "" ""))))
              (let (($ch (make-p-box nil)))
                (p-my-= $ch (p-substr $tmpl $ti 1))
                (p-post++ $ti)
                (p-if (p-str-eq $ch "(")
                ...
                (p-return
                  (if *wantarray* (vector (let ((*wantarray* t)) (p-substr $tmpl 0 $ti)) (let ((*wantarray* t)) (p-substr $tmpl $ti))) (progn (let ((*wantarray* t)) (p-substr $tmpl 0 $ti)) (let ((*wantarray* t)) (p-substr $tmpl $ti)))))))))))))
  ...
  (let ((@_ (p-flatten-args %_args))
    ...
    (catch :p-return
      (block nil
        (let ((*wantarray* :void))
          ;; $checksum += $_[0]
          (let ((*wantarray* *pcl-caller-wantarray*)) (p-incf $checksum (p-aref @_ 0)))
  ...
  (let ((@_ (p-flatten-args %_args))
    ...
    (catch :p-return
      (block nil
        (let ((*wantarray* :void))
          ;; push @result, $_[0]
          (let ((*wantarray* *pcl-caller-wantarray*)) (p-push @result (p-aref @_ 0)))
