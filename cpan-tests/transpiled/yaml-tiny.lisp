...
(box-set $re_capture_double_quoted (pcl::pl-qr "qr/\\\"([^\\\\\"]*(?:\\\\.[^\\\\\"]*)*)\\\"/"))

;; my $re_capture_single_quoted = qr/\'([^\']*(?:\'\'[^\']*)*)\'/
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defvar $re_capture_single_quoted (make-pl-box nil)))
(box-set $re_capture_single_quoted (pcl::pl-qr "qr/\\'([^\\']*(?:\\'\\'[^\\']*)*)\\'/"))

;; # unquoted re gets trailing space that needs to be stripped
;; my $re_capture_unquoted_key  = qr/([^:]+(?::+\S(?:[^:]*|.*?(?=:)))*)(?=\s*\:(?:\s+|$))/
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defvar $re_capture_unquoted_key (make-pl-box nil)))
(box-set $re_capture_unquoted_key (pcl::pl-qr "qr/([^:]+(?::+\\S(?:[^:]*|.*?(?=:)))*)(?=\\s*\\:(?:\\s+|$))/"))

;; my $re_trailing_comment      = qr/(?:\s+\#.*)?/
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defvar $re_trailing_comment (make-pl-box nil)))
(box-set $re_trailing_comment (pcl::pl-qr "qr/(?:\\s+\\#.*)?/"))

;; my $re_key_value_separator   = qr/\s*:(?:\s+(?:\#.*)?|$)/
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defvar $re_key_value_separator (make-pl-box nil)))
(box-set $re_key_value_separator (pcl::pl-qr "qr/\\s*:(?:\\s+(?:\\#.*)?|$)/"))
  ...
  (let ((@_ (make-array (length %_args) :adjustable t :fill-pointer t :initial-contents %_args)))
    (block nil
      (let (($class (make-pl-box nil)) ($file (make-pl-box nil)) ($fh (make-pl-box nil)) ($contents (make-pl-box nil)) ($err (make-pl-box nil)))
        ...
        (pl-unless         (pcl::pl--e $file)         (pl-method-call $class '_error (pl-string-concat "File '" $file "' does not exist")))
        
        ;; $class->_error( "'$file' is a directory, not a file" )         unless -f _
        (pl-unless         (pcl::pl--f _)         (pl-method-call $class '_error (pl-string-concat "'" $file "' is a directory, not a file")))
        
        ;; $class->_error( "Insufficient permissions to read '$file'" )         unless -r _
        (pl-unless         (pcl::pl--r _)         (pl-method-call $class '_error (pl-string-concat "Insufficient permissions to read '" $file "'")))
        ...
        (pl-if (pl-not         $fh)
          (progn
            ;; $class->_error("Failed to open file '$file': $!")
                        (pl-method-call $class '_error (pl-string-concat "Failed to open file '" $file "': " (pcl::pl-errno-string)))
        ...
        (pl-if         (pl-_can_flock)
          (progn
            ;; flock( $fh, Fcntl::LOCK_SH() )             or warn "Couldn't lock '$file' for reading: $!"
                        (pcl:pl-or (pl-flock $fh (Fcntl::pl-LOCK_SH)) (pl-warn (pl-string-concat "Couldn't lock '" $file "' for reading: " (pcl::pl-errno-string))))
        ...
        (pl-if (pl-not         (pl-close $fh))
          (progn
            ;; $class->_error("Failed to close file '$file': $!")
                        (pl-method-call $class '_error (pl-string-concat "Failed to close file '" $file "': " (pcl::pl-errno-string)))
        ...
        (pl-if         (pl-_can_flock)
          (progn
            ...
            ;; sysopen( $fh, $file, $flags )             or $self->_error("Failed to open file '$file' for writing: $!")
                        (pcl:pl-or (pl-sysopen $fh $file $flags) (pl-method-call $self '_error (pl-string-concat "Failed to open file '" $file "' for writing: " (pcl::pl-errno-string))))
            
            ;; binmode( $fh, ":raw:encoding(UTF-8)")
                        (pl-binmode $fh ":raw:encoding(UTF-8)")
            
            ;; flock( $fh, Fcntl::LOCK_EX() )             or warn "Couldn't lock '$file' for reading: $!"
                        (pcl:pl-or (pl-flock $fh (Fcntl::pl-LOCK_EX)) (pl-warn (pl-string-concat "Couldn't lock '" $file "' for reading: " (pcl::pl-errno-string))))
        ...
        (pl-if (pl-not         (pl-close $fh))
          (progn
            ;; $self->_error("Failed to close file '$file': $!")
                        (pl-method-call $self '_error (pl-string-concat "Failed to close file '" $file "': " (pcl::pl-errno-string)))
...
(pl-delete (pl-gethash (pl-stash "YAML::Tiny") "refaddr"))
