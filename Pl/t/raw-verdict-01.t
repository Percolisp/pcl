...
1;
MOD
# task #1004: AoInc's handler RETURNS $_[0], so storing the return and
# discarding it give the same answer -- which is exactly why the bug survived.
# AoMut's handlers return a STRING instead, so the two answers differ: perl
# discards (the handler is a mutator of $_[0]) and the object survives.
write_mod('AoMut', <<'MOD');
package AoMut;
...
# ---- a ++/-- HANDLER IS A MUTATOR: perl discards its return (task #1004) ---
#
# perl calls a `++`/`--` handler as handler($self, undef, '') and THROWS THE
# RETURN VALUE AWAY -- the handler's job is to mutate $_[0].  PCL stored it, so
# a handler that returns anything but $_[0] DECLASSED the object: `ref($a)`
# went '' and `"$a"` gave the handler's string instead of running the class's
# `""`.  Silent, and invisible to #900's rows above because AoInc's handler
# returns $_[0] and the two answers coincide.
#
# Four claims, and the LAST is the one that must not move: the AUTOGENERATED
# `++`-from-`+` really is `$x = $x + 1`, so there the result REPLACES the place
# (a different arm, %p-incdec-autogen, untouched).  Both emission paths, because
# a raw-verdict slot reaches perl-increment by its own route.
my $ovl1004 = qq{${USE}use AoMut (); use AoPlus ();
my \$a = AoMut->new(5); \$a++;
print ref(\$a), ":\$a\\n";
...
my \$g = AoPlus->new(5); \$g++;
print ref(\$g), ":\$g\\n";};
my $ovl1004_want = "AoMut:M(15)\n"
                 . "AoMut:M(15) c=M(15)\n"
                 . "AoMut:M(-5)\n"
                 . "AoPlus:P(6)\n";
test_cl('#1004: a ++/-- handler is a MUTATOR — perl discards its return',
        $ovl1004, $ovl1004_want);
test_cl('#1004: …and the general-form compiler agrees, line for line',
        $ovl1004, $ovl1004_want, 'PCL_OPT=none');
