...
# ── #1001 half (a) + #1005's `x` half: the four compound ops that reached NO
# overload dispatch, and the non-compound `x` that never asked either ──────
# `*=` `%=` `.=` `x=` INLINED their arithmetic instead of delegating to the
# base operator that owns the dispatch, so a class with a `*` handler answered
# 126 (42*3) where perl calls the handler; `x` had no dispatch in EITHER form.
# The `.`/`x` pair is :AUTOGENERATED from `""` in perl, which is why the S1
# rows (a class with only `""`) must NOT move — that answer was already right.
# Every expectation is the live perl 5.40.3 answer.
test_cl('#1001(a)+#1005: *= %= .= x= and x dispatch use overload',
    'package B1;
     use overload q(*) => sub { "MUL" }, q(%) => sub { "MOD" },
                  q(.) => sub { "CAT" }, q(x) => sub { "REP" },
                  q(0+) => sub { 42 },   q("") => sub { "B1" };
     sub new { bless {}, shift }
     package S1;
     use overload q("") => sub { "s" }, fallback => 1;
     sub new { bless {}, shift }
     package main;
     my $a = B1->new; $a *= 3; print "$a\n";
     my $b = B1->new; $b %= 3; print "$b\n";
     my $c = B1->new; $c .= 3; print "$c\n";
     my $d = B1->new; $d x= 3; print "$d\n";
     print B1->new * 3, "\n";
     print B1->new % 3, "\n";
     print B1->new . 3, "\n";
     print B1->new x 3, "\n";
     print 3 x B1->new, "\n";
     my $e = S1->new; $e x= 3; print "$e\n";
     my $f = S1->new; $f .= 3; print "$f\n";
     print S1->new x 2, "\n";',
    "MUL\nMOD\nCAT\nREP\nMUL\nMOD\nCAT\nREP\nREP\nsss\ns3\nss\n");

# The negative half, and the SECOND bug the same copy carried: `x=`'s inlined
# body was `(apply #'concatenate 'string (make-list N …))`, the very spelling
# task #880 removed from `p-str-x` because it passes N ARGUMENTS to
# CONCATENATE — so `$s x= 200000` exhausted SBCL's control stack while
# `$s = $s x 200000` was fine.  `x=` delegates to `p-str-x` now, which also
# gets it the zero/negative/float/array-in-scalar rules from one place.
test_cl('#1001(a): the plain compound paths are untouched, and x= stops blowing the stack',
    'my $n = 7;    $n *= 6;      print "$n\n";
     my $m = 17;   $m %= 5;      print "$m\n";
     my $s = "a";  $s .= "b";    print "$s\n";
     my $r = "ab"; $r x= 3;      print "$r\n";
     my $z = "ab"; $z x= 0;      print "[$z]\n";
     my $y = "ab"; $y x= -2;     print "[$y]\n";
     my $w = "ab"; $w x= 2.7;    print "$w\n";
     my $v = "a";  $v x= 200000; print length($v), "\n";
     my @a = (1,2,3); my $u = @a; $u x= 2; print "$u\n";',
    "42\n2\nab\nababab\n[]\n[]\nabab\n200000\n33\n");

# ── #1005: the math family and `!` dispatch `use overload` ────────────────
# `abs sqrt cos sin exp log atan2 !` were the operators perl overloads that
# PCL never asked about: `abs $x` answered the `0+` number and the rest the
# coerced float.  THREE different fall-throughs, which is why one "just add
# the lookup" pass would have been wrong:
#   * `sqrt cos sin exp log atan2` have NO autogeneration -> `nomethod`;
#   * `!` IS autogenerated (from `bool`, which PCL already answered right
#     through p-true-p's conversion chain), so `nomethod` must NOT see it and
#     only the class's OWN `!` handler was missing;
#   * `abs` autogenerates from `<` and `-` — NOT from `0+` — so a class with
#     those two and no `abs` answers an OBJECT (the AB row).
# `int` is deliberately not here: perl autogenerates it from `0+` and PCL's
# numification already IS that answer.
# Every expectation is the live perl 5.40.3 answer.
test_cl('#1005: own abs/sqrt/cos/sin/exp/log/atan2/! handlers are called',
    'package OW;
     use overload q(abs) => sub { "A" }, q(sqrt) => sub { "Q" },
                  q(cos) => sub { "C" }, q(sin)  => sub { "I" },
                  q(exp) => sub { "E" }, q(log)  => sub { "L" },
                  q(atan2) => sub { "T" }, q(!)  => sub { "B" },
                  q(0+)  => sub { 7 },   q("")   => sub { "OW" };
     sub new { bless {}, shift }
     package main;
     my $o = OW->new;
...
test_cl('#1005: with no handler they reach nomethod (and ! and int do NOT)',
    'package NM;
     use overload q(0+) => sub { 7 }, q("") => sub { "NM" },
                  nomethod => sub { "NM[$_[3]]" };
     sub new { bless {}, shift }
...
# `abs`'s autogeneration and `!`'s, each from its own source, and the plain
# paths beside them.  AB's `<` answers FALSE for `$ab < 0`, so perl's
# `$x < 0 ? -$x : $x` hands back the OBJECT and stringifies it — the `0+`
# answer PCL used to give (0 here) is not perl's.
test_cl('#1005: abs autogenerates from < and -, ! from bool, plain paths intact',
    'package AB;
