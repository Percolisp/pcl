...
our %KIND_A = (
  ...
  'foreach-arrays' => "Pl::Parser::_foreach_bare_arrays + Parser2 foreach (task #1184): a `for my \$x (\@a, \@b)` that already qualifies for the read-only arm AND whose list is nothing but BARE named arrays iterates each array IN TURN (`:arrays t`, %p-make-array-run / %p-run-elt-raw) instead of flattening them into a temporary first -- the flattening is 88 % of the `feread2` bench row.  No new fact: the licence is `foreach-raw`'s own two conjuncts plus the list SHAPE",
