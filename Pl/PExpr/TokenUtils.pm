...
# --- Can this run of code REACH a dynamic loop exit?  (task #1022, #1162) ---
#
# THE LICENCE for a dynamic-exit frame (ir-spec §6.2, Kind-A gate
# `dyn-loop-exit`), as ruled by Fable in s470.  Perl's unlabelled `last`/
# `next`/`redo` acts on the innermost DYNAMICALLY enclosing loop, so a loop
# that can reach such an exit must catch it — and a loop that cannot must be
# emitted exactly as before, because the frame costs ~4.8 MB of SBCL COMPILE
# IR per `catch` (measured, task #1162: 88 frames in one 64 KB top-level form
# took `t/op/loopctl.t` past the default 1 GB heap and it produced no rows at
# all).
#
# $SET is the compilation unit's MAY-DYN-EXIT set (Environment::dyn_exit_subs,
# computed by Pl::Parser2::_may_dyn_exit_set).  A run of code can reach a
# dynamic exit when it either CONTAINS a marked site — which, inside a loop
# body, means a nested `sub {…}` carrying one, the `{ foo(sub { …; last }) }`
