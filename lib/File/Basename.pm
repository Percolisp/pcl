...
#
# PCL SHIM (task #189).  Core's version modifies its argument through the
# `@_` alias (`$_[0] =~ s{…}{}`).  PCL does not alias a plain `my` lexical
# through @_ — a deliberate divergence for the raw-slot speed model, see
# docs/not-supported.md "`@_` argument aliasing" — so under PCL the
# substitution warned "Cannot modify non-boxed value in s///" and did
# NOTHING: `dirname("/a/b/c")` answered "/a/b/" instead of "/a/b", wrong
# for every path, in one of the most-used functions in core.  This copy
# keeps the in-place write (so it behaves identically under real perl) and
# additionally RETURNS the stripped value; the five call sites assign it.
# DELETE THIS SHIM when #189 makes `$_[N]` writes reach lexical arguments.
