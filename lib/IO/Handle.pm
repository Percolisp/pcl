...
# calls DESTROY (deliberate, docs/not-supported.md 'DESTROY called by garbage
...
sub autoflush {
    # PCL: SelectSaver restores the selection from DESTROY, which PCL never
    # calls (docs/not-supported.md).  Same effect, explicitly.
...
sub printflush {
    my $io = shift;
    # PCL: explicit select save/restore instead of SelectSaver (no DESTROY).
    my $old = ref($io) ? select(qualify($io, caller)) : undef;
    local $| = 1;
    if(ref($io)) {
        print $io @_;
    }
