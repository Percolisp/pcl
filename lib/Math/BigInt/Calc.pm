...
# ============================================================================
# PCL SHIM — WHY THIS FILE EXISTS
# ----------------------------------------------------------------------------
# This is a copy of perl's Math::BigInt::Calc with ONE change: the original
# BEGIN block's two precision-probing loops are replaced by hardcoded constants
# (see below).  Everything else is byte-identical to the stock module.
#
# The stock BEGIN auto-detects the platform's optimal "base length" by finding
# the largest $e for which "9"x$e * "9"x$e is still computed EXACTLY in native
# perl arithmetic.  The first loop is an empty-condition `for (;;)` that exits
# ONLY via `last if $yn != $ys` — i.e. it relies on native double/64-bit
# multiplication *losing precision* to terminate.
#
# PCL represents all integers as Common Lisp bignums (arbitrary precision), so
# "9"x$e * "9"x$e is ALWAYS exact, the `last` never fires, and the loop spins
# forever (PCL hangs at load).  This is a fundamental consequence of PCL's
# deliberate unlimited-integer-precision number model, not a transpiler bug —
# any module that probes native numeric precision via a rollover loop has the
# same problem.
#
# Fix: hardcode the values stock perl computes on this platform (perl 5.40,
# x86-64): MAX_EXP_F = MAX_EXP_I = 9, so BASE_LEN = 9, USE_INT = 1.  Because
# PCL's arithmetic is exact at ANY base, these are correct rather than merely
# "optimal" — the probe was only ever an efficiency choice.  PCL's `use integer`
# mul/div/mod were verified to match perl for the base-10**9 element range.
#
# If/when the real Math::BigInt::Calc loads without hanging (e.g. PCL gains a
# native-precision rollover mode), this shim should be deleted.  See
# docs/shipped-modules.md ("Shims that exist to dodge a PCL semantic gap").
# ============================================================================

BEGIN {
    # Stock perl on this platform computes MAX_EXP_F = MAX_EXP_I = 9 here, via
    # two precision-rollover probe loops that cannot terminate under PCL's
    # bignums (see the header comment above).  Hardcoded instead.  We pass the
    # base length (9) and use_int (1) as literals to _base_len rather than
    # threading them through the file-scoped `my $BASE_LEN`/`$MAX_EXP_*`
    # lexicals, which _base_len's own validation reads back.
    $MAX_EXP_F = 9;
    $MAX_EXP_I = 9;

    __PACKAGE__ -> _base_len(9, 1);
}
