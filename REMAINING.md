...
wantarray        # Works for list vs scalar. No void context distinction (see note below)
...
See `PERL_TESTS_STATUS.md` for complete status of all 49 copied test files.
...
See `docs/test-failures-categorized.md` for full categorized analysis of all 103 test files.

**Key blocking issues** (in priority order):
1. **sprintf** — missing %g, %e, %a, %x, %o, %b format specifiers
2. **Transpile failures** — unbalanced parens in codegen (5+ files: anonsub, concat2, die_exit, recurse, splice)
3. **eval/die** — $@ propagation, die with objects, closure capture
4. **Missing builtins** — pack, tie, pos, prototype, UTF8::native_to_unicode

**Deferred (by design):**
- **wantarray void context** — PCL uses two contexts (list/scalar), not three. Perl's
  `wantarray` distinguishes void (undef), scalar (false), and list (true). Our `*wantarray*`
  is nil/t, so code that checks `(defined wantarray)` to detect void context sees void
  where it should see scalar. This is correct for code generation (void and scalar behave
  identically), but 14 tests in wantarray.t fail because they introspect the context.
  Fix would be trivial (three-valued `*wantarray*`: nil/`:S`/t) but low priority since
  real CPAN code rarely branches on void-vs-scalar.
- **String escapes** — `\Q\E`, `\U`, `\L`, `\u`, `\l`, `\F` case-changing escapes and
  `\x{}`, `\o{}` unicode escapes. Deferred pending a decision on character encoding
  strategy (legacy encodings vs modern Unicode). Affects lc.t (45 failures), qq.t (30).

**Bugs found from Perl test suite (prioritized):**

See `PERL_TESTS_STATUS.md` for per-file results.

### Tier 1: Easy fixes, high impact

**Bug 1: test.pl stubs shadow pcl-test.lisp** (blocks: bool.t; affects: any test using cmp_ok/like/unlike)
- `require './test.pl'` transpiles test.pl which defines empty `main::pl-cmp_ok` etc.
- These shadow the real `pcl::pl-cmp_ok` from pcl-test.lisp
- Fix: Remove stub function bodies for functions handled by pcl-test.lisp
- Keep stubs only for functions NOT in pcl-test.lisp (set_up_inc, fresh_perl_is, runperl, etc.)
- **Complication**: This is part of a larger issue with `require` loading files whose
  function definitions shadow inherited package functions. The test.pl case is fixable
  by editing the stub file, but the general problem (any `use`/`require` can shadow
  inherited functions) needs thought.

**Bug 2: wantarray returns boxed value** (blocks: wantarray.t comparisons)
- `is(wantarray(), ...)` shows `#S(PL-BOX` in test output
- Fix: ensure pl-wantarray returns a plain value, or unbox in the comparison

### Tier 2: Common blockers across multiple tests

**Bug 3: `q{}` / `q()` / `q[]` quoting** (blocks: wantarray.t READ error, possibly others)
- Parser doesn't recognize `q{text}` single-quote syntax
- Generated CL has malformed strings causing SBCL READ errors
- Medium effort: need to handle q/qq/qw with arbitrary delimiters in parser

**Bug 4: Undeclared variables crash** (blocks: push.t, delete.t, each.t after initial tests)
- Perl silently treats undeclared package globals as undef
- Our code crashes with "unbound variable" (`$UNDEF1`, `%REFHASH`, `warnings::$VERSION`)
- Fix options: (a) wrap variable access in handler-case, (b) auto-declare on first use,
  (c) generate `(if (boundp '$var) $var *pl-undef*)` for non-`my` variables
- **Sub-bug:** Forward declaration scanner (`_insert_variable_forward_declarations`) excludes
  variables that appear in ANY `let` binding, even if also used as globals at the top level.
  Example: `my $x` inside a sub creates `(let (($x ...)))`, but `$x` is also used as a
  package global outside the sub. Scanner excludes it → crash.
  Fix: need scope-aware analysis, not simple regex matching. Must parse generated CL to track
  whether a variable reference is inside vs outside a `let` scope.

**Bug 5: Package auto-declaration for qualified vars** (blocks: each.t, join.t, many others)
- `$pkg::var` crashes if package not yet declared
- `warnings::$VERSION`, `main::var`, `$o::var` all fail
- Fix: auto-declare package in codegen when generating qualified variable access

**Bug 6: Bare word constants** (blocks: concat.t, concat2.t, my.t)
- Words like `DEEP`, `USE` are parsed as unbound variables instead of bareword strings
- In Perl, uppercase bare words in certain contexts are string literals
- Fix: detect barewords and emit as strings (or at least handle `use constant` output)

### Tier 3: Missing features (each blocks specific tests)

**Bug 7: `do FILE`** (blocks: defined.t)
- `pl-do` function not implemented
- Needs to load and execute a file, returning last expression value

**Bug 8: `*FOO` typeglobs** (blocks: auto.t, sort.t)
- Typeglob syntax not implemented at all
- Significant feature: typeglob holds scalar, array, hash, sub, filehandle under one name
- Partial fix possible: handle `*foo = \&bar` as sub aliasing

**Bug 9: Closures / lexical scoping** (blocks: closure.t, affects real-world Perl)
- All `my` variables use `defvar` (global), so closures don't capture lexical values
- This is an **architecture-level issue** — fixing it requires changing how `my` works
- Options: (a) compile closures to CLOS objects with captured slots, (b) use CL `let` blocks
  with proper scope nesting, (c) gensym-based approach for each closure instance
- High impact but very complex

**Bug 10: `utf8::encode` / `use bytes`** (blocks: sub.t, chr.t)
- UTF-8 encoding functions not implemented
- `use bytes` pragma not recognized
- Could add no-op stubs for basic compatibility
