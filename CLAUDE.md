...
2k. `docs/opus5-review-requests-s376.md` - **s376 asks AWAITING Fable review**: eight asks. (1) the standing rule for a gate test whose EXPECTATION encoded the old wrong output (#275 touched two); (2) perl's "no plan was declared" diagnostic, deliberately left out — file it or close the #202 family; (3) #276's two-predicate shape (`_block_is_empty` beside `_block_is_hash_constructor` — the "second copy" smell, done on purpose) and (4) its ambiguous-brace residue (`f {$k,$v}` / `f {%h}`); (5) **LAYER: bless `builtin::` as PCL's shim-dispatch seam for box-representation predicates?** — `is_dual`/`is_vstring` are not real perl builtins, and `DUALVAR` already was not; `readonly`/`prototype` want the same answer; (6) `uniqnum`'s `pack "d"` key is exact for DOUBLES only (UV > 2^53 collides); (7) **#239's fix shape ruled BEFORE building** — sibling trigger on `_requalify_block_our_after_pkg_switch` + WHICH resolver decides "is this bare name a global", and should an unclassifiable name DIE; (8) does a change under `lib/` make the full sweep non-optional (corpus-diff's corpus is `perl-tests/*.t`, so it never transpiles a shim).
