...
- **131 test files, 4595 tests** with a built pclxs sibling (s339, measured);
  **4581 without** (arithmetic: minus the 14 xs rows).  The gate count is deterministic *per environment*, but it
  ...
  **GOTCHA that cost s319 an investigation: comparing against a `git worktree`
  ...
  and subtract the 14.  What must hold either way is `Result: PASS` and 131
  files.
...
2u. `docs/opus5-review-requests-s346.md` - **OPEN ASKS, awaiting Fable (s346)**: (1) #240 — I GATED the `our`-read-back-in-an-eval-package-region shape rather than ship a silent-wrong; confirm that policy, and choose whether the two-half emitter fix lands before E4.1 or parks behind E5.4 (tension with §5a.2's "every v1 hit is pre-work"); (2) #230/F6 — the run bucket is ALREADY one form per top-level statement, so the ruling's "split at statement boundaries" may not be the applicable cut; locate first (command recorded), then confirm the fallback. Both E4.1 pre-work families are CLEARED (F3 8→0, F1 18→0); neither ask blocks the queue.
