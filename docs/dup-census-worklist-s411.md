...
| the tail (~380 clusters, ~5–12 lines each) | | ~1 700 | **SATISFIED s414** — re-run `tools/dup-census.pl --min-lines 8` over `Pl/**` + `cl/pcl-runtime.lisp` leaves SIX EXACT runs and every one is in code with a standing verdict (four in `Parser.pm`'s `_process_block` / `_process_block_in_tail_context` / `_process_scheduled_block` / `_process_local_declaration` / `_process_tail_stmt` = LEAVE, DELETED-BY E5.3; two in `StringInterpolation.pm`'s subscript walkers = SUPERSEDED-BY #388).  Everything else is SHAPE-only. | the rule was: anything EXACT ≥ 8 lines in a surviving file is an EXTRACT candidate — check the family's verdict above first.  One extra family fell out of the s414 pass and was taken: the `&`-mention arms of `exists`/`defined`/`undef` → `_amp_sub_name` + the existing `_amp_cast_operand_id` (`s414e`), which is also where `defined STDIN` reading false was found. |
