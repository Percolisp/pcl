...
| `re/qr-72922.t` | **14/0 OK (FIXED s322, task #181)** — the root cause turned out to be a THIRD defect neither half of the diagnosis named: the VarAnnotator FROZE the qr variable to its string form (raw-slot verdict) whenever its uses were all stringy, so by the time it reached another regex it was no longer an object at all. Fixes: `write-object` bars that freeze; `p-regex-from-parts` returns the object itself for a lone interpolated qr (keeping only the MATCH-time flags `/g /c /e /r` from the outer modifiers — dropping `/g` first time round made `while (/$re/g)` loop forever); the struct now keeps the perl-side SOURCE text and stringifies from it; `/xx` prints both x's. Original diagnosis below. — 12/**2** DIAGNOSED s321: `qr/$re/` where `$re` is already a qr DOUBLE-WRAPS: perl `(?^:abcdef)`, PCL `(?^:(?:abcdef))`. Perl's rule is that an interpolated qr constituting the WHOLE pattern yields the same regex; only as PART of a larger pattern does the wrapper get embedded. Same code as #181's other half (`/xx` losing an x from the wrapper) — fix together. |
| `uni/tr_utf8.t` | 6/**2** | **DIAGNOSED s321 → task #182, and it is the most serious thing this triage turned up.** Nothing to do with Unicode or `/o`: **`s/…/$h{$k}/` emits LITERAL BRACES.** `s/(a)/$h{$1}/g` gives `{a}` where perl gives `X`; `s/z/$a[1]/g` gives `[1]`. The replacement is built by a hand-rolled mini-interpolator (`_gen_interp_replacement`, `Pl/ExprToCL.pm:5837`) that knows only `\1..\9`, `$1..$9`, `${name}`, `$name` — subscripts fall through as text. Plain `"$h{$k}"` interpolation is correct, so the fix is to reuse `parse_interpolated_string` (CLAUDE.md 11). `s/(\w+)/$map{$1}/g` is a everyday Perl idiom, so this is a CPAN-blocker that produces plausible garbage rather than an error. |
