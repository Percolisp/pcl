...
Updated 2026-06-01 (session 228). **yadayada.t 16→21** (`...` now dies `Unimplemented at $0 line N.` via a runtime-built `:loc`, `Pl/Parser.pm`); **sprintf2.t 1544→1617** (+73; `Integer overflow in format string` guards for `*`-width/`*`-precision/literal-precision, `cl/pcl-runtime.lisp`); **signatures.t 672→775** (+103; two signature-param declaration bugs fixed — params are now mutable boxes, and `local $G=RHS` defaults restore on sub exit; see `docs/variable-declarations-spec.md` §4.1/§4.2). Full sweep 0 new fails, 66 fully passing. Gate 90 files / 3150. NOTE: this session found the catalog significantly stale — list-of-arrays slice, chop-on-assigned-array, `($a=…) .= 'c'`, and LHS array-slice assignment all already pass.
