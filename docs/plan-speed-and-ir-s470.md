...
| 2 | sort-result ADOPTION | **S** | wide (every `my @x = sort …`, `= map`, `= grep`, `= reverse`, `= keys`) | `p-array-=` whose RHS is a FRESH vector (the producer says so) adopts it instead of copying — A5's consumer analysis already proves freshness; the same helps every list-producing builtin | yes: the copy is visible in the A5 profile |  **STILL OPEN after round 33 (s473v)**: #1810 took the two sort rows past their bar with three RUNTIME levers (one typed classification pass, a typed predicate per element kind, the one-source case collected into a right-sized vector) and did not need the emission change adoption is — so this row keeps its own licence question (the producer's freshness) and its own probe list (a `\$x[0]` taken before the assignment).
...
| 8 | typed key sort | **M** | narrow | after adoption (#2): a merge sort specialised for all-fixnum / all-string key vectors | yes: the generic `stable-sort` |  **SHIPPED s473v as #1810, and it did NOT need adoption first.**  The profile said the sort itself was never the cost: the generic `<` as a full call per comparison was 24.3 % of `sortnum` and CL's `string<` — keyword parsing, string-type dispatch, a mismatch INDEX where a sort wants a boolean — was **61.3 %** of `sortstr` across three frames.  A typed predicate per kind plus ONE typed classification pass (replacing the two every/notany check passes) reads sortstr **+148.7 % (1.55x perl -> 0.70x, PCL now faster)** and sortnum **+78.6 % (2.63x -> 1.58x)**, with peak RSS 79.9 -> 60.6 MB.
| 9 | `strcat` residue | measure | narrow | profile first (`sb-sprof` over the row); no design until the profile names it | no |  **MEASURED AND SHIPPED s473v as #1809.**  The profile named it at once: `replace` 35.9 % + `ub32-bash-copy` 14.5 % — HALF the row was a generic sequence call to move ONE character into an adjustable string.  Copying through the UNDERLYING simple string with a one-character arm reads **+123.6 % (2.14x perl -> 0.94x, PCL now faster)**, RSS flat.  The growth policy was the other candidate and is NOT a cost (~20 adjust-arrays for 20 M appends, counted).
| 10 | per-CLASS method + overload-handler cache (#582) | **L** | wide for OO code | blocked on the `@ISA`-write invalidation rule — a Fable design | yes: #73 measured the lookup share |  **OWN-CLASS HALF SHIPPED s473s**: `p-method-call` caches `(class name, method name)` -> the SYMBOL, which needs NO generation counter (every definer writes through `symbol-function`; every removal is caught by the entry's own `fboundp`).  `methret` -27.5 %, `ovlsub` -5.8 %, `json-rt` -4.8 %, `methinh` 0.0 %.  The INHERITED half is untouched -- an own MISS is never cached -- and still wants the @ISA-write marker. |
| 11 | PCRE2 regex backend (#71, + #196 hangs, #477 quadratic `pos`) | **L** | wide for text code, and a PARITY lever | sb-alien binding; the JS target's tier design reuses the classification | yes: the engine itself |  **s474 (USER): the SPIKE is brief s473q — approved 2026-09-07, NOT high priority, AFTER round 30 (s473p) and the remaining S/M levers; before #1187, which stays HELD; ONE engine, never both; bind `libpcre2-32` (zero-copy pinned subjects, offsets = char indexes); ships only with the OS matrix + a macOS leg green.** **SPIKED AND MEASURED s473q (2026-09-07) — `docs/pcre2-spike-s473q.md`, task #71: the stop rule KEPT cl-ppcre and BOTH readings say DO NOT BEGIN THE REPLACEMENT.**  Without JIT PCRE2 LOSES the risk case `subste` 1.188× against a 0.6 % control spread; with `pcre2_jit_compile_32` it wins `subste` 0.972×, `textproc` 0.880×, `json-rt` 0.915× and still loses `regexg` 1.134×.  The FFI half is DONE and the runtime seam is ONE function (`%pcl-build-scanner` → a closure with cl-ppcre's `(scanner function)` protocol; 8 patterns verbatim over five bench rows, 0 fallbacks).  **The strong case is PARITY: +311 / −7 `re_tests` rows against perl 5.40.3, and PCRE2 answers the 18 rows where cl-ppcre HANGS — #196 verbatim, so #71 IS #196's fix.**  **The BLOCKER is portability: the four install-matrix images ship PCRE2 10.39 / 10.42 / 10.42 / 10.46 while the gains are measured on 10.46, so three of four are too old for their own repositories to supply — and there is no macOS leg in CI today at all.**  #477 was NOT retargeted: `pos100k/200k` read 0.922×/0.953× with JIT, 1.28×/1.24× without.  **PARKED by the USER (s476, 2026-09-07): a possible future, re-opened only by an image floor ≥ 10.46 on every supported OS, a vendoring decision with a macOS leg, or #1528's census — see §8 of faster-codegen-suggestions.md.** |
...
|---|---|---:|---:|---:|---|---|---|
| 1 | `list-all-packages` under a system mutex | – | 17.9 | – | **#1189** — moo-objs' profile is LOAD-and-COMPILE dominated (`p-use` was 59.7 % of its samples before Moo was pre-loaded out of the run); this is the A.4.1 module-load cost seen from inside | L | no |  **THE LOOP HALF RE-PROFILED s473x** (N=300 000 through `pcl`'s warm script cache, so the transpile and the program's compile are out of the profile): what is left after #1189 is the CLOS half — `call-with-recursive-lock` **64.1 % TOTAL**, `make-preliminary-layout` 13.9 %, `(fast-method shared-initialize (slot-object t))` 8.4 %, `(BRAID.LISP .arg0.)` 3.9 self, `update-ctors` 1.8, `grab-mutex` + `release-mutex` 2.1.  That is a `defclass` per iteration under a system mutex = **#1518**, the sibling of #1189's `defpackage` half.  Still an **L** (it needs #1189's "is this class already what we would make" predicate, and that is a design), so s473x FILED the share and did not build it. |
| 2 | `search` | 14.5 | – | 11.5 | **#1187** — it is cl-ppcre's OWN scanner calling it (graph-confirmed: `create-scanner-aux`'s lambda, 23.7 % TOTAL in json-rt), not `p-index`.  A.2 row 11 (PCRE2, #71) — or find the non-simple operand first: `do-regex-match` already coerces the SUBJECT (#680), so something else in the pair is generic — **ANSWERED s473q: there is NO non-simple operand.  cl-ppcre declaims `*string*` as the UNION type `simple-string` (`specials.lisp:71`), so its accesses are generic whatever PCL hands it; subject representation is worth 0.8–1.3 %.  #1187 CLOSED unbuilt, this row is #71's.  A DIFFERENT lever at the same site → #1461 (`*use-bmh-matchers*`, `textproc` 0.743×, 8.5 MB per literal pattern)** | L (PCRE2) / S (if it is one coercion) | no |  **#1461 SHIPPED s473s**: PCL supplies `cl-ppcre::create-bmh-matcher` with a 256-way HASHED skip table (2 KB/pattern vs the dense 8.5 MB), so this branch is gone for a literal-prefix pattern -- `textproc` -24.5 % (= +32.5 % faster, control +2.9 %), hashed indistinguishable from dense (1.4381 vs 1.4378 s), peak RSS 99.9 -> 98.2 MB.  `json-rt`/`subste`/`regexg` build ZERO BMH matchers, so they cannot move. |
...
| 7 | a cached-module lambda (`~/.pcl-cache/021484…`) | 6.9 | – | – | JSON::PP's own transpiled body — a PCL-emitted sub; the shape to profile next | M | — |  **PROFILED s473x, and the answer is that there is NO shape**: `json-rt` is the flattest row in the set.  Its four biggest emitted frames are JSON::PP's own `encode`/`decode` workers (3.5 / 3.1 / 1.2 / 1.2 self), not one codegen pattern; above them sit cl-ppcre's `convert-char-class-to-test-function` 7.2 and `create-scanner-aux` 5.1 (JSON::PP decodes with regexes, so this row is #71's too), and below them the general box-and-coercion tax spread thin — `box-set` 2.8, `to-string` 1.9, `%p-str-eq-slow` 1.9 + `string=*` 1.7, `%make-p-box` 1.5, `unbox` 1.5, `box-sv` 1.4, `%p-true-p-slow` 1.4, `%p->=-slow` 1.2, `p-find-overload` 1.2 (for operands that cannot be overloaded at all).  **Nothing here clears a 20 % bar; the codegen lever this row was kept open for is not visible.** |
...
| 12 | `pcl::do-regex-match` | – | – | 4.5 | PCL's own regex entry — the per-match protocol around the scan | M | no |  **RE-PROFILED s473v on the CURRENT tree (task #1803), because this table predates #1250/#1251/#1461 and was therefore stale**: on `regexg` (a 0-group /./g loop) `do-regex-match`'s own self time is 12.6 %, and the per-match protocol AROUND the scan — the eager clear/set/set trio plus four `hash-table-count` calls plus one generic `length` — was **33.6 %**.  **#1804 SHIPPED it**: regexg +55.0 %, subste +35.7 %, textproc +31.8 %, json-rt −0.4 % (predicted: its matches FAIL, so the trio never ran).
| 13 | `package-implements-list` | – | 4.3 | – | #1189 | L | no |
| 14 | `pcl::set-match-vars` | – | – | 3.6 | the per-match `$1`/`$&` bookkeeping; a lever only if a match's captures are provably unread | M | Kind-A candidate |  **SHIPPED s473v as #1804, and the answer to "a lever only if a match's captures are provably unread" is that NOTHING has to be proved**: the seven DERIVED variables ($+, $^N, %+, %-, @-, @+, @{^CAPTURE}) materialise on READ from a per-match record, so an unread capture costs nothing and a read one costs the same single `subseq` it always did.  $1..$20 stay eager.  @-/@+ elements became MAGIC CELLS, which is perl's own model and is what keeps a saved `\$-[0]` reading the current match.  RUNTIME-ONLY — no Kind-A name was needed.
| 15 | `sb-kernel:ub32-bash-copy` | 2.8 | – | – | string copying — `p-string-concat` / `substr` | M | no |  **HALF of it was `$s .= "x"`, and that half SHIPPED s473v as #1809**: on the `strcat` row this function is 14.5 % and the generic `replace` that dispatches to it is another 35.9 %, all of it moving ONE character into an ADJUSTABLE string.  Copying through the underlying simple string with a one-character arm reads strcat +123.6 % (2.14x perl -> 0.94x).  The json-rt 2.8 % above is the OTHER source (`p-string-concat` / `substr`) and is untouched.
