...
|---|---|---:|---|---|
| 1 | lvalue-sub assignment (`f() = …`, `&$r(0) = …`) | 39 | sub_lval.t 33, op+perl-tests substr.t 2+2, try.t 1, signatures.t 1 | **registered** (`not-supported.md` §Lvalue subroutines) + ~~**RULED to stay a loud drop** (`fable-answers-s400.md` §6.3) — permanently EXEMPT from the flip~~ **SUPERSEDED s433 §A.1 / executed s435**: there is no exempt/registered classifier in the emitter (it would be asymmetric in the dangerous direction), so this family dies at run time like every other drop.  It stays a REGISTERED absence — that distinction lives here and in `not-supported.md`, not in the emission |
