...
> **A staged design exists (s495): `docs/scope-exit-destroy-design.md`** — DESTROY at program exit (sound, small), then scope-owned objects (a static licence, no reference counts).  **USER ruling s495: DESTROY at program exit is on the todo list (#2371); the scope-owned stage is PARKED as not worth the trouble (#2370).**  Nothing of it is built yet; the text below is still what PCL does.
...
**This section absorbs one older one**: [`use integer` — large shift /
overflow edge cases](#use-integer--large-shift--overflow-edge-cases).  That
...
was pp_pow's own branch rule, and it is now implemented (`docs/ir-spec.md`
§3.6, task #1248(b)).  Nothing else in this section moved: the three faces
below are unchanged.
...
|---|---|---|
| `sprintf "%u", $uv ** 6` | `18446744073709551615` (clamped to UV_MAX) | `394020061963944792122790401001436138050797392704654466679482934042457217714972106114142662548849156408066279903 06816` — the NV's exact decimal, no clamp |
...
prints `18446744073709551615` on both; and **since s473d the three `**` rows
this table used to carry agree too** — `$uv ** 6` is `3.94020061963945e+115`,
`2 ** 64` is `1.84467440737096e+19`, `do { use integer; 2 ** 63 }` is
`9.22337203685478e+18` on both sides (#1248(b); `use integer` does not
integerize `**` in perl either).
...
(since s473d it builds both by repeated multiplication instead, because `**`
now follows perl and returns an NV), and `lib/Math/BigInt/Calc.pm` builds its
masks with `2**$AND_BITS` — and the
...
## `**` returns an exact integer where Perl returns a float (NV)  *(FIXED s473d — task #1248(b))*

**NOT a divergence any more.**  This heading is kept so older citations
resolve (`docs/difftest-fuzzer.md` cites it by name).  `p-**` now implements
`pp_pow`'s own branch rule — `docs/ir-spec.md` §3.6 — so `2**53` prints
`9.00719925474099e+15`, `2**63` prints `9.22337203685478e+18` and `7**19`
prints the exact `11398895185373143`, all as perl does.
...
computes a power-of-2 base in doubles by choice and gives up on integers when
`exponent * bitlength(base) > 64`, not when the result leaves the 64-bit
range.  It is a per-op branch rule, and implementing it needs no IV/UV
distinction anywhere else: `~0 * 16`, `~0 + 1` and `sprintf "%u"` are
unchanged and still diverge.
