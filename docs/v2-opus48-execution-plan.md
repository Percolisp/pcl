...
**State updated s289 (2026-07-14, Fable): census 99 v2-native / 12 gated,
cache gen v2-31, Pl/t gate green.  M-A SHIPPED (task #67): pack.t
(5638/87, same 87 as v1) + yadayada.t (21/15) de-gated via the interp
rewrite in `_rename_decl_within`, PLUS oversized-extent flattening
($RUN_NEST_MAX force-promotion — pack.t's nested-let emission OOMed the
SBCL compiler — with the $RUN_FORM_MAX assembly gate as by-construction
backstop), PLUS `_premerge_include_prototypes` (cross-require prototypes
now merged before sub lowering/VarAnnotator — fixed a silent
wrong-context class in v2-lowered sub bodies).  Remaining 12 gates:
M-B span family (eval/ref/scalar/sort), chdir, closure, lfs, substr,
array (#63), postfixderef (#45), state/signatures (M-F).**
