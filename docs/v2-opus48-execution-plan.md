...
**State updated s288 (2026-07-13, Fable): census 97 v2-native / 14 gated,
cache gen v2-30, Pl/t gate green (114 files / 4036 tests), commit 92c0046.
Task #60 (E2-prereq void-wrap hoist) is DONE — the sub-body :void regime
ships, the large-sub SBCL heap blowup is gone, substr.t's only remaining
gate is the magic-lvalue foreach.  cl/pcl-pack.lisp is now REGENERATED BY
v2 (same 87 pack.t fails as the v1 artifact — first production artifact
on the new pipeline).  New task #64: bare-block sub tail loses its value
(BOTH pipelines, pre-existing).**
