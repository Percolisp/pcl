...
| string compare | `p-str-eq p-str-ne p-str-lt p-str-gt p-str-le p-str-ge p-str-cmp` (perl's `eq ne lt gt le ge cmp`; the `str-` prefix is what tells them from the numeric row — there are no `p-eq`-style names) | stringify; return `1`/`""` |
| logical | `p-&& p-\|\| p-//` · negation `p-! p-not` | `p-&&`/`p-\|\|`/`p-//` are short-circuit MACROS returning operand values (§3.4).  The two negations are FUNCTIONS returning `1`/`""` — not operand values — and **`p-not` is `p-!`'s low-precedence spelling, with one implementation**: perl's `not` is `!` at a different precedence and shares its overload key, so a translator implements ONE operation and both emitted symbols reach it (both ARE emitted: `unless` lowers to `p-!`, the `not` operator to `p-not`).  Negation consults the class's own `!` handler first and is `:autogenerated` from `bool` — perl DERIVES `!` from `bool`, so a class with no `!` key gets the truthiness answer and `nomethod` is NOT asked (task #1005) |
...
| ~~CL streams~~ | ~~`make-string-input-stream`~~ | **CLOSED s470bo**: the `__DATA__` handle's registration is `p-install-data-handle` (measured: `open $fh, '<', \$s` never reached this).  The quoted handle NAME stays an ARGUMENT of that op and MUST: `DATA` is per-package in perl and `*p-filehandles*` keys on the symbol, so it has to be interned in the emitting file's package.  Interning it in the runtime instead gave the program and every module it loads ONE key, and perl's own `Exporter.pm` `__END__` POD replaced sprintf.t's 566-row table (559 tests -> 1) | — |
| ~~the `loop` macro~~ | ~~`loop`, with its `for` / `across` / `do` keywords~~ | **CLOSED s470bo**: a `\(LIST)` refgen's spread is `p-vector-append` | — |
| a quoted symbol is TWO things (was three) | `'>` `'==` (operator selectors) vs `'start` `'label1` `'undef` `'of` (perl labels and bareword filehandles) — indistinguishable from the token | `p-chain-cmp`, `%p-loop-tag`, `p-readline`, `p-open`, `p-install-data-handle`.  The third meaning, the host TYPE designator `'string`, is gone with `p-literal-string` (s470bo) | #1176 |
