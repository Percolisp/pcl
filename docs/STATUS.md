...
2026-08-21 (internal session s419).
...
* **A dropped statement is loud but not fatal** in file mode (the program
  continues without it).  In `eval STRING` a drop dies into `$@`, as perl's
  compile errors do.  Making file mode die too is planned and gated on the
  drop census reaching its explained floor
  (`docs/drop-census-s419-flip-gate.md` §4).
