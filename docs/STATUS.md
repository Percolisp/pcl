...
| pure-Perl CPAN modules: a 14-distribution board, 183 test files | **79 files PASS / 54 PARTIAL / 50 FAIL; 2,190 assertions ok / 346 not ok** (a PARTIAL file ran most of its suite; FAIL is "zero ok", which also counts the files perl itself skips, such as `*-report-prereqs.t`).  Snapshot [`../baselines/cpan-board14-s474.tsv`](../baselines/cpan-board14-s474.tsv), taken 2026-09-07; its header attributes every row that moved.  The previous one, [`cpan-board14-s467.tsv`](../baselines/cpan-board14-s467.tsv), read 80 / 53 / 50 and 2,154 / 340 as it now stands (the 78 / 54 / 51 and 2,140 / 342 quoted here before were its RAW run, before s470bw hand-edited four rows), and exactly four Scalar-List-Utils files moved between them — they reach their Math::BigInt section for the first time since #1508.  The nine files that had moved DOWN between the s378 and s467 snapshots are each bisected to a commit in the s474 header (#1061, closed) | the command below |
