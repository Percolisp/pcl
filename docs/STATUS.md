...
|---|---|---|
| PCL's own regression gate (`Pl/t/`) | **250 files, 8,481 assertions, all passing.**  Three of the files are the XS-bridge tests, parked (`plan skip_all`) while the experimental [pclxs](#xs) sibling is mid-change; `PCL_XS_TESTS=1` runs them | `tools/prove-core` (or `prove -j8 Pl/t/`) |
