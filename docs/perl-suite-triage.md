...
| 10 | `type-error: The value N is not of type hash-table` | comp/hints, op/coreamp, re/reg_nc_tie, re/regex_sets | **STALE (s316g)**: comp/hints.t no longer crashes — runs 17/14; residue = lexical hint scoping, **user-blessed not-supported 2026-07-28** (not-supported.md §Lexical compile-time hints, expected-tsv row). Re-triage the other three from the fresh faillogs. |
