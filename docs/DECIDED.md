...
- **`PCL_OPT=none` emission is BYTE-IDENTICAL to `ccdb6b7` over all 111 corpus files + 6 shapes files** — the proof that the pass is the only emission change.  With it on, 12 corpus files / 90 changed regions and 66 wide-population files / 152 hunks, every one of them `(p-sort …)` → `(%p-sort-classic …)`; RCDIFF 0.
