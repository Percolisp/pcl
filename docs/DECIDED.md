...
## s470bs (2026-09-05, Opus) — the FILETEST / STAT family gets ONE operand resolver: #1031, #1049, #1048, #1033, #1047 (of the brief's eight; #1042/#1043/#1044 NOT started — machine shutdown)
...
- **#1047 — `_` carries the stat FLAVOUR and the SUCCESS, and neither is derivable from the operand.**  `stat $f; -l _` DIES in perl's own two wordings; after a FAILED stat every filetest but -T/-B answers EBADF from `_`, while -T/-B OPEN the remembered NAME and answer ENOENT (probed; the split `t/op/stat_errors.t` asserts).  Reading `_` also SETS the flavour, as perl's ops do.
