...
- **Filed from the probes, all PRE-EXISTING and all verified on a `1fed80b` worktree**: **#1041** (`sweep-diff.pl` keys a row on file+DESCRIPTION, so bop.t's 14 unnamed failing rows collapse onto ONE baseline entry and 13 are ungated), **#1042** (`$^T` is frozen into the SAVED CORE, so `-M`/`-A`/`-C` answer a negative age that GROWS with the core's age — 184 s on one tree, 2831 s on an older one, 0 in perl), **#1043** (`stat`/`lstat` in SCALAR context answer the 13-element list, not perl's 1 — `my $n = stat($f)` is `ARRAY(0x1)` here), **#1044** above, **#1040** above.
- **Bars**: gate **195 files / 6685 tests**, only the 13 pclxs xs rows failing; full sweep GATE clean, **TOTAL 18266 → 18493 (+227)**, 0 new / 0 fixed, drops 5 = census, and **bop.t is the ONLY mover in 108 files** (per-file `_status.tsv` diffed against a `1fed80b` worktree's own run); a second sweep against the edited baselines TOTAL 18493 (+0); companion `--dir io --dir op --quick`; baselines edited ROW BY ROW (224 rows leave `fail-baseline.tsv` by EDIT, 708 → 484 lines with every non-bop.t line verified byte-identical AND in order; `pass-baseline.tsv`'s bop.t row 253/256 → 480/29 with its cause).  Guards `Pl/t/bitwise-string-01.t` (74 rows, 1.5 s, 54 of 74 FAIL on `1fed80b`) and `Pl/t/bareword-fh-slot-01.t` (54 rows, 1.4 s, 54 of 54 FAIL on `1fed80b` — the program dies at the first `stat(FH)` and produces no rows at all).  `docs/ir-spec.md`: the bitwise family leaves the "numeric ops" row for its own mode-dispatched row.
