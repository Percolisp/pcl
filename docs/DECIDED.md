...
- **`p-||` and `p-` are the SAME symbol** (probed): the `||` is an empty multiple-escape section, so the symbol's name is `P-`.  A text-parsing backend must fold them (ir-spec §11b).  **16 names ir-spec §10 CITES do not exist** (`p-&` `p-x` `p-eq`…`p-cmp` `p-++` …: Perl-facing spellings); `%p-empty-list` is cited but never emitted, and `p-qr` is emitted package-QUALIFIED.
