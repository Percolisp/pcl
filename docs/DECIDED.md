...
## s470bp (2026-09-05, Opus) — the MODULE CACHE stores compiled fasls (the yardstick's biggest number), and two of the three AGGREGATE levers ship, the third is built and measured but NOT committed (shutdown)
...
- **L7/#1205 — BUILT AND MEASURED, NOT COMMITTED (the machine was shut down before its own gate could run; the runtime carrying it is `scratch/s470bp/rt-L6-AND-L7.lisp`, the split script `scratch/s470bp/split-l7.pl` is its inverse).  An rvalue slice in a COPYING consumer reads VALUES, not element cells.**  `p-aslice`/`p-hslice` build ALIASES, which under raw element storage PROMOTE every slot to a box, monotonically and forever.  `my @v = @a[1..5]` copies, so the licence is THE CONSUMER and it is decided where the consumer is visible — in `p-array-=`'s macro, whose VALUE form is syntactically the slice.  Every other consumer keeps the aliasing path untouched (`for (@a[0,1]) { $_ *= 10 }` still writes through).  The READ is the existing one: `%p-aslice-copy` asks `p-aref` and `%p-hslice-copy` asks `p-gethash`.  **It also FIXED a divergence** — `my @a=(1,2); my @v=@a[0..3]; exists $v[3]` was false in PCL and true in perl.  **slices -22.2 %** shipped (-33.3 % hand-replaced), 2.53× → 1.95× of perl.
