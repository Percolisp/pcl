...
- Bars across the three commits: gate **218 files / 7571 rows** (only the 13 pclxs xs rows); corpus-diff 1 of 111 (`split.t`, the `:arrays` family), silent drops 5 unchanged; emission A/B over **1007 files** SAME 966 / DIFF 41 / RCDIFF 0, all 41 re-transpiled against a b5e9f845 extraction and classified as one family; `PCL_OPT=none` byte-identical to main's over 111 + 6 shapes; full sweep GATE clean, **TOTAL passing 18674 (+0)**, drops 5 = census (run three times, once per commit); companion `--jobs 1` over op/for.t op/array.t op/aassign.t op/hash.t op/push.t op/splice.t op/eval.t op/sort.t op/each.t — every file EQUAL to its `baselines/perl-suite-run.tsv` row, ROW DIFF 0/0/0/0; ir-conform 289/0/56/0; ir-host-leak's symbol set identical to main's.  Generation **v2-990**, three artifacts regenerated (only the gen stamp moves).  Guards `Pl/t/eval-cache-01.t` (16 rows) and `Pl/t/perf-levers-01.t` 54 → 68.
