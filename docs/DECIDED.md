...
## s474b (2026-09-07 morning, Fable) — the two slots relaunched after the reboot (s473p + BZ); #1438 RULED leave; #1501 filed (the companion gets the sweep's two triage checks); the CPAN-board question answered
- **Box rebooted 07:26; nothing in flight at start.  LAUNCHED 07:30: s473p** (perf round 30 — #1200 eval disk cache, the single-array foreach lever, #995+#1056, #1182; gen v2-990; IDs 1409–1418) **and BZ = s470bz** (#1302 install layout + #1304 the podman container test; no gen; IDs 1325–1334), both on main `b5e9f845`, briefs re-filled (`s473/s473p/prompt.launch.md`, `s470/s470bz/prompt.launch.md` + its s474b addendum).  **s473t3 brief written** (`s473/s473t3/prompt.md`: the **79** blessed rows whose ONLY cause is a `CATALOG` note — measured, not the record's "~60" — get tasks; IDs 1471–1500; gen v2-1000 only if a fix ships); launches on the next free slot.
- **USER (2026-09-07): "Let the bad counts be, as per your recommendation."  #1438 CLOSED AS DECIDED**: the three stale `plan` counts (bop.t 510/509, eval.t 169/163, method.t 163/161) stay; their 9 rows stay in `baselines/row-shortfall.tsv` under the s473t1 cause `STALE PLAN, not a lost row` — that written-down line IS the resolution.  No `perl-tests/*.t` file edited.
- **USER (2026-09-07): "after those bugs are fixed, add a task to do the same check as for sweeps, but for the rest of the Perl files."  #1501 FILED**: the s473t1 + s473t2 checks for the COMPANION suite (perl's full t/, 528 files).  Sized at filing: 188 files where PCL produces fewer rows than perl (companion shortfall 190 files / 445,102 rows, 125 files UNEXPLAINED); `baselines/perl-suite-fails.tsv` = 18,340 row-level fail rows over 265 files with NO cause column (five fields; ~500-row cap per file, `re/reg_posixcc.t` 7,130).  Sequenced AFTER the s473a–f rounds consume #1431–#1460 (the USER's "after those bugs are fixed"); the first brief SIZES by cluster census and splits per directory; re/ waits for s473q's engine verdict.  Next free task ID **1502**.
- **The sweep's state, answered from the baselines (main b5e9f845)**: 108 files, TOTAL passing 18,674, 61 files with zero fails, 12 PARTIAL; 489 blessed keys = 678 assertions; by first cause: **243 a task (37 distinct), 79 a CATALOG note only, 22 prose (caller() 4-of-11 elements, "no PVBM", #752), 98 a not-supported section, 47 pack.t parked** — i.e. ~340 keys on real defects, ~100 on rulings.  perl-tests rows never produced: 12,213, of which pack.t 8,997 and lc.t 2,577.  "Bad counts" found by the triage: the runner's own `^;` strip hid the cause line (11 of 13 files), #1438's 9 phantom rows, #1432's 47 stubbed assertions (now 11 pass + 36 honest fails), #1439's unconditional `skip_if_miniperl` (15 sites, 9 files), 129 registered skips counted as shortfall (tr 73, ref 31, magic 16, array 9), 4 load-sensitive magic.t rows.  The biggest REAL loser of rows is #1433 (a drop kills its enclosing top-level form: method.t 35, substr.t 44).
- **USER asked: "Do we need to get more CPAN modules to test?"  Fable's answer (recorded; USER decision open)**: not as a wider SCORED board yet — the 14-dist board (183 files: 78 PASS / 54 PARTIAL / 51 FAIL, 342 not-ok rows) is not consumed, its nine s467 down-movers are unattributed (#1061, pending), and its rows have no cause column of the s464 kind (the s343/s344 cause files are a month stale).  Recommended order: (1) #1061; (2) give the board the sweep's treatment — a cause per row, the ≤1 h clusters shipped; (3) THEN widen with pure-Perl dists ALREADY ON DISK in `~/.cpan/build` (108 dists, many XS) that exercise language areas the board does not: Moo (the OO target, `docs/moo-status.md`), Path-Tiny (I/O + overload), Getopt-Long, Pod-Simple (a large regex-heavy parser), Test-Deep (deep structures + overload), Math-BigInt (numeric), Memoize, HTTP-Tiny/HTTP-Message (sockets, headers), and one REAL PROGRAM (App-cpanminus) as an end-to-end target.  The second census population (`cpan-tests/modules/`: Test-Simple, Text-CSV, YAML-Tiny + easy-targets, 308 `.t` files) is a drop-census population only, never scored — scoring it is the cheapest widening of all.
- **USER (2026-09-07): "Sounds good."  THE BOARD ORDER IS RULED: #1061 (the nine down-movers, bisected; brief `s473/s1061/prompt.md`, the filler after s473t3) → #1502 (a ROW-level board file + cause column + CAUSES line; the scoreboard keeps only per-FILE verdicts today) → #1504 (widen in rounds of 2–3 with on-disk pure-Perl dists — all nine candidates measured with ZERO `.xs`: Moo 71 t-files, Getopt-Long 8, Path-Tiny 30, Test-Deep 42, Pod-Simple 74, Math-BigInt 54, Memoize 16, HTTP-Tiny 32, HTTP-Message 23 — plus App-cpanminus as a real-program target); #1503 (score `cpan-tests/modules/`) is an independent filler.  Next free task ID **1513** (s1061 reserves 1505–1512).
- **`t/op/numconvert.t` (1446 companion rows, UNEXPLAINED since the audit) EXPLAINED: the file skips itself.**  Its preamble computes `sprintf "%u", ~0 ** 6` and `use integer; ~0 * 16` and prints `1..0 # Skip unsigned perl arithmetic is not sane` unless both sit inside 64 bits; in PCL both are exact bignums (the 116-digit power; 295147905179352825840) where perl clamps to UV_MAX and wraps to −16.  ONE mechanism with three faces — PCL has NO 64-bit integer boundary (no overflow-to-NV, no `use integer` wraparound, no %u/%d clamp; 9 of 12 probe lines diverge) — filed as **#1513**, a DESIGN item sized and NOT scheduled: the `**` half is the family the USER parked on 2026-06-26, and the fast fixnum paths are Target A's.  Row edited in `baselines/row-shortfall.tsv` with the cause.
- **Unicode/regex share of the failures, measured (asked by the USER)**: companion — 96 % of the 484,921 rows PCL is short sit in `re/` + `uni/` (325k in the registered uniprops tables, 66k in the `uni/variables.t` timeout, 68k in files that run), 82 % even among files that RUN, and 80 % of the running-files hole is TEN case-folding/charset files; sweep — 12 % of blessed rows name a Unicode/regex cause, 14 % of failing assertions sit in regex/Unicode-named files; board — none of the s344 families.  By ROWS it is the hole; by MECHANISMS it is a handful.
- **USER (2026-09-07): "So we should just document that we don't bother with a 64 bit integer boundary?" — Fable: yes, as ONE decision; USER: "Sounds good."  RULED: #1513 is a DOCUMENTATION job** (one not-supported entry absorbing the `**`-exact and `use integer`-shift entries; the explicit-mask rule; the unbounded-accumulator hazard; `op/numconvert.t` registered as an expected divergence; an ir-spec paragraph; a guard row asserting the documented answers), a filler after #1061.  The evidence: 22 `use integer` modules in perl's core lib + the on-disk dists, none wrap-dependent (Tie::SubstrHash bounds its own accumulator at 1e13 — perl's boundary is platform-dependent, so authors never relied on it); pure-Perl digests mask explicitly, which works on unbounded integers.
- **USER (2026-09-07): "do we declare `my` variables as integers, if it is inside `use integer`?"  ANSWER: no, and a declaration would not help — MEASURED (#1514).**  Under the pragma PCL emits bare CL `+ - * truncate rem` with EVERY operand wrapped in `p-int` (to-number typecase + truncate) and declares nothing; that makes the use-integer loop **1.5× slower than the same loop without the pragma** (111 vs 73 ns/iteration) and 1.4× slower than perl, while the plain loop beats perl 2×.  In pure SBCL the same body runs at 5 ns/iteration with an UNTYPED integer slot, identical to a `(signed-byte 64)` modular version, and a fixnum declaration is SLOWER — SBCL's generic arithmetic already takes the machine path for fixnums; a declaration would also be unsafe under the no-boundary ruling (a value past 2^62 = type error or corruption).  So the pragma is a FACT for the operand (drop `p-int` where the raw-numeric facts already prove a number), never a declaration on the variable; and the 73-vs-5 ns gap on the plain loop is op DISPATCH, not variable type — both halves are #1514, queued for the next non-engine perf round.  All numbers taken under load 15 (ratios and slopes only).
## s474 (2026-09-06 night, Fable) — BY + BU merged; the REGEX ENGINE question RULED from two USER questions
- **BY MERGED `0d783d24`** (#1300 #1303 #682 #1338 — the cache surface; Fable gate 216/7517 xs-only with ZERO write-date signatures = #1338's acceptance) and **BU MERGED `8a879a57`** (perf round 29: #1250 per-SITE regex/subst/tr op, #1251 the s/// compiled record; json-rt +99 % vs main, subste +62 %, regexg +32 %, textproc +30 %; runtime-only).  BU was rebased by Fable onto BY; the two records conflicts resolved keep-both, BU above BY.  #770 CLOSED by pointer (it described exactly what #1250/#1251 shipped).  s473t1 ran as the USER's one-off THIRD slot while BY/BU finished; s473t2 took BY's slot.
- **USER (2026-09-06): "Why should we optimize cl-ppcre before we look at if we can go PCRE2?  Do we really want to support both?"  RULED: (1) the #71 spike goes BEFORE #1187** — the July prerequisite on #71 ("plumbing vs engine first") was met by #680 (DECIDED §s454ac: "the engine gap is the next lever, not more plumbing"), and #1187 is a coercion in code the engine phase would delete (the standing "a fix in code a phase deletes is wasted" rule); **(2) ONE engine at run time, never two** — two engines = two semantics, two translators, the sweep twice, "which engine" first on every regex bug; a `PCL_REGEX=ppcre` switch may exist DURING a migration as the A/B instrument and is DELETED at its end; **(3) the spike is a MEASUREMENT with a binding STOP RULE** (PCRE2 loses the many-tiny-matches shape → cl-ppcre stays and #1187 proceeds; PCRE2 wins → the replacement is a Fable-designed PHASE, not something the round starts).  Design point that changes #71's cost estimate: bind `libpcre2-32`, not `-8` — an SBCL character string IS 32-bit code points, so a pinned subject is zero-copy and ovector offsets ARE character indexes; the byte-offset mapping #71 budgeted as the hard part does not exist on that path.  What PCRE2 buys beyond speed is PARITY: the perl pattern goes in verbatim (the six-pass `perl-regex-to-ppcre` translator goes away), and the not-supported entries that name #71 as their lift (control verbs, script_run, `/n`, Unicode properties) plus `(?{…})` via PCRE2 callouts.  Round 30's brief (`~/pcl-agent-scratch/s473/s473p/prompt.md`) rewritten accordingly; s473p takes the NEXT free slot (USER: "Good").
- **USER (2026-09-06): "we are not writing software for our laptop, really.  If PCRE2 works for us, we also need to test it on other O/S."  STANDING RULE: a change that touches the PLATFORM (a foreign library, a syscall family, an installer step) ships only after the install matrix (`tools/install-matrix/`, four glibc images) is green WITH it, plus a macOS leg (GitHub `macos-latest`, Homebrew) — an engine change in particular.**  The spike RECORDS what that test needs (package per image, soname, version, the dlopen name list, the install-time check that refuses LOUDLY like the PPI floor — never a silent fall-back to cl-ppcre); musl and the BSDs are reported, not gated, unless the USER says otherwise.
- **USER (2026-09-07): "Make it so.  But don't prioritize it high, for now.  Do other speed tests first."  RULED: the spike is APPROVED and DEPRIORITISED — it is its own brief **s473q** (`~/pcl-agent-scratch/s473/s473q/prompt.md`, IDs 1461–1470, gen v2-980 if it ever needs one), launched AFTER round 30 (s473p, restored to NON-engine levers: #1200, the single-array foreach lever, #995+#1056 loop-body box reuse, #1182 storage adoption) and after one more round of the remaining S/M levers (§A.2: #7 frame trimming, #883, #924, #813, #1141/#1142, #986 — brief written when s473p reports).  #1187 stays HELD across both (engine-side; the spike decides it).
...
- **The companion `--all --quick` owed since s471 was run on main this session — and its tree MOVED under it** (BX phases 1 and 2 merged mid-run), so its readline/stat_errors "movers" were the OLD code against the NEW baselines; serial re-runs on `d348dcea` read the snapshot exactly.  Six REAL movers, each measured on `git archive` extractions of fcf076f1 (the snapshot's own s470bv bless point) and 637fc58e (pre-BX) before being spliced: **op/tiearray.t 40/35 → 26/29 = #1429** (s471a's #1273 fatal fires on a TIED array whose class sets `$NEGATIVE_INDICES` — the runtime never reads the flag; 20 rows never produced), **op/tie_fetch_count.t 76/65 → 129/12** (55 filetest rows fixed by s470bs's operand resolver; 2 NEW = **#1430**, the range operator FETCHes a tied operand twice), op/filetest_stack_ok.t 71/65 → 98/38 (the same s470bs family), op/gv.t 134/57 → 135/56 (one row, moved between the s470bi bless and fcf076f1, NOT bisected — said so), io/pvbm.t 20/8 → 23/5 (#1043 + the known flapper), run/runenv_randseed.t 2/7 → 3/6 (**#1236, s470bx — the only mover this session's merges caused**).
- **USER (2026-09-06, session end): the s473 sweep readout — 108 files (93 OK / 15 PARTIAL, 0 crash/timeout/transpile-fail), 18,649 pass / 642 fail (479 keys: 152 UNEXPLAINED, 90 task-owned bugs, 60 bug notes without a task, 61 #221, 116 ruled non-support), 12,274 rows never asserted (431 behind the 15 PARTIAL files, 13 of them UNEXPLAINED) — goes on the todo stack SOON as two triage briefs, s473t1 (the 13 early stops) and s473t2 (the unexplained keys); and "skip working more with pack.t" — pack.t's 47 unexplained keys are PARKED with pack/unpack, not probed.**
- **`--bless-fails` / `--bless-shortfall` are DESTRUCTIVE to the hand-written notes** in `baselines/perl-suite-fails.tsv` and `baselines/row-shortfall.tsv` (128 comment lines and every provenance note gone; rows of files the run did not measure re-serialized) — measured this session and REVERTED.  The standing practice (s470bv, s470bx, s471a) is the rule: those two files are edited ROW BY ROW with `:raw` handles, a re-measured file's row block replaced from the run's own rows, every other byte identical (verified by `cmp` after filtering the touched files), and a confirmation run reads 0 NEW / 0 FIXED.  `--bless-rows` (perl-suite-expected-rows.tsv) is the ratified generated-file exception.
...
## s473p (2026-09-07, Opus) — perf round 30: a string eval's TRANSPILE is paid once EVER (#1200, `use JSON::PP` 1.13 s → 0.41 s), ONE bare array is a foreach RUN (#1409, feread 0.47× → 0.30×), and #1182's win turns out to be the GROWTH, not the copy

- **#1200 — the biggest number in the yardstick, halved and then some: `use JSON::PP; print 1` 1.129 s → 0.409 s (0.36×), `eval "1"` 0.304 → 0.172, `use Moo` 0.343 → 0.201, against a byte-identical startup control (0.170 → 0.169).**  A string eval's EMISSION is cached under `<cache>/evals/<md5>.lisp` on **p-eval's OWN in-process key** (perl text, perl package, capture NAMES #296-B1, features #364) plus `*pcl-cache-generation*` — no second spelling of "what makes two evals the same".  MD5 and not sxhash: a run reaches dozens of module paths and **652 distinct eval texts in three perl-tests files**, and a collision would run one eval's code for another's.
- **THE MEASUREMENT CHOSE THE DESIGN, and it is why the fasl half is a separate task**: transpile ~16 ms, READ+EVAL 5.00 ms, COMPILE-FILE 10.00 ms, fasl load 0.05 ms.  The transpile is three quarters of an eval's cost and the only quarter a cache can take with **no semantic question at all** — the string that comes back is the same string.  **#1410** owns the rest, blocked on a real problem: `load` cannot hand back a file's LAST FORM's value, and wrapping that form to capture it destroys its top-level-ness (`p-sub`'s compile-time `%p-reserve-sub-name` with it).  Three shapes were considered and are written down there; a SPLIT (compile all but the last form) is worth nothing, because the expensive form IS the last one.
- **VALIDITY IS THE MODULE CACHE'S, MINUS THE CLAUSE AN EVAL HAS NO SUBJECT FOR.**  No source file to be newer than, so what remains is the #1261 dependency manifest — and it is needed: an eval whose text `use`s a module reads that module's prototypes (verified live — the entry records the module's md5 and is re-transpiled when it changes).  `pl2cl --server`'s request gained a **deps-path line** and writes the sidecar BEFORE it answers; a `.lisp` whose `.deps` is missing or stale is a MISS, since `p-cm-valid-p` defaults to NIL.  That is what makes "deps first, .lisp second" the whole publication rule.  ONE reader: `%p-read-manifest` is now a caller of `%p-manifest-at`, which takes the path.
- **`use JSON::PP` runs 80 string evals, not the ~20 the task guessed** — the same 80 keys in the same order on every run.  The idiom is not JSON::PP's: Class::Accessor, Moo/Sub::Quote, Moose and Type::Tiny all generate accessors this way.
- **THE SWEEP CANNOT SEE THIS CACHE, and that is by design**: every test runner sets `*pcl-skip-cache*` (`./runpcl`, the sweep, `pclperl-for-tests`, `run-dist-t`).  So the sweep proves the UNCACHED path is untouched, and the cached path is covered by twelve probes vs perl (cold run == warm run == `--no-cache` == `PCL_NO_CACHE`, byte for byte), a 70-file cpan-tests population run of `./pcl` twice against a fresh cache directory (**70 SAME / 0 DIFF**, after normalising pids, tids and heap addresses out of a crash backtrace — the first, unnormalised comparison read 58 DIFF and was pure noise), and `Pl/t/eval-cache-01.t`.
- **`<cache>/evals/` is the directory that can grow without bound** — a program can build an eval string per loop iteration — so it joins the 30-day prune, a HIT stamps its pair (last USE, the #1300 rule), and the prune is triggered by an eval MISS as well as a module one.
- **#1409 — the lever round 27 named and left: `feread` 0.47× → 0.30×, −36 %** over two A/B rounds, eleven control rows inside the noise band.  `_foreach_bare_arrays`' `>= 2` becomes `>= 1`, and nothing else.  **#1184 required two arrays because a ONE-element list has no flattening to remove** — `%p-flatten-for-list` hands a lone bare lexical `@a` straight back — and what it hands back is the LIVE adjustable vector, so every access is a hairy data-vector-ref and the bound is re-read per iteration.
- **The live-array question was already answered by the licence that was there.**  `for (@a) { push @a, … }` iterates the new elements (perl 5.40.3 and PCL's live path both) and the run's snapshot would not — but push/pop/shift/unshift/splice/`$#a =`/a whole assignment are all `written_in` for the #1140 array facts, and `escapes` covers every path a callee or closure could take to a `my` array.  Eighteen shapes probed: nine licensed loops take the run, eight write/escape shapes decline to the boxed `p-foreach`, and the lone SLICE and lone DEREF stay read-only-but-not-bare.  **ONE registry name** (`foreach-arrays`), because it is one transform with one fewer element in its shape test.
- **THREE `Pl/t` GUARDS WERE STALE against #1409 and were repaired in its own commit** (the s416 rule, and the gate is what found them): `foreach-raw-01.t` rows 1+4, `array-facts-01.t` rows 48+52, `passes-01.t` row 5 all asserted `(p-foreach-raw ($x @a))` — the pre-#1409 spelling of a shape none of them is about.  Each now names the whole CURRENT form, which is STRONGER than loosening the pattern to accept either.
- **#995's two residues, MEASURED — one is worth nothing and the number says so.**  A `(string, fixnum)` arm in `p-.` that skips the overload dispatch measures **27.0 ns against today's 27.4 ns** (best-of-5 over 5 M calls): `%p-.-slow`'s 23.7 % self is `concatenate 'string` and its allocation, NOT `%with-binary-overload`, whose test for two raw operands is one `p-box-p` per side.  A `simple-string` make+replace spelling is SLOWER than `concatenate`.  **Do not retry either.**  The other residue IS real and is now sized in #995: `$h{$k}++` costs **43.5 ns** against a single-probe raw update's **25.5 ns (−41 %)**, ≈17.6 % of `arrhash-k` — under this round's 20 % bar, so not taken, and the task now carries its licence (the box in the slot is the RUNTIME signal that something aliases the element, because `%p-hash-elem-cell` promotes on first lvalue touch).
- **#1182 RETARGETED BY ITS OWN SIZING: the row it was measured against cannot take adoption.**  `listcopy` is `my @c = @src`, an EXISTING array whose values perl COPIES; adoption needs a FRESH producer (`sort`/`map`/`keys`/`split`/a sub call), and those shapes have no bench row at all — the range case already has #1204's path, which materialises no vector.  So #1182 needs a bench row first, and says so.
- **What the sizing found instead, and it needs NO fact: the cost is `adjust-array`, whose job is to PRESERVE contents a whole-array assignment is about to overwrite.**  Installing a FRESH element vector through `sb-kernel:set-array-header` gets essentially all of adoption's win (5 elements 110 → 50 ms, 50 elements 15 → 9 ms, 2 000 unchanged; full adoption 42/8/5, i.e. SLOWER at the top end).  Shipped as `%p-array-grow-discarding` at the two `p-array-fill` sites whose contents are discarded; the RANGE fill's `adjust-array` is deliberately untouched because it APPENDS.  **listcopy +27…+29 %, slices +20…+21 %** on the interleaved runtime A/B, every other row in the noise band.  Header fields probed one by one against `adjust-array`'s own result; declines for a displaced array or anything not the plain element-storage shape.  **Verified on SBCL 2.6.0.debian and the docstring says so** — the 2.5.2 floor is not on this machine.
- **A MEASUREMENT TRAP THAT COST THIS ROUND AN HOUR: `tools/bench-exec.pl`'s `t(N) − t(0)` CANCELS the constant term, so a lever that shrinks the module load reads as a REGRESSION** (moo-objs +42 %, json-rt +14 % on the #1200 tree).  t(0) falls, the difference grows, and what is reported is the leftover noise.  Absolute interleaved times are the honest instrument for anything that moves a constant — and absolute seconds are the metric (plan §A.0).  Measured absolute: json-rt N=100 2.619 → 1.954 s, N=200 4.208 → 3.533 s with the per-iteration slope unchanged (1.589 → 1.579), moo-objs N=20000 6.144 → 5.619 s.
- **`tools/ir-conform` had never been runnable by hand**: it `require`s `tools/lib/PCLSbcl.pm` by path and puts nothing on `@INC`, so PCLSbcl's own `require PCLPaths` killed every worker and the run produced no verdict at all.  Pre-existing on b5e9f845 (reproduced there); one line, fixed in the #1200 commit, because the WHAT-TO-RUN-WHEN table makes the tool MANDATORY after a `cl/` change.
- Bars across the three commits: gate **218 files / 7571 rows** (only the 13 pclxs xs rows); corpus-diff 1 of 111 (`split.t`, the `:arrays` family), silent drops 5 unchanged; emission A/B over **1007 files** SAME 966 / DIFF 41 / RCDIFF 0, all 41 re-transpiled against a b5e9f845 extraction and classified as one family; `PCL_OPT=none` byte-identical to main's over 111 + 6 shapes; full sweep GATE clean, **TOTAL passing 18674 (+0)**, drops 5 = census (run three times, once per commit — the third finished on the records tree: 0 new / 0 fixed / 0 LOST, `CAUSES: 0 of 478`, 108 files, pack.t OK 5636/89); companion `--jobs 1` over op/for.t op/array.t op/aassign.t op/hash.t op/push.t op/splice.t op/eval.t op/sort.t op/each.t — every file EQUAL to its `baselines/perl-suite-run.tsv` row, ROW DIFF 0/0/0/0; ir-conform 289/0/56/0; ir-host-leak's symbol set identical to main's.  Generation **v2-990**, three artifacts regenerated (only the gen stamp moves).  Guards `Pl/t/eval-cache-01.t` (16 rows) and `Pl/t/perf-levers-01.t` 54 → 68.  Commits `59f2214b` (#1200), `688a7ab4` (#1409), `39e1a656` (#1182).
- **The round was cut by the session rate limit before its last step and resumed the next session** in the same worktree.  The resume closed the task texts, rebased onto main `ee02b69f` and re-ran the bars there: gate **219 files / 7599 rows** (the 13 pclxs xs rows only; the +1 file / +28 rows over the round's own 218/7571 are main's new `Pl/t/pcl-root-01.t`), corpus-diff vs `ee02b69f` 1 of 111 (`split.t`) with silent drops 5, emission A/B over lib + shapes 28 files 25 SAME / 3 DIFF / 0 RCDIFF (`Config.pm`, `English.pm`, `Try/Tiny.pm` — each one single-array `foreach` taking the `:arrays t` run), ir-host-leak identical, `tag-license --check` clean, no file-mode change.  **What main gained between the tree the sweep ran on and the merged one is BZ's installer work (`pcl`, `pl2cl`, `runpcl`, `tools/` incl. the sweep runner and `PCLSbcl`, two `Pl/t` files), s473t3's baseline/catalogue edits and docs — not one `Pl/*.pm`, `cl/` or `lib/` line**, so the sweep could not move and was not re-run (the WHAT-TO-RUN-WHEN table's first row); and main's own sweep through BZ's NEW runner reads the same TOTAL 18674 (Fable, at s473t3's merge), so the two runner versions agree on the number.  `pl2cl` is the one file both sides touched (main's root resolver, #1200's deps-path request line); the rebase merged them without conflict and `Pl/t/pcl-root-01.t`'s 28 rows pass on the result.  **#1411 filed on the way**: `tools/emission-ab.pl --out` with a RELATIVE path resolves side A's output INSIDE the ref worktree, so side A writes nothing and all three pairs read DIFF with an empty `rc A=` — the workaround is an absolute path, the fix is one `rel2abs` beside the input absolutisation the script already does.
## s1061 (2026-09-07, Opus) — #1061 CLOSED: the CPAN board's nine down-movers each have a COMMIT, and two of them were compiler bugs worth fixing

- **THE BISECT RANGE #1061 NAMED WAS WRONG, and that is the first finding.**  The previous snapshot `baselines/cpan-board14-s378.tsv` was MEASURED at `5d941618` (2026-08-09, s378); `23e30051` is only where s440 MOVED the file from `docs/` to `baselines/`.  The real range is `5d941618..d00bb576` — 568 commits, not 238 — and it CONTAINS the announce→DIE flip `f702da31`, which the shorter range would have hidden behind its own baseline.  **A baseline file's name is not its measurement point; `git log --follow` on the file is.**
- **THE BOARD HAD NOT MOVED SINCE s467 BEFORE THIS SESSION'S OWN FIXES.**  A fresh board on main `dfa65afa` was byte-identical to `baselines/cpan-board14-s467.tsv` — 183 rows, 80 PASS / 53 PARTIAL / 50 FAIL, 2154 ok / 340 not-ok, ZERO movers across rounds 24–29, s473t1/t2 and BZ.  So the nine were the whole job.  (STATUS.md's 78/54/51 and 2140/342 were the RAW s467 run; s470bw's four hand-edits moved two rows up and STATUS was never refreshed.  Now it is.)
- **Nine down-movers, nine commits.**  `role-long-package-name.t` = `e79f0a63` (s407a, #362) — a CORRECT fix (code refs numify to their address) unmasking a DEFERRED non-support: Role::Tiny's `_install_does` compares two code refs NUMERICALLY, and while they all numified alike it installed `does`/`DOES` into every composed class, which is the only thing that put a SUB in the stash PCL can see (**#1510**).  `given_when.t` = `048b6871` (s415b, #371 Track A) — the ruled given/when refusal; its one surviving `ok` was an ACCIDENTAL pass (undef == undef).  The four `Capture-Tiny` files = `f702da31` (s435, THE FLIP) — seven pre-existing drops that used to be silent `nil`; the gap under them is that **Capture::Tiny builds its prototypes with `eval "sub $sub(&;@) {…}"`, so the module prototype SCAN finds no literal and `capture { … }` is not a block-form call** (**#1509**).  `dualvar.t` = `b95ad912` (s436, THE PHASE MODEL), `max.t`/`min.t`/`product.t` = the same commit, `oo.t` = `b1847eb7` (s411d), `dump.t` = `a46aa3f0` (s404f), `140-lvalue.t` = `551202b3` (s464a, #964 — its four lost rows were passing on the aliasing bug #964 removed; `:lvalue` is USER-deferred #930).  Only `05_extmul.t` is unbisected, and the task says why (**#1512**: two runs of the SAME tree read 16/14 and 0/0, and a step costs 150–600 s because the file needs ~150 s where perl needs 0.2).
- **THE PHASE MODEL IS THE BOARD'S BIGGEST SINGLE MOVER — four files — and in every case it EXPOSED something rather than breaking it.**  `dualvar.t` has ALWAYS emitted an unreadable token; the phase model moved the BEGIN that holds it ahead of the 16 rows that used to run before the reader got there.  `max/min/product` gained `Foo->new` (so rows 6–7 RUN for the first time) and fail on a `use overload` that is still RUN-phase where perl's `use` is a BEGIN (**#1507**).
- **#1505 FIXED: `import Foo::Bar;` emitted the bare designator `:Foo::Bar`.**  Not a CL token ("too many colons in threads"), so `load` stops at that form and the whole rest of the emitted file is lost.  `Pl::Parser::_cl_pkg_designator` is the single source of truth for that spelling and this one site bypassed it — one line.  corpus-diff IDENTICAL over 111; lib+board A/B 251 files, the only DIFF this line.
- **#1508 FIXED: `$ref->{k} =~ s///` / `=~ tr///` wrote to a VALUE.**  The `=~` write-target gate asked `_is_elem_arg`, whose set is `a_acc`/`h_acc` only — it also serves the `argbox` path, which has no `-deref-argbox` heads — so the DEREF pair never got lvalue_context.  **#960 fixed exactly this omission at `pos()`; the four-kind table is a FAMILY, and a scan for a third site is owed.**  Since `bfa170d9` (s461ar, #911) moved the read-only verdict to run time it stopped being a silent no-op and became perl's "Modification of a read-only value attempted" — and core Math::BigInt writes `$x->{sign} =~ tr/+-/-+/` six times, so every negation killed the program.  Emission A/B over Math::BigInt: the only change is `p-gethash-deref` → `p-gethash-deref-box` at those six sites.  Generation **v2-1010**, the three artifacts regenerated (bodies byte-identical).
- **THE FIX MOVED THE BOARD, AND A MOVE THAT READS WORSE IS NOT ALWAYS ONE.**  The final board (`baselines/cpan-board14-s474.tsv`, 183 rows, 79 PASS / 54 PARTIAL / 50 FAIL, 2190 ok / 346 not-ok) differs from s467 in exactly FOUR rows, all Scalar-List-Utils and all #1508's doing: max 5/2 → 8/2, min 5/2 → 20/2, product 12/1 → 23/4, sum **PASS 8/0 → PARTIAL 15/3**.  `sum.t` used to die after 8 of its 18 planned rows and "zero not-ok" called that a PASS; it now runs all 18 and 15 pass.  **A board label is not a measure — read the ok count** (#1502's whole point).  The six new failures are two causes, both pre-existing: #1507 (three of them) and **#1525, NEW — `scalar(EXPR)` on a HASH REFERENCE returns the RAW hash table**, so `is($v, $v1 + $v2)` (lowered `(p-scalar (p-+ …))`) hands `is` an unboxed Math::BigInt and its `""` overload is never found; `my $c = scalar($href)` stores the KEY COUNT.  `p-scalar`'s ARRAY arm carries the box-preserving guard and its own comment says the hash arm already did — the hash arm only DECLINES for a boxed hash and falls through to `(t v)`.  Rule 11, two copies, one repaired.
- **A drop in a BOARD dist's `t/` file is counted NOWHERE.**  The drop census's cpan population is `cpan-tests/modules/**/t/**`; the 14-dist board's own `t/` files are in no population, so Capture-Tiny's seven drops have been invisible to every counter.  The board also has no registered-timeout mechanism (the perl-suite runner has `baselines/perl-suite-timeouts.tsv`), which is the whole reason 05_extmul.t reads FAIL 0/0.  Both belong to #1502/#1504.
- **BARS.** Gate `PCLXS_DIR=~/pclxs tools/prove-core` **219 files / 7,603 rows**, Result FAIL = only the 13 pclxs xs rows.  Full sweep `--jobs 4`: **GATE clean, TOTAL passing 18674 = baseline (+0)**, 0 new / 0 fixed, drops 5 = census, shortfall +0, CAUSES 0.  `tools/corpus-diff.pl bfa11c72` IDENTICAL over 111 files after BOTH fixes; `tools/emission-ab.pl --shapes` over lib/ 27 files SAME / 0 DIFF / 0 RCDIFF.  Board re-run on the final tree (after the s473p rebase) = the blessed snapshot, and it is byte-identical to the run taken before that rebase — perf round 30 moved no board row.  Measured on the tree REBASED onto s473p's merge (`bfa11c72`): generation stays **v2-1010** (above main's v2-990) and the three artifacts, regenerated there, come back byte-identical to what the rebase produced.  Filed **#1506**, **#1507**, **#1509**, **#1510**, **#1511**, **#1512**, **#1525**.

## s473t3 (2026-09-07, Opus) — the 79 blessed rows whose only cause was a CATALOGUE NOTE now name a task or a section, and six of the notes were measured wrong about their own rows

- **Population (A) — column 6 STARTS with `CATALOG` — is 79 rows on `d9a2aac0`, and is now 0.**  Population (B), the 10 rows where `CATALOG …` is a courtesy suffix after `#NNNN`/`NS:`, is byte-identical to main.  The 79 edited rows differ in **column 6 only** (verified field by field through `:raw`); the old note is kept as a suffix so `docs/sweep-bug-catalog.md`'s prose stays reachable.  Full sweep `--jobs 4` on the final tree: **GATE clean, TOTAL passing 18674 (+0), 0 new / 0 fixed, drops 5 = census, `CAUSES: 0 of 478`.**  No code changed.
- **25 tasks filed, #1471–#1495.**  Two rows go to an existing task (**#155**, tied HASH — PCL announces `tie: a HASH … is not implemented`, so `hash.t 3` and `hash.t 13` were owned all along); 15 rows go to a `docs/not-supported.md` section (7 `DESTROY` called by garbage collector, 4 NUL bytes in identifiers, 3 Error compatibility for invalid Perl input / principle 9, 1 Error message text and format, 1 Context propagation into string eval).  Every filed task carries a reproducer with BOTH outputs, its row keys, the catalogue note it replaces, and its family.
- **SIX NOTES WERE WRONG ABOUT THEIR OWN ROWS.**  `array.t 126` is not arylen magic at all — the arylen half works and the row is **indirect object with a LOWERCASE class name** (#1479: `Pl/PExpr.pm`'s general pre-pass requires `/^[A-Z]/`, so `pling peen` with `package peen` compiles as `pling(peen())` and dies; the all-caps branch beside it already has a known-package exception the lowercase branch never reaches).  `length.t 34` is not tie semantics — the VALUE path is right and the **HARNESS** is wrong (#1482: `cl/pcl-test.lisp:396` `test-undef-p` reads `p-box-value` raw, so one program prints `defined=0` and `got: ''` two lines apart).  `local.t 258/259/260` has no `local` in it and the match spelling passes — `chop`, a bare `-X` and a **plain assignment** are the three that skip a tied `$_`'s magic (#1483).  `wantarray.t 11` is not "void context through `||`" (that block passes) — it is `eval $qcontext` in void context, which `docs/not-supported.md` "Context propagation into string eval" already names by row.  `sprintf.t 157/165` contain no positional specifier — the `+` flag is applied per vector ELEMENT (#1474).  `sprintf2.t 1509–1519` is not rounding — the subnormal SIGNIFICAND is wrong (#1476), a different fault from the genuine tie row 1534 (#1477).
- **`sort.t 26` reproduces after all** (s464 §1c had recorded that it did not): the shape needs the comparator to redefine ITSELF, `*twoface = sub { *twoface = *Backwards_other; $a <=> $b }`, and then `sort twoface 4,1,9,5` is `1 4 5 9` in perl and `9 5 1 4` in PCL — perl holds the CV for the whole sort, PCL re-resolves the name per comparison (#1491).
- **#1486, located by a five-variant probe: `*Pkg::AUTOLOAD = sub {…}` installs the handler from INSIDE `package Pkg` and nothing from `package main`** — while an alias to a named sub works from main, and a non-AUTOLOAD glob assignment works from main.  The call then returns **undef silently**, an answer perl can never give.  (method.t 103's NUL-byte method name is a red herring; with the AUTOLOAD installed in-package, PCL answers it correctly.)
- **A STALE BLESSED ROW IS NOT REMOVED ON A PARTIAL FILE'S EVIDENCE.**  `ref.t 67`'s recorded got is `SCALAR(0xf)` — the 15th reference stringified in the run, which the s219 `LVALUE` refs pushed the vstring off; the whole 24-row ref-type table is identical to perl today, and the full sweep runs ref.t to test 242 of 245 without failing it.  `sweep-diff.pl` still calls a PARTIAL file's absent baseline rows UNVERIFIED, never FIXED, so the row **stays**, with #1495 carrying the evidence and the release condition.  Same discipline as s473t2's magic.t 184/185/186/201.
- **Two ≤1-hour fixes were in reach and were NOT taken, on purpose**: #1479 widens a parser rule (needs the break-case probe, a generation bump and the four-population A/B) and #1482 is the harness row of the WHAT-TO-RUN-WHEN table (full sweep + companion `--all --quick` + row-by-row baselines).  Filing was the deliverable; both tasks name the bar they would have to meet.
## s470bz (2026-09-07, Opus) — #1302 + #1304: ONE root resolver, `pcl` installed and uninstallable, and the installer tested in a container (which found #1327 on its first run)

- **`PCLPaths::root($hint)` is the ONE answer to "which PCL tree am I part of", and `PCLPaths::root_of($path)` is the same derivation without the environment override.**  Five hand-written spellings agreed only by accident of depth (`pcl`, `pl2cl`, `runpcl`, the sweep, `tools/pclperl-for-tests`, `tools/runt`, `tools/clt`, `PCLSbcl::_installed_core`).  The rule: `$PCL_ROOT` when set — explicit ALWAYS wins, and an unusable one DIES naming both candidates rather than falling back silently — else the caller's own real directory and then its PARENT, exactly two, each verified by `cl/pcl-runtime.lisp` being there.  Not a walk to `/`: that answers with whatever checkout sits above, which is worse than dying.
- **THE BOOTSTRAP STAYS AND IS ASSERTED.**  A script cannot call `root()` until it has found `tools/lib`, which is in the tree it is asking about, so one `use lib` per script is permanent.  What must not grow is a SECOND rule beside it: `Pl/t/pcl-root-01.t` writes out each caller's old spelling and demands it equal `root()`'s answer, and runs all seven scripts under a bogus `$PCL_ROOT` demanding the resolver's own message — a caller still deriving its own root passes silently otherwise.
- **`pl2cl`'s `@INC` IS PART OF ITS OUTPUT** (`*p-core-inc-dirs*` in every program preamble), so it loads PCLPaths through a `local @INC` + `require`, never a permanent `use lib`.  **Requiring it by ABSOLUTE PATH instead is a `%INC` bug with a loud blast radius**: the path becomes the key, the sibling `require PCLPaths` in `Pl/ProtoCache.pm` and `tools/lib/PCLSbcl.pm` reads the file again, every sub is redefined, and ten warning lines land on the stderr of EVERY transpile — five gate files failed on that noise alone.  `Pl/ProtoCache.pm::cache_dir` already had the right shape; the sibling was there to be copied (rule 11).
- **An installation is `pcl`, `pl2cl`, `runpcl` — ONE list** (`@COMMANDS`) that the tree copy, the wrapper loop, the smoke test and `--uninstall` all read, so a fourth command is installed, wrapped, tested and removed by construction.  `pcl` had never been installed at all while the README called it "the everyday command".
- **`--uninstall` removes only what it wrote**: the wrappers are recognised by CONTENT, so a `pcl` somebody else put in that bin directory survives and is said to survive; it refuses naming the directory when `<prefix>/lib/pcl` has no runtime (rule 12); and it requires NO dependencies, because the usual reason to uninstall is that something about the machine changed.  A per-user cache is not part of an installation and is left alone.
- **An installer's SOURCE must not be redirectable.**  `tools/install-pcl` resolves its own root with `root_of($0)` and deliberately ignores `$PCL_ROOT`: what it installs is the tree it is part of, not whatever an environment variable names.
- **THE VERSION AN INSTALLED TREE REPORTS HAS TWO SOURCES AND SAYS WHICH**: `git describe --tags`, else `CHANGELOG.md`'s top heading (what a tarball or a `git archive` extraction has), else "unknown" plainly — a guessed version is worse than none in a bug report.  One writer (`tools/install-pcl`), one reader (`pcl`'s `pcl_version`).
- **The fresh-machine recipe is ONE recipe in TWO halves** — `tools/install-matrix/deps.sh` + `verify.sh`, with `install-and-verify.sh` the two in sequence so the CI workflow is untouched.  Each half sets its own environment (an exported PATH does not cross to a sibling process), and `verify.sh` picks up `$HOME/sbcl/bin` itself, conditionally.  Two deliberate behaviour changes, declared: `PPI_MIN` is now EXPORTED (the floor check is a child perl reading `$ENV{PPI_MIN}`; unexported it compared against undef and could not fail — CI set it as a job env, which is why it worked there and only there), and `verify.sh` runs `pcl` beside `runpcl`.
- **`tools/t/install-container.t` (podman, docker accepted, plan-skips naming both when neither answers) runs the matrix locally plus the two shapes it cannot reach**: a non-root user installing into `$HOME/.local`, and a SHARED `/opt/pcl` install serving a different user with an empty home.  The base image is tagged by a hash of `deps.sh` + its Containerfile + `SBCL_VERSION`, so an edit rebuilds rather than reuses; the repo enters as a `git archive HEAD` extraction bind-mounted READ-ONLY, so the test measures what is COMMITTED and says so when the tree is dirty.  87 s cold, 47 s warm.
- **#1327, found by leg (c) on its FIRST run and reproduced without a container: a saved PCL core memoises `asdf:*user-cache*` as the CORE BUILDER's home.**  `uiop:xdg-cache-home` is read at call time and is correct; `*user-cache*` is not, because PCL saves cores with a bare `save-lisp-and-die` and UIOP's image-restore hooks never run.  Any ASDF work in another user's process then targets a directory it cannot write — which is what a system-wide install invites, since a sysinit loading Quicklisp is the ordinary way to make cl-ppcre visible to everyone.  The #1303 family one layer out.  NOT fixed here: it is a `cl/` change owing the full bar, and clearing ASDF's configuration on every start has a cost that must be measured against Target A first.
- Bars: gate **218 files / 7569 tests** (only the 13 pclxs xs rows); `corpus-diff b5e9f845` IDENTICAL over 111, silent drops 5, run twice; `PCL_SHOW_SBCL` byte-identical vs a `git archive b5e9f845` extraction for `runpcl` and the sweep, on the cached-core AND the `PCL_NO_CORE` leg (normalising only the tree root, the core's PATHKEY — which IS `sha1(abs runtime path)` — and per-run temp names); full sweep `--jobs 4` GATE clean, **TOTAL passing 18674 (+0)**, 0 new / 0 fixed / 0 LOST, drops 5 = census, shortfall 12213 (+0), CAUSES 0, `_status.tsv` compared file by file against the OLD runner on that extraction.  Guards: `Pl/t/pcl-root-01.t` 28 rows (inverse: 28/28 fail on the extraction, and with only the new `PCLPaths.pm` copied over exactly the seven caller rows fail), `tools/t/install-pcl.t` 15 → 57 rows / 21.7 s (inverse: 23 of 57 fail on the extraction), `tools/t/install-container.t` 17 rows.  Filed **#1325**, **#1326**, **#1327**.

## s473t2 (2026-09-07, Opus) — EVERY blessed sweep failure now names its cause: 152 causeless keys attributed, 15 tasks filed, and the read-only-scalar family is bigger than its entry says

- **The bar is met and the runner reports it: `CAUSES: 0 of 468 blessed row(s) have no cause`** (the sweep's own line, previously 152).  `baselines/fail-baseline.tsv` column 6 was written on 152 of the 479 keys; columns 1–5 are byte-identical to `0d783d24` on all 479 data rows and the 34 existing comment lines are unchanged (verified with a `:raw` field compare, not a diff eyeball).  No row added, removed or re-blessed.  Bars: gate 216 files / 7517 rows, FAIL = only the 13 pclxs xs rows; full sweep `--jobs 4` **GATE clean, TOTAL passing 18649 (+0), 0 new / 0 fixed / 0 LOST, drops 5 = census**.
- **pack.t's 47 keys read `PARKED: pack/unpack (USER 2026-09-06)` and were not probed** — the USER's instruction, executed literally.  The other 105 were clustered by the assertion's got/expected shape, ONE representative per cluster probed against perl 5.40.3 with `./runpcl` (13 probe files, `scratch/s473t2/probes/` in the agent worktree), and attributed.  **Counted by the FIRST cause a row names: 67 rows name one of the 15 new tasks #1446–#1460 (several of them alongside #221 or a not-supported section), 21 name only an existing task (#155 #174 #221 #233 #408 #449 #1190), 14 name only a `docs/not-supported.md` section (5 the read-only-scalar entry, 4 `DESTROY` called by garbage collector, 2 principle 9, and one each for the utf8 flag, `local` RT #130727 and regex code blocks), 3 a CATALOG note or the sibling brief's shortfall.**
- **A row's cause comes from the test SOURCE, and two of them were owned all along.**  `bless.t 105` "blessing through tied refs" is **#408**, whose own description names `perl-tests/bless.t:179`; `multideref.t 24/27/30/33` are **#1190**, the local-element family — probed: `local $h{k}` and `local $b[1]` restore correctly, `local $a[3]{foo}[1]{c}` does not (the nested accessor hands back a detached box, so the restore writes where nobody reads).  Neither had ever been written into the row.
- **THE READ-ONLY-SCALAR FAMILY IS BIGGER THAN ITS ENTRY SAYS.**  `docs/not-supported.md` "A LITERAL in a `foreach` list is writable" carries the general rationale ("a read-only scalar has nowhere to carry the flag in PCL's box model"), and three separate clusters are that same fact: `scalar.t 34/36/40` (`open(F,'>',\43)` — perl refuses with EACCES, **PCL opens it and TRUNCATES the literal**, so the following read gets nothing), `method.t 155` (a constant class name is not read-only) and `local.t 264`.  Cited as that section with the shape named per row.
- **Fifteen tasks, each with its probe and its row count.**  #1446 an in-memory `\$scalar` handle binds a STRING, not the SV — tie FETCH never fires and the SBCL print form of the tie proxy reaches the program's own output (7 rows); #1447 a >0xFF target must fail the open with EINVAL (9); #1448 `${^LAST_FH}` is hardwired to `p-undef` at `Pl/ExprToCL.pm:322` (3); #1449 a `die` in a compile-phase block skips the remaining QUEUED phases and prints no "BEGIN failed" line (5); #1450 `@DB::args` is never populated (6); #1451 `local` in a non-leading expression position emits `(pl-local …)`, a call to a function that does not exist (2); #1452 list assignment does not resolve its LHS lvalues first (5); #1453 `*FH{IO}` yields the GLOB, not an IO::File object (2); #1454 the postfix glob deref `->**` is unparsed (3); #1455 a string eval in a named sub binds a FILE lexical declared LATER (1); #1456 a symbolic array/`$#{}` deref through a non-identifier name (6); #1457 the `pos()` residue — `s///eg`, a glob, a foreach alias (3); #1458 the `%ENV` store's utf8 downgrade/encode rules and `local $ENV{K}` deleting the key (8); #1459 a pool of seven one-row shapes; #1460 the shapes behind the COLLAPSED unnamed keys of eval.t and ref.t.
- **#1452 IS s464 review §4c item 1, now probed and owned**: `my $x=1; my $sref=\$x; ($sref,$$sref)=(2,3)` gives perl `2/3` and PCL `3/1` — PCL assigns left to right, so `$$sref` dereferences the value it just wrote.
- **A row with an EMPTY description stands for ALL of its file's unnamed failing rows** (`sweep-diff.pl` keys on `file\tdescription`, #1041).  `eval.t 99` and `ref.t 194` are each one such key over six-to-eight real assertions, so their cause is compound and #1460 holds the shapes: `eval EXPR` must impose SCALAR context (`my @b = eval qw(a b c d)` is `4` in perl, empty plus `Undefined subroutine &main::ARRAY` in PCL); an eval inside a sub in package `DB` sees the CALLER's pad; `local(*bar) = *bar` loses the value; and a `runperl(prog => …)` child reports `at - line 1` where perl reports `at -e line 1` (that last one is `tools/pclperl-for-tests`'s program NAME, so it reaches every `prog =>` child in the suite).
- **FOUR ROWS PASS WHEN MEASURED ALONE and were KEPT, not removed** — `magic.t 184/185/186/201`.  They failed in the blessed `--jobs 4` run and pass under `--jobs 1` and `tools/runt`; the four `%ENV` store shapes they test all agree with perl in isolation (`p4-env.pl`).  magic.t clears `%ENV` wholesale earlier in the same block, so #1458 (c) says: measure `%ENV = ()` against the child environment first.  Removing them by edit would have made the next full sweep report them NEW.
- **`tools/runt` is not the sweep**: it runs the children under the REAL perl (no `PCLPERL`), so `split.t 32` (`runperl` with `-Dt`) passes there and fails in the sweep, and `ref.t` stops at 108 rows under runt against 211 in the sweep.  Use runt to read a file's TAP, never to decide whether a row fails.
- **NOT DONE, and why: the ~60 rows whose cause is a CATALOG note still have no task.**  The brief's secondary ask needs one task per cluster and the 15 assigned IDs (1446–1460) were all consumed by the causeless-row clusters; `docs/plan-post-s473.md` reserves 1461+ as the next free range, and squatting on it risks two tasks with the same number.  The clusters, ready to file in one pass: `sprintf.t` version objects in `%vd`/`%vx` (11), `sprintf.t` INVALID/REDUNDANT/MISSING markers (6), `array.t` freed-array length (6), `qr.t` PVLV/blessed-`qr//` stringification (5+1), `method.t` AUTOLOAD via `@ISA` (5), `sprintf2.t` subnormal `%a` rounding (4+2), `ref.t` NUL/UTF-8 symbolic-ref stash names (4), `split.t` chained list-assignment-as-lvalue (3), `readline.t` `tell()` after unglob (3), `local.t` `local $_` with filetest/match (3), `tr.t` 223-224 + 257 (4), `sprintf.t` reordered positional+vector (2) and `%.0g` edges (2), `readline.t` `<>` autovivification (2), `local.t` `local *{$pkg}{method}` (2), `do.t` 63-68 + 70 (3), `delete.t` `\delete` address equality (2), `array.t` arylen residue (2), plus six singletons (`wantarray.t`, `ref.t` vstring refs, `sort.t` in-file interaction, `length.t` 34, `hash.t` ×2, `args.t` 23).

## s473t1 (2026-09-07, Opus) — the 13 early-stop files TRIAGED: one written-down cause each; the sweep was hiding the answer in the lines it strips

- **WHY THEY WERE "UNEXPLAINED": `tools/sweep-perl-tests.pl` folds stderr into stdout and then deletes every `^;` line — which is exactly where `p-load-with-recovery` prints `; PCL recovery: top-level form aborted (recovered): <error>`.**  Rerun a file through the sweep's own recipe with stderr KEPT and 11 of the 13 name their cause in one line.  Instruments (kept in the agent's scratch, not shipped): `triage.pl` (both sides — PCL, and the SAME file under real perl 5.40.3 run from the perl build root so `require "./test.pl"` finds perl's test.pl and not PCL's stub), `anchor-diff.pl` (join on NAMED rows: perl labels an unnamed row `[at FILE line N]`, PCL labels it with nothing, so an LCS over descriptions mis-aligns), `formmap.lisp` (top-level FORM boundaries by SBCL's own reader).
- **A textual `^(` scan does NOT give the top-level form boundaries — the emitter writes some NESTED forms at column 0** (`(p-local-glob …` inside a `p-let`).  Use the reader.  This is not cosmetic: it is the difference between "method.t's drop costs 1 row" and the truth, 35.
- **#1433 (NEW): the announce→DIE flip's blast radius is the ENCLOSING TOP-LEVEL FORM, and a file-level `my` makes that the rest of the file.**  `perl-tests/method.t`: ONE drop (line 72, the ruled indirect-object non-support) dies inside the `p-let` binding `my $obj` from line 66, whose form spans source lines 66–199 → **35 assertions never run**.  `perl-tests/substr.t`: two lvalue-sub drops cost 40 and 4.  `perl-tests/ref.t`'s drop is its own form and costs **0**.  not-supported.md's "costs only the rows … inside its own top-level form" now says so explicitly in three places.
- **#1024 CLOSED — `_kv_slice_node` was a THIRD copy of the subscript path.**  `_parse_subscript_ix` exists because the interpolation and expression paths had already drifted into two copies of the lone-bareword Word case; the five kv-slice sites went through `_kv_slice_node`, which called `parse()` directly, so `%h{i}` / `%$hr{i}` / `%{$hr}{i}` / `$hr->%{i}` / `delete %h{i}` ALL compiled the key as a sub call and died.  One argument (`$type eq 'kv_slice_a_acc'`) routes it through the existing predicate; perl's rule is positional and sigil-blind (a HASH subscript autoquotes even when a sub of that name exists; an ARRAY subscript calls it) and PCL now matches on both halves.  kvhslice.t 38 → 39 rows, OK.
- **#1431 (NEW, FIXED): `+EXPR` is perl's pure disambiguator, and a site that INSPECTS the argument node must see through it.**  `gen_prefix_op_form`'s `+` arm already passed the operand through, but `_elem_container_key` — the ONE reading of container-and-key, shared by `exists`/`delete`/`pos` — saw the `prefix_op` wrapper, declined, and the caller emitted a ONE-argument `(p-exists VALUE)`: a raw CL "invalid number of arguments: 1" that aborted the whole form.  `_thru_unary_plus` repeats the emitter's own two steps at one point.  multideref.t 52 → 65 rows, OK.
- **#1432 (NEW, FIXED): a `perl-tests/t/` SHARED HELPER had been replaced by a stub, and a `do FILE` that returns 1 having run nothing looks exactly like success.**  `perl-tests/t/op/caller.pl` was 4 lines where perl ships 175 and 47 assertions ("Stub: XS hint_fetch/hint_exists not available in PCL", 6b8e524c) — and the premise is false: `perl-tests/caller.t` defines both subs itself, in plain perl, over `(caller $n)[10]`.  Restored from perl 5.40.3: caller.t 65 → **112 rows** (+11 pass, +36 honest fails, one cause — PCL's `caller EXPR` returns FOUR elements where perl returns eleven and element 10, the `%^H` hints hash, is absent).  Guard row in `Pl/t/tap-assert-01.t`.
- **#1439 (NEW): `perl-tests/t/test.pl`'s `skip_if_miniperl` skips UNCONDITIONALLY while its own `is_miniperl` returns 0** — 15 call sites in 9 files.  Same family as the already-fixed `skip_without_dynamic_extension`.  MEASURED with perl's own body (`skip(@_) if is_miniperl()`) and reverted: magic.t +5 pass, ref.t +3 pass, **method.t −25 produced rows** (its 26-row IO::File block crashes its form when it actually runs).  So it is a per-call-site job with the rows read, like the s464ay kvhslice/splice restoration — not one edit.
- **#1438 (NEW): three perl-tests files carry a STALE `plan` — 9 rows of shortfall no fix can remove.**  `bop.t` 510 vs 509, `eval.t` 169 vs 163, `method.t` 163 vs 161; each is an old PCL edit that commented tests out and left the count, and **REAL PERL runs the lower number too** (measured).  Changing a `plan` is an edit to a `perl-tests/*.t` file, so it needs the USER.
- **A row can be MISSING rather than FAILING whenever the assertion lives inside a callback that never fires** — `pass()` inside `$SIG{__WARN__}` (length.t 2 rows, #221), `is()` inside `DESTROY` (ref.t 3, postfixderef.t 4), a child program's `print "ok $tn[4]"` through an untied array (magic.t 3, #155).  The shortfall bucket is the only one that sees these.
- Also filed: **#1434** (`*glob->method` / `(\*glob)->method` die "Can't call method self on unblessed reference"), **#1435** (`DESTROY { }` without a leading `sub` is not installed), **#1436** (a BAREWORD invocant of a postfix deref — `foo->@*` — is compiled as a sub call where perl takes it as a symbolic ref), **#1437** (`local *STDOUT` does not detach the handle: the block's output glues itself to the next TAP line and costs that row its verdict — yadayada.t's whole shortfall), **#1440** (a kv-ARRAY slice reports the normalized integer index where perl echoes the index EXPRESSION: `%a[-1]` keys −1, `%a[1.7]` keys 1.7).
- **BARS (re-measured on the tree REBASED onto main `48d2c384`)**: gate 217/7541 with only the 13 pclxs xs rows; full sweep `--jobs 4` GATE CLEAN, TOTAL passing 18674 (+0 against the edited baseline; the +25 over main is caller.t 11 + multideref.t 12 + kvhslice.t 2), drops census 5 = current 5, shortfall +0; corpus-diff 1 of 111 files (multideref.t) against both the launch commit and the new main; emission-ab SAME over 25 + 93 + 604 files bar t/op/multideref.t, RCDIFF 0; companion op/ leg 0 NEW ROW / 16 FIXED ROW / 0 LOST with a 0/0/0/0 confirmation; the three artifacts regenerate byte-identical after the rebase; generation v2-960.  The UNEXPLAINED shortfall population drops from 49 files to 36.
## s470bu (2026-09-06, Opus) — perf round 29: the regex/subst/tr LITERAL is built once per SITE (#1250) and the s/// op CARRIES its compiled record (#1251); json-rt 3.60x perl -> 1.81x

- **A regex/subst/tr LITERAL now builds its op ONCE per emitted call SITE (#1250).**  `%p-op-once` = `(load-time-value (cons nil nil) nil)` + `(or (car site) (setf (car site) FORM))`, the #1180 `p-symref-site` shape.  It is LAZY on purpose: the constructor can announce on stderr (`%pcl-strip-regex-code-blocks` stripping a `(?{code})` block) and can reject a modifier letter, and a `(load-time-value … t)` would move both to the LOAD of the file, so a site that never executes would announce.  Fires only when EVERY ingredient is a compile-time string (a literal or a `(p-esc "…")` payload).
- **`p-qr` deliberately does NOT memoize** — two evaluations of `qr/a/` are distinct references in perl (ir-spec §10), which is why `%p-qr-parts` was never memoized either.  Nor does a s/// whose replacement is a CLOSURE: it may capture lexicals, and a site cell would freeze the first capture.
- **The s/// op now carries its COMPILED record (#1251) — m//'s #680, four years late and much larger.**  `do-regex-subst` re-ran `perl-regex-to-ppcre` (SIX `cl-ppcre:regex-replace-all` passes over the pattern text), the replacement translation, seven `member` walks of the modifier list, an `append`ed options list and a FORMATted scanner-cache key on EVERY evaluation; `perl-regex-to-ppcre` alone was **33.1 %** of the `json-rt` macro row's samples.  The record is a pure function of the op's three data slots and nothing writes those after the op is built, so THE OP IS THE CACHE — there is no invalidation rule to get wrong.
- **THE SLOT HOLDS A CONS, NOT THE RECORD — that is what makes the lever worth taking.**  An s/// with a closure replacement (every interpolated replacement, every /e — JSON::PP's hot ones) must build a FRESH op per evaluation, so a cache living in the op caches nothing.  The record does not depend on the closure (a `functionp` replacement is always eval-p), so the cons can be a per-SITE cell: `%p-op-site` is passed to `%p-subst-parts` whenever the `p-subst` macro proved the PATTERN and FLAGS constant.  A run-time pattern gets a private cons and behaves as before.
- **A cache whose construction can SIGNAL is built inside the caller's `handler-case`, and stored only on success** — a bad s/// pattern still warns and answers 0 exactly where it did when the scanner was built per call, and a retry re-signals rather than reading a half-built record.
- **MEASURED, quiet box (load 1.99), `tools/bench-exec.pl` with `BENCH_RT_B` = main `31473881`'s runtime, interleaved best-of-5, startup subtracted:** json-rt 3.2013 -> **1.6064 s (-49.8 %)** = 3.60x perl -> **1.81x perl**; subste 0.6433 -> 0.3980 (-38.1 %); regexg 1.5295 -> 1.1602 (-24.1 %); textproc 3.7141 -> 2.8516 (-23.2 %).  Controls intloop+= -1.9 %, strcat -1.0 %, sortstr +1.2 %, moo-objs +0.5 % = the spread.  A second run at load 5.4 agreed within 2 points on every row.
- **THE PER-CALL COST OF s/// IS INVISIBLE ON A LONG SUBJECT.**  Isolating #1251 from #1250 (one program, two runtimes, B = HEAD~1): `subste` (20 short fields, /ge + /g) **-43.5 %**, `substg` (the same two s/// over a 900-char string) only -5.4 %.  So a benchmark that measures ONE s/// scanning a big string cannot see this lever at all — `tools/bench-exec.pl` gains `subste`, the many-s///-on-short-subjects shape CPAN code actually has.
- **Neither lever changes the emission** (`%p-op-once` / `%p-op-site` are inserted by the runtime's own macros and never appear in emitted text): corpus-diff IDENTICAL over 111 files, emission-ab 27/27 SAME, no `PCL_OPT` name, **no generation bump and no artifact regeneration** — the #1181 / #1203–#1205 precedent for a runtime-only speed change.
- Bars: gate 216/7478 with only the 13 pclxs xs rows; full sweep GATE clean (0 new / 0 fixed / 0 LOST, TOTAL 18649 +0, drops 5 = census, shortfall +0); ir-conform 289/0/56 known/0 stale; companion `--jobs 1` over eight regex-heavy re/ + op/ files against a `git archive 31473881` extraction with **zero movers** (the one TIMEOUT -> DIFF flip at identical counts did not reproduce serially on either tree — load noise, nothing spliced); guard `Pl/t/perf-levers-03.t` 11 -> 18 rows, inverse-verified (nine mechanism rows fail on the extraction; the six negatives and three RUN rows pass, which is the design).

## s470by (2026-09-06, Opus) — #1300 SHIPPED: the cache directory is a PROCESS fact (#1303), an entry is kept while it is USED (#682), the directory is 0700, the four commands exist — and #1338, the shared cache is racy BY DESIGN
- **#1303 FIXED: `PCL_CACHE_DIR` is read at PROCESS START, never at load.**  `%p-default-cache-dir` + an `sb-ext:*init-hooks*` entry (the shape `$$`, the FP modes, the standard handles and `PCL_RAW_ELEMS` already use); the `defparameter` initform used to run when the runtime LOADS, which on the normal path is when a saved CORE is built — so an installed core would have sent every user's modules into the builder's home.  Probe: before, `PCL_CACHE_DIR=<tmp>` put `proto/` in the tmpdir and the module `.lisp` in `~/.pcl-cache` (530 → 533 files); after, `~/.pcl-cache` unchanged — and a core BUILT with `PCL_CACHE_DIR=A` and RUN with `=B` writes into B.
- **ONE Perl-side reading of it: `PCLPaths::cache_root`** (+ `cache_root_source` = `env`/`default`), called by `pcl`, `PCLSbcl::core_cache_dir` and `Pl::ProtoCache::cache_dir`; there were three copies of the expression and a fourth, in the runtime, evaluated in the wrong process.
- **Validity is the #1261 manifest ONLY; the age rule is disk hygiene keyed on LAST USE** (F2, folds #682).  A used entry is re-stamped by `%p-note-cache-use` — the `.lisp`, the `.deps` and this runtime's `.fasl` TOGETHER, because the prune reads each file's own mtime and half an entry is not an entry — at most once a day per entry, once per source per process; `p-cleanup-old-cache` drops what nothing reached for 30 days (`*pcl-cache-max-age*`), in `modules/` AND `proto/`.
- **The prune scan is claimed once a day because it was MEASURED**: 590 module files 7.0 ms + 1112 prototype files 14.0 ms = **21 ms per scan**, warm, invoked on EVERY cache miss; `%p-claim-prune` publishes a `.last-prune` stamp (temp + `rename(2)`).  The touch is one `utime(2)` = **0.0020 ms**, and the warm A/B is flat: `use Moo` 435/438 ms before vs 455/421 after, `use JSON::PP` 1476/1422 before vs 1396/1399 after (best-of-5, two rounds, box load 6.9–7.6 beside a sibling agent).
- **The cache root is created `0700` and an unsafe one is REFUSED loudly** (F8): another uid, or group/world-writable, dies perl-shaped and trappable, naming the directory, the mode and `chmod 700 DIR` (rule 12).  Only the ROOT is asked — at 0700 nobody else traverses into it.  **This box's `~/.pcl-cache` was 0775**; the four dirs were `chmod 700`ed.  **And the refusal fires on the ordinary accident**: a cache dir made by hand with `mkdir` under the usual `0002` umask is 0775, which is how the first cold board run of this session ended — every transpile refused, with the message naming the fix.  A directory PCL creates itself is 0700 and never hits it.  What a mode check cannot see (a rename out from under a non-sticky world-writable parent) is written into ir-spec §9.2b instead of assumed.
- **`--no-cache` used to skip READS only**: `*pcl-skip-cache*` still WROTE the `.lisp` and its manifest (measured: 2 files), so the one flag for "is it the cache?" repopulated the thing in question.  `%p-load-module-uncached` transpiles to a temp file outside the cache, loads it, deletes it; `pcl --no-cache` forwards `pl2cl`'s existing flag (no second mechanism).
- **A NEW LOAD PATH MUST REBIND `*pcl-current-package*`** — found by the full sweep, invisible to the gate: `%p-load-module-uncached` returned early from `p-load-module-cached` and skipped its rebinding, so a module's `package` leaked into the caller and later unqualified names resolved in the wrong one (hexfp.t 107 → 100, sub.t 53 → 51, every moved row `got=undef`).  **The whole perl-tests sweep runs with `*pcl-skip-cache*`** — that is why a `cl/` change owes a sweep even when the gate is green.
- **`pcl --cache-info` prints what the RUNTIME resolves, not a second answer**: the compile policy line comes from `p-cache-info` because `PCL_COMPILE_DIRS` / `PCL_NO_COMPILE_DIRS` are `realpath`'d at call time; the core line is asked of `PCLSbcl`, the one command-line builder.  45 ms warm; `pcl --version` (PCL / cache generation / SBCL / PPI) 64 ms; `pl2cl --help` exists; `tools/install-pcl` writes `<libdir>/VERSION` so an installed `--version` is not "unknown".
- **#1338 (absorbed, the same mechanism): a reader never dies over the shared cache.**  `<cache>/` belongs to the USER, not to a process — two worktrees, two CI jobs, the eight workers of a `prove -j8` — so anything listed, stat'ed or stamped can be gone before the next call.  **`%p-mtime` is now the runtime's ONLY `file-write-date`** (grep: exactly one) and it answers NIL, which means SKIP on the hygiene side and MISS on the validity side — the s329 boundary, effect-only, so not rule 12's die; `%p-load-module-fasl` gains a file-error arm narrowed to "the fasl is no longer there", so a file error raised BY the module's own code still propagates.  The die #1338 was filed on was already gone here (items A–D had rewritten the prune with the stat inside `ignore-errors`), which is why 6 of the 9 guard rows pass on the parent commit too.
- **#1338 round 2: the file the race was measured on was a WRITER'S IN-FLIGHT TEMP, so the prune does not ask about those at all.**  `%p-cache-temp-path` is the one construction both writers publish through and `%p-cache-temp-p` the one recognition (they each built the name inline before — the duplication that lets a reader's idea of "a temp" drift from a writer's).  The suffix is `-tmp<pid>`, not `-<pid>`: an entry's name is uppercase hex, and a bare `-<digits>` would collide with the ~0.5 % of fasl identity fields that are all decimal digits.  THE COST, in the docstring: a temp whose writer CRASHED is never pruned — one small file per crash; the alternative is to delete a file another process owns.
- **BARS.** Gate on the rebased tree 216 files / 7517 rows, Result FAIL = only the 13 pclxs xs rows.  Full sweep `--jobs 4` (after the `*pcl-current-package*` fix): **TOTAL passing 18647 = baseline (+0)**, 0 new / 0 fixed / 0 LOST, drops 5 = census, GATE clean.  `tools/corpus-diff.pl` vs both `cd34df72` and `31473881`: emission IDENTICAL over 111 files, silent drops 5 unchanged (**no emission change, so no generation bump**; the tree carries main's `v2-840`).  `tools/ir-host-leak.pl` byte-identical to the extraction (31 distinct leaked symbols over 111 files, both).  **CPAN board 14 dists, COLD (`PCL_CACHE_DIR=<fresh>`) and WARM: 183/183 rows identical to `baselines/cpan-board14-s467.tsv` and to each other** — no cold/warm difference, so no cache bug.  Companion `--jobs 1` over `op/do.t op/inccode.t op/incfilter.t op/require_errors.t comp/require.t`: the first four = snapshot; comp/require.t reads 911/836 here and the BASE extraction reads 911/836 parallel / 909/838 serial on ONE tree — a spread of its own, NOT attributable, snapshot NOT edited.  `PCL_SHOW_SBCL` / `pcl -v`: the SBCL command line is byte-identical before and after except the core NAME (content+path keyed) and the per-run `/tmp` temp names.  Guards `Pl/t/cache-surface-01.t` (50 rows, NEW) + `Pl/t/module-fasl-cache-01.t` (42 → 49), inverse-verified on a `cd34df72` extraction.
- **FILED: #1315** — `pcl` is the one SBCL-spawning entry point that ignores `PCL_SHOW_SBCL` (it prints its command line under `-v` only, while `runpcl` and the four dev runners read the variable).  Measured taking the SHOW_SBCL bar; one line, filed rather than folded in so the batch would not owe a second gate.
...
## s470bx (2026-09-06, Opus) — the s470bs residue: `$^T` and `rand` per RUN (#1042/#1236), `stat`/`lstat` answer their CONTEXT (#1043), and a BAREWORD in an EXPR handle slot is a handle NAME (#1231/#1044)

- **A `defvar` INITFORM is per IMAGE, and since s439b the image is a SAVED CORE** — so `$^T` was the core's BUILD moment and `-M`/`-A`/`-C`, which are `($^T - filetime)/86400`, came out NEGATIVE for any file written after the core was built.  Measured 17 s here, 184 s and 2831 s in #1042's own report: **the offset IS the core's age**.  Fixed the way `$$` (the pid) already was — a p-box refreshed from `sb-ext:*init-hooks*` at boot.
- **`$^T` is ASSIGNABLE in perl, and a bare integer is not a place** — `$^T = 12345` was a silent no-op, the `$0` bug of task #512 in a second variable.  That is why the box and the hook are ONE change: the same variable, the same family.
- **#1236 is the same audit's other member with the opposite failure**: `rand` was never seeded, so two runs printed the SAME number.  perl seeds from entropy on FIRST USE (`PL_srand_called` in pp_rand), so PCL does too — a `*p-drand48-seeded*` flag, NOT an init hook, which would seed before the program and erase the first-use boundary `srand`'s own return value documents.  `%p-drand48-set-seed` is now the ONE writer of the state; the entropy seed carries the pid and the run-time counter, because the wall clock alone gives two processes started in the same second the same seed.
- **#1043 — `stat`/`lstat` answer their CONTEXT: the 13-element list, or perl's `&PL_sv_yes`/`&PL_sv_no`.**  The failure answer is the empty STRING, not undef, so `defined(scalar stat "/no/such")` is TRUE.  PCL handed the vector back everywhere, so `my $ok = stat $f` stored an ARRAY REF and `$ok == 1` was an address comparison.  **Truthiness agreed either way** — a non-empty list and 1 are both true — which is exactly why the sweep and the companion were clean with the bug in place.  Two edits: `%WANTARRAY_SENSITIVE` gains the pair, and `%p-stat-answer` is the ONE reading of the context for both ops.  t/op/stat_errors.t 510/128 → 532/106 (22 rows, its `is $res, !!0` assertion).
- **#1231 + #1044 are ONE predicate seen from two sides, and the fix is keyed on WHICH SLOT.**  perl's rule for an EXPR handle slot (`stat`, `lstat`, the 26 filetests): a DECLARED sub is CALLED, anything else is a FILEHANDLE NAME.  PCL asked "is this bareword a handle REGISTERED IN THIS FILE", so `-e NOPE` compiled to a CALL and DIED unless some other statement in the same file had put NOPE in a handle slot.  The FACT is one flag — `expr_slot => 1` on `stat`/`lstat` in `Pl::Environment::_builtin_prototypes`; the two consumers are `_read_star_slot_bareword`'s builtin half (#1044) and `PExpr::_expr_slot_bareword_operand` (#1231, a filetest is an OPERATOR and has no prototype record), sharing `_bareword_slot_word` and `_expr_slot_keeps_call`.  **t/op/stat_errors.t 532/106 → 636/2** — 104 rows, 0 new.
- **THE STRING-EVAL HORIZON IS WHERE A COMPILE-TIME RULE HAS TO STOP.**  An eval fragment is transpiled without the enclosing program's sub table, so every bareword looks unplaceable there — and answering "handle name" for all of them turns `eval "-e SPATH"`, a DECLARED sub, into an unopened handle.  The compiler now DEFERS in eval mode and `%p-fh-arg` gains a third mode `:expr-slot`, whose `(pl-NAME)` arm fires only when NAME is not `fboundp`: the macro expands INSIDE the eval, in the image where `pl-SPATH` either exists or does not.  **One rule, asked at whichever moment can answer it.**  Caught by a probe, not by any suite — the first version shipped the regression silently.
- **A GUARD THAT REUSES ONE BAREWORD PASSES ON THE BROKEN TREE.**  The old discriminator was FILE-WIDE, so a name that appears in ANY handle slot anywhere in the program was already read correctly everywhere else: a first draft of the #1231 rows used one name for `-e`, `stat` and `-M` and FOUR of them went green against `fcf076f1`.  Each of ZZFTONLY / ZZSTONLY / ZZMONLY is now used ONCE, in ONE slot.
- **Accepted residue**: a LOWER-CASE bareword in an EXPR slot stays a CALL, though perl reads it as a handle name too (probed: `-e nope` is EBADF even under `use strict`).  #266's asymmetry — PCL's compile-time name knowledge is incomplete, so an absence of knowledge must fail LOUDLY.  The gate is `fh_bareword_shape`, the same one every sibling site uses.
- **Bars**: gate on the final rebased tree (main `637fc58e`) **215 files / 7451 rows**, FAIL = only the 13 standing pclxs xs rows — `use-require-01.t`'s two rows died in the shared-cache prune race **#1338** (`p-cleanup-old-cache` stats a key a sibling process has just replaced) and are **70/70 alone**; the per-commit gates on the pre-rebase tree read 7361 / 7389 / 7415.  Generation **v2-840**, three artifacts regenerated (their diff is the gen stamp and nothing else).  corpus-diff IDENTICAL over 111, SILENT-DROP 5 unchanged — three times, the third against `637fc58e` on the rebased tree.  emission-ab over lib + perl's own t/ + cpan-tests (1035 files): SAME 1030 / DIFF 5 / RCDIFF 0, every diff exactly `(p-stat X)` → `(p-scalar-ctx (p-stat X))`.  ir-host-leak 31 leaks over 111, byte-identical to the base.  **gate-SET scan over BOTH populations (638 files each) vs a fcf076f1 extraction: IDENTICAL** — mandatory because the classifier WIDENED.  Full sweep GATE clean, 0 new / 0 fixed, drops 5 = census, TOTAL passing 18647 (+0).  Baselines edited ROW BY ROW, never `--bless-fails`.
- **A `baselines/*.tsv` IS BINARY — edit it through `:raw` handles ONLY.**  `re/charset.t`'s descriptions carry raw Latin-1 bytes (the `«` `»` perl prints in them) and `op/lex.t` row 37 carries more, so the file is not valid UTF-8.  Read it through ANY decoding layer — `:encoding(UTF-8)`, `-C`, or a text tool that decodes on read and encodes on write — and each of those bytes returns as **U+FFFD**: 25 rows here were re-encoded while removing 126 others, and the rows still LOOK right (`git diff` shows them removed and re-added, byte-different, text-identical).  **The description IS the companion runner's JOIN KEY** (`docs/tap-assertion-audit.md`), so a re-encoded row silently stops pairing with the run and resurfaces as a NEW / unrefreshable row.  The four edit scripts were already `:raw`; the damage came from an out-of-band edit of the file's HEADER COMMENT.  Guard: `scratch/s470bx/check-baseline-bytes.pl <base-ref> <file>... --allow <row-key-re>` — every base line not covered by an `--allow` must still be present byte for byte, and no U+FFFD may appear that the base did not already have.
- **#1237 — a READ from a DIRECTORY handle: perl answers undef AND sets `$!`; PCL answered only the first half, and two entries of the family KILLED THE PROGRAM.**  On Linux `open F, '.'` SUCCEEDS (O_RDONLY on a directory is legal) and every read then fails.  ONE helper `%p-read-fail` fstats the descriptor **on the failure path** — no per-handle flag to keep in step with dups and reopens, nothing paid on the happy path, and NOT a re-read of the C errno (an allocation between the failing call and the read can clobber it, which s470bs ruled out).  **The entries do not agree and the difference was MEASURED, not assumed**: sysread/read/readline/slurp report **EISDIR**, `getc` reports **EBADF** (its 'handle is not open for reading' exit), and `eof` reports NOTHING and answers **1** — a read it cannot perform reads as EOF, the rule its closed-handle branch already had.  `$!` must be cleared AFTER the open to see any of this: perl's own open leaves ENOTTY behind from its isatty() probe, and a sloppy probe reads that instead.
- **`getc` and `eof` had NO handler at all** — `%p-getc-impl`'s `read-char` and `%p-eof-impl`'s `peek-char` reached SBCL's unhandled `simple-stream-error` and took the whole program with them (the base tree dies at probe row 5 and prints nothing after it).  `eof` was found only because a probe row after `getc` finally ran.  Both new handlers re-raise `p-exception` the way `%p-read-impl` does (rule 11): a perl-level die passing through is the caller's, not a read failure.
- **A shortfall row's CAUSE can name a bug the shortfall never had.**  `baselines/row-shortfall.tsv`'s `perl-tests/readline.t 6` was blessed to #1237 at the s470bs merge, but its six unproduced rows are the file's early stop at test 32 — fixing #1237 moved 23/7 → 25/5 and left the shortfall at 6.  The cause is now UNEXPLAINED with the boundary named, not a task number that would go stale.
- **The COMPANION population was BELOW its own blessed snapshot and no bucket said so**: `t/op/readline.t` reads C 23/9 on the base tree against a snapshot of 25/7, with the two `readline( DIRECTORY )` rows as unblessed NEW failures; the fix puts it back at 25/7 with ROW DIFF 0/0/0/0.  A snapshot that is AHEAD of the tree is only visible when something makes it agree again.
- **Filed**: **#1290** (`\FUNC(...)` does not spread over the returned LIST — `my @r = \stat($Curdir)` is 1 ref where perl gives 13; PRE-EXISTING, `\localtime(0)` already answered 1 ref on the base tree, and the refgen operand has been annotated SCALAR all along), **#1291** (#1044's rule (a): a `()`-prototype sub or `use constant` in a GLOB slot — `close(CPATH)`, `tell CPATH` — is INLINED by perl before the slot sees it; measure its population first).  **#1292** (a `<...>` GLOB whose interpolation yields THREE OR MORE pieces emits an N-ARY call to the BINARY `p-.` — `</etc/$h*>` dies at LOAD with `invalid number of arguments: 3` and the program never starts; two pieces work, which is why the common `<$dir/*>` spelling hid it; PRE-EXISTING at `fcf076f1`).

## s470bt (2026-09-06, Opus) — B6 the IR CONFORMANCE CORPUS (`tools/ir-conform`, 345 cases) and B7 the two target notes as tables

- **"A BACKEND for PCL's IR is done when it answers every case the way real perl does."**  `tools/ir-conform` is `tools/pcl-conform`'s sentence one level up: `ir-conform/cases/NNN-<topic>.pl` (345 programs, 23 topics) + `.expected` (perl 5.40.3's stdout + exit code) + `.ir` (the DATA form, `pl2cl --emit-sexp`) + optional `.rules`.  `ir-conform/README.md` is the backend author's page.
- **THE ORACLE IS stdout + THE EXIT CODE; stderr is never compared** — the interleaving of the two streams is buffering-dependent, so a merged compare would bless a flake.  A case that pins a diagnostic prints it to stdout itself.
- **A case is normalised by a NAMED, CLOSED projection or rejected — never blessed flaky.**  Three, each implementable in any language: `die-location`, `hex-address`, `sort-lines`.  Rule 12: a case with no `.expected` DIES.
- **Path dependence is caught, not blessed**: every candidate was run TWICE under TWO DIFFERENT FILE NAMES and its normalised output compared.  Harvest: 752 probe files under `~/pcl-agent-scratch/{s469,s470}` → 731 distinct bodies → 347 kept (self-contained, deterministic, produces stdout), 2 then dropped as harvest artifacts -> **345**.
- **Three legs**: no args = PCL's own CL target (the corpus's soundness proof); `--oracle` = perl again (its self-test); `--backend CMD` = `CMD <case>.ir` in a fresh cwd, printing the program's stdout and exiting with its code — nothing PCL-specific.
- **MEASURED**: PCL 287 pass / 0 fail / **58 known** / 0 stale (65 s at `--jobs 2`, load ~5); perl 345/345 (19 s).
- **A harvest of REVIEW PROBES is dense in exactly the places PCL is still wrong — that is what makes it worth having.**  The 58 are PCL bugs, each in `ir-conform/known-fail.tsv` with an OWNING TASK: the case still runs and prints `known`; an UNLISTED failure fails the run like a regression; a listed case that starts PASSING is reported **STALE** and fails the run, so a fix cannot land without its row leaving the file.  `--strict` counts them all — the number to watch shrink.  Rows leave BY EDIT, one at a time.
- **Filed #1240–#1249**, each with a minimal probe vs perl: #1240 every RUNTIME-raised die carries `at (eval 0) line 0.` (an explicit `die` already gets it right — `p-die` carries `:loc`); #1241 an RVALUE nested subscript does not autovivify the intermediate (#1058 fixed only the `++` spelling); #1242 `*a{ARRAY}` after `undef *a` (#1020 residue); #1243 `local($X) = LIST` assigns the COUNT / `local` in an arg list is never restored / `local ${'sym'}` never sets; #1244 loop control from a called sub dies when the loop has a `continue` block, `eval q{last}` never reaches the loop (#1022 residue); **#1245 a CRASH** — `printf "%-8s"` of a raw double from `p-eval` reaches an SBCL type-error in `sprintf-apply-width`; #1246 a failed `open($a[0],…)` does not autovivify (#1271 residue); #1247 no `$!` after a failed read on a directory handle + an uncaught die exits 1 where perl exits `$!`; #1248 `%` with Inf is NaN / `2**63` stringifies exactly / `SvREADONLY` makes `scalar(@a)` return the elements; #1249 seven singletons.  138-local → #221, 345-tie → #155.
- **The `<topic>` half of a case's name is a COARSE LABEL, not a promise** — a first-match classification over the case body, so a case that mentions `sort` while pinning something about numbers files under `sort`.  Every case in `known-fail.tsv` was read during the divergence triage and carries a CHECKED label (twenty were re-topiced, e.g. `251-regex` -> `251-numeric`, `295-sprintf` -> `295-string-eval`); the rest carry the classifier's.  Two harvest artifacts were DROPPED rather than relabelled: both glob a directory that does not exist, so their loop body never runs and their output pins nothing.
- **Gate row `Pl/t/ir-conform-01.t` (8 rows, 6-9 s) = the structural invariants over the WHOLE corpus + a 20-case SAMPLE.**  The full corpus is a WHAT-TO-RUN-WHEN entry (after a `cl/` runtime or `Pl/` emission change), not a gate row — wall time is the metric.
- **The `.ir` snapshots are NOT gated against the generation stamp** (deliberate): they would churn on every emission change and gate nothing the behaviour comparison already gates.  `--record-ir` refreshes them.
- **The corpus `.pl` files are OURS**: `ir-conform` joins `PCLLicense`'s `@ROOTS`; the `.expected` / `.ir` / `.rules` files are data with no code extension, so the scan never reaches them.
- **B7: `docs/js-target-plan.md` Part II §II.0–§II.7 is now TABLES over the generated inventory** (684 → 580 lines).  The four worked probe programs are gone because everything they taught is derivable: §II.2 = one row per B1 family (54 families / 698 names), §II.1 = the §B.3 FACTS table with a JS column and a ✓/✗ per fact, §II.3 = the §11b kernel groups, §II.7 = the five reader rules.  §II.8 untouched (settled design); §II.9's acceptance re-pointed at the corpus, sliced by each milestone's manifest `NEEDS`.  History: `git show 981480f4~1:docs/js-target-plan.md`.
- **`docs/c-target-notes.md` (new) = the same tables with a C column.**  Findings: **GC is the one decision the IR cannot make for you** — refcounting recommended (identical `DESTROY` timing, and pclxs's ABI expects it), with the class facts keeping most values out of refcounted cells; **`setjmp` only where `p-sub`'s `:needs` says so** (it defeats register allocation across a call — the biggest C-side cost avoided); **closure conversion is a direct read of the capture manifest**; **PCRE2+JIT compiled once at load from the structured literal**, where `:tier` is the REFUSAL boundary rather than a routing decision.  C gets `tagbody`/`go` = `goto` and `int64_t` ≡ perl's IV for free (so the range proof is a SPEED lever there, not a correctness one) and owes strings and the unwind path, which the two hosted targets get from the host.
- **Bars**: gate green apart from the 13 standing pclxs xs rows (214 files / 7383 rows); `tools/corpus-diff.pl cd34df72` emission IDENTICAL across 111 files, silent drops 5 unchanged.  No emission change, no generation bump, no sweep, no companion.

## s471b (2026-09-06, Opus) — #1301 SHIPPED: the three root scripts move into `tools/`; and the SCOPE of a rename's reference sweep is "living documents only"

- **`runt`, `clt`, `sweep-perl-tests.pl` are now `tools/runt`, `tools/clt`, `tools/sweep-perl-tests.pl`; `run-perl-test.pl` is DELETED** (dead, zero code references).  The repo root keeps exactly `pcl`, `pl2cl`, `runpcl`, `xs-pin`.  The old spellings do not work: `./runt`, `./clt` and `perl sweep-perl-tests.pl` are gone, and `.claude/settings.json`'s two permission strings moved with them.
- **Each moved script derives the root in its OWN idiom, deliberately NOT through a shared helper** — `PCLPaths::root()` is #1302's, and a second root resolver added here is the thing #1302 exists to remove.  `runt`/`clt`: `dirname(abs_path(dirname($0)))`.  The sweep: `use lib "$FindBin::RealBin/lib"` + `$project_root = dirname($FindBin::RealBin)`.  The moved sweep differs from the old one in exactly four lines.
- **Everything the sweep writes still lands at the PROJECT root, and that is MEASURED**: `$log_dir = $ENV{PCL_TEST_LOG_DIR} || "$project_root/.faillog"`, a relative `PCL_TEST_LOG_DIR` absolutised against `$project_root` — after a one-file run `.faillog/` is at the repo root, there is no `tools/.faillog`, and it is still the repo root with `cwd=/tmp`.
- **The RUNNERS row's byte-identical bar was met on THREE legs, zero differences**: with the cached core, with `PCL_NO_CORE=1` (no core at all), and against the control of the OLD runner run twice.  **Only root-derived or run-derived fields were normalised**: the tree root; the core file's PATHKEY, which IS `sha1(abs runtime path)[0,8]` (`PCLSbcl::cached_core`) while its SOURCE hash matched on both sides; and `File::Temp`'s per-run directory, which the control shows the same runner varying from itself.
- **`tools/runt` vs `./runt`: stdout BYTE-IDENTICAL RAW (3348 B).**  `/tmp/<name>.lisp` is byte-identical after the five root strings the preamble embeds by design (#217); `/tmp/<name>.out` after stripping SBCL's own `{HEXADDR}` object addresses — **which the SAME runner on the SAME tree varies from itself run to run** (measured, 6 lines; identical after stripping).  An SBCL object address is never evidence about a runner.
- **THE SCOPE OF A RENAME'S REFERENCE SWEEP IS "LIVING DOCUMENTS ONLY"** (ruled Fable s471b, standing).  `CLAUDE.md`, `README.md`, `docs/STATUS.md` and the how-to docs change spelling; **dated past-session plan docs (`docs/plan-*-sNNN.md`, the `*-plan.md` family), `docs/history/`, `docs/session-log.md`, `docs/DECIDED.md` and the `baselines/*.tsv` header notes KEEP the old spellings** — they record what was run at the time, and re-spelling them falsifies a measurement's provenance.  Same reason keeps CLAUDE.md's compressed-history bullet 2i unchanged while its four live paths move.  Counts measured against `d65f4a7c`: 16 new spellings in 11 code/config files; 54 in 13 live docs, plus 14 `run-perl-test.pl` mentions replaced.
- **A doc that names a DELETED script gets its CLAIMS corrected, not just its path.**  `docs/debugging-hangs-crashes.md` was written around `run-perl-test.pl`, and `tools/runt` differs exactly where it made claims: runt redirects SBCL output into `/tmp/<name>.out` rather than a backtick, MERGES stderr into it instead of swallowing it, and already HAS the per-test timeout the doc's own wishlist asked for (`RUNT_TIMEOUT`, 300 s default, `0` disables).  Same for `docs/test-infrastructure.md` and the three `docs/bug-finding-strategy.md` recipes.
- **TRAP: write-to-`<path>.new`-then-rename DROPS THE EXECUTE BIT** (created at `0666 & ~umask`).  A mechanical rewrite turned `pl2cl`, `tools/rebuild-pack`, `tools/run-perl-suite.pl` and `tools/sweep-diff.pl` non-executable; `git diff` showed no content difference for it and the gate's first row said `sh: 1: ./pl2cl: Permission denied`.  **`git diff --summary <base> HEAD | grep mode` is the cheap check after any scripted file rewrite.**
- Bars: gate 213 files / 7347 tests, only the 13 standing pclxs xs rows failing; `corpus-diff.pl d65f4a7c` emission IDENTICAL across 111 files (silent drops 5, unchanged); full sweep `--jobs 4` GATE clean, 0 new / 0 fixed, drops 5 = census, shortfall 12274 (+0), **TOTAL passing 18647 (+0)**; the OLD runner run once afterwards on a `git archive d65f4a7c` extraction gives a `.faillog/_status.tsv` **byte-identical to the moved runner's** after normalising the root (4233 B, 108 files, no column differs) — no contention flake, no serial re-run owed.  `tag-license --check`, `rebuild-pack --help`, `dup-census.pl` all green.  NO generation bump (nothing under `Pl/`/`cl/` changed but comments).

## s471a (2026-09-06, Opus) — #1273 the negative subscript on the store path (a 59-row regression), the rule-12 fatal that came with it, and #1271's autovivified handle

- **A COPIED IDIOM WITH THREE MISSING COPIES is what killed `t/run/fresh_perl.t`** (#1273): the negative-subscript rebase `(if (< i 0) (+ len i) i)` lived by hand in SEVEN array accessors and was ABSENT from `p-autoviv-aref-for-hash`, `p-autoviv-aref-for-array` and `p-array-set`, which handed the raw index to `AREF`.  Latent until #1057 routed a compound assign's CONTAINER through the chain walker; then `$prgs[-1][0] .= $_` died at LOAD with `Invalid index -1 for (vector t 1)` and the file produced **no TAP at all** — 59 passing rows gone, invisible to every bucket but the shortfall instrument.
- **ONE `%p-array-index` (inlined, beside `to-number`) is now the only resolution of an array subscript against its container, and all TEN accessors call it** — read, write, eager lvalue, `@_` alias, `delete`, `exists`, alias-slot and the two autoviv walkers.  The read and write halves cannot disagree about what -1 means, and the next accessor inherits the rule instead of re-deriving it.
- **A subscript before the START of the array is perl's FATAL on every LVALUE use, and PCL used to drop the write** (rule 12, the #138 silent-wrong one level down): `%p-non-creatable-index` fires from `(setf p-aref)`, `p-aref-box`, `p-array-set` and the two walkers.  **Which sites die is MEASURED, not guessed** — 19 one-file probes vs perl 5.40.3: assignment / `.=` / `+=` / `++` / `||=` / `$a[-4]{k}=1` / `\$a[-4]` / `chop` / `$r->[-4]=9` / a foreach alias / a sub-argument alias / a write to an EMPTY array all die; a READ, a SLICE read, `exists` and `delete` are quiet, so those three accessors take the helper and NOT the fatal.  The death is trappable (`eval { $a[-4] = 9 }` leaves `$@` matching `/^Modification of non-creatable/`).
- **run/fresh_perl.t 0/0 DIFF → 60/31 XDIFF, SHORTFALL 91 → 0** — ONE row BETTER than the `3016011e` bless of 59/32, because the file's own `sub NewShell` (`my($m2) = $#Shells++; $Shells[$m2]{HOST} = $Host` = `$Shells[-1]{HOST}`) is the same mechanism; its `perl-suite-expected-rows.tsv` registration was therefore stale and left.
- **A FIX CAN MAKE ITS OWN REGISTRATION STALE IN THE SAME RUN**: `perl-tests/array.t` test 82 (`eval '$a[-1] = 0'`) printed `# REGISTRY-STALE` in the very run that fixed it.  Dropped — and **its REASON was wrong as well as stale**: `$a[-1] = 0` on an empty array is VALID Perl that dies at RUN time, so principle 9 (do not detect invalid INPUT) never applied.  `docs/not-supported.md`'s claim to cover it is corrected.  Dropping a stale entry is COUNT-NEUTRAL by construction — the registry only relabels a FAILING row.
- **#1271: perl AUTOVIVIFIES the lexical BEFORE it attempts the open**, so a failed `open(my $fh, …)` still leaves a fresh CLOSED glob (`defined` 1, `ref` GLOB, TRUE, a copy is the same handle).  PCL wrote the variable only on success, so `close $fh if defined $fh` and `if ($fh)` took the opposite branch, silently.
- **MEASURING THE REPRESENTATION FIRST turned that into five lines**: `Symbol::gensym` + a failed open ALREADY matched perl in PCL, so only the vivification was missing.  `%p-autoviv-failed-handle` builds the glob ref through `p-backslash`'s typeglob arm — the ONE construction that sets `is-ref`, which is what makes `ref($fh)` answer GLOB rather than `""`.  **Only an UNDEF box vivifies**: a string box is perl's symbolic-filehandle spelling and keeps its value, a glob box is already the handle, a bareword is not a box (all three probed).
- **`io/perlio_fail.t` reads 11/4 on THIS tree and on an `ed302e13` extraction against a blessed 6/9** — a STALE snapshot row, not a mover; spliced with that measurement as its cause and five rows out of `baselines/perl-suite-fails.tsv` by EDIT (the four `Got warning` rows stay: no warnings-gated diagnostic, #221).  **`io/pvbm.t` was NOT touched** — 20/8 blessed, 23/5 base, 22/6 here is the documented flap, and re-blessing a flapper only moves which run looks wrong.
- **Filed**: #1305 (one array.t skip pattern covers two rows, one now passing — stale on BOTH trees; split it, and re-probe the residue its own text calls provisional), #1306 (`p-aref-argbox`, the LAZY `@_` accessor, still drops the write where perl dies — it also serves SLICE reads, where perl is quiet, so one accessor is asked two questions), #1307 (`close` on a never-opened handle: perl's DEFINED `""` vs PCL's undef, the #403/#416 family, and the only closed-handle answer still divergent), #1308 (a SUCCESSFUL `open my $fh` leaves `ref($fh)` = `""` — PCL now gives a FAILED open a more perl-shaped handle than a successful one; do not fix without measuring, it moves every lexical read onto a name lookup), #1309 (`open($a[0], …)` SUCCEEDS and `<$a[0]>` reads nothing — silent wrong on the success path, and the hash-element spelling works).

## s470bw (2026-09-06, Opus) — #1261: a cached module transpile is only valid while its DEPENDENCIES still hash as read, and which entries get a fasl is TWO DIRECTORY LISTS

- **THE HOLE WAS REAL AND IT WAS BOTH LAYERS, reproduced before anything was written.**  A module A's *parse* consumes facts about the modules it `use`s — a `(&@)` makes a trailing block a code ref, an empty prototype makes a bareword a TERM, a bareword is a CALL only for a known exported sub — so A's cached CL encodes B's prototypes and exports, while the key was A's path + the generation and the check was A's own mtime.  Editing B only (A's mtime untouched), on the `.lisp` cache AND on #1188's fasl: perl `HASH:1 ZAP|1` / PCL `CODE:1 ZAP|1` for a dropped prototype — **silently stale** — and perl `CODE:1 zap|1` / PCL **CRASH** `Undefined subroutine &A2::zap called` for a dropped export.  perl re-parses A on every run; that is the oracle.
- **`Pl::ProtoCache` is NOT a second hole of the same shape — measured, not assumed.**  Its entries already record every dependency the walk resolved and re-resolve + re-stat each one, so transpiling A twice across a B edit gives one `lambda` then none.  What it lacked was a CONSUMER on the runtime side.
- **ONE dependency walk (rule 11): ProtoCache's frames, bracketed one level higher.**  `pl2cl` wraps a whole transpile in one OUTERMOST `begin_walk`/`end_walk`, so the frames that already report each nested prototype walk now report to a frame standing for THIS source; `end_walk` is the transitive list.  `pl2cl --deps FILE` writes it beside the cached emission; `pl2cl --manifest` publishes the same list as `depends`.  One walk, two consumers.
- **The hash is CONTENT (MD5), never an mtime** — a `git checkout` restores an old mtime, and a stale dependent is the silent-wrong class this project refuses.  Digest::MD5 is core on the Perl side, `sb-md5` a contrib on the Lisp side, so both ends compute the same string.  Cost is below the noise floor: `use JSON::PP` (9 dependencies) warm reads 0.999 s with the check against 1.118 s without it on the base tree.
- **`p-cache-valid-p` stays THE ONE validity predicate** (both the `.lisp` and the fasl go through it) and **loses its 7-day age clause**: the age was a stand-in for the staleness the manifest now states directly, and it made "an untouched module is a HIT" untestable.  `p-cleanup-old-cache` still prunes by write date — disk hygiene, not validity.  A missing or unparseable manifest ⇒ INVALID, never trusted (rule 12); that is also what every pre-#1261 entry looks like.  Transitivity is free: B is validated when B loads.
- **WHAT IT DOES NOT COVER, said out loud in `docs/ir-spec.md` §9.2b rather than assumed**: a dependency that MOVES (a new `-I` shadowing a shim) is not detected, because the runtime cannot re-run the transpiler's `_find_module_file` with the transpiler's search path.  The transpiler's own layer does re-resolve; the remedy here is `pcl --clear-cache`.
- **Which modules are compiled to native code is TWO DIRECTORY LISTS in PERL5LIB syntax (Fable ruling, s471), not a policy word**: `PCL_COMPILE_DIRS` (`*` = all; UNSET = perl's installed library dirs + PCL's own `lib/`) and `PCL_NO_COMPILE_DIRS`, which WINS on any match (`*` = compile nothing); `PCL_NO_FASL_CACHE=1` is kept as the alias of the latter's `*`.  Entries are realpath'd at CALL time and memoised on the raw environment string, so a saved core cannot bake in the core builder's answer.  A module that is not compiled still gets the `.lisp` cache and the manifest check, and loads from the cached text — correctness is the manifest, this is a belt.
- **`*p-core-inc-dirs*` is NOT the default's source, and that is a MEASUREMENT**: it is pl2cl's own `@INC` as the preamble found it, so `PERL5LIB=/tmp ./pl2cl prog.pl` emits `/tmp` into it — exactly the directories a developer edits in.  The installed set comes from %Config's six `*libexp` keys plus PCL's `lib/`, in ONE classifier (`Pl::ProtoCache::trust_class`), whose verdict reaches the runtime in the manifest's `source` line.
- **Module cache entries moved to `<cache>/modules/`**, beside `<cache>/core/` and `<cache>/proto/` — free at this moment and never again, because every pre-manifest entry was invalid anyway.
- **A GUARD CAN BE WEAKENED BY A DEFAULT, not only by an edit**: #1188's five ordering cases run each fixture through "the fasl build pass" and "the fasl hit", and those fixtures live in a tempdir — under the new default both passes would have taken the TEXT path and the rows would still have passed while testing nothing.  Every fasl-path row now passes `PCL_COMPILE_DIRS='*'` explicitly, and the default is asserted by its own rows.
- Bars: gate **213 files / 7359 rows** (only the 13 pclxs xs rows; +26 guard rows over main's 7333); corpus-diff `9ee95f2` IDENTICAL over 111 with silent drops 5; emission-ab `--shapes` over `lib/` 28 SAME / 0 DIFF / 0 RCDIFF; `ir-host-leak` 31 symbols / 111 files (the standing number); full sweep `--jobs 4` GATE clean, **TOTAL passing 18646 (+0)**, drops 5 = census, shortfall +0 — and its 4 UNSTABLE + 15 unverified rows reproduce BYTE-FOR-BYTE on a `git archive 9ee95f2` extraction, so they are pre-existing crash-file noise *measured*, not asserted; companion `--jobs 1` over `op/require.t op/do.t op/inccode.t op/incfilter.t op/require_errors.t` identical file-for-file to the same run on that extraction, ROW DIFF 0/0/0/0.  Guards `Pl/t/module-fasl-cache-01.t` 22 → 42 and `Pl/t/manifest-01.t` 47 → 53, inverse-verified on the extraction (12 rows fail there).
- **#1286, FOUND BY READING THE BOARD PAIR AND FIXED: a `List::Util` block/callback is called with NO ARGUMENTS.**  perl sets `$_` and passes nothing (probed: `scalar(@_)` is 0 for first/any/all/none/notall); the shim called `$code->($_)`.  Invisible in the `{ BLOCK }` form, fatal in the `\&NAME` form — `Scalar-List-Utils t/first.t`'s `sub rec { my $n = shift; … $v = first \&rec, 1,2 }` saw the ELEMENT where perl gives undef and recursed forever, ending in a binding-stack exhaustion at **12 of 24 rows**.  Fixed: **23 of 24** (the residue wants perl's `Can't undef active subroutine`).  **The bisection is the lesson**: #1261's runtime hunks flipped that file from "SBCL prints the backtrace and exits" to "SBCL re-exhausts while printing and LIVELOCKS", the flip follows the compile-dirs hunks *even in a probe that never calls the new code*, and 100 inert defuns of the same size do not reproduce it — the trigger was an image knife-edge in the disabled-debugger backtrace print, the CAUSE was the infinite recursion, and fixing the recursion removes both.
- Filed: **#1284** (`pcl -I DIR` does not reach a MODULE's own transpile — the runtime spawns `perl pl2cl --module` with no `-I`, so a module's own `use` does not resolve at transpile time and its dependency's parse facts are silently lost; PERL5LIB works only because the child inherits the environment), **#1285** (a DECLARED sub with NO prototype followed by a block: perl builds an anonymous HASH, PCL passes the block's VALUE — the other half of #478).
- **THE MERGE BARS, RE-TAKEN ON THE REBASE ONTO MAIN `d65f4a7c` (2026-09-06), because a `lib/` commit owes a sweep and a board and neither had been run since it landed.**  Rebase conflicts were the two record files ONLY (both resolved keeping both sides); `cl/pcl-runtime.lisp` and `docs/ir-spec.md` auto-merged, parens balanced, generation stays main's `v2-830`.  Gate **213 files / 7374 rows** = main's 7347 + this batch's 27 guard rows (20 + 6 + 1), only the 13 pclxs xs rows failing; `corpus-diff d65f4a7c` IDENTICAL over 111, silent drops 5; full sweep `--jobs 4` **GATE clean, TOTAL passing 18647 = baseline (+0)**, 0 new / 0 fixed / 0 LOST, drops 5 = census, shortfall +0; companion `--jobs 1` over the six reachable files (`op/aassign.t`, `op/lc.t` — the only companion-population files that `use List::Util` — plus `op/do.t op/inccode.t op/incfilter.t op/require_errors.t`) **identical to the `perl-suite-run.tsv` snapshot file for file**, ROW DIFF 0/0/0/0, 0 drops.  `op/require.t` DOES NOT EXIST in perl's t/ (the earlier leg's name was a slip); the real module-load file of that name is `comp/require.t`, quick-capped and blessed as multi-step volatile, so it yields no comparable verdict and was not run.
- **THE CPAN BOARD COLD AND WARM ARE BYTE-IDENTICAL (0 differing rows).**  That is the pair the brief asked to read with care, and it says the module cache — the `.lisp` layer, the fasl layer and the new manifest check together — introduces NO divergence between a cold and a warm tree.  Cold = `rm -rf ~/.pcl-cache/modules ~/.pcl-cache/proto`, NEVER `pcl --clear-cache` (that deletes the cached CORES, including a sibling agent's, mid-gate).
- **FOUR board rows differ from blessed, and EVERY ONE WAS BISECTED TO A COMMIT BEFORE IT WAS EDITED IN** ("pre-existing" is WHEN, not WHY).  `Scalar-List-Utils first.t` 12/1 → **23/1** = this batch's #1286.  `Try-Tiny when.t` (FAIL 0/0 rc 2 → **PASS 1/0**) and `given_when.t` (rc 2 → **rc 0**) = `0cbe7831` (s466bc, **#1037**): **rc 2 is `run-dist-t.pl`'s TRANSPILE-FAIL exit**, so a whole-file `given/when` refusal meant NO TAP at all; a statement-level refusal lets both files transpile (TRANSPILE-FAIL at `0e0b0b9c`, TRANSPILE-OK at `0cbe7831`).  `Role-Tiny extend.t` 2/2 → **4/0** = `0d1b5c18` (s470bk, **#1058**, the nested-element autovivification) — the file's whole subject is `$apply_steps{$to}{$role}{before}++` (2/2 at `37d78687`, 4/0 at `0d1b5c18`).  A board bisect is CHEAP when the verdict is a transpile: `git archive` + `pl2cl` is ~1 s a step, and only the run-level row needed the ~8 s SBCL probe.
- **`baselines/cpan-board14-s467.tsv` gained a HEADER**: the four rows were edited by hand and the file now carries a comment block naming each row's cause.  Nothing reads it programmatically (checked) and the data rows stay diffable against a fresh `--tsv`.
...
- **The array-assignment BULK FILL (#1181, `cc83209`, RUNTIME ONLY — no PCL_OPT name, there is no emission to switch)**: `@x = LIST` is ONE block copy when every element would be stored AS ITSELF, and the licence is **the existing write rule read backwards** — `%p-storable-raw` is asked, never re-derived.  **listcopy -52..-57 % (0.94× → 0.42× of perl)**, sortnum -6..-11 %, sortstr -3..-7 %.  A BOX must be copied into a FRESH box (perl copies values); a nested vector / hash / flatten marker declines, which is also what makes skipping the aliasing snapshot safe.
- **`numeric-slot` (Kind-A, #1183, `3f24edc`) — and #1153's RULED PREMISE IS FALSE.**  A raw slot CAN hold a blessed box, by two probed routes: a raw compound write with a NON-literal delta (`$s += $obj` leaves the object IN the slot, so a later `$s *= 3` has a literal delta and a blessed CUR) and a B-regime write, where `%pcl-to-number-strict` DECLINES the freeze and stores `(p-box-init V)`.  So the licence is about the SLOT: VarAnnotator's `numonly` = every write stores a compile-time NUMBER.  **NOT "num-FAMILY"** — `$x = $a + $obj` is num-family and yields an object.  **-59.2 %** of #1153's 4e6 `*=`/`%=` loop (acceptance was ~0.015 s of loop; reads 0.0135).
- **What the compound-assign overload guard costs is the BRANCH, not its tests** — measured: each HALF alone costs as much as the whole (0.028 / 0.030 s against 0.029 s).  With the branch gone SBCL compiles the body as straight-line arithmetic.  Do not re-litigate the guard's SHAPE (#972's five-candidate table stands); the only way to recover the cost is to remove the branch, which needs the licence.
- **`foreach-arrays` (Kind-A, #1184, `00b26f8`)**: `for my $x (@a, @b)` over BARE named arrays iterates each in turn with no flattened temporary — **feread2 -78.3 %** (ceiling -88.3 %).  No new fact: `p-foreach-raw`'s own two conjuncts plus the list SHAPE.  **The shape occurs in ZERO of the 1035 files of the four populations** (four spellings exist; all decline, one because it writes the loop variable).  `tools/bench-exec.pl` gains `feread3` as the array-count scaling control.
- **`feread2` 1.32x -> 0.28x and `feread3` 0.30x, while the SINGLE-array `feread` stays 0.47x**: with the run in, a two-array loop is FASTER than a one-array loop over the same 1000 elements, because the run indexes each source's element-storage simple-vector with `svref` while `p-foreach-raw` over one live array goes through `%p-foreach-elt-raw` on the array OBJECT.  The cheapest unclaimed lever this round found (ease S, the expander has both shapes); NOT taken because the single-array path's live-array semantics differ — a `push` during it DOES extend the iteration, which the run deliberately does not.  Plan §A.4.3.
- **THE YARDSTICK'S HEADLINE (#1188, the biggest number measured this round): `use JSON::PP` costs 6.44 s WARM and 13.42 s cold, `use Moo` 3.50 s** (perl 0.0098 / 0.0081), because `*pcl-cache-fasl*` is NIL and the module cache stores `.lisp` TEXT that SBCL recompiles every run — a session-251 correctness workaround for the module double-execution bug.  It is larger than everything else in the ranked table put together, and A.0's metric is PCL's ABSOLUTE seconds.  Ease L (the double execution must go first, `docs/module-double-exec-bug.md` options C/D).  Startup 0.169 s, string eval 0.292 s.
- **More than a THIRD of both text-heavy macro rows is inside ONE cl-ppcre scan** (#1187): `search` 14.5 / 11.5 %, `vector-hairy-data-vector-ref` 11.6 / 12.5 %, the scanner closure 8.7 / 8.2 %.  The graph proves `search`'s caller is cl-ppcre's own `create-scanner-aux` closure, **not `p-index`**.  `do-regex-match` already coerces the SUBJECT (#680), so a generic `search` inside that scan means some OTHER operand is not simple — measure that before committing to the PCRE2 backend (#71).
- **The ranked table is `docs/plan-speed-and-ir-s470.md` §A.4** — the deliverable the next rounds read.  Macro-row ratios: json-rt 3.4–3.6×, textproc 4.5–5.6×, moo-objs 36–55×.  **None of the four round-27 levers appears in it**: each removed the whole of a microbench row's cost, and none of those shapes is where a real program spends its time.
- **A LOADED BOX CANNOT RESOLVE A 10 % CHANGE, and `tools/bench-exec.pl` cannot say so on its own**: `perl`'s own column moved 40 % between consecutive runs of the same tree, and the macro rows' intercept/slope decomposition came out NEGATIVE for json-rt because both engines grew superlinearly.  The usable instrument is `ab-lisp.pl` (two `.lisp` files, ONE core, interleaved, best-of-K, a byte-identical CONTROL PAIR timed in the same window and printed beside the result); the usable quantity across runs is the RATIO, because both engines see the same load.
- **A program with a `use` cannot be `compile-file`d before profiling**: the `p-use` sits in an `eval-when` (principle 3) and runs BEFORE the plain load-time `(setf @INC …)`, so it dies "@INC contains:" empty.  Profile by LOADING the source and name the compiler's share, or pre-`p-use` the modules into the driver — which also needs the program's `*pcl-pl2cl-path*` line, because Moo's own load does a string eval (#1189).
- Filed from building the macro rows: **#1185** (JSON::PP under PCL encodes an integer as a STRING — `{"a":"1"}` vs perl `{"a":1}`; the IV/PV distinction, not a coercion bug) and **#1186** (`Text::Wrap::wrap` dies "This shouldn't happen" at Wrap.pm:78 — a regex/`pos` divergence with a small reproducer waiting inside Wrap.pm's own three matches).  **#1182** records what L2 did not take, with its number: true storage adoption is a further -30 % of what is left.
- Also probed and PRE-EXISTING (identical with the transform switched off), not fixed: `$#{"main::ga"}` answers the NAME's length (7) where perl answers 2 — `p-array-last-index` has no symbolic-string arm and a string IS a vector; `local ${'main::g'} = 9` reads back undef where perl reads 9.
...
- **#1022 half (a) STANDS as the trappable die (the s329 announce boundary does not cover an untaken loop exit); half (b) SHIPPED under #1162: a loop is framed only when its body directly names a same-unit sub in the MAY-DYN-EXIT fixpoint set — one `catch` per loop ENTRY, one frame in the whole perl-tests corpus, all bench rows byte-identical; an exit through an indirect call, a string eval, or across compilation units takes the perl-shaped die (residue).**  The wide "any user call" licence blew SBCL\x27s compile heap on t/op/loopctl.t (88 catches ≈ 4.8 MB IR each in one 64 KB form).
- **RT #130727 = an ACCEPTED DIVERGENCE** (`@{local $x[0][0]} = 1` on an undef intermediate: perl dies without vivifying, PCL vivifies like the non-local spelling); multideref.t rows 47/49 were ACCIDENTAL passes before #1058 — skip-registry + not-supported `local` section.  #1190 filed beside it.
- **`use open` is LEXICAL (#1222)**: a module\x27s `use open` must never reach its caller\x27s opens — the emitter carries the in-scope defaults to each open site; only `:std` is a runtime effect.  `no open` is a NO-OP in perl itself (open.pm has no unimport).  Found by Fable\x27s probe after #1115 made the default BYTES.
- **`DATA` is PER-PACKAGE**: an op that installs the handle takes the quoted handle name from the emitter (`(p-install-data-handle \x27DATA …)`); one shared key made every module\x27s POD replace the program\x27s `__END__` table (sprintf.t 532 → 0 — the first sweep after BO caught it).
- **A5\x27s licence corrected by measurement**: `values`, `grep`, a nested `sort` and a `do { }` tail hand ALIASES through; `map`, `keys`, `split`, sub calls, `eval { }` and sub tails copy; five sort modes, not four; the Kind-B pass RATIFIED.  #1140 (array facts) owns foreach-raw\x27s write-to-the-array hole.
- **#1153 RULED emitter-side** (a raw twin with a literal delta needs no overload guard — shipped by BN as `numeric-slot`, per-SLOT because a raw slot CAN hold a blessed box, probed).
- **The merge-time sweep is NON-NEGOTIABLE for `cl/` work**: today it caught #1118 (eval.t −36, BG), the BO `DATA` key (sprintf.t −532), and BR\x27s byte-default fallout (substr.t hang, magic/tr/readline −89) — three regressions the agents\x27 own legs did not see.
...
## s470bv (2026-09-06, Opus) — #1224: a PerlIO layer name's leading colon is OPTIONAL; and the companion snapshot brought honest, 51 rows with a cause each

- **#1224 — `binmode FH, "utf8"` is `binmode FH, ":utf8"`, and there is ONE reader of a layer string.**  perl's `PerlIO_parse_layers` treats `:` and whitespace as SEPARATORS, an empty name as skippable, and the leading colon as OPTIONAL; an UNKNOWN name makes the whole call FAIL (`undef`, `$! = ENOENT`) with NOTHING applied.  `%p-split-layers` was rewritten to that grammar and every consumer routed through it (`%p-split-open-mode` now splits on the SIGIL run and returns the first unknown layer; `%p-binmode-impl` and `%p-open-impl` fail as perl does; `p-use-open`'s word classifier dropped its colon test).  **t/op/read.t 1852/264 DIFF -> 2116/0 OK.**  Guard `Pl/t/io-layers-01.t` 27 -> 36 rows, 7 of the 9 new ones failing on a `424cabc` extraction.  ir-spec §7.7.
- **The SAME grammar has a second reader, in the COMPILER, and it was not fixed here: `Pl::Parser::_use_open_layers` keeps only words matching `/^:/`, so `use open IO => "utf8"` is silently discarded (#1270).**  It is a `Pl/` change and owes a generation bump + the three artifacts + corpus-diff + emission-ab, which do not belong in a runtime-only commit — and the spelling occurs in NONE of the four populations, so it is guard-row territory.
- **`utf8::is_utf8` answering 1 on a byte-read string is ALREADY BLESSED** (`docs/not-supported.md`, "The per-scalar UTF-8 flag") — verified before filing anything, so no task.
- **FIFTY-ONE companion snapshot rows spliced by hand, each with a MEASURED cause** (`baselines/perl-suite-run.tsv`; every one re-run ALONE by the #366 serial pass, all "REAL MOVE, both runs agree").  The batch's own change moves exactly ONE file and the snapshot already said its number, so none of the 51 is this batch's.  Sixteen moved inside the batches merged the same day — #1115 (comp/utf.t, op/print.t, op/warn.t, re/bigfuzzy_not_utf8.t, re/charset.t, run/switchC.t's −3) and #1221 (op/chop.t +48, op/index.t +10, op/postfixderef.t, op/utf8magic.t, op/utftaint.t, opbasic/cmp.t +1202, re/reg_60508.t, uni/parser.t, uni/method.t, run/switchC.t's +1) — each PINNED by running the file at `a3d6d8e` and at `b25afbc`, not inferred.  **op/ref.t is TWO steps** (199/43 → 201/41 at b25afbc, → 202/40 only at 424cabc: the last row is s470bs's #1048, "IO slot of the temporary glob is set correctly").
- **THE TWO LOSSES WERE BISECTED, and they are different animals.**  `op/multideref.t` 43/9 → 41/11 bisects to `0d1b5c18` (#1058) and its two rows are `RT #130727 array not autovivified` (+ part 2) — an ACCEPTED DIVERGENCE already ruled in DECIDED §s470, so they were ACCIDENTAL passes and no task is owed.  `run/fresh_perl.t` 59/32 → **0/0** bisects to `9d43ccf2` (#1057, "a compound assign to a nested element vivifies its container") and the file now dies at LOAD with `Invalid index -N for (vector t N)`: a 59-row COVERAGE LOSS and a real regression — **#1273**, with the negative-subscript probes to write first.
- **A STALE REGISTRATION HAS TWO SHAPES and they need different repairs.**  The runner says "expected-divergence row now PASSES — remove it" when the file MATCHES perl (registration DROPPED: op/print.t, mro/basic_04_c3 ±utf8, mro/vulcan_c3 ±utf8 — five rows out of `perl-suite-expected.tsv`, five orphan rows out of `perl-suite-expected-rows.tsv`), and "N registered row(s) no longer diverge — re-bless with --bless-rows" when only SOME rows stopped diverging (registration KEPT, ROWS re-blessed: the 21 other mro files, **41 rows removed, every one read first**; all are `got the right MRO` / `can(method) resolved itself` / `method dispatch` rows, i.e. exactly what the reason covers, and the `mro::` API and `next::method` rows still fail).  Dropping the registration in the second case would have moved explained rows into UNEXPLAINED — strictly worse and against the all-or-nothing bar.
- **THE mro FAMILY IS NOT THE mro PROVIDER'S DOING.**  `git log --oneline -- lib/mro.pm cl/pcl-mro.lisp` shows `lib/mro.pm` unchanged since s401c (a licence tag) and every `cl/pcl-mro.lisp` commit an artifact regeneration at a generation bump.  The 25 files moved because generic method resolution got better over many sessions.  Attributed by git log, NOT re-run — and recorded as such.
- **A RAW `.suitelog/*.fails.tsv` IS NOT THE ROW BASELINE'S ALPHABET.**  perl interpolates its own `$0` into unnamed-test descriptions, and the runner invokes perl with an ABSOLUTE path, so the raw log says `[at /home/.../t/op/magic.t line 213]` where the baseline says `[at t/op/magic.t line 213]`.  The difference is the KEY PROJECTION (`PclTapAlign::rowkey_desc` strips `$tdir`), not drift — a raw diff of the two "finds" hundreds of phantom moves.  Project before comparing; the s470bv splice script calls the runner's own projection.
- **`--bless-fails` / `--bless-shortfall` regenerate their file's header** and would have dropped every accumulated provenance note (the s470bs trap, now hit twice).  Both files were edited row by row instead: `perl-suite-fails.tsv` 24 files re-measured, 1962 rows replaced by 1353, **11 NEW rows each read and explained before being written** (run/switchC.t's three `-C` rows → #1272, op/multideref.t's two → the RT #130727 ruling, uni/method.t's two → #1274, op/magic.t's one → #1275, io/perlio_open.t's one → newly PRODUCED, and two `*summary*` rows that follow their file's count); `row-shortfall.tsv` 27 rows (20 deleted, 5 lowered, 2 added — one of them `t/run/fresh_perl.t 91`, the whole file).
- **A COUNT THAT DOES NOT MOVE CAN STILL HIDE A ROW SWAP.**  `op/magic.t` reads 176/31 at 614c6af, a3d6d8e, b25afbc and HEAD, but at `b25afbc` (#1221) one row started passing (`ENV store encodes high utf8 in SV`) and another started failing (`.. and the UTF-8 version is written`): `$0 = <wide string>` must store the UTF-8 OCTETS, and PCL keeps the wide string — **#1275**.  Only the ROW baseline can see this; the snapshot cannot.
- **Bars**: rebase onto main `9ee95f2` clean, and a FINAL rebase onto `f3abf773` (main moved by two DOCS-ONLY commits) with the gate re-run there IDENTICAL; gate **213 files / 7342 rows**, only the 13 pclxs xs rows; FULL perl-tests sweep `--jobs 4` **TOTAL 18646 = baseline (+0), GATE clean, 0 new / 0 fixed / 0 LOST, drops 5 = census, shortfall +0**; `tools/ir-host-leak.pl` **31 leaks here and 31 on a `9ee95f2` extraction**; the 63-file companion identical to the pre-rebase measurement for 62 of 63 (the one mover is op/eval.t 130/38 → 131/37 = s470bq's own #1219, already on the snapshot).  No generation bump — the change is `cl/` only.
- **Filed**: #1270 (compile-time `use open` colon), #1271 (a FAILED `open(my $fh,…)` leaves the lexical undef; perl autovivifies the glob), #1272 (the `-C` switch is implemented nowhere — 12 rows), #1273 (fresh_perl.t's 59-row loss), #1274 (a method call on a non-ASCII package stringifies `HASH(0xb)` — the #418 family), #1275 (`$0` and wide characters), #1276 (the attribution debt: 33 gains spliced without a bisect, listed).
...
- **Round 3 (2026-09-06): the residual +2 of the +63 is `postfixderef.t` 42 + 46, cause #1221** (`utf8::encode $name_utf8` on `chr 9787` at line 206 — the name is 3 octets and a different symbolic reference).  MEASURED on both trees (`027ba9c` 100+21, HEAD 102+19, SAME last row 121/128 so the shortfall stays 7) and probed byte for byte (`1/3/Face/undef` on perl AND HEAD, `1/1/Face/Face` on the base).  Full sweep on `4afd8ec`: 0 new / 0 fixed / 0 LOST, drops 5 = census, shortfall +0, **TOTAL 18642 -> 18644**, GATE clean.
- **A STALE skip registration is a no-op for the counts** — `cl/pcl-test.lisp` emits a plain `ok` for a registered test that PASSES, so dropping one removes only the `# REGISTRY-STALE` diagnostic.  The ten `index.t` `:utf8` entries (0 stale lines on `027ba9c`, 10 on HEAD = the proof they are #1221's) were dropped with index.t reading `120/0/0/120` before and after.  `chop.t`'s own stale entry is PRE-EXISTING (stale on `027ba9c` too) and stays.
- **`docs/not-supported.md`'s `utf8::encode`/`utf8::decode` entry was FALSE since #1221** and is now **the per-scalar UTF-8 flag** (`utf8::is_utf8`, still always 1; `utf8::upgrade` answers perl's octet count but sets nothing).  Its two documented second-order consequences — chop.t's self-skipping guard and the ten registrations — are gone with it.  The runbook's decision-tree pointer, `sweep-bug-catalog.md`'s index.t section + ranked row 6, and `questions.org` were corrected in the same commit.
- **#1224 — `binmode FH, "utf8"` (a layer name with NO leading colon, which perl accepts) is not recognised, so the handle stays a BYTE handle.**  A #1115 REGRESSION worth **264 rows**: `t/op/read.t` OK 2116/0 -> DIFF 1852/264, bisected in worktrees to `3a89a5c` (#1115 + review fixes, ONLY) and unchanged through the batch.  Probed: perl reads back 5 characters for BOTH spellings, PCL reads back 10 UTF-8 octets for the colon-less one and warns "Wide character in print" on the write side.  Before #1115 it passed for the wrong reason (binmode was a no-op AND the default was UTF-8).
- **It was invisible because the batch never ran a companion `op/` leg**: round 1 ran only `io/` and `uni/`, round 2 ran none, and `t/op/read.t` is in `t/op/`.  The DECIDED s470 rule ("the merge-time sweep is NON-NEGOTIABLE for `cl/` work") wants its companion half read the same way — **a `cl/` I/O change runs at least the `op/` leg too**.
- **"Mover" is a claim about the BASE, not about the snapshot.**  Of the companion's nine NEW-ROW files, four (op/multideref.t, io/perlio_open.t, op/magic.t, op/rt119311.t) plus both NEW shortfalls (run/fresh_perl.t 0->91, mro/method_caching.t 0->3) read IDENTICALLY on `027ba9c` — the SNAPSHOT is stale, the batch did nothing.  `baselines/perl-suite-run.tsv` was deliberately NOT spliced in the registry commit: none of the movers is that commit's, and splicing them would launder #1224 into the baseline.
## s470bp (2026-09-05 + resume 2026-09-06, Opus) — the MODULE CACHE stores compiled fasls (the yardstick's biggest number) and ALL THREE aggregate levers ship (the third on resume, with the batch's owed full sweep and companion legs)

- **#1188 — the module cache stores COMPILED FASLS, and the double-execution bug is an ORDER, not a count.**  `*pcl-cache-fasl*` had been NIL since session 251, so a cached module was CL TEXT that SBCL recompiled on EVERY run.  The reason was `docs/module-double-exec-bug.md`: compiling and loading in the SAME image installs a `sub NAME` at BOTH passes and runs a `BEGIN` block at the COMPILE pass only, so the load pass re-installs the plain body OVER a BEGIN-time replacement whose idempotency guard now skips — Moo subclasses came out with empty attrs.  **`*pcl-fasl-build*`**, bound around and only around the `compile-file` that builds a module's fasl, moves exactly the forms that are not idempotent into the fasl and leaves the ones that are: `p-BEGIN` compile+execute → load+execute (a BEGIN block is the only module form that is arbitrary, ordered and guarded); `p-sub`'s LAMBDA INSTALL to load time while the compile pass keeps `%p-reserve-sub-name` (shadow + intern, which the READER depends on — without the shadow a module method named `push` reads as the inherited `pcl:pl-push` and installing it clobbers the runtime's builtin); `p-CHECK` discarded, which is what a source load already does.  **`p-eval-always` is UNCHANGED, and that is a MEASUREMENT, not a choice**: suppressing it fails JSON::PP's compile with "Package Exporter does not exist" — it is `(p-use "Exporter")` running in the compile pass that creates the package before the reader meets a symbol in it.  Option C of that doc done in the RUNTIME's macros, so the emitted text does not change at all and corpus-diff cannot move.
- **A cached module FASL is keyed by `*pcl-runtime-identity*` as well** — a CONTENT hash of `cl/pcl-runtime.lisp` plus the SBCL identity, computed once from `*load-truename*` while the runtime loads (so it is baked into the cached core and costs nothing per run; 9 ms in source mode).  A fasl has this runtime's macro expansions BAKED IN and only this SBCL can load it; an mtime would let a fasl carrying a removed macro's old expansion load, which is the silent-wrong this project refuses.  The `.lisp` key is untouched, so existing text caches stay valid.  Failure is never fatal: a compile-file failure writes a `.failed` marker (one hour, like the cached core's) and the module loads from its text; an `invalid-fasl` is refused before any of its forms runs; every other error propagates, because a module that dies at load dies the same way from its text and swallowing it would leave the module half-loaded.  A nested load reached from a compile-pass `use` does not build a fasl.
- **MEASURED** (interleaved on one tree and one core; `PCL_NO_FASL_CACHE=1` is exactly the pre-#1188 path; startup 0.164 s as the control): `use Carp` 0.406 → 0.216 (**1.88×**), `use Scalar::Util qw(blessed)` 0.337 → 0.170 (**1.98×**), `use Moo` 1.127 → 0.417 (**2.70×**), `use JSON::PP` 2.751 → 1.432 (**1.92×**).
- **The `use JSON::PP` residue is NOT compilation — it is STRING EVAL, and that is the next module-load lever (#1200).**  With every module hot, PCL_FASL_DEBUG shows `PP.pm -> FASL HIT (0.858s)` while its four dependencies take 1 ms each, and `sb-sprof` attributes the frames under it to `sb-c::ir1-convert` reached from `pcl:p-eval`: JSON::PP runs ~20 string `eval`s in a loop at load time to generate its property accessors (`lib/5.40.3/JSON/PP.pm:79-99`), each a `pl2cl --server` round trip plus an SBCL compile, EVERY RUN.  It is not JSON::PP-specific — generate-accessors-with-eval is the pure-Perl idiom (Class::Accessor, Sub::Quote, Moose, Type::Tiny).  #1200 has the design: the same three key ingredients make `p-eval`'s existing in-PROCESS cache a DISK cache, because its key (text + capture NAMES) is already process-independent.
- **The EXTENSION load has the same disease and is USER-PARKED (#1202)**: `pack("N",1)` costs **8.3 s** because `p-load-extension` plainly `load`s a checked-in transpiled artifact, recompiling it every run.  The #1188 machinery applies verbatim; not taken, because the USER parked the extension load with `pack`.
- **L5/#1203 — a CONSTANT-subscript slice assignment is N direct stores.**  `@h{qw(a b)} = …`, `@a[1..3] = …`, `@a[0,2] = …`: the licence is `%p-const-subscript-values`, a compile-time reading of the place form, and exactly three shapes occur in PCL's emission for a literal slice (a self-evaluating number or string, a quoted literal, a literal numeric range).  A DESCENDING literal range yields the EMPTY list — perl's own answer — so the call site reads empty as "no licence".  The RHS is still evaluated once and SNAPSHOTTED (#818), a missing value is still `*p-undef*`, and the STORE is the same `(setf (p-aref …))`.  **sliceasgn -43.5 %** shipped (-62.7 % hand-replaced), 2.20× → 1.17× of perl.
- **L6/#1204 — an array assignment from a RANGE is not materialised.**  `@a = (1..$n)`, `@a = (1..20, $_)`: `%p-fill-segments` licenses a bare range or a literal `(vector …)` with at least one range piece; requiring a range is the point, because without one the general path already has #1181's block copy.  Every piece — the range BOUNDS included — is evaluated and each non-range piece SNAPSHOTTED before the destination is cleared, so `@a = (1..2, @a)` still reads the old @a.  Non-range pieces go through `%p-array-add-items`, EXTRACTED from `p-array-fill` rather than copied, and the oversized-range refusal through `%p-range-size-check`, shared with `p-..`.  A magical STRING range has no counting form and goes through `p-..`.  **arrfill -58.3 %** shipped (-93.9 % hand-replaced), **1.47× → 0.61× of perl** — from slower than perl to well under it.
- **L7/#1205 — built and measured here, COMMITTED ON RESUME (the machine was shut down before its own gate could run; the runtime carrying it was `scratch/s470bp/rt-L6-AND-L7.lisp`, the split script `scratch/s470bp/split-l7.pl` its inverse; see the resume bullets below).  An rvalue slice in a COPYING consumer reads VALUES, not element cells.**  `p-aslice`/`p-hslice` build ALIASES, which under raw element storage PROMOTE every slot to a box, monotonically and forever.  `my @v = @a[1..5]` copies, so the licence is THE CONSUMER and it is decided where the consumer is visible — in `p-array-=`'s macro, whose VALUE form is syntactically the slice.  Every other consumer keeps the aliasing path untouched (`for (@a[0,1]) { $_ *= 10 }` still writes through).  The READ is the existing one: `%p-aslice-copy` asks `p-aref` and `%p-hslice-copy` asks `p-gethash`.  **It also FIXED a divergence** — `my @a=(1,2); my @v=@a[0..3]; exists $v[3]` was false in PCL and true in perl.  **slices -22.2 %** shipped (-33.3 % hand-replaced), 2.53× → 1.95× of perl.
- **None of the three levers takes a PCL_OPT name, and that is task #1181's precedent, not laziness**: all three are runtime MACROS with no emission to switch (`pl2cl`'s output is byte-identical with and without them, corpus-diff IDENTICAL over 111 files).  So their guard rows are **MACROEXPANSIONS** — the macro's own answer, read directly — each with a NEGATIVE beside it (a dynamic subscript list, a non-range RHS, a slice in a consumer that does not copy).  Without the negatives a lever that fired everywhere would pass every row and be wrong everywhere else.  `Pl/t/perf-levers-02.t`, inverse-verified on the base runtime.
- **A ±10 % swing on `feread2`/`feread3` is this box's LAYOUT NOISE, not a regression — measured three ways.**  The two rows moved in OPPOSITE directions (-8.6 % and +12.8 %) reproducibly; a `push`-built variant of feread2, which contains no code either lever touches, reads **+10.9 %**, and a control A/B of two BYTE-IDENTICAL runtimes on the same row reads **-4.8 %**.  At ~4 ns per element read over 30 M reads, a data-vector alignment difference is worth that much.  Re-measure on a quiet box at merge review.
- Filed: **#1200** (the string-eval disk cache — the biggest remaining module-load number, with its design and its five "check before building" questions), **#1201** (a CHECK block in a `use`d module never runs; perl runs it at the end of the requiring unit's compile phase — PRE-EXISTING, both paths agree), **#1202** (the parked extension load), **#1206** (a slice assignment in SCALAR context yields the source vector where perl yields the COUNT — PRE-EXISTING, both paths agree; `p-hash-=` has the two-armed return to copy, and the LIST-context half diverges too).
- **RESUMED 2026-09-06 — the OWED measurements taken, L7/#1205 SHIPPED.**  The full sweep on `485661b` (the only instrument that sees #1188, since module loading and fasl caching are invisible to corpus-diff) is **GATE clean, TOTAL passing baseline 18581 / current 18581 (+0)**, 0 new / 0 fixed, drops 5 = census, shortfall +0; the six companion legs (`op/hashassign.t op/aassign.t op/list.t op/array.t op/hash.t op/local.t --jobs 1`) reproduce `baselines/perl-suite-run.tsv` FILE FOR FILE with 0 NEW ROW / 0 FIXED ROW / 0 LOST — nothing moved, so no base-tree measurement was owed.  **The "expect 18312" the stop note carried was STALE**: main had moved to `614c6af` and its baselines carry 18581 — the TREE's own `baselines/pass-baseline.tsv` is the authority, never a number written down in a doc.
- **L7/#1205 shipped**, from the runtime the shutdown had left in scratch, its diff first verified to be EXACTLY the four edits `split-l7.pl` names.  Gate **211 files / 7149 rows** (only the 13 pclxs xs rows); the second full sweep, owed because L7 is a `cl/` runtime change, is again **TOTAL 18581 (+0)** GATE clean.  `tools/corpus-diff.pl 614c6af` IDENTICAL over 111 with the SILENT-DROP line unchanged, `tools/emission-ab.pl --ref 614c6af --shapes --list lib/**/*.pm` SAME / 0 DIFF / 0 RCDIFF, `tools/ir-host-leak.pl` adds no leak, and `PCL_OPT=none` on a slice-assignment program is byte-identical to main — all four as predicted, because the lever is a runtime MACRO with no emission to switch.
- **`probe-slice2.pl` — the second probe file, never run before this — is 20 of 20 against L7.**  Its ONE reported DIFF is the probe harness's own shell quoting: `''` inside `perl -e '...'` closes the quote, so **perl itself** answers "syntax error near `ne ?`" on that row; run from a FILE the dualvar row agrees on both sides, and the repaired probe — no quote literals at all — reads **0 divergences of 20**.  A probe that disagrees with perl on BOTH sides is a probe bug, not a finding.
- **L7's guards are inverse-verified, and the inverse run is what proves the row is about L7**: `Pl/t/perf-levers-02.t` 11 → 17 rows, and on a `485661b` tree exactly rows 12, 13 and 17 fail — 17's failing line being the `exists $v[3]` divergence L7 fixes (`N` on the base where perl says `E`).  The foreach NEGATIVE is the non-vacuous one: `p-foreach` is itself a macro, so its expansion demonstrably still carries `(p-aslice @a 1 2)`; the `p-print` negative can only ever be vacuous, since `macroexpand-1` of a function-call form is the identity.
- **The four population instruments on the FINAL tree, all as a runtime macro predicts**: `tools/corpus-diff.pl 614c6af` emission IDENTICAL over 111 files with silent drops 5 unchanged; `emission-ab --shapes --list lib/**/*.pm` 27 SAME / 0 DIFF / 0 RCDIFF plus a 204-file wide leg (cpan `.pm` + the perl-tests corpus) likewise; `tools/ir-host-leak.pl` **byte-identical** to the pre-L7 run (31 leaked symbols over 111 files, the standing number); `PCL_OPT=none` byte-identical to main once the preamble's embedded BUILD PATH is normalised — that path is the ONLY diff and it is task #217, not this change.
- **L7 answers identically with `PCL_RAW_ELEMS=0`** (the all-boxed A/B world), and that is structural rather than lucky: `%p-aslice-copy` ASKS `p-aref` instead of storing anything itself, so the trap that caught `%p-array-fill-range` in this same session — a fast path that must disappear when the gate is off — cannot apply to it.  The twelve-shape probe program is byte-identical to perl under both settings.
- **The `slices` A/B re-run is a CONFIRMATION, not the quiet re-measure** (that is the reviewer's, on a quiet box): A (L7) 0.1694 s vs B (`485661b`) 0.2153 s, **B/A +27.1 %** best-of-7, taken at load 4.6 with a sibling agent running — the same size as the −22.2 % / −22.8 % recorded at the stop.
- **REBASED ONTO MAIN `f01dcd6` (BR merged, generation v2-820) and re-measured on the combined tree — runtime-only, so main's generation string and main's regenerated artifacts are KEPT, no bump and no regeneration.**  Gate **212 files / 7183 rows** (BR's `io-layers-01.t` included), only the 13 pclxs xs rows; full sweep **TOTAL passing 18644 / 18644 (+0)** — main's post-BR baseline — GATE clean, 0 new / 0 fixed, drops 5 = census, shortfall 12277 → 12277 (+0); the six companion legs reproduce `baselines/perl-suite-run.tsv` as BR left it, ROW DIFF 0 NEW / 0 FIXED / 0 LOST; `corpus-diff.pl f01dcd6` emission IDENTICAL across 111 files, silent drops 5 unchanged; `ir-host-leak.pl` identical to a `git archive f01dcd6` extraction's own run (31 symbols / 111 files) — no leak ADDED.
- **A REBASE IS PROVED, NOT ASSERTED**: `git diff ce4fbc3 HEAD -- cl Pl lib`, with blob hashes and hunk line numbers normalised, is IDENTICAL to main's own `614c6af..f01dcd6` delta over the same paths.  That is the statement "only the incoming hunks came in, and nothing of this batch was lost" as a measurement rather than a reading of the conflict list — and it is cheap, because both diffs already exist.
- **`./runpcl` CANNOT SEE THE FASL CACHE — it sets `*pcl-skip-cache*` by design.**  Probing #1188 through it reports `TEXT` on every run and every module, which reads exactly like a broken cache; `./pcl -E 'use Carp; …'` on the same tree shows `fasl-build` then **`FASL HIT`**.  The runner was the wrong instrument, not the cache.  (`~/.pcl-cache` also holds a `*.fasl.failed` marker left by `module-fasl-cache-01.t`'s own deliberate failure fixture — expected residue, not a live failure.)
- **Rebased a SECOND time, onto `d59e58c`**, because main advanced again while the first rebase's sweep ran (two further s470br commits: the ten stale `index.t :utf8` skip registrations dropped — `cl/skip-registry.lisp` is a HARNESS file, so the sweep is the instrument that could have moved).  Same proof (`git diff f315cf5 HEAD -- cl Pl lib` == main's own `f01dcd6..d59e58c` delta), and **every bar re-read on that tip with every number unchanged**: gate 212/7183, sweep TOTAL 18644 (+0) GATE clean drops 5 = census, companion 0 NEW / 0 FIXED / 0 LOST, corpus-diff identical over 111, host-leak identical to a `git archive d59e58c` extraction.

## s470bs (2026-09-05 + merge legs 2026-09-06, Opus) — the FILETEST / STAT family gets ONE operand resolver: #1031, #1049, #1048, #1033, #1047 (of the brief's eight; #1042/#1043/#1044 NOT started — machine shutdown)

- **`%p-stat-operand` IS the reading of a stat / lstat / filetest operand, and nothing else in the family looks at its own argument** (rule 11).  Kinds `:over` / `:fd` / `:path` / `:bad`, dispatched by an `ecase` (rule 12); `%p--stat-test`, `%p--access-test` and the -T/-B scan are its only consumers.  The drift it replaces is why `-e STDOUT` answered false while `stat STDOUT` answered thirteen elements.
- **#1031 — a FILETEST runs its operand's overload, in two halves.**  A `-X` handler ANSWERS (passed the operator's LETTER and a false-but-defined third argument — measured `($obj,"e","")`; no stat runs and `_` is untouched), and anything else blessed STRINGIFIES through `""`.  **The `""` half was one line: the funnel PEELED THE BOX before stringifying**, so `to-string` saw the payload and the handler never ran — `-e $path_object` was a silent "no such file".  `stat`/`lstat` are not filetests and never ask the `-X` handler (probed).
- **#1049 — a STRING is a PATH, always; `-t` is the ONE exception and it was MEASURED**: `-t "STDIN"` answers "" with ENOTTY (it found the handle), `-t "/etc/passwd"` answers undef with EBADF.  So `-t` keeps the handle-NAME reading and joins the resolver for the overload and the errno.
- **#1048 — a glob VALUE, a glob REF, an IO ref, a STANDARD STREAM and a DIRHANDLE are handles.**  Resolution goes through `%p-resolve-fh` + `%p-fd-of-stream`, the two readings `fileno` already uses.  A dirhandle has no dirfd here, so `opendir` remembers the DIRECTORY in `*p-dirhandle-paths*` **keyed on the handle's own `(index . names)` cons** — the object both `opendir DIR` and `opendir my $dh` hold, so one lookup serves both — and `closedir` drops it, which is what makes a closed dirhandle EBADF.
- **`-l` NEVER takes a handle.**  It stringifies anything that is not a bareword (probed: `-l $lex` and `-l \*FH` are ENOENT on the glob's own string; op/filetest.t asserts `-l \*foo` names a file), and a bareword is EBADF.
- **#1033 — EBADF and ENOENT are different FACTS and every member now sets `$!`.**  EBADF = a handle that is not open; ENOENT = a missing path, **including one containing a NUL byte**, which perl fails ([perl #131895]) and the C layer TRUNCATES — `-f "TEST\0-"` answered TRUE for the file TEST.  ONE failure exit + ONE syscall wrapper that reads errno off the POSIX condition, never re-reading the C variable an allocation can clobber.
- **#1047 — `_` carries the stat FLAVOUR and the SUCCESS, and neither is derivable from the operand.**  `stat $f; -l _` DIES in perl's own two wordings; after a FAILED stat every filetest but -T/-B answers EBADF from `_`, while -T/-B OPEN the remembered NAME and answer ENOENT (probed; the split `t/op/stat_errors.t` asserts).  Reading `_` does NOT set the flavour — see the merge-legs bullets below, where the companion leg caught the claim as written here being wrong.
- **#1047's IO-REF half was NOT where the task pointed.**  The value never reached the stat funnel: `*$fh` on a lexical handle ran `p-dynamic-typeglob`, which STRINGIFIED the stream into a glob NAME, so `*$fh{IO}` was undef.  perl's `*$fh` is the anonymous glob the open created; PCL has no such object, so the handle itself is handed through and `p-glob-slot` answers IO (the handle) and GLOB (the operand) from it.
- **`-r`/`-w`/`-x` STAT before access(2)** — perl's `cando` reads the stat buffer, which is what fills `_` and what makes a missing file ENOENT rather than access(2)'s errno.  The access(2) verdict (ACL-aware) is unchanged.
- **CL's NIL *is* a symbol**: an undef operand fell into the bareword-handle arm and became EBADF.  perl's `-e undef` is the EMPTY PATH (ENOENT) — undef is classified before the symbol arm.
- **Bars**: three commits, RUNTIME ONLY (no `Pl/` file, no export, no generation bump, no artifact regeneration; corpus-diff / emission-ab cannot move by construction).  Gate after each; the last **209 files / 7188 rows, only the 13 pclxs xs rows**.  Guard `Pl/t/bareword-fh-slot-01.t` **55 → 133 rows**, every expectation the SAME program's output under perl 5.40.3, **inverse-verified by running that program against 027ba9c's runtime** (35 of 39 new rows differ there).  `docs/ir-op-inventory.tsv` regenerated.  **The companion legs and the sweep were NOT run — stopped for a machine shutdown.**
- **Filed, all PRE-EXISTING**: **#1230** (a box's STRING cache poisoned with a raw DOUBLE — `%pcl-dualvar-p` false-positives on ordinary floats and `%p-dualvar-copy` copies the raw VALUE into the sv slot; `printf "%s"` then dies "not of type SEQUENCE" and the program is lost), **#1231** (an UNDECLARED bareword in a filetest operand slot is emitted as a CALL and DIES — the reason `t/op/stat_errors.t`'s 311 errno rows still fail, since that file is `eval "$op $arg"` over BAREWORDS), **#1232** (`t/op/stat.t` = ZERO rows on `is_linux_container`, a sub at the END of perl's own t/test.pl; 108 perl rows), **#1233** (a closed LEXICAL handle is ENOENT, not EBADF — `%p-forget-fh` clears the box, deliberately, and #529 depends on it), **#1234** (a STACKED filetest over a `-X`-overloaded operand re-dispatches at EVERY level in perl; two probes prove the `_`-chain desugaring cannot do it — 54 rows, a COMPILER change), **#1235** (`-t`/`-T`/`-B` on an IO REF run `""` in perl; PCL cannot tell an IO ref from a lexical handle — 4 rows), **#1236** (`rand` is deterministic ACROSS RUNS — no auto-seed; found by #1042's "sweep that family", whose full audit is in the task).

- **READING `_` PERFORMS NO STAT, SO IT DOES NOT CHANGE THE FLAVOUR** (s470bs-merge; this CORRECTS the s470bs bullet above, which claimed the opposite).  `lstat $f; -e _; -l _` is legal in perl, and so is a plain `stat _` in the middle of the chain.  The ONE exception is `-T`/`-B`, which OPEN the file and stat the descriptor — and only when there was a valid buffer to work from: `lstat "/nope"; -T _; -l _` is legal, `lstat $f; -T _; -l _` dies.  Probed as eleven shapes vs perl 5.40.3; five were wrong, all eleven agree now.  **The companion leg is what caught it** — the first "after" run carried ONE NEW row (`stacked -l uses previous stat, not previous retval`, t/op/filetest.t:125) and a shortfall of 2 → 8, because the file's own abort had become #1047's new DIE.  A new DIE is exactly the kind of change whose cost is invisible to a green gate.
- **The batch's OWED bars, taken at merge time on the tree rebased onto main `48d8e39`.**  Rebase clean, and PROVED by arithmetic: `git diff a130812 HEAD -- cl Pl lib` with hunk headers normalised is byte-identical to main's own `git diff 614c6af 48d8e39`.  Gate **212 files / 7261 rows**, only the 13 pclxs xs rows.  `tools/ir-host-leak.pl` **31 leaks over 111 files, byte-identical to a `git archive 48d8e39` extraction** — no leak added.  Guard `Pl/t/bareword-fh-slot-01.t` 133 → **143** rows.
- **Companion: `op/filetest.t` C 180/254 → 369/65 (+189) and `op/stat_errors.t` C 305/333 → 510/128 (+205); the other four files unchanged.**  ROW DIFF **0 NEW / 394 FIXED / 0 LOST**, SHORTFALL 120 = base.  **`op/stat_errors.t` was predicted NOT to move** (its barewords die in the compiler, #1231) — the prediction was wrong by 205 rows, because #1231 only blocks the bareword operands and every non-bareword `error from -X <operand>` row was reachable all along.  Each "before" was re-measured on a `git archive 48d8e39` extraction (0/0/0/0 ROW DIFF there), which is what makes "the mover is ours" a measurement and not an assumption.
- **Sweep: GATE clean, 0 new / 0 fixed, drops 5 = census, TOTAL passing 18644 → 18645.**  The +1 is `ref.t` 195/16 → 196/15, test 46 (`$ref = *STDOUT{IO}; is($ref, *{$ref}{IO})`) — #1047's IO-ref half — measured serially on BOTH trees; the twin assertion in `postfixderef.t` (the `$ref->*{IO}` spelling inside a string eval) still fails and stays blessed.
- **A FIX THAT MAKES `$!` REAL EXPOSES A SKIP THAT WAS PASSING ON NOTHING** (the s438 pattern, a third time).  `perl-tests/readline.t` rows 12/13 are gated on `open F, '.' … sysread …; my $err = $! + 0;` and used to SKIP because `$err` was 0.  #1033 makes the BEGIN block's failing `-d 't'` leave ENOENT behind, as perl does, so the SKIP stops firing and the rows run — and fail on the half PCL still lacks, a read from a DIRECTORY handle that sets no errno where perl sets EISDIR.  **#1237** filed with the two-tree probe; the rows blessed with that cause; `row-shortfall.tsv` 8 → 6.  23/5/4 → 23/7/2 on both trees, serially.
- **`--bless-fails` DESTROYS the header of `baselines/perl-suite-fails.tsv`** — it replaces the accumulated per-session provenance with a bare `# taken-at:` line.  Restore the old header by hand in the same edit, and keep the unmeasured files' rows from the previous bless byte-for-byte (only the six measured files' rows may move).
- **`sweep-perl-tests.pl` on named files CLEARS `.faillog`**, so a single-file re-measure after a full sweep destroys the full run's per-file logs.  Use `PCL_TEST_LOG_DIR` for every re-measure (that is what the flagged-file attribution used).
- **`docs/ir-spec.md` §10c** now carries the family: the operand-kind table, the three operator-dependent exceptions (`-l` never takes a handle, `-t` keeps the handle-NAME reading of a string, an EXPR slot CALLS a declared sub), `$!` as part of the answer, and the two things `_` remembers.

## s470bq (2026-09-05, Opus) — the IR as DATA: structured regex literals with the engine TIER, `p-esc`, `--emit-sexp`, `--facts`, `:needs` (Part B item B5 + §B.3)

- **The regex literal is STRUCTURED and carries its engine TIER (#1210/#1211, flag-day, gen v2-800)**: `(p-regex :pat "…" :flags "…" :tier :native)` and the same keyword shape for `p-qr` / `p-regex-from-parts` / `p-subst` / `p-tr` (which has no `:tier` — a transliteration needs no engine).  All five are MACROS now; **the OLD positional form DIES at macroexpansion naming the site** (rule 12 — two shapes at one entry point is exactly what a compatibility arm would hide).  The run is byte-identical: the delimiter/flag parse simply happens at the transpiler.  Grammar + the per-tier construct table: `docs/ir-spec.md` §10-tier; friction `generated-cl-ir-review.md` §3.3 marked CLOSED.
- **The emitter side was FOUR copies and is now ONE** (`_regex_literal_form`): gen_leaf and gen_leaf_form each had an m// and a qr// arm, byte-identical but for the head symbol — which is why the tier could not have been added in one place before (rule 11).  Routing `:pat`/`:flags` through the one string writer also closed #419's unrepresentable-code-point hole inside regex literals, for free.
- **`Pl::RegexTier` is one pure function** — native / pcre / refused, plus **`dynamic` for an interpolated pattern**, whose tier is a run-time question and is DECLARED rather than guessed.  Its one bug, found by probe: `\p{L}+` read `}+` as a possessive quantifier, so the `}` arm needs the counted-`{n,m}` test.  **RULED: an unrecognised modifier letter answers `pcre`, not an error** — rule 12's die boundary (s329) is "die when the missing case produces a value the program consumes", and nothing here reaches the program.
- **`pl2cl --manifest`'s `regex.tier` stops saying `unclassified`** and prints a histogram over the four classes (ir-spec §10b; the stale manifest-01.t row repaired in the same commit).
- **`p-esc` (#1212) closes friction §3.2**: a literal holding a character below 0x20 is `(p-esc "…")`, decoded at macroexpansion, so **no emitted string literal spans a line** and a line break in the file is always syntax.  The payload alphabet IS the data form's (`\\ \" \n \t \r \uXXXX`), so a backend implements ONE unescape for both.  Heredocs and `__DATA__` were routed through the one string writer to get it — and gained #419's split, which their own inline escapers lacked.  **The two NON-evaluated plist slots** (`p-let`'s `:perl`, `p-sub`'s `:prototype`) keep the plain literal via `cl_string_datum`: a form there would be read as data.
- **`pl2cl --emit-sexp` (#1215) is a SECOND PRINTER over the same CLForm tree**, never a text transform: every symbol pipe-quoted (a keyword's colon INSIDE the bars), strings 7-bit with six escapes and JSON-style surrogate pairs, numbers decimal, one form per line, no comments.  Grammar + a Perl AND a JavaScript reader: **ir-spec §12b**.  **The review doc's warning ("a sexp dump with embedded text islands is worse than no dump") is answered, not waited out**: every island is DECLARED (`(|p-cl-text| …)`, `(|p-cl-text-wrap| …)`) and COUNTED in the file's own trailer form.  What it still omits — the assembly's own text lines — is stated in §12b and owned by **#1217**.
- **Order is the program**: the collector buffers by (section, bucket) and replays in the #469 phase order.  `Pl::Passes::run` forwards the BUCKET to the observer hook; `(|p-bucket| SECTION :BUCKET :PHASE)` marks each run.
- **`pl2cl --facts` (#1213) prints every licence that HELD as `(p-fact (NAME …) FORM)`** — §B.3's "facts, not rewrites", so `PCL_OPT=none --facts` is the general form with the compiler's proofs attached.  **The design point: the FACT is computed BEFORE the switch at each wired site** (a gate written `enabled(N) && FACT` short-circuits the fact away), and a DISABLED Kind-B pass still runs under `--facts` in ANNOTATE-ONLY mode.  Ten licences wired; the verdict-COVERAGE family is NOT, because its licence is a declaration's CLASS and printing the counterfactual means running the verdict with the switch off — **#1216**, stated in ir-spec §12c so the boundary is in the spec, not only in a task.
- **`:needs` on `p-sub` (#1214)** — the obligation classes the body exercises, in §10b's own class names, from `Pl::Manifest`'s own walk made REENTRANT (no second table).  **ALWAYS printed, possibly `()`, because the ABSENCE is the half a target uses**: nothing can throw ⇒ no frame; nothing localizes ⇒ no save/restore stack.
- **A headed KEYWORD PLIST now breaks between PAIRS in the CL printer** — the same argument the headless facts plist already made; without it a long interpolated pattern printed `:flags` and `"i"` on separate lines.

**Merge session (2026-09-06), rebased onto `d59e58c` and then onto `424cabc`, at generation `v2-830`:**

- **#1219 — AN UNTERMINATED REGEX LITERAL MUST DIE, and the batch's FIRST full sweep is what found it.**  `_parse_regex_content` locates the closing delimiter with `rindex`, which for an unterminated literal lands on the OPENING one, so `$foo = /` read an EMPTY pattern with the newline as FLAGS; an empty pattern matches, so `eval '$foo = /'` **returned 1** where perl raises "Search pattern not terminated" and returns undef (probed 5.40.3).  The function is PRE-EXISTING and byte-identical to the base — **what changed is that the keyword form makes its output the EMITTED `:pat`/`:flags`, where before only the interpolation gate read it and the raw string went to `p-regex` for the runtime to re-parse.  A tolerant parse behind a runtime that re-does the work becomes an authoritative one that INVENTS a value; the base at least died (with a leaked CL type error).**  THE RULE WORTH CARRYING: *a parse that MOVES from a consumer to the emitter must be re-read for rule 12 at the move.*  One line, perl's own text because it is free here.  perl-tests/eval.t test 6 now PASSES; sweep TOTAL 18644 → **18645**, `pass-baseline.tsv` edited ROW BY ROW; guard `Pl/t/ir-data-form-01.t` 57–58, inverse-verified, its negative half six terminated delimiter spellings.
- **The flag-day's emission residue is TWO families, both measured behaviourally NEUTRAL** (all four populations A/B vs `d59e58c`, **RCDIFF 0 everywhere**; mechanically explained 110/111 + 28/28 + 590/598 + 292/293).  (1) Five files cross `_cap_inlining_if_huge`'s 20 000-character threshold because the new forms are LONGER and gain the pre-existing `(locally (declare (notinline …)))` wrapper — `p-defcell` counts identical, and the wrapper only suppresses inlining in a cold top-level form.  (2) Four are **#1218**'s `_blank_string_innards` p-defcell parity bug (op/sort.t's `@result`, op/lex_assign.t, op/threads.t's `$SIG`, win32/system.t's `@echo`).  A companion run of the six runnable ones reads its SNAPSHOT exactly, file for file, ROW DIFF 0/0/0/0.
- **THE TIER, MEASURED over 1030 files / 6291 regex literals** (ir-spec §10-tier now carries the table): **native 5030 (80.0 %), dynamic 1085 (17.2 %), pcre 112 (1.8 %), refused 64 (1.0 %)**.  A target that implements only the native tier already runs four fifths of what PCL emits.  **`:dynamic` is the bucket to size first** — it is not a construct verdict at all but "the pattern is built at run time", so the classifier never sees the text.
- **A COMPANION MOVER IS ATTRIBUTED BY RE-RUNNING IT ON THE BASE, NOT BY THE SNAPSHOT'S DATE.**  s470bq's `--all --quick` flagged **40 movers, all "REAL MOVE (both runs agree)"**, plus ROW DIFF 276 NEW / 620 FIXED.  Re-running the 45 movers on a `git archive d59e58c` extraction gave **42 of 45 IDENTICAL** — the mro/* STALE family, the io/* moves and op/read.t's 264 rows are s470br's/#1115 and s470bn's, left unspliced by their own records.  The remaining three were re-run on the SAME FOOTING (`--jobs 1`, same file set) on both trees: io/open.t 154/34 both, io/pvbm.t 20/8 both (the known batch-contention flake, six times since s437), and **op/eval.t 130/38 → 131/37 = the batch's ONE mover, the same #1219 row**.  `perl-suite-run.tsv` spliced BY HAND for that row only, carrying its cause AND the reason the other 44 are untouched — a splice without a cause launders another batch's work into a baseline.
- **The stale-guard population predicate**: the s416 rule's grep for this flag-day is NOT `:writes-args|p-regex "|p-subst "` (it names four of the eight files and misses `prototype-01.t` and `refusal-site-01.t`) but "mentions `p-sub`, a regex-literal macro, `p-esc`, a facts key, or a control-character string expectation" — 58 files, 2701 rows.  Twelve rows repaired under the s377 rule, every edit STRENGTHENING (the `[^)]*` wildcards became the literal plist).
...
- **The generator reads the LOADED runtime, never the source text** — `p-+`, `p-*`, the six numeric compares, the six string compares and the compound-assignment family have NO textual `(defun` (they are `%def-overloaded-arith`/`-cmp`/`-str-cmp` expansions), so a text scan misses 20+ of the most-used ops in the IR.
- **An op's machine-readable contract is a `Contract:` docstring tail** (normative, ir-spec §10a): `ctx= coerce= magic= dies= dynamic= phase= host=`, seven keys, CLOSED value sets; an unknown key/value DIES and so does a PARTIAL tail (absent must never read as a default).  **The tail states the op's PERL contract — a divergence from it is a BUG, not a weaker contract** (`p-%` is `dies=yes`; PCL's NaN is #1173).  Seeded on 55 ops; 627 still UNCLASSIFIED, counted per run.
- **`p-||` and `p-` are the SAME symbol** (probed): the `||` is an empty multiple-escape section, so the symbol's name is `P-`.  A text-parsing backend must fold them (ir-spec §11b).- **ir-spec §10 named symbols that DO NOT EXIST, and was CORRECTED against the generated inventory** (s470bm merge review — the spec must name real symbols, which is B1's whole point): four rows cited Perl-facing spellings (`p-&`-style bitwise, `p-x`, `p-eq`…`p-cmp`, `p-++`-style increment) and now name `p-bit-and p-bit-or p-bit-xor p-bit-not` / `p-str-x` (+`p-list-x`) / `p-str-eq`…`p-str-cmp` / `p-pre++ p-pre-- p-post++ p-post--`, **each verified EMITTED before the edit**, each with a clause saying why the spelling differs from perl's operator.  `%p-empty-list` was cited as the empty list's SPELLING and is never emitted (measured over 111 + 592 files) — the row now describes the VALUE (a zero-length vector) and marks the helper runtime-internal.  `tools/ir-inventory.pl`'s citation list is the standing drift check: **18 stale citations → 1**, the survivor being `pcl::p-qr`, genuinely unexported (#1177).
- **`pl2cl --manifest FILE` prints what a program demands of a target** (normative, ir-spec §10b): `uses` + `uses_other` (nothing is silently dropped), `needs` (the obligation classes, every one PRESENT with a zero where unused), `facts` (`fired / candidates` per Kind-A/Kind-B licence + the #1035 verdict census).  **ONE walk, hooked into `Pl::Passes::run`** — the place a lowered tree already becomes text — as an OBSERVER whose return value is discarded, installed via `set_form_hook`/`set_text_hook` so no core module gains a dependency on a flag-only one.  corpus-diff IDENTICAL over 111 + 6 shapes.  (#1171)
- **Measured over four populations**: the phase model and I/O are universal; **string eval is in 55 of 111 perl-tests and 205 of 592 perl t/ files but only 14 of 287 CPAN dist files** — CPAN test code is far more portable than perl's own suite; `local` + magic-global writes ~45 % of files; `goto` 5 files, `formats` 0 corpus / 9 perl-t, `xs` 0 everywhere.  The regex TIER prints `unclassified` — a DECLARED absence, B5 owns the classifier.
- **`local` has TWO emission shapes and counting heads gets it wrong both ways**: `(p-local-cell $g (p-box-for-local 2))` (the second is the VALUE wrapper) vs `(let (($/ (p-box-for-local ":")))` (it is the site's ONLY marker).  Sites = p-local-* heads + p-box-for-local − the wraps.  Also: two consecutive `my` STATEMENTS are TWO `p-let` forms, not one with two entries.
- **The IR contains only three vocabularies, and the CL kernel is 100 NAMES** (normative, ir-spec §11b, with each form's JS and C rendering + the five reader rules a text-parsing backend needs).  `tools/ir-host-leak.pl` measures it; gate row `Pl/t/ir-host-leak-01.t` (924 ms) runs one fixture per leak family and asserts the EXACT set in both directions, plus §11b's stated size against the tool's `%KERNEL`.  The population census (~40 s) is a WHAT-TO-RUN-WHEN item for `cl/**` and `Pl/**`, not a gate row (8.4 s for six real corpus files).  (#1172)
- **A leak census must be POSITION-aware or it reports nothing but noise**: `(block start …)`'s `start` is a perl LABEL and is the same SHAPE as a leaked CL function.  Four positions — op / quoted / bind / arg — and only the first two are claims about the host; **a HEADLESS list's first element is a BINDING NAME** (a list whose own first element is a list is headless — every binding list, lambda list and plist); **a `pcl::`-qualified token is NOT a program identifier** (treating it as one hid five emitted runtime internals, #1177).
- **Census results**: perl-tests 38 distinct leaked symbols over 111 files; `lib/**` adds only `use integer`'s; **perl's own t/ (592 files) added two REAL BUGS nothing else had** — **#1178** (`ok((open my $fh, …))` emits the NON-EXISTENT op `p-my` and the whole file dies at load; the plain `my $ok = open my $fh, …` is fine) and **#1179** (`use parent qw( -norequire Foo )` makes `-norequire` a SUPERCLASS — `@ISA` `[-norequire Foo]`, `->isa("-norequire")` true; the COMMA spelling is correct, so the qw() path does not strip the flag).
- Filed, not fixed: #1173 (`5 % 0` → NaN where perl DIES, plus `substr` past the end returning `""` and `exists` on a non-ref returning undef), #1174 (`1/0` dies with SBCL's own text), #1175 (the CL-function leak families), #1176 (a quoted symbol is three unrelated things), #1177, #1178, #1179.

## s470bk (2026-09-05, Opus) — the nested-element autoviv pair (#1058 + #1057) and four of the six overload-dispatch members (#1021, #1004, #1001(a) + #1005); BI's companion bless chore

- **`my %h; $h{a}{b}++` left the element UNDEF where perl says 1, silently** (#1058): `p-gethash-box`'s container arm returned a fresh DETACHED box when handed an undef box — a PLACE it could have vivified into — so `++` incremented something unreachable while `exists $h{a}` still said 1.  **The two EAGER lvalue accessors (`p-gethash-box`, `p-aref-box`, and there are only two — ExprToCL emits them for l-value position only) now vivify a WRITABLE container through the existing `p-ensure-hashref`/`p-ensure-arrayref`**; a container that is not a box keeps the detached box, which is what stops a read from creating structure.
- **NIL is the array-slot spelling of that undef**, so `$a[0]{k}++` CRASHED rather than merely losing the write (`%p-elem-cell` promotes a HOLE to a box of NIL, and NIL reached SBCL's GETHASH).  ONE condition, TWO failure modes.
- **The coercing compound assigns crashed on the same chain** (#1057) because the STORE, not the read, is where the container is needed: `%store-back-form` builds `(setf PLACE (op PLACE VALUE))` and `(setf p-gethash)` is handed a VALUE.  **The mechanism already existed — `expand-autoviv`, the compile-time place-chain walker `p-setf` reaches through `p-autoviv-set`** — so the compound store binds its container through THAT (rule 11) and `$h{a}{b} = 1` / `$h{a}{b} .= "x"` cannot drift.
- **Routing the compound store through `p-setf` instead would have been a NEW silent wrong, and that is measured**: `p-setf`'s nested arm stores with a raw `(setf (gethash …))` which REPLACES the slot, so an alias taken earlier goes stale (`xy` in perl, `x` that way).  Binding the container and storing through `(setf (p-gethash CONT KEY) …)` keeps `%p-gethash-store`'s write-through rule.  The plain nested `=` still loses that alias: **#1151**, pre-existing, one-line fix known.
- **`%p-store-back` DELETED** — a second copy of the store decision that served `&.=` `|.=` `^.=` and carried the identical crash.
- **A `LC_ALL=C sort` is the check for a pure code MOVE; a locale-collating `sort` is not** (it reorders equal-collating lines: 2542 spurious differences on a 58-line block move).
- **perl vivifies an INTERMEDIATE dereference on a pure READ too** — `my $v = $h{a}{b}` leaves `keys %h` = 1, and so do `exists`/`defined` of the inner key.  PCL does not, and it needs the EMISSION (the runtime sees only a value; `$h{a}` and `$h{a}{b}` lower the same element access): **#1150**, the only residue of a 30-row matrix that went 3 → 23 identical.
- **`sort @objs` ignored an overloaded `cmp`** (#1021): perl `3 2 1`, PCL `1 2 3`.  **sort's default comparator IS the `cmp` operator** (perldoc -f sort), so it routes through `p-str-cmp` — the same function the explicit `{ $a cmp $b }` block already used.  Silent because a `cmp` that AGREES with its `""` is right by accident.
- **The overload question is asked ONCE PER SORT, not per comparison** (`%p-sort-plain-elements-p`: O(n) beside O(n log n), first test the `*p-any-overload-registered*` memory load), so a default `sort` over plain strings keeps its raw `string<` comparator verbatim.  `%p-sort-classic`'s `:default` arm inherits the fix through `p-sort`, as its docstring predicted.
- **A `++`/`--` overload handler is a MUTATOR of `$_[0]` and perl DISCARDS its return** (#1004); PCL stored it, which DECLASSED the object.  It survived 58 green rows because a `++` handler normally returns `$_[0]` and the two answers coincide — the guard class must return something else.  The AUTOGENERATED `++`-from-`+` still REPLACES the place (a different arm).
- **`*=` `%=` `.=` `x=` reached NO overload dispatch, and `x` had none in either form** (#1001(a) + #1005's `x` half): all five inlined their arithmetic instead of delegating to the base operator.  One builder (`%compound-arith-form` gains a form-BUILDER alternative to its `cl-op` symbol) + three one-line slow halves.
- **`x=`'s inlined body was the control-stack-exhausting `(apply #'concatenate 'string (make-list N …))` spelling task #880 had already removed from `p-str-x`** — the second bug a second copy always grows.  `x=` delegates outright now.
- **MEASURED COST, flagged: the `*=`/`%=` overload guard costs a raw-numeric loop +9.4 % of process time and roughly DOUBLES the loop itself** (0.015 → 0.031 s for 4 M iterations); PCL still beats perl there 3.4× where it beat it 7×.  `.=` is unchanged (the `str-buffer` Kind-A emission owns that path).  **#1153** carries the discriminating question: does the raw-numeric verdict PROVE a raw twin's slot is never a p-box?
- **A change on within-noise evidence is churn**: putting `(p-box-p …)` before `*p-any-overload-registered*` read 0.1859 s against 0.1893 s — inside both spreads — so #972's measured five-candidate order STANDS, with the attempt's numbers left in the code.
- **`abs` does NOT autogenerate from `0+`** (#1005): perl derives it from `<` and `-` (`$x < 0 ? -$x : $x`), so a class with those two and no `abs` answers an OBJECT.  `sqrt cos sin exp log atan2` have NO autogeneration; `!` IS autogenerated from `bool`, so `%with-unary-overload` gains the `:autogenerated` key its binary twin has carried since #972 and only the OWN handler was missing.  **Three fall-throughs — one "just add the lookup" pass would have been wrong for `abs`.**  `int` stays absent by measurement (perl derives it from `0+`, which PCL already answers).
- **`p-not` is `p-!`** — perl's `not` is `!` with a different precedence and no separate overload key; the two were copies of one line.  **BOTH SYMBOLS ARE STILL EMITTED and both must stay exported**: `unless` lowers to `p-!`, the `not` OPERATOR to `p-not` (probed on the rebased tree — a four-line program emits two `p-not`), and `Pl/Parser.pm` has three `(p-not …)` sites with a `Pl/t/clform-01.t` row asserting the spelling.  `docs/ir-spec.md` §10's logical row now says so, and also corrects a PRE-EXISTING error in the same row: the two negations are FUNCTIONS returning `1`/`""`, not short-circuit macros returning operand values.
- **The #1153 slowdown is RULED (Fable, s470): the correctness fix stays; the guard-free path is a PERF lever, not a revert.**  For a RAW twin whose DELTA is a compile-time literal number the only open question is whether CUR is a blessed box, and a raw-numeric-verdict slot cannot hold one (the checked coercion declines the freeze rather than storing a box) — so the EMITTER, which knows both facts, emits a guard-free variant.  That makes it an emission change: routed to the perf agent's round 27 as lever L4, with s470bk's measurement as the acceptance (the 4e6 loop back to ~0.015 s of loop time).
- **Where the family stops, and why**: **#1000** (`use integer` bypasses every operator's dispatch) needs a `Pl/` emission change plus a new `p-+-int` runtime family; **#1006(a)** (the `=` copy constructor) needs a notion of a SHARED overloaded place PCL cannot represent.  **#1006(b)** is the only residue of #1005's matrix and stays declined with its measurement (`nomethod` in ZERO of the 25 `use overload` files across all four populations, against a hot-path @ISA walk).
- **A companion row can only be blessed from a tree that CONTAINS the commit that moved it**: BI's seven ROW-DIFF wins read `0 FIXED` on the pre-rebase branch and appeared only after `git rebase main`.  `--bless-fails`/`--bless-shortfall` rewrite the accumulated `# taken-at:` block to one sha — restore it by hand.
- **A stale companion NOTE is a mover too**: op/closure.t's `aborted-forms:1` was cleared because the file aborts none now, attributed to BI's batch as a whole rather than bisected within it.
- **A NEW forward `declaim` must match any existing proclamation exactly.**  `(function (t) t)` against the existing `(function * *)` for `p-ensure-hashref`/`p-ensure-arrayref` printed an SBCL warning that polluted four output-CAPTURING gate files (81 rows) while the behaviour under test was fine.  A forward reference inside a function body needs no declaim at all.
...
- **The default for an unclassified position is ESCAPE.**  `_tw_walk` gained a 6th argument `$aread` — "an `@name` leaf here is only READ" — passed at the ~20 classified recursion sites and absent everywhere else, so a spelling nobody thought of costs an optimization and never a value.
- **The read-only builtin list is SHORT BY MEASUREMENT, and three of the guesses were wrong**: `reverse @a`, `sort @a` with NO block and `values @a` all hand the caller ALIASES (`$_++ for reverse @a` increments `@a`, probed perl 5.40.3), so they are neither reads nor escapes — they are **alias-TRANSPARENT** (they pass their own position's licence through, and an assignment copies).  `map`/`grep` are NOT (their block sees `$_` aliased and can `\$_` it — `my @r = map { \$_ } sort @c` really hands out `\$c[i]`).  What copies: `print`/`say`/`printf`/`sprintf`/`join`/`scalar`/`defined`/`warn`/`die`, `push`/`unshift`'s non-target arguments, `return @a` and a sub's tail value.
- **A SYMBOLIC REFERENCE CANNOT REACH A `my` ARRAY** — the #1140 design sketch called for a blanket `@{"a"}` / `@$name` escape and it is unnecessary: `no strict 'refs'; my @a=(1,2); push @{"a"}, 5` leaves `@a` at 2 and puts the 5 in `@main::a` (probed).  The only path to a lexical is `*a = \@a`, which takes a reference and is already an escape.
- **A `sort { … }` escape is RELAXABLE, and it had to be or A5 would have regressed.**  The comparator's only channel to the array is the alias pair `$a`/`$b`, and this same walk already records every write/`\`/alias of those (the block body is walked with the seam flag).  Without the relaxation `for my $x (sort { $a <=> $b } @src) { push @o, $x }` lost the raw arm — and with it `classic-sort`'s whole foreach licence, whose guard row is in `Pl/t/classic-sort-01.t`.  Residue (#1142): a comparator that CALLS a sub writing the package `$a`/`$b` is invisible here.
- **The foreach conjunct lives at the LOWERING SITE, not in `foreach_ro`** — the verdict is name-keyed and two loops can share `$x` while iterating different arrays.  And it asks about **every array NAMED in the list**, not a bare `@a`: `sort`, `reverse`, `values` and a comma list all hand the SOURCE array's elements through, so `for my $x (sort { $a <=> $b } @fs) { $fs[1] = 99; print $x; last }` is perl's 99 too.
- **`classic-sort` needed NO code change** to become exact: it consults the `p-foreach-raw` head by design (s470a5), so the conjunct reaches it by construction.  Both #1140 reproducers now print what perl prints (99 / 99) in default AND `PCL_OPT=none`.
- **`local-push` = Kind-A `%p-push1`** (`%p-array-store-scalar` + the new length — deliberately the SAME function `p-push-impl` calls, so the copy/blessed/dualvar/raw-element rules cannot drift).  Two halves: the ARRAY is a proven fact (`my @a`, `!escapes`, and the emitted symbol is a let-bound lexical), the ITEM is a shape at the site (`_push_item_is_scalar`).  Bench `pushloc` **0.48× → 0.28×, −43 %** over three A/B rounds (control `cfor` +1.3…+4.7 %); `arrhash-k` 1.15× → 1.03×, **−10 %**.
- **`_node_is_definitely_scalar` was NOT widened** although `local-push` needed exactly that predicate: its other consumer (`gen_progn_form`'s `vector` vs `p-flatten-args`) would change emission corpus-wide.  The wider one lives beside it as `_push_item_is_scalar`, and folding them is task **#1141** (with a bench row first — no standing row builds a list of computed values).
- Generation **v2-720**; guard `Pl/t/array-facts-01.t` (61 rows, 8.4 s), inverse-verified on an `e73038f` worktree (exactly 23 rows fail there).  Filed **#1141**, **#1142** (relax map/grep/sort blocks that only read), **#1143** (`Internals::SvREADONLY(@a,1)` leaves `scalar(@a)` reading the elements — pre-existing), **#1144** (a topic/modifier `for` over `@a` escapes it).

## s470bi (2026-09-05, Opus) — the s469bi correctness pool finished (#1045, #1084, #1020, #1083, #1022(a)) + a main-breaking emission bug (#1118) and a cache-key collision (#1119)

- **#1045**: a `goto` ANYWHERE in a sub body makes the sub context-SENSITIVE.  `_sub_return_facts` read `return` statements only, so a `goto &NAME` body looked like a body with NO exits — provably wantarray-insensitive — and the call site dropped its bind.  `"@{[ rgoto() ]}"` gave `3` (the COUNT) where perl gives `7 8 9`; `scalar(gw())` gave `L` where perl gives `S`.  Every `goto` counts: `goto &NAME` REPLACES the frame (the target runs in this sub's CALLER's context) and `goto LABEL` can land anywhere.  Verdict routed through the one an unknown callee already gets.  Guards `Pl/t/goto-sub-phase-01.t` 17→19 + `Pl/t/parser2-01.t` +2.
- **#1084**: `do FILE` reads a file as SOURCE, exactly as `require` does.  TWO silent defects, both hit by ONE U+2019 in a COMMENT: `p-do` sized its buffer with FILE-LENGTH (OCTETS) while `read-sequence` filled it with DECODED characters and its count was discarded (junk past the end → PPI failed to tokenize → the whole `do` died); and the read used the DEFAULT external format (UTF-8) where perl reads OCTETS and lets `use utf8` decide.  New `%p-read-source-octets` (latin-1, honours the count).
- **#1084 second half**: `Pl::Parser::_maybe_decode_utf8` guarded on `!utf8::is_utf8($src)` — a question about perl's INTERNAL REPRESENTATION.  Source over the `pl2cl --server` pipe arrives FLAGGED while still one character per octet, so a `use utf8` file read as octets was never decoded.  `utf8::downgrade($copy, 1)` asks the real question ("is every character a byte value?").  All three spellings (`do` / `require` / string eval) now agree with perl.
- **#1118's SWEEP COST, measured by Fable at the merge review**: on main `e73038f` the corrupted `:captures` cost `perl-tests/eval.t` **36 passing rows (128 → 92)**; s470bi's tree restores 128/32, last test 163.  `PCL_OPT=-raw-op-family` and `-raw-return-family` each MASKED it — the form only reaches the file under the default emissions — so **an emission bug a PCL_OPT bisect makes disappear is still an emission bug**.  #1125 (filed by the A5 agent from the byte-compare end) is closed as a duplicate pointing at #1118.
- **#1118 (FILED AND FIXED)**: a `p-sub` FACTS PLIST matched `CLForm::to_string`'s p-let DECLARATION-ENTRY layout shape ("an atom, then a keyword atom" — `(:returns :str …)` fits), so its tail went through `join` and `:captures`'s ARRAY REF was stringified into the emitted CL as `ARRAY(0x…)`.  NON-DETERMINISTIC emission (main differs from itself), an ODD-LENGTH plist, and death at LOAD in `%p-check-facts` — `t/op/try.t`, `t/opbasic/concat.t`, cpan YAML::Tiny and Test::Tester::Capture each lost a whole file, PRE-EXISTING on main since #1035 step 3.  Fix: one conjunct (a declaration entry's first element is a NAME, never a keyword) + a plist layout that breaks between PAIRS.
- **#1020**: `undef *GLOB` cleared NOTHING, in silence — `p-undef` handed the callee the CL package NAME ("MAIN"), which it case-INVERTED to "main", found no package, and swallowed its whole body.  Also: the CODE slot never removed the `*p-declared-subs*` entry and the IO slot was untouched.  The three clear halves come OUT of `%p-glob-copy-*-slot` as `%p-glob-clear-{var,code,io}-slot`, shared with `*A = *B` (#602's rule), taking the PACKAGE OBJECT + already-inverted name.  Every VALUE spelling now matches perl (incl. `print FH` after `undef *FH` → undef, $! = EBADF).
- **#1117 (FILED)**: perl REMOVES an aggregate slot on `undef *a` (`*a{ARRAY}` is undef until a READ re-vivifies it — order matters when probing); PCL empties it.  `makunbound` was tried and reverted: it gives perl's answer AND `push` re-vivifies, but a plain READ crashes on the unbound symbol-macro cell, so it needs vivify-on-read first.  Inherits `perl-tests/sub.t` row 24's cause (re-pointed by hand in both baselines).
- **#1083**: perl clones a `sub {…}` per evaluation only when the body can COMPILE PERL AT RUN TIME (PL_cv_has_eval).  PCL answered `same` to all nine measured shapes — and got the six `same` ones right BY ACCIDENT, because SBCL hoists a lambda with no free lexical into a constant.  Classifier + runtime `p-cloned-sub`.  **A NESTED sub is NOT excluded — measured**: `sub { sub { my $q = sub { eval "1" }; 2 } }` is DIFFERENT in perl.  **A discarded reference does not keep a closure**: SBCL deletes it and re-hoists (`(lambda () (progn c 1))` is EQ to itself, `(lambda () c)` is not), so the wrapper closes over the lambda it calls.
- **#1022 half (a)** (Fable ruling, s470): a bare `last`/`next`/`redo` whose nearest enclosing loop is not inside the sub body now DIES at run time, trappably, naming the sub; with no loop anywhere it keeps perl's own `Can't "last" outside a loop block`.  ONE document walk + ONE emission site, the verdict riding the token through `@PPI_ADHOC_KEYS`.
- **What counts as a loop, for #1022** — probed, and the two non-obvious ones came from the emission A/B, not from a probe: PPI compound types `for` (C-style — missing it would have killed my.t and time.t), `foreach`, `while` (= `until`), `continue` (a BARE BLOCK is ALWAYS this type); **a `for`/`foreach` STATEMENT MODIFIER is a loop with no Compound around it — core Exporter.pm's `import` is written `… and last foreach (@_);` on purpose, so marking it would have made every `use` die**; a `while` modifier is NOT (`Can't "last" outside a loop block`, like `do BLOCK while`); the walk's arm ORDER is load-bearing (`Statement::Sub` isa `Statement`).
- **#1022(a)'s MEASURED COST — RULED at the s470bi merge review: the die STAYS, and the cost is blessed VISIBLY.**  `perl-tests/loopctl.t` OK 63/1 → PARTIAL 40/0 (23 passing rows), `op/loopctl.t` 63/4 → 40/0, `op/rt119311.t` 11/2 → 8/2 (six `{ foo(sub { …; last }) }` sites).  The s329 boundary covers an EFFECT-ONLY miss in code that otherwise runs correctly; an untaken loop exit changes the CALLER's control flow, so the trappable die is the correct interim and **half (b) is the immediate next item — it wins every one of these rows back**.  Blessed ROW BY ROW with ONE cause across five files: `pass-baseline.tsv`, `fail-baseline.tsv` (row 41 leaves BY EDIT and is NOT fixed — it no longer RUNS), `row-shortfall.tsv` (3 → 27 by hand for the sweep key; the two `t/op/` keys by `--bless-shortfall`), `perl-suite-run.tsv` (spliced by hand with sigs), `perl-suite-fails.tsv` (25 rows, `--bless-fails` after READING each from `.suitelog/*.fails.tsv`).  Verified: no loopctl.t row in any `sweep-diff.pl` bucket, and `planned rows not asserted` 27 → 27 (+0).
- **#1119 (FILED)**: the module cache is keyed by the HAND-MAINTAINED `*pcl-cache-generation*`, so it identifies a session's intent and not the compiler.  Two gate runs lost: a STOPPED predecessor's v2-690 entries (written before #1035's facts plist) and then a SIBLING WORKTREE also on v2-700.  The saved-CORE cache is content-keyed since s439 and immune; key the module cache the same way.  Generation is **v2-710** (v2-712 in the commit that made the jump; renumbered above main's v2-700 at the final rebase).  The sibling was the A5 agent, whose runtime has `%p-sort-classic` -- the function the poisoned `File::Path` entry called.
- Also filed: **#1115** (PCL's `open` decodes UTF-8 by default where perl reads BYTES — srclen 101 vs perl's 107; wider than the `do` bug that exposed it), **#1116** (`do FILE` does not populate `%INC`, so a later `require` re-runs the file), **#1120** (`s///ee` behaves as `s///e` — the second `e` never evals the RESULT).

- Bars on the tree rebased onto main `e73038f`: gate **Files=202, Tests=6853**, only the 13 pclxs xs rows failing; corpus-diff **7 of 111**, silent drops 5 unchanged; emission-ab over **1048** files **SAME 1003 / DIFF 45 / RCDIFF 0**, every DIFF classified BY MECHANISM (15 #1045, 13 #1083, 1 both, 2 #1022, 14 the #1118 pair-layout); #1072's body-comparing staleness row green.  NO full sweep and NO `--all` companion — Fable's legs, and #1022(a) needs a ruling first.
## s470a5 (2026-09-05, Opus) — #996 half A5 SHIPPED: the classic sort comparators (`classic-sort`, the registry's FIRST Kind-B pass); sortnum 7.4× → 3.2×, sortstr 3.5× → 1.9×

- **The `sort` fast path is licensed by the CONSUMER, not by the comparator** — `sort` returns ALIASES (`$_++ for sort { $a <=> $b } @a` writes back into `@a`, in perl and in PCL), which is why `%p-collect-list` promotes every raw source slot to a box.  Two licences, either enough: **A** a copying consumer (array/list assignment, `[ … ]`, push/unshift, join/print/say/printf, return, and a `foreach-raw` list — the loop variable is only ever read); **B** every top-level argument is a fresh producer (literal, `keys`, `map`, `split`, `readdir`, `glob`, a range, a user sub call).  Measured over 250 sorts in lib + cpan-tests + perl-tests: **149 are a licensed SHAPE (60 %) and 110 also carry a licence (44 % of all sorts)**.
- **`values` IS NOT A FRESH SOURCE — the design said it was, and it is wrong**: `for my $v (sort { $a <=> $b } values %h) { $v .= "!" }` writes back into the hash, in perl AND in PCL (probed both).  So do `grep` and a nested `sort`.  A user sub call IS fresh even when the sub returns `@arr` or `values %h` — the call boundary copies (probed).
- **The licence is a fact about the CONSUMER, and the consumers are six emitters with no parent link in the PExpr tree — so `classic-sort` is a Kind-B CLForm pass, not a Kind-A emission site** (a deviation from the task's design, flagged for review).  `Parser2::_lower_program`'s `Pl::Passes::run` seam is "the one place a lowered tree becomes text" (rule 11), and the lowered tree already carries the `p-foreach-raw` head — i.e. VarAnnotator's read-only verdict — so the pass CONSULTS that fact instead of computing a second one.
- **FIVE modes, not four**: `:default` (`sort LIST`) is separate from `:str-asc` (`{ $a cmp $b }`) because their FALLBACKS differ — `p-sort`'s no-comparator arm stringifies (`string<` on `to-string`) while the block dispatches a `cmp` OVERLOAD.  Conflating them would silently change `sort @objs`.  An unknown mode DIES naming it (rule 12).
- **The per-element safety test is `%p-plain-scalar`, the gate-free core of `%p-storable-raw`'s rule** (boxed-aggregates §4.1): a plain number, string or undef carries no bless class, no is-ref flag, no dualvar halves, no tie proxy and no magic — so its numeric and string readings are pure and may be taken ONCE.  `*p-any-overload-registered*` is NOT consulted: an overloaded value is a blessed box and the per-element test already excludes it, so the program-level flag would be a second, weaker copy of the same question.
- **NaN falls back**: `p-<=>` answers undef for a NaN pair (= "not less" both ways) and no CL predicate on a key reproduces that order, so a NaN key sends the sort to the generic path — which is what keeps `sort { $a <=> $b } (3, NaN, 1, 2)` = `1,2,3,NaN` and the descending twin `3,NaN,2,1`, both perl's.
- **`sort LIST` decides comparator-vs-list at RUN time** (`p-sort` takes a first argument that unboxes to a function as the comparator), so `%p-sort-classic` makes that test itself, once per call, in `:default` mode — the compiler cannot know and a static whitelist would have been a second answer to the same question.
- **`PCL_OPT=none` emission is BYTE-IDENTICAL to `0faac04` over all 111 corpus files + 6 shapes files** — the proof that the pass is the only emission change.  With it on, 12 corpus files / 90 changed regions and 66 wide-population files / 152 hunks, every one of them `(p-sort …)` → `(%p-sort-classic …)`; RCDIFF 0.
- **PRE-EXISTING, filed #1125: emission is NONDETERMINISTIC** — a Perl arrayref stringification leaks into the `:captures` slot (`:captures ARRAY(0x…)`), so two runs of the SAME compiler on `t/op/try.t` differ.  It defeats a byte-compare and would make a module-cache key unstable.
- **MERGE REVIEW (Fable, s470a5): APPROVED, Kind B RATIFIED, `values` correction adopted.**  Three probe facts added to the record: a **`do { … }` TAIL IS NOT a copying consumer** — perl hands its aliases through (`$_++ for do { sort @d }` writes back), and the pass already left it generic (guard row added); **`eval { }` and a sub tail DO copy** (the sub tail is licensed, guard row); `reverse` passes aliases through and `$a`/`$b` are untouched after — all identical to perl in both `PCL_OPT` modes.
- **`sort @objs` with an overloaded `cmp` is perl `c b a`, PCL `a b c` in BOTH modes — that is PRE-EXISTING #1021, not A5's.**  The `:default` fallback runs through `p-sort`, so #1021's fix carries automatically; the runtime docstring now POINTS at #1021 instead of claiming the no-block arm "stringifies" as if that were perl.
- **classic-sort's foreach licence INHERITS `foreach-raw`'s own condition — no better, no worse — and it STAYS.**  `foreach-raw` proves the loop VARIABLE is only ever read; neither verdict covers a write to the ARRAY by another path during the loop, because VarAnnotator has no array facts.  Measured on one tree: `for my $x (@fa) { $fa[0] = 99; print $x; last }` is perl 99 / PCL 1, restored by `PCL_OPT=-foreach-raw` ALONE; the `sort`-fed twin is perl 99 / PCL 1, restored by `-classic-sort` OR `-foreach-raw`.  **Task #1140** (the VarAnnotator ARRAY-fact family) closes both and also unblocks #996 A3, whose licence IS that fact family.  No `Pl/t` row asserts the wrong answer.
## s470c (2026-09-05, Opus) — #1072: the artifact-staleness row REGENERATES each artifact and compares the BODY; `tools/rebuild-pack -o PATH`

- **The `gen=` stamp is HALF the promise.**  Comparing line 1 against `*pcl-cache-generation*` catches "the generation was bumped and the artifact was not regenerated" and is blind to the other half — a stamp that is CURRENT over a body the compiler no longer emits.  Measured on main `9bee19b` (s468bc): `cl/pcl-pack.lisp` was stamped with the tree's own generation, the row was green, and the regenerated body differed by ~134 lines, so every pack.t run in between measured an emitter the tree no longer had.  `Pl/t/artifact-staleness-01.t` now regenerates each discovered artifact into a temp file and compares everything after line 1 — three new rows, 8 in the file.
- **A CHECKER RUNS THE RECIPE; it never carries a copy of it.**  `tools/rebuild-pack` gained **`-o PATH`** (write the result elsewhere, tree untouched; a relative path resolves against the CALLER's cwd because the script `cd`s to the repo root; the hand-written appendix still comes from the checked-in artifact, its only home), and mro/warnings go through `./pl2cl --extension` + `tools/tag-license` — the recipe in the file's own header.  A re-spelled recipe is free to drift from the recipe it is checking.
- **An artifact's emitted CL embeds THE SOURCE PATH AS SPELLED, so a regeneration check must run from the repo root with the recipe's relative path.**  `./pl2cl --extension /abs/…/lib/mro.pm` and `./pl2cl --extension lib/mro.pm` differ in two lines: a `die` in the module becomes the literal `"lib/mro.pm line 71"` in the output.  That is the only caller-dependent thing there is, and the answer is to run the recipe verbatim, NOT to normalise.
- **NOTHING else is normalised, and that is measured.**  `--extension` emits no program preamble (#349), so no absolute path reaches an artifact at all (`grep /home/ cl/*.lisp` = nothing; `Pl/t/extension-preamble-01.t` is the row that keeps it so, and #217's build-machine paths are gone).  Two regenerations of the same artifact are byte-equal.  A normaliser that is not needed is a place for a real drift to hide.
- **Rule 12 on the discovery loop**: an artifact the stamp loop finds and `%RECIPE` cannot classify DIES naming it (proved with a fourth stamped file — the three real rows still run and print first).  Silently not checking a new artifact is the hole the file exists to close.
- **The gate's wall does not move**: `prove --timer` on a quiet box, the file **25 ms → 3.56 s** against the gate's slowest file `Pl/t/autoviv-01.t` at **130.6 s** in the same run (2.7 %); under the loaded cold gate, 7.8 s against 399.8 s.  The task's fallback — moving the comparison out of the gate into `rebuild-pack`-style tooling or a WHAT-TO-RUN-WHEN row — is therefore NOT needed and was not implemented.
- Bars: cold gate **199 files / 6796 rows, only the 13 known pclxs xs rows fail**; `tools/corpus-diff.pl 9083d2e` **identical across 111 files, silent drops 5 unchanged**; no generation bump (a test and a tool changed, nothing under `Pl/`); no sweep, no companion (excluded by the brief).

## s469 (2026-09-04/05, Fable) — ROUND 24: BH (#1046 + #1056 + #1105 + #996 measured) MERGED ff `57969a2` gen v2-670; BG (#1035 steps 2–4 + the manifest follow-up) MERGED ff `600ed53` gen v2-680; BF + BI in flight; STOPPED for shutdown with a restart recipe (session-log §469)

- **A bench leg that "ran" is not a measurement until its log shows numbers**: s468's bench board died at `Permission denied` (invoked without `perl`) and the round closed calling it "not run".  s469 ran it first on the rebooted box; every row within noise of the README table, `arrhash-k` 1.14× the one row still slower than perl after #995.
- **The rename manifest must reach EVERY declaration form a renamed name is bound by**, not only `p-let`: a promoted cell (`p-defcell $x__file__0`) and a `p-raw-params` entry are declarations too, and a consumer that has to parse `__file__0` off the symbol is the rule-11 shape the manifest exists to remove.  Bar for the family: registrations == printed `:perl`/`:why` pairs, per family, over the corpus.
- **#1105 (BH) is the rule-12 archetype in the runtime: a missing case that stores NOTHING is an infinite loop, not a wrong number.**  `box-set` on a non-box returns the value by design; the incdec macros' default arm relied on it and a raw slot never moved.  One store-back decision for four macros; the emitter-side fix (ExprToCL cannot see verdicts) stays residue.
- **A5 before A3 (#996), by the rows BH added**: `push` on a local array is already 0.48× of perl; `sort { $a <=> $b }` is 7.3× SLOWER (ceiling −48 %), and `%p-collect-list` promotes every raw element of the source array to a box — monotone — so the ceiling is a floor on the prize.  A3 needs a new array-fact family in VarAnnotator (scalar-only by construction); A5 needs only a comparator shape.
- **README opening (USER)**: half the text, short sentences, two paragraphs; the SBCL explanation is the link's job; "perl is not involved at any point" was false (perl runs the compiler; string `eval` needs it at run time) and is now stated accurately.
- **Shutdown protocol (USER "set it up so we can restart")**: every in-flight agent commits finished work, leaves half-done diffs uncommitted, writes `scratch/<label>/STOP.md`; the worktrees are KEPT; the session log carries the relaunch recipe; the round-end legs are OWED and listed.

## s469bf (2026-09-05, Opus) — the re/ comp/ run/ half of the companion ROW DIFF attributed: 199 NEW / 74 FIXED / 84 UNVERIFIED / 2 LOST over TEN files and SIX causes, and the volatile class is now measured on the PERL ORACLE alone

- **A ROW-ALIGNMENT volatility can be measured WITHOUT PCL, in ten seconds a file: run perl's own `.t` three times and compare the TAP description SEQUENCES.**  `scratch/s469bf/perlorder.sh` + `ordercmp.pl`.  re/pat.t (3×1188 rows), re/pat_advanced.t (3×1675) and re/regexp_unicode_prop.t (3×1110) each give three different ORDERS with the SAME multiset, and the first divergence lands exactly on the block the row diff blamed (row 270 = `for (keys %ans)`; rows 29-32 = `each %d`; row 244 = `each %SHORT_PROPERTIES`).  This beats repeating the companion run: it measures the source, costs no SBCL, and answers the "at least THREE runs" bar of #1082 in one step.  TRAP paid for once: compare the multisets under `LC_ALL=C` — a locale-collating `sort | uniq -c` splits byte-identical descriptions across groups and reports a false difference.
- **Three more `*rows-unstable*` members (#1082): re/pat.t, re/pat_advanced.t, re/regexp_unicode_prop.t** — 1,438 rows leave the ROW gate, COUNTS still gate (231/138, 951/729, 778/332, each read three times and each equal to the snapshot).  Companion draws on ONE tree: 8/12, 8/6, 8/12-with-different-keys; 3/6, 3/6, **6/3** (the first two AGREED — the op/inc.t trap, again); 149/51, 80/38, 161/42.  The family is now SEVEN files / 2,070 rows, which is the number that should decide #1082's option (b) (`PERL_HASH_SEED=0` for the perl ORACLE child): none of the three new members is a test OF hash quality, so freezing the seed costs them nothing.  A third option is now named too — pair equal descriptions as a MULTISET in `align_taps` instead of positionally within a 20-row window — and carries #1051's bar (a before/after verdict diff of all 111 registered files).
- **The 500-row log cap is a NOISE SOURCE, not only a hole (#1051).**  A truncated file writes exactly 500 PAIRED rows (extras are appended after the cap and consume none of it), so a volatile block EARLY in the file moves the window's right edge: re/pat.t's last paired row 718 → 722, re/pat_advanced.t's 1186 → 1189, carrying four `(?il), utf8=…, locale=…` rows and three `((?^:\v))` rows in and out with nothing wrong with them.  That is why the registration has to be the whole file, and why #1051's option 3 (a digest beside the first 500) would not help these two.
- **A PER-RUN TOKEN IS A PER-RUN TOKEN WHICHEVER SIDE WROTE IT.**  run/cloexec.t's four rows moved every run from BOTH ends: perl's `tmp_<alpha(PID)>_<alpha(n)>` (normalised since s468be, but BE's leg never opened run/, so the baseline still carried `tmp_CQZP_C`) and **PCL's own** `/tmp/pcl-test-$$-$n` from `perl-tests/t/test.pl:343`, which nothing normalised.  One line added to `PclTapAlign::rowkey_desc` + 2 rows in `tools/t/tap-align.t` (23 → 25) and the four rows are blessed, 0/0 forever.  They still do not PAIR (the two stubs use different NAMES, so both assertions PASS and still count as four diverging rows) — **#1085** makes the stub answer perl's shape, a HARNESS change worth 16/6 → 18/4.
- **comp/require.t 909/835 → 910/837 is PRE-EXISTING, MULTI-STEP, and partly a SPEED effect — spliced by hand, and the bisect deliberately stopped after two steps.**  910/837 in five readings (s465az, s466, s468 parallel+serial, s469bf parallel+serial); the 909/835 it replaces was last confirmed at s396 (2026-08-15) and this QUICK-CAPPED file (450 s > the 120 s cap) is reached only by a full `--all`.  A `d00f893` (s440) worktree reads **906/836**, and a `2065b89` (round 19) worktree **TIMEOUTs at 868/795 inside the 450 s allowance** — so the file's counts are partly a function of how FAST the tree is, which makes a count bisect non-monotone.  The three rows that moved are past the 500-row cap and cannot be named without #1051.
- **A TIMEOUT file's rows can change BUCKET without changing at all.**  re/speed.t's 34 NEW rows in s468 exist only because that run's #366 serial re-run happened to finish (DIFF 25/34); on the same tree in s469bf it stayed TIMEOUT (25/0 ‖ 25/25) and the identical rows landed in the "no comparable TAP — noise" bucket.  Two consequences recorded on #326: the s468 SHORTFALL advisory `re/speed.t 4 -> 0 … EDIT the baseline row` must NOT be acted on (blessing 0 fails every future completing run), and the I4 stamp records the VERDICT OF THE RUN THAT STAMPED IT — for a hang-set member it answers *when*, never *what*.
- **A registered XDIFF row that starts PASSING is invisible to the ROW DIFF â look at the STALE column.**  run/fresh_perl.t read STALE 59/32 (snapshot 57/34) in s468 and again here, in no NEW/FIXED bucket, because `row_gated_here` keeps XDIFF files out of I1 so that one row has exactly one gate.  BISECTED to **#1032**: `1fed80b` 57/34 XDIFF with the sig **`unbound:H`**, `0e7c5e3` (after BA's #1028+#1032) 59/32 STALE with the same two rows, main identical.  The rows are that task's own two builtins â `open(H,â¦)` (the bare CL symbol that WAS the `unbound:H`) and `write REMITOUT` (t/run/fresh_perl.t:572-586).  BA's leg was `--dir io --dir op`, so run/ was never opened and a round-23 win reached main uncounted.  `--bless-rows` on the file alone: 2564 â 2562 rows, exactly the two.
- **io/open.t and io/pvbm.t confirmed NOT movers by running them ALONE**: 154/34 and 20/8, both exactly the snapshot and both **0 NEW / 0 FIXED** against the row baseline, against 153/35 and 21/7 in the 528-file `--all`.  The documented full-session flip (io/open.t's own s460ap caveat) and the standing flapper (DECIDED s437/s438).  Their 1 NEW + 1 FIXED + 1 of the 2 LOST rows are that, and nothing was blessed.

## s469bg (2026-09-05, Opus) — #1035 steps 2–4: the compiler's OWN FACTS on the declaration forms; a rename is explained WHEREVER it is declared; step 4 decided, measured and deferred

- **The rename manifest lives where the rename is MINTED, never in the suffix text.**  Six sites (`_rename_spanning_lexicals`, `_promote_captured`, `_state_rewrite_routes`, `_rename_state_vars`, `_rename_exception_mys`, `_gate_seam_my_shadow`) call ONE registrar `_reg_rename($old, $new, :WHY)` with a CLOSED reason set (`:spanning :captured :state-cell :exception-global :seam-shadow`; an unknown reason DIES), and ONE reader `_decl_facts` prints it back as `:perl "$x" :why :FAMILY`.  A printer that parsed `__family__N` off the symbol would be the shape rule 11 forbids — the compiler knows why it renamed, so it says so.
- **A RENAME HAS THREE CARRIERS, not one, and that is what the first attempt got wrong.**  A `my` binds a `p-let` entry; a signature parameter binds a `p-raw-params` entry; a PROMOTED lexical binds nothing — it publishes a package CELL.  The first version annotated `p-let` entries only, so `:captured` and `:spanning` — the two families the task named FIRST — reached emission as nothing but `$x__file__0` (caught by the s469bg merge review).  All three carriers now read the same `_decl_facts`, through ONE new printer `_decl_cell`, and **all eight Parser2 `global_decl_form` calls route through it** — routing every site rather than the ones judged able to see a rename is the point: a name with no manifest entry emits what it emitted before, so no family can be missed by a judgement call.  Exception-set names keep plain CL `defvar` (its lambda list would read a facts tail as a docstring), guarded on the declarer.
- **A NAME CAN BE RENAMED TWICE, and recording only the last link is worse than saying nothing.**  `perl-tests/aassign.t` renames `$a` → `$a__excl__6` (exception-set) → `$a__excl__6__file__2` (captured by a named sub), and the first version's `:perl` then named `"$a__excl__6"` — a spelling nowhere in the perl program.  `_reg_rename` FOLLOWS the chain: `:perl` is the source at its root, `:why` lists every reason in the order they applied.  **`:why` is ONE keyword for a single rename and a LIST for a chain** — self-describing shapes, so a consumer wanting one value takes the last.  The intermediate entry is MARKED superseded rather than deleted, so an audit can tell "nothing declares this" from "a hole".
- **THE ARITHMETIC IS THE BAR, and it is closed by MATCHING, not by counting.**  A temporary manifest-dump instrument (added, measured, removed; the removal byte-verified against a pre-instrument snapshot) let every registration be joined to a printed pair over the 111-file corpus: **283 registrations, 282 annotated, 0 unannotated** — `:exception-global` 130, `:captured` 87, `:state-cell` 43, `:spanning` 14, `:seam-shadow` 7, chained 1; 417 `:perl` tokens emitted (a name can be carried by both a cell and an entry).  The ONE absent is `state.t`'s `$spam__state__6`, **verified**: its `given (state $spam …)` statement is a RULED REFUSAL, so the declaration never reaches emission at all.  The first version's "129 = the count" was an arithmetic over `p-let` entries only — a count that closes over the wrong population closes over nothing.
- **`:captured t` is the CAPTURE, not the veto — under-reporting is the unsafe direction.**  `Pl::VarAnnotator`'s `nested_sub` reading is published as a `%vi` key and printed on the binding: the heap-vs-stack fact a target without native closures needs, and the one thing the emitted SHAPE never showed.  Narrowing it to the capture-plus-boxing-event veto would have missed read-only captures, which still make a binding outlive its frame.  Text-shaped, so it over-reports — the safe direction, stated in ir-spec §2b.2a.
- **A per-sub FACT is impossible to add until the three `p-sub` printers are ONE.**  Done first and byte-identically (its own commit: corpus-diff IDENTICAL, lib 22/22 SAME, no generation bump).  Then `p-sub` gained a facts plist at a FIXED position after the lambda list — **always printed, possibly `()`** (155 of the corpus's 661 print `()`, mostly `use constant`) — with six keys the compiler already proved and threw away: `:returns` (#77), `:wantarray-insensitive` (**true-only**: the walk answers 0 on any doubt), `:writes-args` (**both directions**: `_sub_writes_args` answers 1 on any doubt, so `nil` is a real proof that arguments may pass by value), `:string-eval`, `:captures`, `:prototype`.
- **Two sources had to be made to answer PER SUB.**  The file-wide eval scan lost its `last` early-out (the first `eval` the walk met is not a per-sub answer) and `_inside_named_sub` returns the NODE; `_captured_in_subs` gained an optional collector that suppresses its first-hit early-out, so the promotion records WHICH subs it promoted for — keyed by PPI refaddr, so two same-named subs in different packages cannot be confused.
- **Every parameter class in the corpus is `:scalar`, and that is a fact about the ANALYSIS, not a placeholder** — the B-regime freeze verdicts exclude parameters by construction, because a parameter's initial value is caller-bound and not a wrappable write.
- **It is all syntax, and the bar says so.**  `PCL_IR_PLAIN=1 tools/corpus-diff.pl 57969a2` IDENTICAL across 111 files, drops 5 unchanged; form counts identical on both trees (2 214 `p-let`, 661 `p-sub`, 27 `p-raw-params`); emitted bytes +0.91 %; compile time on `perl-tests/pack.t`, interleaved 5 rounds, **2.76 s → 2.79 s = +1.1 %** (budget 50 %).  The switch's ONE reading is `Pl::CLForm::ir_plain`, asked by both emitters.
- **`PCL_IR_PLAIN=1` output is for COMPARISON, not for RUNNING, and that is new since this step.**  The facts plist is a POSITIONAL slot, so the plist-less `p-sub` the switch prints binds its `facts` parameter to the first body form.  That is the price of an unambiguous lambda list, and it costs nothing — the switch's only consumer is a byte-diff against a tree that predates the step.  **The same property makes `tools/bench-emission-ab.pl` unusable here** (it times two emissions against ONE core; the base emission does not load) → **#1095**; the zero-cost evidence is macroexpansion equality instead, which is stronger: the compiled S-expression is the same form, so there is nothing to time.  Coarse per-tree wall A/B with its spread: sub-call-heavy 1.50 → 1.55 s against a base series spanning 1.39–1.66; loop 0.49 → 0.51 against 0.47–0.51 — both inside their own control.
- **The runtime's declaration vocabulary is ONE block, ahead of all three macros** (`*p-let-classes*`, `*p-let-fact-keys*`, `*p-sub-fact-keys*`, `%p-check-facts` — one checker for every plist — `%p-let-binding`, `%p-param-name`).  It had sat beside `p-let`, 2 600 lines BELOW `p-sub` and `p-raw-params`.  Every set closed; every violation an error at macroexpansion; expansions unchanged, proven by `macroexpand-1` EQUAL.
- **STEP 4 (statement source lines): the grammar is a COMMENT, `;; @ FILE:LINE`, not a `(p-line N)` form — and it needs its own step-0, so it is NOT implemented.**  A marker is a real form the host compiler must see and delete, and in TAIL position it would change a block's value; a comment cannot change what is evaluated, and ir-spec §9.2's pipeline marker is the precedent for a normative comment.  Measured: **39 717 PPI statements** over the corpus; 28 bytes a line = **+38.4 %** of emitted bytes; the CL reader costs **1.4 ms → 1.8 ms** per read of the largest emitted file at the upper bound of one marker per LINE — negligible beside the compile.  The blocker is structural: a per-statement marker must reach **71 return paths** (`_lower_block` 45, `_lower_stmt` 17, `_lower_compound` 8, `_lower_scope` 1) — there is no ONE statement printer, exactly the rule-11 shape that made step 0 necessary for declarations.  ir-spec §1 reconciled: one comment is normative today, this grammar is RESERVED as the second.
- Bars: gate **199 files / 6793 rows, only the 13 pclxs xs rows**; guard `Pl/t/decl-facts-01.t` (35 rows, 3 s wall) — one per family per carrier, the identity promotion that is correctly NOT a rename, the chain, `:captured` presence AND absence, and twelve rows proving an unknown class / key / stray atom DIES at macroexpansion; **28 shape rows in 10 `Pl/t` files rewritten as STRENGTHENING edits**; generation **v2-680**; the three artifacts regenerated on the rebased tree.  **No sweep-visible surface** — the expansion is identical, so nothing the sweep measures can move.  `PCL_CHECK_FACTS` (the optional item) NOT done; #1035 says what it still owes.  Residue filed: **#1095** (the bench instrument), **#1096** (the package-QUALIFIED twin of a spanning cell carries no facts — the same cell IS annotated at its primary declaration in the same file, so it is polish, not a hole).

## s469bh (2026-09-05, Opus) — #1046 the explicit-`return` bench rows; #1056 all eight access spellings classify their subscript; #1105 a value-consuming ++/-- on a raw slot stored NOTHING; #996 measured, designed, not shipped

- **A bench row that cannot see the transform it was written for is not a metric (#1046).**  Every sub in `tools/bench-exec.pl` ended in an IMPLICIT tail expression (`fib`) or in `return … if …` MODIFIERS (`gcdrec`), and neither shape reaches the `tail-return` transform — so #994's ~31 % was measured in a one-off scratch instrument that is now gone.  `fibret` / `gcdret` / `subret` / `methret` are the twins that can: on a quiet box, `PCL_OPT=-tail-return` moves them **+39.6 / +15.6 / +37.2 / +8.8 %** (perl control column agreed to 1.0–5.9 % between the two runs; pcl run-to-run spread ≤ 1.1 %).
- **`gcdret`'s SHAPE was chosen BY MEASUREMENT**: the obvious spelling (two `if (…) { return gcd… }` blocks and a trailing `return $x`) leaves the recursive returns NON-tail and the row reads −0.0 % under `-tail-return` — it measures the transform not firing.  A row must be able to see what it exists for.
- **ALL EIGHT access spellings now classify their subscript's USE (#1056), not just `h_acc`/`a_acc`.**  `%ACCESS_NODE` carries the use class as its VALUE (hash subscript → `strkey`, array subscript → `num`) instead of a bare 1, so the six deref and slice types stop falling through to the generic `opaque` tail: `$r->{$k}` gets the same B-regime string freeze `$h{$k}` gets, `$r->[$i]` the numeric one.  The BASE walk stays opaque (a deref root is written back by autovivification).  ONE table for the shape and the class — two tables is how they disagreed.
- **Emission over `perl-tests` is IDENTICAL, so the sweep CANNOT move and was not run**; the widening is visible only in the wide A/B: 111 corpus files identical (silent drops 5), lib 19 SAME / 1 DIFF, wide **1033 files 1027 SAME / 6 DIFF / 0 RCDIFF** — every diff ONE family (a slot whose only blocker was `write-shape`, now frozen because its use is a subscript through a deref or a slice: List::Util `shuffle`/`zip`/`mesh`, `t/re/pat_psycho.t`, `t/porting/test_testlist.t`, Test2 `Event.pm` + `API/Context.pm`, `TestML/Tiny.pm`).  Gate 198/6756 xs-only; generation **v2-670**, all three artifacts regenerated with bodies byte-identical to main's.
- **`use overload` anywhere in the FILE turns the whole B regime off** (`$host->{_overload_in_file}`) — which is why widening the freeze to `->{}` keys cannot meet an overloaded `""`.  Probed anyway with the freeze verified firing: a reference, undef, a number and a plain string as keys through both spellings, `"3abc"` as an index through both — identical to perl and under `PCL_OPT=none`.
- **A missing case that stores NOTHING is the rule-12 family, and this one was an INFINITE LOOP (#1105, PRE-EXISTING on `c80b1a0`, fixed s469bh).**  `box-set` on a non-box returns the value and stores nothing — deliberately (`undef = val` is a perl no-op) — and that is the DEFAULT arm of `p-pre++`/`p-post++`/`p-pre--`/`p-post--`, the arm a bare `$x` reaches.  On a raw-verdict slot the new value was computed and thrown away: `for (my $i = @a; -- $i;) { $t += $a[$i] }` never terminated, reproduced on a base worktree with byte-identical emission.
- **The hole is exactly one position: an incdec in a C-style for CONDITION**, either fixity.  Parser2's raw twin `p-incf-raw`/`p-decf-raw` fires only when the incdec IS the whole statement; everywhere else VarAnnotator records `write-incdec` and the slot stays boxed (measured with `PCL_B_DEBUG` over eleven shapes).  **The fix is ONE store-back decision shared by the four macros** (`%p-incdec-store-form`: a SYMBOL place tests `p-box-p` and SETFs when the slot is raw), not four emitter branches and not a new boxing event — an event would have made the two agree by BOXING the slot, i.e. by throwing the optimization away.  `Pl/ExprToCL.pm` cannot see the verdicts at all, which is why the emitter fix is the bigger one and stays as residue.
- **A guard whose bound is written on the accumulated VALUE hangs the gate instead of failing it.**  With the bug the counter never moves, the sum stays 0, and `last if $t > 99` never fires — measured: `raw-verdict-01.t` with a value-bound never finished on a `c80b1a0` worktree.  Every loop in the guard carries an INDEPENDENT iteration counter, and row 55 then fails there in 6 s with `0 168 14 14` against perl's `14 7 14 14`.
- **#996's own priority is the other way round, and only a bench row could say so.**  Three rows added first (`pushloc`, `sortnum`, `sortstr`): **`push` on a local array is already 0.48x — 2x FASTER than perl** (the catalogue's ~7x was measured before raw elements s455 and `%p-vpush`'s fast path #811 collected it), while **`sort { $a <=> $b }` is 7.3x SLOWER** and `cmp` 3.5x — the worst ratios on the board apart from the pack oracle.  Hand-written ceilings: sortnum −48.4 %, sortstr −40.4 %, pushloc −40.9 %, against byte-identical control pairs at +1.1 / −2.2 % in the same window.  **A5 is the Target-A item; A3 is the smaller prize AND the bigger job** — `Pl::VarAnnotator` is scalar-only by construction (`next unless $var =~ /^\$\w+$/`), so A3's escape licence is a new fact family over array names, while A5 needs only a comparator SHAPE.  Neither shipped; both designed in #996 with their correctness constraints.
- **A5's cost is not where the catalogue put it**: besides the per-comparison `catch :p-return`, the `*wantarray*` bind and two `make-p-box`, `%p-collect-list` PROMOTES every raw element of the source array to a box before the sort starts, and promotion is MONOTONE — a plain `sort { $a <=> $b } @a` makes `@a` pay box indirection on every later read forever.  The measured ceilings are a FLOOR on the prize, since they still go through that collector.

## s468 (2026-09-04, Fable) — ROUND 23 COMPLETE: BD (#995) + BC (#1037) + BE (the companion ROW DIFF attribution) MERGED ff; the prompts of a round now live in the worktrees

- **An agent's prompt is written to its worktree BEFORE launch** (`scratch/s468xx/prompt.md`, untracked, never committed).  s466 died with three agents in flight and s467 spent its morning reconstructing what each had been told; a session's transcript is not durable, a worktree's `scratch/` is.  The relaunch rule is now mechanical: a fresh Opus agent on the saved file, told to `git status` / `git log main..HEAD` first.  Same for leg logs (`scratch/s468xx/*.log`).
- **A stale guard is the ONLY thing that blocked #995, and the respell was a STRENGTHENING**: main's #1035 step 1 (`p-let`) had changed the declaration spelling after BD's guard was written; every row now asserts the CLASS the tree emits (`:scalar` for the raw slot, `:box` for the box), the two `unlike` rows are no longer vacuous, and the inverse guard is measured on a MAIN worktree (rows 1-4, 6-12, 14 fail there) because any base older than `f330e5f` fails every row for the wrong reason.  Standing rule: an inverse guard names a base that already has every spelling the guard depends on.
- **#995's perf claim re-measured on the quiet box after the rebase**: `arrhash-k` −21.7 / −24.3 / −22.3 % (median −22.3) against a byte-identical `feread` control at −0.3 % in the same window, via `tools/bench-emission-ab.pl` (one core, both emissions, interleaved).  Sweep TOTAL 18493 (+0), pack.t 5636/89 on the regenerated artifact, lib A/B 20 SAME / 2 DIFF (List::Util `shuffle`'s `$j`, Math::BigInt::Calc) / 0 RCDIFF.  Review residue filed on #1056: an lvalue `substr($h{$k},…) = …` still boxes the key (the root is a funcall, not an access node); `push @{$h{$k}}`, `local $h{$k}` and a foreach alias over an element are already raw (probed).
- **#1037's second rebase and the companion half**: gen v2-650, artifacts regenerated with bodies byte-identical to main's; gate 198/6743 xs-only; corpus-diff exactly `state.t` (nothing → 1310 lines with ONE `;; RULED REFUSAL:`), drops 5; wide A/B 999 SAME / 17 DIFF / 17 RCDIFF = the 16 refusal files + `t/bigmem/stack.t`, all transpile-FAIL → rc 0; gate-SET scan 638×2: exactly those 17 verdicts moved; sweep GATE clean, **TOTAL 18493 → 18581 (+88 = state.t entering by EDIT)**.  Sixteen companion files blessed per file (op/state.t XDIFF 88/4, op/tie_fetch_count.t DIFF 76/65, op/smartmatch.t XDIFF 55/295, op/coreamp.t DIFF 9/46, op/defer.t XDIFF 4/8, op/switch.t XDIFF 2/0, class/inherit.t XDIFF 0/1, class/threads.t NOTAP, nine class/*.t XDIFF 0/0); **two XDIFF registrations DE-REGISTERED after re-reading their rows** (op/coreamp.t's 46 `&CORE::` rows and op/tie_fetch_count.t's 65 `FETCH called just once` rows are not the refusal their reasons cited — now gated row by row), op/state.t's kept with a two-section reason (its 4 rows are computed `goto`, blessed by name).  Census 17/57 → **19/62** by two rows (t/class/inherit.t 1, t/op/coreamp.t 4) — the legal rise, a file that emitted nothing now emits CL and its drops become countable (s409); perl-tests population unchanged at 5.
- **The companion ROW DIFF is attributed row for row (BE)**: 178 NEW / 407 FIXED / 2 LOST over TEN files and FOUR causes — #1028 (op/bop.t 217 FIXED, uni/overload.t 6, both BISECTED on 1fed80b / 73edcff / 0e7c5e3 worktrees), #1032 (op/write.t 86/86 = one count gain read as two-sided churn through the 500-row log cap #1051; op/filetest.t 4 NEW → #1047; op/closure.t 2 FIXED), the volatile-key class (op/hash.t, op/inc.t, op/undef.t, op/utfhash.t — perl interpolates its own hash order into the DESCRIPTIONS; measured over four runs of ONE tree; registered `*rows-unstable*` in I1 with the opt-out named on every run, COUNTS still gate, #1082), and a PID-derived tempfile name (op/require_errors.t — normalised by perl's own `$::tempfile_regexp` in the ONE key projection `PclTapAlign::rowkey_desc`, beside the `TYPE(0x…)` rule).  The 2 LOST were load (io/crlf_through.t + io/through.t OK 942/0 serial).  **Two agreeing runs are not evidence of a stable row set** — op/inc.t was identical twice and drew a third pattern on the next run; measure at least three before registering or blessing a member of this family.  **A companion mover can hide from its own agent's leg**: op/closure.t 273/3 → 274/7 is +1 pass AND +4 fail, which a count comparison reads as a wash; read both columns.
- **The UNVERIFIED hole is one file plus a dir set**: comp/require.t (quick-capped, 450 s allowance) and 12,298 blessed rows in 59 files a dirs-limited run never opens (re/ 36 files 11,272 rows — `re/reg_posixcc.t` alone 7,130 — run/ 18, opbasic/ 3, base/ 1, cmd/ 1); only the full `--all` (I4) reaches them — see the round-end leg below.
- **Filed this round**: #1072 (`artifact-staleness-01.t` compares the `gen=` stamp only — on `9bee19b` main's own `cl/pcl-pack.lisp` body did not match main's own compiler; BD's regeneration fixed the instance, the hole stands), #1073 (op/coreamp.t's `File::Spec::Functions` abort + its `&CORE::` rows), #1074 (`STDOUT->binmode(...)` — no IO::Handle methods on a bareword handle), #1082 (the volatile-key class), #1083 (op/closure.t's three now-honest rows: PCL hands back the SAME code ref for a `use re 'eval'` / `s///ee` body but a DIFFERENT one for `eval "1"`), #1084 (`do FILE` re-parses through the INLINE-CODE path and dies on a non-ASCII byte the file path handles — one U+2019 in `t/op/closure_test.pl`'s comment is closure.t's whole remaining abort and shortfall).
- **Round-end legs on the final tree `a715608`**: cold gate 198/6743 xs-only; sweep GATE clean, TOTAL 18581 (+0), drops 5 = census; the full companion `--all --jobs 4 --bless-stamps` (13 stamps blessed at s468; op/list.t + op/pack.t still NEVER, #160) is **NOT ROW-CLEAN — 199 NEW / 74 FIXED / 84 UNVERIFIED / 2 LOST**, all in the dirs a limited run never opens (re/ above all: regexp_unicode_prop.t, pat.t, pat_advanced.t; io/open.t is its known flap; re/overload.t TIMEOUT; comp/require.t 910/837 is a REAL snapshot mover, first full measurement of the quick-capped file).  **NOTHING blessed** (a new row is explained, never re-blessed over); the attribution, by BE's method, is the NEXT SESSION's first item.  The bench board was NOT run (the USER stopped the round before it) and stays owed.
- **NEXT**: #1035 steps 2–4 (an Opus agent: the FACTS keys `:perl`/`:why`, `p-raw-params` classes, the `p-sub` facts plist); the correctness pool #1022 / #1020 / #1045 / #1084 / #1083; perf #1046 / #996 / the #1056 residues; #1072 (a body-comparing staleness gate) as a tools filler.

## s466bd (2026-09-03/04, Opus, round 23 resumed) — #995: the subscript of an element write is a READ

- **A write to a container ELEMENT writes the element; every subscript on the way down is an ordinary READ, and only the ROOT of the access chain can be a scalar the write reaches.**  `Pl::VarAnnotator`'s tree walk had this rule in the `=` arm only; `++`/`--`, the compound assigns, `=~`, the mutating builtins (chomp/read/tie/pos/4-arg substr/open), an `@_`-writing sub's argument and `\` all marked the WHOLE operand subtree, so `$h{$k}++` / `$h{$k} .= "x"` / `chomp $h{$k}` boxed the KEY and `$a[$i]++` the index.  ONE marker now, `_tw_mark_lvalue` (rule 11), over the whole access family `%ACCESS_NODE` = h_acc / a_acc / h_ref_acc / a_ref_acc / slice_h_acc / slice_a_acc / kv_slice_h_acc / kv_slice_a_acc, all shaped `[BASE, SUBSCRIPT...]`.
- **The root decides, and there are exactly three answers**: a plain `%h`/`@a` root writes NO scalar (`$h{$k}{$j} = 1` vivifies into `%h`'s own element box — so the earlier "mark every $scalar under the base" was over-marking, key AND base token); a SCALAR root (`$r->{A}[0]`, `$$r{k}`, `@{$r}{…}`) takes `write-deref-viv` and stays boxed because autovivification writes the new container BACK into its box (exists_sub.t t13); anything else keeps the whole subtree with the caller's own reason.
- **The perf claim is measured with a NEW instrument, because the old one is blind to it**: `tools/bench-exec.pl`'s `BENCH_RT_B` shares one transpile between its columns (runtime A/B), and a tree-vs-tree run of the whole tool carries a per-tree offset (byte-identical `feread` read −8 % across trees on a loaded box).  ONE core, both compilers' emissions of the same program, interleaved best-of-K, with emission identity printed per row — **`tools/bench-emission-ab.pl --ref REF PROG.pl N_BIG`**, new and checked in: **arrhash-k (the only bench row whose emission differs) −25.4 % median of five runs** (−17.2 … −35.9), against byte-identical controls spanning −5.7 … +3.2 % over eight runs.  Against perl the row goes 1.37–1.55x → 1.06–1.23x.  New bench row `arrhash-k`; the literal-key `arrhash` row could never see this shape.
- **Emission is IDENTICAL over the perl-tests corpus** (111 files + 6 shapes) — the raw-slot verdict's OTHER conditions (one `my`, no string eval in the region, raw-shaped writes) are what those files fail.  The change is visible only in the 1006-file A/B (987 SAME / 19 DIFF / 0 RCDIFF, all one family) and in the guards, so **`Pl/t/lvalue-root-01.t` IS the bar** (23 rows; rows 1-4, 6-12, 14 fail on 57848f3).  Gate 197/6719 xs-only; sweep GATE clean TOTAL 18493 (+0) drops 5 = census, SHORTFALL/CAUSES unchanged; `PCL_OPT=none` identical; generation v2-640.
- **Three PRE-EXISTING bugs found by the probes, all filed, none fixed here** (a runtime fix in the same session would have muddied the attribution of every sweep number): **#1058** — `my %h; $h{a}{b}++` leaves the element **undef** where perl says 1, because `p-gethash-box` returns a FRESH DETACHED box when its container is an undef box instead of vivifying through it (rule 12: a missing case producing a value the program consumes; `exists $h{a}` is 1, so it is invisible to `exists`/`keys`, and `$a[0]{k}++` crashes); **#1057** — a coercing compound assign to a nested element (`$h{a}{b} .= "x"`, `+=`) lowers the chain through the plain READ accessor `p-gethash` and dies with an SBCL type error, while `++`/`=`/`||=` work and `$h{a}[0] .= "x"` works but `$a[0]{k} .= "x"` does not; **#1056** — only h_acc/a_acc classify their subscript's USE, so a key read through `->{}` or a slice is `opaque` and cannot take the B-str freeze the same key gets under `$h{…}`.
- **A tie probe proved the rule rather than breaking it**: perl's `STORE` assigning `$_[1]` does not reach the caller's key variable — the key an element write receives is a copy, so a subscript really is only a read.


- **s468bd finish (2026-09-04): the only merge blocker was a STALE GUARD, and a guard goes stale by SPELLING.**  Main's #1035 step 1 (`f330e5f`) respelled every `my` binding `(p-let ((NAME CLASS INIT)) …)`; `Pl/t/lvalue-root-01.t` still grepped `(let ((` and failed 21/23 — with its two `unlike` rows passing VACUOUSLY, which is how a stale guard hides.  All 23 rows respelled and STRENGTHENED with the emitted class (probed per row: every raw row `:scalar`, every box row `:box`), the `unlike` rows made non-vacuous against `($k :box (make-p-box nil))`.  **An inverse guard's BASE must be a tree that already has the new spelling** — on 57848f3 every row fails for the WRONG reason — so the base is now a MAIN worktree, `9bee19b`, where rows 1-4, 6-12 and 14 fail.
- **Post-rebase bars, all met on `9bee19b`**: gate **197/6719** xs-only; `corpus-diff` IDENTICAL over 111 + 6 shapes with silent drops 5; emission A/B lib **20 SAME / 2 DIFF / 0 RCDIFF** and WIDE (1035 files = 22 lib + 605 perl t/ + 402 cpan + 6 shapes) **1015 SAME / 20 DIFF / 0 RCDIFF**, every diff read and all one family; sweep **GATE clean, TOTAL 18493 (+0)**, LOST empty, drops 5 = census, SHORTFALL +0, **pack.t OK 5636/89** on the regenerated artifact; `PCL_OPT=none` identical on 21 probes; companion `op/groups.t` OK 4/0 + two DIFFs with ROW DIFF 0/0/0/0 (`re/reg_mesg.t`'s sig text differs on MAIN too — the runtime is byte-identical to main but for the gen stamp).  Bench on a quiet box vs main: **arrhash-k −21.7 / −24.3 / −22.3 %** against a byte-identical `feread` control at **−0.3 %** in the same window.

## s466bc (2026-09-03, Opus, round 23) — #1037: a RULED REFUSAL is a STATEMENT-level event

- **A ruled refusal takes the drop form's shape** — normative in `docs/ir-spec.md` §9.3b.  The USER's s465 ruling: a construct PCL recognises and will not translate is an event AT A STATEMENT, so the file transpiles, the refused statement becomes a perl-shaped trappable run-time `die` at its own site, and every other statement runs.  **A refusal that aborted the whole transpile was a bug in the refusal, not a property of the feature**: `perl-tests/state.t` and `t/op/state.t` each lost ~160 rows to ONE `given` block.
- **The marker is `;; RULED REFUSAL:`, never `;; PARSE ERROR:`**, and the stderr announcement has its own verb, `PCL: refused statement at F line N: TEXT -- REASON`.  Both because the other strings are the drop CENSUS's key and the runners' `drops` column — and **a refusal is not a gap the census exists to shrink**: no fix to the term grammar will ever close one.
- **String-eval mode is unchanged, and that is the same #363 reasoning rather than an exception**: the emission is produced by the `pl2cl --server` subprocess and discarded on error, so nothing can carry a run-time form, while perl's own contract for `eval STRING` is that what does not compile sets `$@`.
- **TWO ROUTES, ONE BUILDER**: `Pl::Parser::refusal_statement_cl` + `announce_refusal`, reached by the drop classifier (`_ruled_refusal_for_drop`, the five Track A families) and by Parser2's `class NAME ;` site through `refused_statement_form`.  The second route had its own hand-spelled die until s466 — the drift rule 11 exists to prevent.  A prototype-extraction parse announces nothing, the gate the drop route has always had.
- **CLASSIFICATION of every other transpile-time refusal (step 2 of the task).**  **(a) statement-shaped → converted**: the five drop-site families + `class NAME ;`.  **(b) EXPRESSION-shaped → already right**: `Pl/ExprToCL.pm`'s four dies are caught by `_parse_expression*` and emitted as the drop form, whose unit is the enclosing statement.  **(c) NOT statement-shaped**, each with its reason now in its `not-supported.md` entry — the `our`-alias refusal (the unit is a REGION; a die at the declaration leaves the region compiled with the wrong variable identity, still reachable through a sub defined inside it), the oversized generated top-level form (the unit is a GENERATED form), the two string-eval refusals (the unit is the eval).  The `Parser2 TODO:` dies are compiler gaps, not ruled refusals.
- **A MODULE containing a refusal now LOADS** — the "module-mode increment" the flip dissolved, probed live: the module's other subs work and only a call into the refused sub dies, trappably, with `--module` staying silent (ruled s403 — the statement says it itself, when reached).
- Filed, all PRE-EXISTING silences: **#1053** (a stripped `format NAME = … .` disappears with NO announcement — PPI swallows the picture lines and every following statement into one Statement), **#1054** (`defer { … }` at END of file is lowered as a CALL to a sub named `defer`, body evaluated EAGERLY; add one statement after it and the same file refuses), **#1055** (the drop/refusal die stops the enclosing TOP-LEVEL form, and a file-level `my` makes that the rest of the file — `perl-tests/state.t`'s whole 78-row shortfall; worth a sizing pass over the census).
- Record: session log s466bc; guard `Pl/t/refusal-site-01.t` (19 rows, 13 fail on `57848f3`); generation **v2-630**.
- **s468bc finish (2026-09-04): rebased twice (main `9bee19b`, then BD's `0e0b0b9`), generation v2-650, artifacts regenerated with ALL THREE BODIES byte-identical to main's.**  Bars: gate **198/6743** xs-only; `corpus-diff` 1 of 111 (`state.t`, one `;; RULED REFUSAL:`, zero `;; PARSE ERROR:`), silent drops 5 unchanged, shapes identical; emission A/B **22 lib SAME** and **1016 files 999 SAME / 17 DIFF / 17 RCDIFF**, every RCDIFF transpile-FAIL → rc 0 (the s373 die-scan leg); `gate-set-scan` over both populations moves **exactly those 17 verdicts**; full sweep **GATE clean**, TOTAL **18581** (+88 vs main's 18493, `state.t` entering `pass-baseline.tsv` by EDIT), drops 5 = census, shortfall +0, LOST 0.
- **AN EXPECTED-DIVERGENCE REGISTRATION HAS TO BE RE-READ WHEN A FILE STOPS REFUSING WHOLESALE, and the reading is per ROW.**  While a file produced no TAP its registration was trivially all-or-nothing true; once it produces rows, the bar ("EVERY failing row explained by a blessed `not-supported.md` section") is a measurement.  Of the sixteen: **thirteen keep** their reason unchanged, **`op/state.t` keeps one with a rewritten TWO-section reason** (its 4 rows are `computed goto`, which `not-supported.md` blesses BY NAME — the bar permits two sections, it does not require one), and **`op/coreamp.t` + `op/tie_fetch_count.t` LOSE theirs** (46 `&CORE::` rows / 65 `FETCH called just once` rows, neither of them the refusal their reason cited).  The two de-registered files are ordinary DIFF files now, gated ROW BY ROW in `baselines/perl-suite-fails.tsv`; a partially-explained file stays UNEXPLAINED on purpose, because it still holds a fix target.
- **The census rise is the legal kind (ruled s409): 17 files / 57 drops → 19 / 62**, `t/class/inherit.t` 1 + `t/op/coreamp.t` 4.  A file that used to die at transpile now emits CL, so drops that were always there become COUNTABLE for the first time.  The perl-tests population is unchanged at 5, so the sweep's DROPS bucket does not move.
- **`op/write.t` is not this change's**, though it moved in the same window: its emission is byte-identical to main's, its snapshot row already reads 99/32, and its stale shortfall row belongs to #1032 (agent BE's).  `t/bigmem/stack.t` is an eighteenth refusal file outside both dir sets — noted, not blessed.
- Filed: **#1072** (`artifact-staleness-01.t` compares the `gen=` stamp only, so a checked-in artifact can be BODY-stale within its own generation — measured live on `9bee19b`, where main's `cl/pcl-pack.lisp` body did not match main's own compiler), **#1073** (`t/op/coreamp.t`: `File::Spec::Functions` aborts 15 forms; 46 `&CORE::` rows), **#1074** (`STDOUT->binmode(...)` — no IO::Handle methods on a bareword filehandle).

## s468be (2026-09-04, Opus) — the companion ROW DIFF ATTRIBUTED row for row: every one of 178 NEW / 407 FIXED / 2 LOST has a cause, and the volatile-key class gets the `*rows-unstable*` opt-out (#1082)

- **The diff s466 could not read is fully attributed.**  `baselines/perl-suite-fails.tsv` (I1) was blessed by AZ from a tree BEFORE BB's #994 (`73edcff`) and BA's #1028 + #1032 (`0e7c5e3`), so main's first companion runs reported 178 NEW / 407 FIXED / 500 UNVERIFIED / 2 LOST with nothing naming them.  **Ten files, four causes, no residue** — the table is in the session log.  Method: the three bisection runs (`1fed80b` / `73edcff` / `0e7c5e3` worktrees) first, then every file re-run ALONE on 9bee19b, twice, and the two s466 runs of ONE tree used as the volatility discriminator.
- **#1028 owns 223 FIXED rows** (op/bop.t 217 + uni/overload.t 6, the six "something xor itself is zeros"); **#1032 owns op/write.t's 86/86, op/filetest.t's 4 NEW (→ #1047) and op/closure.t's 2 FIXED**.  op/write.t's 86 NEW *and* 86 FIXED are one event: the file went 18/23 → 99/32, and its per-test log is **TRUNCATED at 500 of 537 diverging rows (#1051)**, so the window slid — a *count* gain reads as a two-sided *row* churn in any truncated file.
- **A COMPANION MOVER CAN HIDE FROM ITS OWN AGENT'S LEG.**  BA measured `--dir io --dir op` and reported "two movers, both mine"; op/closure.t was in that dir set and did not show, because its move is `273/3 → 274/7` — MORE failing rows *and* one more passing row, which the count comparison reads as a wash unless you read both columns.  uni/overload.t was simply outside the leg.  Both are spliced into `baselines/perl-suite-run.tsv` with their bisection.
- **THE ROW KEY IS NOT ALWAYS A KEY.**  Four files build their test descriptions out of perl's own hash state, which is randomised per process: op/hash.t ("uses >0 heads (6)", "has >0 array size (32)"), op/undef.t ("k1: delete" — the `X::DESTROY` order), op/utfhash.t ("with 3 keys, key of length 4" — and BOTH sides interpolate their own order, so the *pairing* moves), op/inc.t ("key '2' incremented correctly" — `keys %inc` order decides which row the aligner cannot pair).  Measured over four runs of ONE tree: op/hash.t drew 45/45, 57/57 and 46/46; op/utfhash.t's *diverging-row count* was 51 / 43 / 19.  **They are registered `*rows-unstable*`** — the same hand-placed, bless-surviving opt-out `perl-suite-expected-rows.tsv` has carried since s336 (mro/package_aliases_utf8.t) and s399 (op/assignwarn.t, the identical cause) — extended to I1 in ~20 lines, and NAMED on every run so it is never silent.  Their COUNTS still gate (snapshot + shortfall).  Task **#1082** carries the measurements and the two alternatives that were *not* taken (per-file description normalisers; `PERL_HASH_SEED=0` for the perl oracle only, the durable fix, a decision about the suite's contract).
- **TWO AGREEING RUNS ARE NOT EVIDENCE OF A STABLE ROW SET.**  op/inc.t was measured IDENTICAL in two consecutive runs and was on course to be blessed; the very next run produced a third draw (40 diverging rows instead of 36).  Before registering — or blessing — a member of this family, measure at least THREE.
- **The one normaliser that IS general was added**: t/test.pl's `tempfile()` names its files `tmp_` + the PID in base 26, and perl's own test.pl declares the shape as `$::tempfile_regexp = 'tmp_[A-Z]+_[A-Z]+'`.  op/require_errors.t's four rows moved every run on that alone; normalised, 0/0 forever.  It lives with the `TYPE(0x…)` rule in **one** function, `PclTapAlign::rowkey_desc` — the row-baseline KEY projection, beside the description PAIRING it is the baseline half of — read by the ONE reader both row baselines share.  Guards: `tools/t/tap-align.t` (+9 rows, incl. the negatives "a hex CONSTANT keeps its text" and "a word starting with tmp is not a tempfile"), `tools/t/audit-instruments.t` (+5, incl. a check over the REAL baseline that an opt-out entry never sits beside ordinary rows — which would be a silent no-op).
- **The 2 LOST were LOAD, measured not assumed**: io/crlf_through.t and io/through.t came back TIMEOUT in the s466 parallel run (snapshot OK 942/0) and **OK 942/0 on this tree run serially** — the documented behaviour of that pair (perl-suite-run.tsv's own header, s457/s438).  io/open.t (153/35 ↔ 154/34) and io/pvbm.t (20/8 ↔ 21/7) flapped in BOTH directions between the two runs and were deliberately NOT blessed.
- **The 500 UNVERIFIED is one file** — comp/require.t, QUICK-CAPPED (registered allowance 450 s > the 120 s quick cap).  A dirs-limited run additionally leaves **12,298 blessed rows in 59 files it never looked at** (re/ 36 files 11,272 rows — `re/reg_posixcc.t` alone 7,130 — plus run/ 18, opbasic/ 3, base/ 1, cmd/ 1); only the full `--all` form (I4) reaches them.
- **Filed**: **#1082** (the volatile-key class above), **#1083** (op/closure.t's three now-honest failures: the `format`/`write` row and two `isnt($s[0],$s[1])` closure-identity rows where PCL hands back the SAME code ref for a `use re 'eval'` or `s///ee` body but a DIFFERENT one for a plain `eval "1"` body — probed against perl 5.40.3), **#1084** (`do FILE` re-parses through the INLINE-CODE path and dies on a non-ASCII byte the file-mode path handles: one U+2019 in `t/op/closure_test.pl`'s comment is closure.t's whole remaining abort *and* its whole remaining shortfall; three-file reproducer in the task).

## s465ba (2026-09-03, Opus, round 23) — #1028: the bitwise MODE DECISION (the largest cluster in the sweep baseline) and #1032: a bareword filehandle is a NAME in a stat / filetest slot

- **#1028 — `& | ^ ~` ask perl's question ONCE, and the answer is not "is it a string".**  perl's `pp_bit_and` asks `SvNIOKp(left) || SvNIOKp(right)`; if neither operand CARRIES A NUMBER, `do_vop` stringifies both and operates byte by byte.  PCL asked `(and (stringp v) (not (looks-like-number v)))`, i.e. it sent every NON-string state to the NUMERIC side, so a reference, glob, `qr//`, blessed object or undef answered an address or 0 where perl answers the bit-string of its stringification — **229 blessed `perl-tests/bop.t` rows, 33 % of the whole baseline, behind one predicate and with no recorded cause**.  ONE classifier `%p-bitwise-operand-kind` (:NUMBER / :STRING / :REFERENCE) over the same box states `stringify-value` and `box-nv` enumerate; an unlisted state DIES naming its type (rule 12 — the arm it replaced answered :NUMBER for everything, i.e. turned an unknown value into 0).
- **THE THIRD VERDICT IS REAL: unary `~` and the binary trio disagree on exactly one state.**  `~ [1]` is the complemented ADDRESS while `[1] | "x"` is the bit-string op, because `pp_complement` takes its string branch only for an SV that HAS a PV and a reference has neither.  Measured over all 14 states, not deduced: `~ undef` and `~ *glob` are STRING, `~ \*glob` is NUMERIC.
- **TWO STATES THE PROBE CORRECTED, both would have been silent regressions**: CL **NIL is PCL's OTHER spelling of undef** (`%pcl-definedp` calls NIL and `*p-undef*` alike undefined, and `my ($a,$b);` fills the boxes with NIL) → :STRING; a first version called it PL_sv_no and `$u1 | $u2` printed 0 where perl prints `""`.  CL **T IS PL_sv_yes** (IOK) → :NUMBER; perl's FALSE reaches the runtime as the STRING `""` (`p-bool` answers 1/"").
- **The error half is the same dispatch**: a bit-STRING op whose operands hold a code point above 0xFF is FATAL in perl, in four wordings (`~.` says `string 1's complement (~)`), raised through `p-die` so it is perl-shaped and trappable, and it lives INSIDE the string path — `5 | "\x{100}"` is the numeric op and must not die.
- **NOT FIXED, deliberately → #1040**: a numeric-LOOKING string still takes the numeric side (`"12" & "10"` is `"10"` in perl, `"8"` here).  `looks-like-number` is PCL's stand-in for perl's per-SV NIOK flag, and removing it requires **`use feature 'bitwise'` to exist first** — under that pragma `&` is ALWAYS numeric and bop.t asserts exactly that for `"22" & "66"` (11 rows that pass today by accident of the heuristic).  #1040 also carries the `p-box-nv-ok` arm that would answer the `$!`-dualvar case exactly, and names the one consequence: under that pragma `"\x{100}" | "\xFF"` now DIES here where perl runs the numeric op (it was silently wrong before).
- **#1032 — the handle-slot contract is the RUNTIME's `%p-fh-arg`, and `p--t` was the sibling all along.**  `stat FH` / `-e FH` emitted the bare CL symbol, an UNBOUND VARIABLE that killed the whole program even for an OPEN handle (~810 companion rows: `t/op/stat_errors.t` 333, `t/op/write.t`'s abort + 477).  The task pointed at #452's compiler-side `_bareword_fh_p`, but eight builtins — `open close eof tell seek binmode fileno readline` and **`p--t`, itself a filetest** — already take the bare symbol and quote it in a runtime macro, and `%p-stat-arg` (the one funnel `stat` and `%p--path` share) has always accepted a symbol naming a handle.  So: 28 public names become macros over `%…-impl` bodies through one generator, `%define-fh-slot-op`.  RUNTIME ONLY — emission unchanged by construction, no generation bump.
- **TWO EXCLUSIONS, both load-bearing and both probed**: `_` is the stat-cache VARIABLE (`defvar _`), not a handle named "_"; and the `(pl-NAME)` arm is switched OFF for this family, because **in a READ slot perl CALLS a declared sub** (`sub SPATH {…} -e SPATH` is TRUE; `use constant CPATH => …; -e CPATH` reads the constant) and the filetest emitter ALREADY emits `(p--e (pl-SPATH))` — turning that call back into a handle name would have been a NEW silent wrong.  `open` keeps the arm ON: its first argument is a GLOB slot, not a read slot.
- **AN AUDIT OF ALL 44 HANDLE-TAKING BUILTINS found one more: `p-write`** (`scratch/audit-handle-slots.pl`).  Its docstring promises to return 1 "rather than crashing, so a stray write() call does not abort the whole program" — and it aborted anyway, on the ARGUMENT: `t/op/write.t` **18/23 → 99/32 (+81 rows)**, its abort moving from `The variable OUT is unbound` to the later, blessed `Undefined subroutine &main::formline`.  `p-select` is already quoted by the COMPILER and `p-print`/`p-say`/`p-printf` by the `:fh` marker; **`p-chdir` is deliberately LEFT** — quoting there would replace a loud "fchdir unimplemented" refusal with a silent chdir failure.
- **ir-spec §7.5's normative claim was WRONG and is corrected here.**  It said the builtin handle slots are ONE group in which "the bareword is always the handle even when a sub of the same name is declared", citing `sub FILE1 () {42}; tell FILE1` is `-1`.  Measured with the warning TEXT as the discriminator (`tell` answers −1 either way): **THREE rules** — a `()`-prototype sub (`use constant` included) is INLINED before any slot sees it, so that example's handle is named `42`; an ordinary sub in a GLOB slot does not displace the bareword (`tell SPATH` warns on the EMPTY name); an ordinary sub in an EXPR slot (`stat lstat -X chdir`) IS called.
- **THE RESIDUE IS MEASURED, NOT ASSUMED → #1044.**  `stat(CONSTANT)` now answers the handle-name reading where it used to CRASH — the same answer `eof(CONSTANT)`/`close(CONSTANT)` have always given (the paren-ARGUMENT path never asks the bareword classifier; the filetest OPERAND path does).  Population of the colliding shape across `perl-tests/`, `lib/`, `Pl/t/`, `cpan-tests/`, `examples/` **and perl's own t/** — 1452 files, 2412 bareword operand slots — is **ZERO** (all 33 raw hits are `_`, which is excluded, or a bareword inside a test description STRING).
- **`op/filetest.t` 184/250 → 180/254 is the s435 pattern, explained per row**: the file guards eight rows with `skip … unless eval { -T STDERR; 1 }`, and that eval DIED on `1fed80b` **of this very bug**, so four MANUFACTURED passes became honest failures (row total unchanged at 434).  Their two real causes are probed and filed as **#1047**: the `_` cache carries no stat KIND (`-l _` after a `stat` never dies), and a stat through an IO ref (`stat *$fh{IO}`) does not fill the cache at all.  **`op/stat_errors.t` did NOT move (305/333)** — the task attributed its 333 rows here; they are the errno half (#1033), so the ~810-row estimate was really ~495.
- **Filed from the probes, all PRE-EXISTING and all verified on a `1fed80b` worktree**: **#1041** (`sweep-diff.pl` keys a row on file+DESCRIPTION, so bop.t's 14 unnamed failing rows collapse onto ONE baseline entry and 13 are ungated), **#1042** (`$^T` is frozen into the SAVED CORE, so `-M`/`-A`/`-C` answer a negative age that GROWS with the core's age — 184 s on one tree, 2831 s on an older one, 0 in perl), **#1043** (`stat`/`lstat` in SCALAR context answer the 13-element list, not perl's 1 — `my $n = stat($f)` is `ARRAY(0x1)` here), **#1044** above, **#1047** above, **#1040** above.
- **Bars (on the tree REBASED onto `d9faf2b`, i.e. with s465bb's #994 underneath)**: gate **196 files / 6696 tests**, only the 13 pclxs xs rows failing; full sweep GATE clean, **TOTAL 18266 → 18493 (+227)**, 0 new / 0 fixed, drops 5 = census, and **bop.t is the ONLY mover in 108 files** (per-file `_status.tsv` diffed against a `1fed80b` worktree's own run); re-run against the edited baselines and again after the rebase, TOTAL 18493 (+0) both times; companion `--dir io --dir op --quick --jobs 4` = **265 files, TWO real moves, both mine** (op/bop.t 252/245 → 469/28, op/filetest.t 184/250 → 180/254; the two io/ TIMEOUTs re-ran serial and match).  Baselines edited ROW BY ROW: 224 rows leave `fail-baseline.tsv` by EDIT (708 → 484 lines, every non-bop.t line verified byte-identical AND in order), `pass-baseline.tsv`'s bop.t row 253/256 → 480/29 with its cause, `perl-suite-run.tsv` spliced three rows (1642 lines unchanged).  Guards `Pl/t/bitwise-string-01.t` (74 rows, 1.5 s, **54 of 74 FAIL on `1fed80b`**) and `Pl/t/bareword-fh-slot-01.t` (55 rows, 0.8 s, **55 of 55 FAIL there** — the program dies at the first `stat(FH)` and produces no rows at all).  Both changes are RUNTIME ONLY: no `Pl/` file touched, so no generation bump, no artifact regeneration, and corpus-diff / emission-ab cannot move.
## s465bb (2026-09-03, Opus, round 23) — a TAIL `return` does not throw (#994)

- **A `return EXPR` that IS a sub body's LAST statement lowers as the tail expression** — Kind-A emission `tail-return` (`Pl/Passes.pm`).  Spellings: `(p-tail-value X)` (single-statement body), `(p-caller-ctx (p-tail-value X))` (multi-statement `:void` regime), `(p-return-empty)` (bare `return;`).  Everything else still throws: a statement modifier, a multi-element `return (A,B)` (v1 seam), a `return` nested in a compound/loop, and a `return` inside `eval { }` or a sort comparator (a different `catch`).  Normative: `docs/ir-spec.md` §5.3; guard `Pl/t/tail-return-01.t`; record: session log s465bb.
- **THE ELISION IS NOT "drop `p-return-value`".**  Two of its arms fire on a value the frame exit (`%p-leavesub`) does not repeat — an ARRAY in scalar context is its element COUNT, raw `nil` in list context is the empty list — and dropping them printed `ARRAY(0x1)` for `(return @a) . ""` where perl prints 3.  `p-tail-value` is that call with its IDENTITY fast path inlined: *a value that is neither raw nil nor a raw non-string vector comes back unchanged* (every BOX arm is re-applied by `%p-leavesub`, so a box may pass through).  If `p-return-value` grows an arm firing on anything else, that test must grow with it.
- **The array→count rule may NOT move into `%p-leavesub`.**  It reads `*wantarray*`, and the `insensitive-call` Kind-A gate deliberately omits the context bind for subs whose returns are provably scalar-rooted — a VALUE rule there would be right only by accident.  (It would also miss the no-sub spelling; the real bug is #1039, the string coercion.)
- **Method for "the shape is everywhere, explain every diff":** re-transpile the populations with the Kind-A name switched OFF and byte-compare against the base worktree.  s465bb: perl-tests SAME 111 / DIFF 0, lib SAME 22 / DIFF 0, SHAPES SAME 6 / DIFF 0, 80 CPAN modules SAME 80 / DIFF 0 — which proves the general form is untouched and the whole corpus-diff is the gate.
- **#1039 (PRE-EXISTING silent wrong)**: a raw ARRAY in STRING context stringifies as `ARRAY(0x1)`, not its count — `my @a=(7,8,9); print @a . ""` is 3 in perl; `lc`/`length`/`eq` follow the wrong answer.  Identical on base and under `PCL_OPT=none`.  Fix belongs in the string coercion, not the frame exit.
- **#995 MEASURED (not implemented): `++` on a hash/array ELEMENT is attributed as a WRITE to the KEY.**  `Pl/VarAnnotator.pm`'s prefix/postfix-op arm `_tw_mark`s the WHOLE operand subtree as `write-incdec`, so `$h{$k}++` boxes `$k` (`B-DEBUG $k unboxable=0 reasons=[write-incdec] uses={strkey=1}`; the same program with `$h{$k} = 1` is `unboxable=1` and emits a RAW slot).  That ONE event is #995's whole 15.4 % `%make-p-box` + 8.1 % `box-set`; neither candidate fix the task proposed (raw-string slot / box reuse) is needed.  Fix = mark the lvalue PATH, not the subscript/key subtrees; it is a boxing-verdict WIDENING, so sweep-as-gate + gate-SET scan over both populations.  Measurement, located line and acceptance list are in task #995.
...
- **FOUR USER RULINGS on the audit plan (§5), taken as recommended (2026-09-02):** (1) **budget = INTERLEAVED, standing slot** — phase 0 (the four instruments) as ONE dedicated Opus agent now; afterwards every round = one perf agent + one AUDIT agent + one correctness agent (the s452 shape with the audit as the standing correctness slot).  (2) **the Unicode case-fold / property-table family is OUT OF SCOPE for v0.2 as a CLASS** (~370,000 perl rows; `not-supported.md` "Unicode case folding and property tables"; owner **#1036**, post-v0.2); phase 1 still reads each member's FIRST error.  (3) **`perl-tests/` refresh = WHOLESALE from 5.40.3, re-blessed ROW BY ROW**, principle-9 edits kept as a LISTED set, the six longer-than-upstream files separated first — **#1038** (phase 4, absorbs #965's remainder).  (4) **STANDING RULE: a ruled refusal is STATEMENT-level** — a perl-shaped trappable die at the site (the drop form's shape, ir-spec §9.3); a whole FILE lost to one refused statement is a BUG — **#1037** (given/when first: state.t's 157 rows).  Decision §5.4 (phase-3 reading budget + one-representative blessing) is DEFERRED to the phase-3 launch.
- **ROUND 23 LAUNCHED (three Opus agents, worktrees from the session-start commit, told to rebase onto `1fed80b` first): AZ = phase 0 instruments I1–I4 (runner/baseline work only); BA = #1028 (bitwise ops numify non-plain operands; ONE dispatch, rule-12 read) → #1032 (bareword handle in a stat/filetest slot; ONE predicate, #452's family) → #1037 if budget; BB = #994 (`tail-return` Kind-A emission, gen v2-610) → #995 measured.  BA's gen string v2-600.  Fable does the records + the `not-supported.md` class section; #1035 (export the compiler's facts into the IR — the part (a) of the #1034 pair, USER-confirmed intent: "annotate what variables are used for … in case someone wants to compile to C/C++") waits for the round to merge (emission collision).**
- **BB (#994) REVIEWED + MERGED ff as main `73edcff` (2026-09-03), gen v2-610.**  Fable legs: diff read (40 lines Parser2 + the `tail-return` registry line + `p-tail-value` / `p-return-empty`); own 40-row battery vs perl 5.40.3 on default / `PCL_OPT=none` / `-tail-return` — identical except two PRE-EXISTING rows: #1039 (`"s" . @a` → `ARRAY(0x3)`, BB's filing) and **#1045** (a sub ending in `goto &ra` is classified context-INSENSITIVE, so `my @x = g()` gets the COUNT; right under `-insensitive-call`); gate on BB's tree 194/6567 xs-only; sweep on the merged main TOTAL 18266 (+0), GATE clean, drops 5 = census.  BB's own bars stand (corpus-diff 31/111 all attributed by the `-tail-return` byte-compare over four populations; emission-ab RCDIFF 0; companion op/ 221 files zero movers; bench K=5 with a ±6.2 % control: fibret 0.698 → 0.478 s, −31.5 %; subret/methret/gcdret −8 to −11 %; the implicit-tail rows unchanged).  **Ruling recorded from BB: the array→count rule may NOT move into `%p-leavesub`** (it reads `*wantarray*`, which `insensitive-call` deliberately does not bind).  Bench-alone on the final tree = round end.  **Two lessons:** (1) BB COMMITTED its 51 `scratch/` files — `scratch/` is untracked and NOT gitignored; dropped with `git rm -r --cached` + `--amend` before the ff (copy: scratchpad `p994/scratch-bb`); every agent prompt now says scratch/ is never committed; (2) the standing bench rows were BLIND to the transform (fib's tail is implicit) — **#1046** adds the explicit-return rows.  #995 MEASURED by BB and left in the task: `$h{$k}++` marks `$k` as `write-incdec` (the whole operand subtree), boxing the KEY — one false positive, a verdict widening (sweep-as-gate + gate-SET), not the two fixes the task proposed.
- **BA (#1028 + #1032) REVIEWED + MERGED ff as main `0e7c5e3` (2026-09-03), runtime-only, four commits REWRITTEN without `scratch/` (filter-branch; the second agent this round to commit scratch — the rewrite also deletes the files from the worktree's disk, copy first).**  Fable legs: diff read (`%p-bitwise-operand-kind` = ONE classifier over the enumerated box states with a rule-12 die, `~` reading :reference as :number — measured; `%define-fh-slot-op` = stat/lstat/26 filetests/write as macros over `%…-impl` bodies through the EXISTING `%p-fh-arg` contract with the call-form arm OFF and `_` excluded; the ir-spec §7.5 "one group" claim corrected to glob slots vs EXPR slots with constants inlined first); the 224-row `fail-baseline.tsv` edit verified: non-bop rows byte-identical and in order (708 → 484); scan for function-position uses of the new macro names before their definition: none; gate on BA's tree 196/6696 xs-only; sweep on the merged main **TOTAL 18493 (+0 vs BA's baselines; +227 vs 18266)**, GATE clean, drops 5 = census, bop.t 480/29 (still PARTIAL at 509/510 — the `formline` child drop).  Probe batteries: bit.pl 26 rows → 4 diffs vs perl (main: 16), fh.pl 20 rows → 4 diffs (main: 20, the file died) — every remaining diff PRE-EXISTING and now owned: **#1040** (numeric-looking string stays numeric; `use feature 'bitwise'` making `|` always-numeric is in its scope), ref-address LENGTH (PCL's `ARRAY(0x1)` is shorter than perl's — the documented ref-string divergence), **#1050** (perl's FALSE is a dualvar with IV 0, PCL's is `""` — `(1==0) | "abc"` is 0 in perl, `abc` in PCL), **#1048** (a glob VALUE / glob REF / DIRHANDLE / std stream in a filetest slot answers false; perl fstat()s the handle), **#1049** (a STRING naming an open handle resolves to the handle; perl: a string is a PATH).  BA's own bars stand (companion io/+op/ 265 files, two movers both its own: op/write.t 18/23 → 99/32, op/filetest.t 184/250 → 180/254 = four manufactured passes made honest → **#1047**; op/stat_errors.t did NOT move — its 333 rows are #1033's errno half, the task's "~810 rows" was ~495).  BA filed #1040–#1044 + #1047 and did NOT start #1037 (measured: two files, ~323 rows).
- **SESSION END (USER: out of tokens; 2026-09-03): ALL THREE AGENTS STOPPED by TaskStop, worktrees KEPT.**  State at the stop: **AZ** (`agent-a72791da66f5e296c`) — phase 0 instruments I1–I4 DONE and COMMITTED, four commits on top of main `c9398d9`, clean tree, HEAD `542e878` (its last commit re-merges fail-baseline.tsv against main's post-#1028 row set); it was about to re-run the companion `--all` on the rebased tree.  **NEXT SESSION'S FIRST JOB = the AZ merge review** (read the diff of the runners + baselines + tools/t; verify the row-level bless against the count snapshot per file; gate + sweep on its tree; the companion `--all --jobs 4` it did not get to run; then ff + records + the I4 CLAUDE.md line it proposes in its session-log section).  **BC** (`agent-a9ca34a71273d52ef`, #1037) and **BD** (`agent-a474d73ddc7aa408f`, #995) had NO commits — each has ONE uncommitted file (Pl/Parser.pm; Pl/VarAnnotator.pm), minutes of work: resume as FRESH Opus agents in the kept worktrees (`git diff` first) or restart from the prompts in the in-flight memory.  Bench-alone leg on the final tree still owed for BB/BA (#994/#1028/#1032).
- **#1035 DESIGN DIRECTION (USER 2026-09-03): the class goes ON THE DECLARATION FORM** — reclaim the dead legacy `p-let` as `(p-let ((NAME CLASS INIT …facts)) body)` with a CLOSED class set (:box :num :str :str-buffer :array :hash; unknown DIES), the rename manifest as `:perl`/`:why` keys on the entry, per-name classes on `p-raw-params`, a facts plist on `p-sub`; expansion = today's `let`, zero runtime cost; the separate `p-facts` block idea is SUPERSEDED for variables.  Cost measured: ~17 Pl/t files / ~70 shape rows keyed on `(let ((`.
- **#1034 REWRITTEN as part (b) — the USER-supplied hints; two spellings recorded**: an ATTRIBUTE (`my $i :int`, valid perl syntax through a `MODIFY_SCALAR_ATTRIBUTES` shim) or DISTINCT DECLARATORS (`int $i = 0`, the USER's suggestion 2026-09-02 — reads best; real perl needs a source-filter shim; `int` is a builtin and `Int $i` is indirect-object syntax, so the declarator set comes from the shim and PCL recognises it generically).  Spelling = a USER decision when scheduled; both lower to #1035's fact vocabulary flagged `:declared`.

## s466 (2026-09-03, Fable) — s465az REVIEWED + MERGED (main `57848f3`); BC + BD relaunched; #1035 steps 0 + 1: `p-let` carries the declaration CLASS

- **s465az APPROVED with three review fixes**: the shortfall definition comment said `planned - (pass+fail+skip)` in four places while the code, the unit test and the blessed 12,257 COUNT skips (`planned - (pass+fail)`); `eq 'UNEXPLAINED'` vs `/^UNEXPLAINED/` on the two sides; the UNEXPLAINED total recounted from the blessed file — **82,666 rows / 183 files** (70,409 companion / 132 + 12,257 sweep / 51), not the report's 86,126 / 185, which did not add up to its own 459,454.  **Standing: a quoted count must add up to the record's total — recompute from the blessed FILE.**
- **I4 CADENCE (CLAUDE.md, the companion paragraph): the full `--all --jobs 4` companion AT LEAST ONCE PER ROUND, as `PCL_SESSION=sNNN tools/run-perl-suite.pl --all --jobs 4 --bless-stamps`**; a stamp older than the current round is the reason to run it; a NEW ROW in the ROW DIFF is explained or fixed, never re-blessed over.  Bars on the merged tree: gate 196/6696 xs-only; sweep TOTAL 18493 (+0), shortfall +0, 108/108 files identical in verdict across the runner change; companion `--all --jobs 4 --bless-stamps` 528 files — **the first ROW DIFF found four instrument bugs (fixed, `e660d12`): a stale per-file log after a serial re-run that turns OK (342 phantom NEW rows), an address-bearing description key (`TYPE(0x…)` → `TYPE(0xADDR)` in the ONE reader), a TIMEOUT file's shortfall is a load reading (excluded), the per-file breakdown leads the report**; the real movers (op/bop.t 217 FIXED = #1028, op/filetest.t 4 NEW = #1032, op/write.t's 86/86 window shift = #1051, op/hash.t's 57/57 volatile descriptions, op/closure.t / uni/overload.t / op/utfhash.t / io/open.t / io/pvbm.t to bisect) go to agent BE for attribution + PER-FILE blesses.  **Standing: NEVER ff-merge a compiler/runtime change into main while a runner is measuring main** (the `p-let` merge landed under the validation run; its tail ran new emission on the old core and was killed and re-run).  **Standing (USER): Fable does not execute what an Opus agent can do from a written design** — #1035 steps 0+1 were done by Fable and should not have been; steps 2–4 go to an agent.
- **#1035 steps 0 + 1 SHIPPED: every perl `my` is printed by ONE printer (`Pl::Parser2::_decl_entry` / `_decl_let`, sixteen sites routed) as `(p-let ((NAME CLASS INIT)) …)`; the runtime `p-let` expands to exactly the old `let` (zero run-time cost, proven by macroexpansion); the class set is CLOSED (:box :scalar :num :str :str-buffer :array :hash — `:scalar` added for the plain unboxable verdict; unknown DIES at both ends); `PCL_IR_PLAIN=1` is the verification switch (not an optimization — the normaliser bar is `PCL_IR_PLAIN=1 tools/corpus-diff.pl <step-0-sha>` IDENTICAL); ir-spec §2b.2a normative; gen v2-611; artifacts regenerated; 68 shape rows in 10 Pl/t files strengthened with their class.**  Bars: step 0 byte-identical (corpus-diff IDENTICAL, lib 22/22 SAME); step 1 gate 196/6696 xs-only after two fixed rows, sweep TOTAL 18493 (+0).  Next: the FACTS keys (the rename manifest lands ON the entry — ir-review §4b.3 updated), `p-raw-params` classes, the `p-sub` facts plist.

## s465az (2026-09-03, Opus, round 23) — PHASE 0 of the ignored-tests audit: the four instruments I1–I4 BUILT and BLESSED (#993)

- **A blessed failing row carries a CAUSE; a cause-less row is QUEUE, not baseline.**  `baselines/fail-baseline.tsv` has a SIXTH column (task number / `not-supported.md` anchor / `UNEXPLAINED`) — **552 of 708 attributed, 156 not**, from `docs/blessed-fails-review-s464.md` §3 row by row (**#1028 owns 229 = 33 %**).  `sweep-diff.pl` joins on `(file, description)` as before, splits at SIX, prints the cause-less count every run, and `save` WARNS rather than silently erasing the column.  Reader: `docs/test-debugging-runbook.md` §4f.
- **"OK" never meant "the plan was produced" — the SHORTFALL is now an instrument** (I2, `baselines/row-shortfall.tsv`, SHARED by both runners through `tools/lib/PCLShortfall.pm`, keyed `perl-tests/<name>` / `t/<rel>` like the drop census, same contract: rows leave BY EDIT, MORE fails the run).  **459,454 rows over 260 files; 376,788 caused, 82,666 UNEXPLAINED in 183 files = the audit's queue (recounted from the blessed file in the s466 review; the run report had said 86,126)**, printed every run.  The sweep's oracle is the file's own `1..N` plan, the companion's is real perl's run; the sweep also splits the two halves — of its 12,257, **222 produced no TAP at all** (the PARTIAL class) and **12,035 were SKIPPED** (pack.t 8,997, lc.t 2,577).  A GROWN shortfall joins the #215 serial re-run.  Runbook §4c.
- **The companion's DIFF files are blessed BY ROW** (I1, `baselines/perl-suite-fails.tsv`, **21,097 rows / 267 files**), keyed `(rel, PERL's description)` as a MULTISET through the SAME reader as the #185 expected-rows baseline; XDIFF/FIXTURE files stay out — one row, one gate.  Buckets NEW ROW / FIXED ROW / UNVERIFIED / LOST.  **The 500-row log cap is announced in the log and counted in the report** (19 files; #1051).  Runbook §4d.
- **A never-run file carries the session that last measured it** (I4, `baselines/perl-suite-notrun-stamps.tsv`, 13 files, 2 NEVER); membership comes from the TABLES, not from a run's `%not_run`.  Runbook §4e.
- **Standing, from three bugs the instruments hit on their own first run: every registry answer comes from the state the run was MEASURED against** — the ROW DIFF's "NOT CHECKED" test read `-e` at report time and a `--bless` run writes the file first, so the first bless called all 20,097 of its own rows NEW.
- **Of 11 companion snapshot movers, 9 were LOAD** (they reproduced the snapshot alone).  The two that did not are **#1052** — reported, NOT spliced, each with its discriminating measurement: `comp/require.t` 909/835 → 910/837 (time-bounded at its 450 s allowance — the count may be a clock reading) and `re/overload.t` TIMEOUT 3/0 → 0/0 (a #326 hang-set file, and an OVERLOAD file one round after the overload work).
- Bars: gate 194/6567 xs-only; sweep 0 new / 0 fixed / 0 LOST, TOTAL 18266 (+0), drops 5 = census, and `sweep-diff.pl diff` exits 0 once the shortfall is blessed; companion `--all --jobs 4` 41 min 17 s / 528 files with both SNAPSHOT holes zero.  No compiler or runtime file touched, so no corpus-diff, gate-SET scan or generation bump.
...
- **AW, #972 (`fe93a89`)**: the ten bitwise keys (`& | ^ << >> ~ &. |. ^. ~.`) dispatch `use overload` through `%with-binary-overload` and a new `%with-unary-overload` (the runtime's only unary dispatch had been `neg`, hand-spelled); `nomethod` consulted with perl's FOURTH argument at every no-handler point incl. #934's `++` refusal (perl's own `t/lib/overload_nomethod.t` 0/3 → 3/3 — a file NOT in the companion scan set); the conversion chain `%p-conversion-handler` (own → derived `""`↔`0+`↔`bool` → nomethod; `fallback => 0` keeps PCL's ordinary answer until #960(a)) — **a blessed ref is no longer unconditionally TRUE** (perl: a class with `0+ => 0` is false); a comparison handler's return is the operator's VALUE (was truthified); the third handler argument is `""`, not undef (undef is perl's mutator marker); `<<=`/`>>=` delegate (they were a second copy of the shift WITHOUT the clamp: `$z <<= 70` → 5902958103587056517120, perl 0).  perl-tests/bop.t 464 passes; 465 fails differently (`'0'`, #1001 compound family).  **AW, #987 (`3d666ea`)**: `%p-leavesub` at `p-eval-block` and the string-eval catch (pp_leaveeval); `do { }` is not a frame.  Fable probe ovl.pl 37 rows: 33 identical to perl on both PCL_OPT paths; the 4 diffs are all "perl DIES, PCL runs on" = the declined #960(a).  Filed #1000–#1008 (#1002 `join` flattens a ref/object argument to nothing; #1003 `"$a$a"` one copy; #1004 `++` handler return stored; #1007 `$aryref x N` loses the ref; #1008 `do { $lexical }` no alias — all pre-existing).
- **AX, #985 (`209f7d2`)**: `%p-flatten-slice-args` answers (simple-vector, count) — the single-vector case is the argument's OWN backing store, nothing allocated; all eight callers walk `svref`; the two array-delete arms SNAPSHOT (the store is the live container — `delete @s[@s]` on `(1,2,0)` discriminates); the store is CAPACITY not length (a popped array).  **#982 (`300d47f`)**: `%p-fixnum-string` serves 0..1023 from a read-only shared table (every in-place string writer audited to copy first; a poisoned-entry probe proves the 16-route test can see sharing).  Interleaved saved-core wall A/B with control: `slices` +19.9 % over both commits (0.2036 → 0.1699 s, **3.02× → 2.48×**); `numstr-small` +20.7 %; other rows within their control.  Instrument left behind: `wallab.pl` (interleaved wall-time A/B of two saved cores — resolves 1 % where bench-exec on a shared box resolves 15 %).  #986 step 1 taken on a quiet box: the gap does NOT dissolve (machine-state arm killed; the round 15–18 bisect remains).  Filed #1010 (aliased slice element vivifies a missing key in perl), #1011 (`%a[-1]` index text), #1012 (a float of magnitude exactly 1e15 prints as integer text; perl `1e+15` — `stringify-value`'s LOG-derived exponent).
- **AY, the review (`c16aee0` `f21ce04` `ea34c0f`), record `docs/blessed-fails-review-s464.md`**: (0) **the #965 census was WRONG by 37 %** — `grep "ok(1, 'SKIP"` misses the double-quoted spelling: 210 statements in 14 files, not 132 in 11 (`sort.t` 32, `kvhslice.t` 25, `splice.t` 2 were invisible); a THIRD spelling `skip "… not supported in PCL", N` hides ≥ 21 more rows; **"fully passing" 62 → 58** — five files were PASS on manufactured rows (`sort.t` "204/205" is 182/22).  (1) 109 statements = 115 rows restored: **31 pass today** (fixes shipped and never counted — all seven `sort.t` "wantarray regression" rows, both `splice.t`, `sub.t`'s #964 row and its string-eval row), 57 registered with a verified reason, **11 had a FALSE written reason** (the #964 pattern: `sub.t` "@_ aliasing not supported" ×2, `loopctl.t` "foreach aliasing not supported — PCL copies" (flatly false), `each.t` "refaliasing not supported … removed in 5.40", `time.t` "no tzset" (passes), `local.t` "eval string not supported"), 14 held back as fix targets.  Nine of eleven files are now byte-identical to perl 5.40.3's; left: `state.t` 46 (transpile-fails whole), `lex.t` 38 (older extraction).  (2) three registry entries claimed "@_ elements are copies" — corrected.  (3) the 695 blessed rows in 128 clusters: 81 attributed (298 rows), 47 unexplained (397); **the biggest: `bop.t` 229 rows = 33 % of the whole baseline, ONE cause, no owner (#1028)**; #221 warnings ~62 rows; #233 caller 29.  (4) **the companion's t/op rows read individually for the first time (6201)** — `t/op/sort.t`'s failing rows are EXACTLY the 22 that appeared when `perl-tests/sort.t` was restored and `t/op/sub.t`'s exactly the 11: the two suites never differed, one copy had been edited and the other blessed as a number.  Sweep TOTAL 18346 → **18265 (−81)**, every row explained, both baselines row-edited; `kvhslice.t` OK → PARTIAL (39 planned / 38 produced — the manufactured rows had filled the hole instrument I2 is for).
- **THE FOURTEEN BUGS FILED BY THE REVIEW (all pre-existing, 12 re-probed by Fable vs perl 5.40.3 on main — every one reproduces)**, ranked as #964-class: **#1028** `undef | "abc"` → perl `abc`, PCL `0` — the bitwise ops NUMIFY a ref/glob/qr/object/undef operand where perl bit-strings it (229 sweep + 259 companion rows); **#1031** `-e $overloaded_path_object` → PCL no (237 of filetest.t's 252 rows); **#1032** `open(FH,…); stat(FH)` → "The variable FH is unbound" (~810 rows behind #452's predicate); **#1022** an unlabelled `last` inside a called sub does not exit the caller's loop (perl n=0, PCL n=300); **#1020** `undef *x` clears NOTHING, silently; **#1021** `sort @objects` ignores an overloaded `cmp`; **#1023** `scalar(%h{'c','d','e'})` → perl `E`, PCL `6`; **#1025** `s/X/join ':', 'a'..'e'/e` → perl `a:b:c:d:e`, PCL `1E0` (the range read as flip-flop); **#1030** `$ENV{X} = undef` leaves the value DEFINED; **#1029** `$SIG{INT} = "h"` stores `h`, perl `main::h`; **#1033** `-e "/no/such"` leaves `$!` 0 (perl ENOENT); **#1024** `%h{i}` bareword key called as a sub (crash); **#1026** `map {}` in an `s///e` replacement fails the transpile; **#1027** each.t 64/65 in-file only.  Ten more #964-class CANDIDATES ranked and unprobed in the record §4c (op/aassign.t evaluation order, op/array.t `@_`-to-nonexistent-index, op/repeat.t `\$_[0] == \$_[1]`, op/inc.t NV boundary, op/anonconst.t `:const` does not freeze the captured value — Fable probed: perl 1, PCL 2 — …).  USER items in the record §5: two stale `not-supported.md` entries (TZ/localtime; "context propagation into string eval" pointing at rows that diverge with no eval), the 9 `skip "not supported in PCL"` calls, `kvhslice.t`'s planned/produced hole.
- **Fable-filed perf findings from profiling two WINNING rows (USER: "there should be more optimizations where PCL already beats perl")**: **#994** a tail-position `return EXPR` is a THROW through the frame catch + p-return-value — ~10 % of fib(27) by direct A/B; **#995** a loop-body `my $k = EXPR` allocates a fresh box per iteration (15 % of arrhash in `%make-p-box` + 8 % box-set), `"k" . NUM` takes the slow concat path, `$h{$k}++` builds a box per element; **#996** the catalogue's A3 (`push` on a non-escaping local → `vector-push-extend`, ~7×) and A5 (classic sort comparators → native sort, ~6×) were never taken.  A profile lead that was FALSE: SBCL's type system in the fib profile was the program's own COMPILE (the profile included `load`).
- **Legs on the final tree `ea34c0f` (Fable):** gate 193 / 6557 (only the 13 pclxs xs rows); sweep TOTAL 18266 (+0 against the row-edited baselines: 18346 → 18265 by AY's honest restorations, +1 by AW's bop.t row), GATE clean, drops 5 = census; bench K=3 ALONE on the box, PCL absolute seconds main vs an 80b715c worktree: slices 0.2027 → 0.1687 (**3.01× → 2.54×**), ovlsub 0.1371 → 0.1356, arrhash 0.0815 → 0.0843, fib(27) 0.4152 → 0.4117, gcdrec 0.0944 → 0.0947, symref 0.0319 → 0.0319 — every row but slices inside the ±5 % band; Fable probes: ovl.pl 37 rows (AW) and ay.pl 16 rows (AY's findings re-probed, 12 reproduce on main), both PCL_OPT paths.  Records commit `be5c611` + this fix-up; CI to check next session.
- Filed this session in total: #987, #988, #993–#996, #1000–#1008, #1010–#1012, #1020–#1033.  Round-22 worktrees: AW/AX prunable; **AY's `scratch/` (128 probe files the filed tasks cite) is copied to the session scratchpad and the worktree is KEPT until the probes are re-homed.**


## s464aw (2026-09-02, Opus, round 22) — #987: AN EVAL IS A FRAME (the sibling of #964's sub-frame leave rule)

- **perl's `pp_leaveeval` mortal-copies the value an `eval { }` or a string `eval` leaves with, exactly as `pp_leavesub` does at a sub's exit** — and #964 deliberately scoped that out, so the eval frame still handed the caller the variable's own box and every aliasing consumer wrote through it.  EIGHT probe rows leaked, identically under `PCL_OPT=none`: `for (eval { $x }) { $_ = 5 }`, `\eval { $x }`, the two string-eval twins, `eval { @l }` in a foreach, `eval '@l'`, `eval { %h }`, and an `@_`-writing callee given `eval { $x }`.
- **The fix is the mechanism #964 already built, at two more sites and nothing else**: `%p-leavesub` at `p-eval-block`'s `catch :p-return` and at the string-eval's.  No new rule, no new helper — the vector-shape trap, the dualvar arm and the `:void` rule are all inherited, which is why the four traps the task listed cost nothing to clear.  The copy sits in the **`prog1`'s value position**, so `box-set $@ ""` still runs after the body and the error path still yields a bare `nil`.
- **`do { }` is NOT a frame and did not get one.**  `\do { $x }` IS `\$x` in perl (probed), and PCL's `do` has no `catch` of its own to hang a copy on, so nothing had to be excluded — the boundary held by construction.
- **A guard row for that boundary had to use `our`, and finding out why produced #1008**: `do { $lexical }` does not alias in PCL for ANY lexical kind (file, block or sub), only for a package variable — PRE-EXISTING, verified on an 80b715c worktree, and the OPPOSITE direction from #964/#987.  The discriminator is that the bare `for ($f) { $_ = 5 }` on the same file lexical DOES alias, so the aliasing machinery works and the `do` lowering drops it.
- **Bars:** gate **192/6540**; full sweep **GATE clean**, TOTAL 18347 (+0), drops 5 = census; companion op/ + comp/; the 23-row probe file identical to perl on BOTH emission paths (8 rows moved, 15 held — including the eval's VALUE, its scalar/list context, `$@` on the die path, a blessed return's `ref`, a `$!` dualvar, the empty list and undef); guard `Pl/t/return-copy-01.t` 6 → 8 rows, both inverse-verified FAILING on an 80b715c worktree.  Runtime-only: no generation bump.  `docs/ir-spec.md` §5.3 + §6.3 updated — §5.3's "Not frames" bullet named this task and now names `do { }` instead.

## s464aw (2026-09-02, Opus, round 22) — #972: THE THREE OVERLOAD PREREQUISITES of #960(a) (bitwise dispatch, `nomethod`, the conversion derivations)

- **The BITWISE family dispatches `use overload` like every other operator.**  `&` `|` `^` `<<` `>>` `~` and the string twins `&.` `|.` `^.` `~.` coerced with `to-number` / `p-string-bit-op` and never asked, so `bless([],"Baz") | "x"` answered the object's ADDRESS.  Ten keys, one macro each: the five binary ops and the three string-binary ops through `%with-binary-overload`, the two unary ones (`~`, `~.`) through a NEW `%with-unary-overload` — which is also where `%p-neg`'s hand-spelled `neg` lookup went, the only unary dispatch that existed and therefore the reason `~` had none to copy.  Acceptance: `perl-tests/bop.t` 464 / `t/op/bop.t` 466 `| overload works`, the ONE mover in either population.
- **`p-<<=` / `p->>=` were a SECOND COPY of the shift and the copies disagreed** — the compound form had no clamp, so `my $z=5; $z <<= 70` answered 5902958103587056517120 where perl and `5 << 70` both answer 0.  They delegate to `p-<<` / `p->>` now, which also gets them the dispatch.  The other eight compound ops are task **#1001**: `*=` `%=` `.=` `x=` reach NO overload dispatch at all (they inline instead of delegating), and the ones that do delegate report the BASE operator where perl names the compound one.
- **`nomethod` is consulted, with perl's FOURTH argument.**  `%p-nomethod-binary` sits at the fall-through of `%with-binary-overload`, `%with-unary-overload`, `%def-overloaded-cmp` and `%def-overloaded-str-cmp`, and at `%p-incdec-autogen`'s refusal.  **That last one is not cosmetic: #934's shipped `Operation "++"` death KILLED a `nomethod` class that perl runs** — perl calls the handler (op=`++`, other=undef, swapped=`''`) and DISCARDS its return, leaving the object in place.  perl's own `t/lib/overload_nomethod.t` goes **0/3 DIFF → 3/3 OK**.
- **`nomethod` is NOT reached where perl AUTOGENERATES**, and that distinction is a macro flag (`:autogenerated`), not a guess: `.` and `x` come from `""` (probed — a class with `nomethod` and `""` concatenates), the `== != < > <= >=` family from `<=>` and `eq ne lt gt le ge` from `cmp`.  Only `p-.` carries the flag; `x` has no dispatch at all (#1005).
- **perl's THIRD argument is `""`, not undef, for an ordinary call.**  It is three-valued and every state is observable: `""` = ordinary, `1` = swapped, `undef` = *this handler is being called as a MUTATOR* (a `+=`-family key), which PCL never produces.  PCL passed `*p-undef*` for not-swapped, so a handler asking `defined $_[2]` saw every call as a mutator call.  One token in `p-call-overload`.
- **A comparison handler's RETURN VALUE is the operator's value — perl does not truthify it.**  `use overload '==' => sub {"PQR"}` makes `$x == 2` answer PQR; PCL wrapped every arm in `p-true-p` + `p-bool` and handed back 1.  `p-bool` moved INWARD in both comparison macros, to the two arms that actually COMPUTE a boolean (the `<=>` / `cmp` derivation, and the ordinary comparison) — the derivation keeps it because there perl itself is comparing -1/0/1 against 0.
- **THE CONVERSION DERIVATIONS, and they are a fixed preference order (perl's `Perl_amagic_call`), measured one class per row:** `""` ← `0+` ← `bool`; `0+` ← `""` ← `bool`; `bool` ← `0+` ← `""`.  ONE reading, `%p-conversion-handler`, consumed by `box-sv` / `box-nv` / `%p-true-p-slow` — which had three bare `p-find-overload` calls and no derivation at all, so a `0+`-only class stringified to `N0=HASH(0x1)`.  `fallback => 0` forbids the derivation (perl dies there; PCL keeps the address form until the binary refusal lands with it).  Rule 12: a KEY that is not one of the three DIES naming itself.
- **The scary half is real and it IS perl: a blessed reference is no longer unconditionally TRUE.**  A class whose `0+` returns 0, or whose `""` returns `""`/`"0"`, is FALSE — and `0+` WINS over `""` (a class with `0+ => 0` and `"" => "yes"` is false).  Measured across the whole sweep + the whole companion: zero rows moved either way.
- **What #972 does NOT ship, each measured and filed rather than guessed:** the refusal itself (#960(a) — still 6 probe rows where perl dies and PCL runs); `use integer`, which emits CL primitives INLINE and so bypasses every operator's dispatch (**#1000**, a `Pl/` change); the compound family (**#1001**); the `=` copy constructor and `fallback => 0`-forbids-`.` (**#1006** — `nomethod` appears in ZERO of the 25 `use overload` files across all four populations, so the fallback walk that row needs would cost the `""` concat hot path for a case nothing measures).
- **Four PRE-EXISTING silent-wrongs found by the review probes, all verified identical on an 80b715c worktree:** **#1002** `join(",", $hashref_or_arrayref)` FLATTENS the reference away and yields empty (scalar refs are fine, `"$ref"` is fine — it is the argument flattening, and it hits every `join` over overloaded objects); **#1003** `"$a$a"` — the same box interpolated twice ADJACENTLY yields ONE copy (`"$a$b"` and `"$a $a"` are fine); **#1004** a `++`/`--` handler's return value is DISCARDED by perl (the handler is a mutator of `$_[0]`) and STORED by PCL, which declasses the object — the `%p-incdec-overload` comment asserts the opposite and is wrong; **#1007** `$aryref x 2` is LIST repetition and loses the reference, blessed or not (hash-refs and scalar-refs are fine).
- **Bars:** gate **192/6527** (only the 13 pclxs xs rows); full sweep **GATE clean, TOTAL 18346 → 18347 (+1)**, drops 5 = census, 0 new / 0 fixed; companion `--dir op` **221 files, ONE mover** (op/bop.t 251/246 → 252/245, attributed row by row on an 80b715c worktree — `| overload works` present in one `.fails.tsv` and absent from the other, nothing else in the file) and `--all --quick`; both baselines edited ROW BY ROW (the `| overload works` row leaves `fail-baseline` by EDIT; `|= doesn't break string overload` stays a fail with its observed value retagged 9 → 0, the `0+`-from-`""` derivation); guard `Pl/t/overload-01.t` 23 → 34, ten of the eleven new rows inverse-verified FAILING on an 80b715c worktree.  Runtime-only: no generation bump, no artifact regeneration, corpus cannot move.
- **A coverage hole named in passing:** perl's two overload test files live in `t/lib/`, which the companion's scan set does not include, so `overload_nomethod.t` had never been run — it was 0/3 on `main`.  Belongs with **#993**'s inventory.
## s464ax (Opus, round 22 PERF slot, 2026-09-02) — the slice-argument flattener answers a VECTOR (#985)
## s464ax (Opus, round 22 PERF slot, 2026-09-02) — the slice-argument flattener answers a VECTOR (#985) and small fixnums stringify out of a shared table (#982); `slices` 3.02× → ~2.5×

- **`%p-flatten-slice-args` returns a SIMPLE-VECTOR and a COUNT, never a list (#985).**  Every slice the corpus emits hands it ONE non-string vector (`@a[1..5]` = `(p-aslice @a (p-.. 1 5))`, `@h{@k}` = `(p-hslice %h @k)`), and the list shape made that case allocate twice — `coerce … 'list` plus LOOP `APPEND`'s copy, 4.3 % self of the `slices` bench row.  The single-vector answer is now the argument's OWN backing store (`%p-vec-data`), so nothing is allocated; the eight callers walk `0..COUNT-1` with `SVREF`.  **The COUNT is a second value because the store is the array's CAPACITY** — `my @k = (0..9); pop @k; pop @k;` is 8 entries in a 10-slot store, and reading `(LENGTH store)` slices TEN (inverse-guard measured: 10 of 11 probe rows change).  **The store can be a LIVE container**: reading it is safe (`%p-alias-aelem`'s promotion preserves the value), but the two array DELETES write the array they index by, so they snapshot on `(eq flat (%p-vec-data a))` — perl evaluates the index list onto the stack first; without it `delete @s[@s]` on `(1,2,0)` answered `2,1,U|3` where perl says `2,0,1|0`.  One caller cannot take two values (the list-assign LHS macro) and gets the thin wrapper `%p-slice-args-vector`.  **Measured −5.5 % on `slices` (3.02× → 2.78–2.92×), all four interleaved A/B runs outside a ±1.7 % byte-identical-runtime control band; every other row inside its band.**  The `&rest` list is NOT a cost (1 sample of 1308 — #985's second question, answered; no `dynamic-extent` added).  Record `docs/faster-codegen-suggestions.md` §0.2g; guard `Pl/t/slice-args-01.t` (15 rows, inverse-verified twice).
- **Two PRE-EXISTING divergences found by the #985 probe battery and FILED, not fixed** (both reproduce on `80b715c`): **#1010** — perl VIVIFIES a missing key when a hash slice's element is ALIASED (`f(@h{'a','zz'})`, `for (@h{'a','zz'}) {}` create `$h{zz}`; PCL does not); **#1011** — `%a[-1]` yields the index AS WRITTEN in perl (`-1,50`), PCL normalises it (`4,50`).
...
- **USER question: how common are `:lvalue` subs on CPAN?**  Measured locally: TWO real definitions in the installed perl 5.40.3 privlib + sitelib (989 .pm files) — core `JSON::PP::incr_text` and core `Thread::Queue::limit`; support code in Class::Method::Modifiers (it PRESERVES the attribute when wrapping); none new in `~/.cpan/build` (2432 .pm); zero in `cpan-tests/` (94 .pm).  A MetaCPAN-wide count (grep.metacpan.org API) was started and abandoned by the USER.  **RULED (USER): #930 is DEFERRED — "worth doing, but we can push it a bit forward"; #964 ships alone.**  Its switch is `p-sub-frame`: an `:lvalue` sub will be emitted with a frame that OMITS the copy.  Recorded in task #930.
- **THE RETURN PROTOCOL (task #964 DESIGN section; normative text `docs/ir-spec.md` §5.3):** an ordinary sub returns a COPY of every value it returns — perl's `pp_leavesub` — applied ONCE at the frame exit by `%p-leavesub` through the macro `p-sub-frame` = `(%p-leavesub (catch :p-return …))`, used by `p-sub` and EMITTED by the anon-sub wrapper (Parser2 ×2, Parser.pm ×1).  A plain scalar box UNBOXES (the raw value IS the copy — no allocation; **a sub's return is now routinely the raw `5`, not a box holding 5**, USER question); a ref / blessed / container box or a dualvar becomes a FRESH box; in list context a returned vector is copied SAME-SHAPE (array vs list-temp is semantic: a list temp's aggregate elements stay unflattened for the consumer — copying one into an adjustable vector killed every Moo role application, moo-01.t 15/15) with the ELEMENT rule per element — a box → the scalar rule, a nested aggregate copied same-shape too, a deferred `p-flatten-marker` rebuilt around the copied array (s464b, the Fable-review residue: `sub f { (0, @a) }` / `(@a, @b)` still aliased) — and a raw non-blessed hash-table copied as a table.  `:void` takes the scalar rule (PCL's void regime wraps a whole discarded statement, so an inner CONSUMED call sees `:void` — two probe rows fail otherwise).  NOT at `p-sort-cmp`, `p-eval-block` / string eval (perl copies there too — `pp_leaveeval` — **#987**, a sibling rule), `p-goto-sub`.  Measured on a8b4043 first: 24 of 38 probe rows leaked, identical under `PCL_OPT=none`; `return $str` aliased because p-return-value's container arm tested `(vectorp v)` and a CL string is a vector — an accident that was LOAD-BEARING for `$!` (a dualvar's numeric half lives on the box), now an explicit `%pcl-dualvar-p` arm.
- **Cost:** `%p-leavesub` inline, raw arm first — one struct typep + one special read per return.  Agent bench (all rows, best-of-5, WITH a byte-identical-runtime CONTROL A/B): the control itself reads symref ±14 %, intloop= ±5 %, so nothing smaller is readable; every named row inside that band; `ovlsub` ≈ −4 % has a mechanism (two blessed-box copies per iteration, perl's own cost).  No compile-time escape needed.  Fable spot-check on the merged tree, K=3, PCL absolute seconds main `4fd661b` vs an a8b4043 worktree, alone on the box: intloop+= 0.0225 / 0.0215, intloop= 0.0187 / 0.0199, fib(27) 0.4226 / 0.4101, gcdrec 0.0916 / 0.0956, ovlsub 0.1382 / 0.1353, symref 0.0311 / 0.0300 — mixed signs, every row within ±6 %, i.e. inside the control band; the ratios vs perl unchanged (fib 0.28×, ovlsub 3.3×, symref 1.3×).
- **Companion cost, ACCEPTED: 25 rows that passed BY ACCIDENT now fail honestly** — op/sub_lval.t 54/47 → 31/64, op/kvaslice.t 24/14 → 23/15, op/kvhslice.t 20/18 → 19/19, every one through an `: lvalue` sub (they "worked" only because every sub returned the box); spliced row by row with cause = #930, which owns their restoration.  The gain is the one row in either suite population that ever saw the bug: op/sub.t 53/12 → 54/11 ([perl #91844] "result of shift is copied when returned").
- **HOW #964 SURVIVED (USER: "how could such a big difference live so long despite all the tests?"):** its test existed TWICE and was neutralised both times — `perl-tests/sub.t`'s row replaced on 2026-05-19 (`fddd67b`) by an inline `ok(1, 'SKIP: @_ aliasing not supported …')` with the WRONG diagnosis, and the real suite's row blessed inside op/sub.t's COUNT; the Pl/t guard used a sub-local `my`, the one unobservable shape.  **`docs/plan-test-audit-s464.md` (task #993) is the inventory of everything the two suites do not run or do not READ, and the phased plan — to be PRESENTED s465 (USER instruction) after re-measuring on this tree.**  Headline numbers (a8b4043): companion — 2 files quarantined since 2026-08-02, 8 hang-set + 3 slow files only in full runs (last full run found: s398), **10 TRANSPILE-FAIL files producing ZERO rows every run (~2,030 perl rows: op/coresubs.t, op/lvref.t, op/svleak.t, op/for.t, op/goto.t, …)**, 273 DIFF files with 53,619 failing rows blessed as COUNTS (no per-row list); sweep — 132 inline SKIPs in 11 files (#965), 695 blessed fails with NO cause column, 14 PARTIAL files (~400 rows past the abort), state.t's 158 rows lost whole since s415, lc.t 82 of 2,659 planned rows and called OK, 43 of 105 extracted files stale vs 5.40.3 (`while.t` = a 4-test file vs 223 lines).
- **Legs on the final tree (Fable, main `4fd661b`):** gate 192 / 6527 (only the 13 pclxs xs rows); sweep TOTAL 18346 (+0) GATE clean, 0 new / 0 fixed, drops 5 = census; probe batteries leak.pl 38/38 + edge.pl 28/28 byte-identical to perl 5.40.3 on BOTH emission paths; companion op/ `--quick` 221 files zero movers (agent, s464b tree = final tree).  Guards: `Pl/t/return-copy-01.t` (40 rows in four programs + 2 shape rows, inverse-verified on a8b4043 AND on 551202b), `foreach-aliasing-01.t:103` de-vacuated, `parser2-02.t` two spelling rows.  Gen **v2-590**; the three artifacts regenerated.  Filed: #987, #988 (pre-existing: `p-flatten-args` spreads one level where `%p-flatten-list-general` recurses), #993.
...
## s464a (Opus, round 21, 2026-09-02) — THE SUB-FRAME LEAVE RULE: an ordinary sub returns a COPY (#964)

- **perl's `pp_leavesub` is now implemented, ONCE, at the frame exit.**  A non-lvalue sub returns a mortal COPY of every value it returns; PCL used to hand the tail expression's BOX out, so foreach, the `for` modifier, `map`/`grep`'s `$_`, `\f()` and an `@_`-writing callee all wrote through into the RETURNED VARIABLE.  24 of the 38 probe rows in the new `Pl/t/return-copy-01.t` leaked, identically under the default optimizer and `PCL_OPT=none` — the return PROTOCOL, not an optimizer regime.  Runtime: `%p-leavesub` + `%p-leavesub-scalar` / `%p-leavesub-list` / `%p-leavesub-vector`, reached through the macro `p-sub-frame` (= `(%p-leavesub (catch :p-return …))`) which `p-sub` uses and the anon-sub wrapper EMITS (Parser2 `_lower_embedded_anon` ×2, Parser.pm's v1 seam ×1).  NOT frames: `p-sort-cmp`, `p-eval-block` / string eval (→ **#987**), `p-goto-sub`.  Normative: `docs/ir-spec.md` §5.3, rewritten.
- **`:void` does NOT skip the copy — measured, not cautious.**  PCL's void regime binds `*wantarray*` to `:void` around a whole DISCARDED STATEMENT, so an inner call whose value IS consumed sees `:void` too: `my $r = \f(); $$r = 10` and `sub mod { $_[0] = 12 } mod(f())` both leaked when `:void` returned the value untouched.  `:void` takes the SCALAR rule.
- **A VECTOR'S SHAPE IS SEMANTIC — a copy must preserve it.**  PCL distinguishes an ARRAY (adjustable + fill pointer; slots are the values) from a LIST TEMP (a simple vector whose aggregate elements are still UNFLATTENED, expanded later by `%p-flatten-list`).  Copying a list temp into an adjustable vector told every consumer it was an array, a nested aggregate stopped being flattened, and `sub steps { qw(x), $_[0]->SUPER::steps }` yielded `ARRAY(0x1)` — the gate's whole `moo-01.t` (every Moo role application) died.  `%p-leavesub-vector` mirrors `array-has-fill-pointer-p` / `adjustable-array-p`.  **s464b:
the per-element rule is `%p-leavesub-element`, not the scalar rule** — a raw aggregate ELEMENT is COPIED same-shape too (copying is not flattening; the element stays one element and the consumer still flattens it).  The emitter writes an `@array` element of a comma list as `(p-flatten @a)`, a deferred-flatten MARKER around the LIVE array, so the copy reaches through the marker; without it `sub nest_a { (0, @a) }` and `sub two_arr { (@a, @b) }` wrote through a `foreach` alias into the callee's arrays (`return (@a, 9)` was already right — `p-return` flattens first).  Probe: the 28-row `edge.pl`; guard: the last two rows of `Pl/t/return-copy-01.t` program B, inverse-verified on `551202b` (the s464a commit) where exactly those two rows fail.
- **A GENUINE DUALVAR carries its identity on the BOX.**  `p-return-value`'s container arm tested `(vectorp v)`, and a CL string IS a vector, so `$!` / `Scalar::Util::dualvar` rode out of it intact BY ACCIDENT.  Adding the honest `(not (stringp v))` dropped the numeric half (`Pl/t/errno-01.t`'s four dualvar rows); the arm is now explicit via the runtime's existing `%pcl-dualvar-p` / `%p-dualvar-copy`, in `p-return-value` AND in `%p-leavesub-scalar`.
- **The copy is of the REFERENCE, never the referent** — an object survives with its class and its `==`; only `\f()` and `$_[0] = …` stop writing through.  **Never in place**: `&f;` and `goto &f` SHARE `@_` with the caller.
- **#930 (`:lvalue` subs) is a switch on this macro** — an lvalue sub gets a frame that OMITS the copy, which is what the bare `catch` already yields.  Not implemented here.
- **THE `:lvalue` ACCIDENT IS NOW MEASURED, and it confirms the USER's "one owner" ruling.**  Because PCL returned the box from EVERY sub, an `:lvalue` sub looked like it returned the place.  Removing that accident costs **25 companion rows in three files, all `: lvalue`**: `op/sub_lval.t` 54/47 → **31/64** (23 rows; the file's own `get_lex`/`get_st`/`id`/`id1`/`inc` are all `: lvalue`, lines 34-38), `op/kvaslice.t` 24/14 → **23/15** and `op/kvhslice.t` 20/18 → **19/19** (both `sub foo:lvalue{ %a[@i] }; $_++ foreach foo()`).  Attributed by a base-worktree run of the same four files; spliced ROW BY ROW with the cause.  **#930 restores them** — an `: lvalue` sub gets a frame that OMITS the copy, and `p-sub-frame` is exactly that switch.  The GAIN is `op/sub.t` 53/12 → **54/11**, row 9 `result of shift is copied when returned` ([perl #91844]) — the one row in either suite population that ever saw this bug.
- **The cost bar, and what the instrument can actually resolve.**  Interleaved runtime A/B (this runtime vs the same runtime with `%p-leavesub` short-circuited), best-of-5, five runs.  A CONTROL A/B of the runtime against a byte-identical COPY of itself reports `symref` +14.2 % and `intloop=` −5.0 % with a consistent sign across three runs — so on this box the consistent-sign method has a per-row bias of ±5 % (symref ±15 %) and nothing smaller is interpretable.  Against that floor the only row with a mechanism is **`ovlsub` ≈ −4 %** (two blessed-box mortalcopies per iteration — perl pays the same), and every row the cost bar names (gcdrec, collatz, intloop+=, intloop=, arrhash, symref) is inside the control's own band.  Branch-vs-main absolutes agree: every row within ±3 % except `intloop=` +6 %, which has no sub call in its hot loop.  The rule is INLINE with the raw arm first (a raw return is already a copy: one struct typep + one special read, no call); out of line and list-first it cost 4-9 % on the call-heavy rows.
- Filed: **#987** (eval `{ }` / string eval are frames too — `pp_leaveeval`; four probe rows, `do { }` correctly unmoved) and **#988** (PRE-EXISTING silent-wrong, verified on a base worktree: `p-flatten-args` spreads ONE level where `%p-flatten-list-general` recurses, so `join("|", D1->steps)` prints `ARRAY(0x1)`).
- Record: session-log §464a.

## s463f, part 3 (2026-09-02, Fable) — ROUND 20 COMPLETE: AV (#950 + #980 + #981 + §0.2f) REVIEWED + MERGED, main `fc96b08`; batch legs on the final tree GREEN; AV's three asks RULED; round 21 = the return protocol, solo

- **AV APPROVED as shipped.**  Diff read first: ONE `(declaim (optimize (speed 3)))` at the top of the runtime (the end-of-file `(speed 2) (safety 1) (debug 0)` still governs generated code); `%pcl-def-ext-stub` moved above its first user so pack/unpack stop being the hand-written twin (the `(> speed debug)` local-self-call hazard, #980); `p-magic-cell`'s two closures declared `function` with erroring defaults (248 of the 3179 speed-3 notes were that one missing declaration); `%p-fill-lower-bound` + one guarded `adjust-array` before `p-array-fill`'s push loop (#981).  Rebased on `bb62fc0`; parens balanced; no emission change, no generation bump.  Fable probes vs perl: pack/unpack incl. unpack's `$_` default, six array-fill shapes, a deleted-element copy, a recursing sub redefined through a glob and through a string eval — all SAME, and SAME under `PCL_RAW_ELEMS=0` and `PCL_OPT=none`.  Harness/xs files redefine NO runtime defun (checked, LC_ALL=C), so the local-call hazard has no second victim.
- **Ask 1 (`(speed 3)` for 21k lines) — KEEP.**  The verdict method is sound (six interleaved A/B runs, the SIGN of all six is the verdict, nine rows 1.4–7.2 % faster, no row consistently slower, gcdrec's +11.8 % shown to be load); the one side effect was a real bug class, fixed at its mechanism and guarded by a SOURCE invariant; the cost is +1.1 s per core build, once per runtime edit.  **Standing rule: a runtime function that must be redefinable at run time delegates through SYMBOL-FUNCTION; the policy is never lowered for it.**
- **Ask 2 (interleaved timing series in both bench modes) — the STANDING FORM.**  On a shared box only interleaving makes two series comparable; `BENCH_RT_B=<runtime>` is how a runtime-only change is A/B'd from now on.
- **Ask 3 (coverage rows for a semantically invisible change) — RIGHT BAR, with the label AV gave them.**  A capacity reservation cannot be inverse-guarded; its bar is the A/B measurement plus rows that exercise every ARM of the new predicate, stated in the file as coverage, not guards.  A change with no observable behaviour still gets its arms covered; it never gets a row that claims to be an inverse guard.
- **Batch legs on `fc96b08`**: gate COLD 191 / 6521 (13 pclxs xs rows only); sweep TOTAL 18346 (+0) GATE clean, drops 5 = census; bench rows at their §0.2f values (slices 2.96×, listcopy 0.91×, feread 0.46×, symref 1.30×); companion `--all --quick` 528 files: **two attributed movers spliced ROW BY ROW** — comp/opsubs.t 31/5 → 32/4 = **#962** (bisected: 31/5 on 2065b89, 32/4 on aafb02c; test 2's shifted argument list lines up) and run/runenv_hashseed.t 8/0 → 9/0 = **#939, round 19** (already 9/0 on 2065b89; AT's A/B had named the shape; run/ was in no earlier leg) — plus the three known noise rows (io/open.t's multi-file flip, io/pvbm.t's flapper, uni/variables.t's unstable TIMEOUT count), untouched.
- **ROUND 21 (USER, 2026-09-01): the RETURN PROTOCOL, solo** — #964 (ordinary subs copy a returned scalar variable) + #930's scalar half (`:lvalue` subs return the place), ONE switch at the return site under ONE owner of the return family, never with a perf slot.  Then the correctness pool in this order: #972 (bitwise DISPATCH first) → #966 + #967 + #971 (the pos/substr place family) → #965 in chunks (sub.t first) → #963.  Perf pool after that: #982 (numeric hash key re-stringify, answer the sharing hazard first), #985 (slice-argument list), #986 (settle on a quiet box, ten minutes), #983 (the note census).
- Record: session-log §463f part 3.
...
- **Ask 1 (#960(a) declined) — RIGHT CALL.**  A death that fires where perl runs is worse than the four blessed rows it would move; #972 (nomethod, bitwise dispatch, `""`-from-`0+`) goes in the round-21 correctness pool AFTER the return-protocol task (below), the bitwise DISPATCH half first because it is additive.
- **Ask 2 (the 4-arg substr keeps `when`, not the rule-12 death) — RATIFIED as a reading of the s329 boundary**: a write that perl itself DISCARDS (a temporary in loose lvalue context) loses no value the program consumes, so silence there is perl's own answer; the loudness for a MISSED place lives in the emitter (`_lvalue_target_form`), not the runtime.  The read-only LITERAL case is #936's mark, not a runtime test.
- **Ask 3 (`return X while 0` residue) — STOP THERE.**  perl's `while 0` and `until 1` answer differently for the same untaken loop; a rule that matches one optree artifact would be silent-wrong on its twin.  Recorded in #920, no not-supported entry (it is not a supported-vs-not question).
- **Ask 4 (two generation bumps in one session) — CORRECT, and the standing rule**: every emission-changing COMMIT bumps, because the module cache is keyed by generation and two emissions under one string would reuse each other's cached transpiles; the reviewer adopts the last.  Artifacts regenerated at each bump, as AU did.
...
- **A Fable probe row can be INVALID PERL — run the perl side alone and read its stderr before calling a DIFF a finding**: `substr("lit",0,1) =~ s///` is a compile-time "Can't modify constant item" in perl, and `declared_sub / 3, "\n"` is perl's "Search pattern not terminated" (a bareword sub followed by `/` starts a MATCH in perl too — which is exactly the #940 licence).  Two of three probe DIFFs were mine.
- **#939 moved one shape from an ACCIDENTAL death to the #936 silent write**: `for my $e ("const") { substr($e,0,1) =~ s/c/C/ }` died on 12eacb5 (no place — #911's death for the wrong reason) and writes silently on 6e6f191 (the foreach-alias box has no read-only mark), which is the consistent state #936 sizes; no baseline row depended on the accident.  Appended to #936.
- **The re/ companion TAIL AT could not run (the one unverified bar) had ONE change-attributed mover, and it is an improvement with an honest new fail**: re/subst.t 200/72 → 205/67 — `%p-write-match-target` STOREs through a TIE instead of clobbering the proxy with the result string (rows 203/205/207/209/210/211 pass honestly), and row 212 had passed ONLY because the clobber had already destroyed the tie before `$s = \1` ran; with the tie alive it hits PRE-EXISTING **#963** (a reference CONSTRUCTOR assigned to a tied scalar reaches STORE dereferenced — `\1`/`\$v`/`[..]`/`{..}` all lose their ref-ness; the same value through a variable is right; identical on 12eacb5 by probe).  Snapshot spliced ROW BY ROW with the cause.  re/charset.t 2776/2776 → 2776/2775 is the s455b "perennial churn" file producing one row fewer, pass count unchanged — not a mover, snapshot untouched.
- **#962 duplicates #459** (a failed capture-less m// in LIST context yields one false element in an ARGUMENT list where perl yields the empty list; the list-ASSIGNMENT path is right) — AU closes both with one fix.
...
- **A raw-slot eager freeze DECLINES at runtime; it does not die (#890,
  `docs/raw-numeric-verdict.md` §"Checked coercion — the runtime DECLINE",
  which SUPERSEDES the s286 "die loud / never weaken this check" section).**
  The B-regime licence is a TEXTUAL scan of the transpiled file for `use
  overload`, so it cannot see an overloaded object arriving from a module —
  `version->new('5.30.0')` in a numeric-only slot was FATAL.  No classifier
  fix closes the class (the regime fires exactly when the write shape is
  UNPROVEN, so the object can come from a hash element, a sub return or a
  string eval too; widening the scan to loaded modules is both incomplete and
  a far bigger de-optimization).  **What the decline stores is
  `(p-box-init V)` — the general form's own representation** — so the
  optimized emission runs identically to `PCL_OPT=none` BY CONSTRUCTION, not
  by a case analysis of consumers.  `ir-spec` §2.2's "a raw slot never holds
  a box" invariant now names this exception.
- **The freeze check asks "does this class overload ANY operator", not
  `""`/`0+`.**  The narrow question was a SILENT WRONG: a `<=>`-only class
  gets every numeric comparison autogenerated from it, and the frozen slot
  compared the ref's stable ID (`CmpOnly->new(42) > 10` answered 0, perl 1).  ONE
  reading, `%p-class-overloaded-p`, shared with `overload::Overloaded`
  (`p-overloaded`), and it walks `@ISA` because **perl's own
  `overload::Overloaded` is inheritance-aware** (probed 5.40.3).
- **`perl-increment` must numify the BOX, not the unboxed payload** — `unbox`
  throws the class away before `to-number` can dispatch `0+`, which gave one
  program two answers (`$o++;$o++` → 3 under `PCL_OPT=none`, 7 by default)
  the moment #890 let values survive the write.  Residue **#900**: perl
  AUTOGENERATES `++` from a `+` handler and keeps an OBJECT; PCL keeps a
  number on both paths.
- **A guard added in front of a fast path must be measured against the path
  it precedes — and so must a guard REMOVED from one.**  Collapsing the
  freeze check to a single inline `p-box-p` test took **`collatz` 0.40× →
  0.25× perl** (0.7704 → 0.4859 s), because it has two B-regime writes per
  iteration.  The win was predicted verbatim by the s458ak DECIDED note and
  went uncollected for two rounds.
- **PRE-SIZING `p-flatten-args`'s result is a NET LOSS — measured, rejected,
  do not retry (#883).**  Its `(make-array (length args) …)` really is the
  count of ARGUMENTS where a count of ELEMENTS is meant, so a whole-array
  argument grows from 2 by doubling — but the O(1) pre-pass taxes the
  overwhelmingly common SCALAR-argument call: `fib(27)x` +3.8 %, `gcdrec`
  +2.3 %, against `feread2` −2.1 % (A/B best-of-5).  The numbers live in a
  comment above the function.  Corollary killed with it: comparing two
  array SIZES does NOT discriminate growth-copying — both grow from
  `(length args)`.
- **#883's arm is CONFIRMED but SMALL: a multi-array foreach list is already
  in the all-boxed world** (`feread2` measures identically with
  `PCL_RAW_ELEMS` on and off, where `feread` gains 31 %) — so
  `p-flatten-args` really does promote before `p-foreach-raw` sees a slot.
  The prize is only the raw-vs-boxed READ difference, **~15 % of the row,
  not ARM A's 40 %**, because ~⅔ of that row is the flattener's own
  per-element work.  Not worth an emission boat for 2 of 32 arm sites.
- **E2c′ (#862) INVERTS the safety polarity of `writes_args` for arrays —
  Fable ruling wanted before it ships.**  Today a whole-array argument
  promotes unconditionally, so a wrong `false` still writes through; E2c′
  makes it a silent lost write (the boxed-aggregates design's own §4.3(b)
  "unaffordable failure mode", merely gated on a fact).  #189 already trusts
  the fact permissively for SCALAR args, so it is no new KIND of trust, but
  the blast radius widens.  Population measured: 91.6 % of subs eligible,
  only 7.5 % of call sites ever pass a whole array.  Structural note that
  shrinks the change: **`p-flatten-args` runs in the CALLEE**, so the fact is
  `_sub_writes_args($sub)` at the sub being lowered — not call-site plumbing.
- **The `symref` cost is NAME-STRING MANGLING, not the package lookup and not
  the cell read (#812, sb-sprof).**  `%p-symref-symbol` is 87 % of the row;
  the lookup proper is ~11 % and `unbox` is 0.5 %, while ~76 % is splitting
  `main::g` on `::`, case-inverting, upcasing and concatenating — per access,
  for a compile-time-constant string.  **A name → SYMBOL memo has no #582
  invalidation blocker** (a symbol's identity for a (name, current-package)
  pair is stable; a later glob assignment changes what it HOLDS, which the
  cell read picks up); #582 gates only a memo on the CELL.
...
- **#561 computed magics DESIGNED** — `docs/computed-magic-design-s449.md`
  (the authoritative text): scalar half = canonical MAGIC BOXES (`|$!|`,
  `|$^E|`) beside `|$.|`, the accessor IS the getter, emission unchanged,
  ZERO glob-path changes (`p-glob-copy` already shares slot boxes — the bug
  was only that `|$!|` was unbound); `%!` half = a REAL hash whose VALUES
  are `p-magic-cell` boxes over a new `*p-errno-table*` (POSIX names via
  sb-posix at boot), `%p-hash-unbox-elem` gains the magic arm
  `p-aref-unbox-elem` already has; NOT an `%ENV`-marker.  `lib/Errno.pm`
  shim = follow-on task (rule 9a).  Round 8, cl/-only.
- **#602 `*A = *B` clear-then-copy: shape (a) APPROVED** (design doc §5) —
  glob-to-glob arm only, clear only destination-bound slots, empties =
  `p-glob-undef-name`'s; shape (b) one-GV stays with the boxed-aggregate
  family.  Same round-8 session as #561 (pat.t:1715 needs both).
- **#551 numeric symbolic-ref: PARKED behind the boxed-aggregates design**
  (post-v0.1) — the unblocking change (container reads keep the scalar-ref
  wrapper) is a representation change in exactly that family; string
  spelling stays correct, canary row stands, s335 "no new box slot" stands,
  never retry the name-exists heuristic.  Ruling in task #551 + ir-spec §8.
- **#541 false-conditioned `local`: fix shape APPROVED** — conditional
  SAVE/RESTORE, never a conditional value: `p-local-cell-if` for the
  ordinary-global half, `progv` with runtime-computed lists for the
  defvar half; RHS of `local X = RHS if COND` must not evaluate when false;
  bar leg 1 = unconditional spellings BYTE-IDENTICAL in corpus-diff.
  Round 8.  Ruling in task #541.
- **#542 stdio buffering: APPROVED as its OWN session** — ONE policy
  function shared by boot + every rebuild site: STDOUT line-buffered iff
  isatty(1) else `:full`, STDERR probe-and-match perl, `$|` keeps
  overriding; REQUIRED: every normal exit path incl. `p-die`→exit flushes
  (or block buffering turns aborts into invisible LOST rows); full sweep +
  io/ leg price accepted, rows edited by hand.  Ruling in task #542.
- **#560 disk-cached prototype facts: APPROVED** — PER-MODULE entries under
  `~/.pcl-cache/proto/` (a hit prunes the recursive walk); key = path +
  mtime + size + generation, repeated inside the JSON (mismatch = ignored +
  rebuilt); value = the `PCL_PROTO_ORACLE` record shape; atomic
  temp+rename; in-process memo stays as L1; corpus-diff IDENTICAL is the
  first bar leg.  The budget stays REJECTED.  Opus builds.  Ruling in #560.
- **#592's fcntl shape SETTLED** (was round 7's entry condition):
  sb-posix:fcntl f-setfd/fd-cloexec, perl's `> $^F` threshold,
  parent-end-only for fork-pipes, builtin `fcntl` (piece b) first.
  Assigned to round-7 agent Q.
- **ROUND 7 LAUNCHED off `c8a8311`** — three Opus agents in worktrees:
  Q = io residue #591+#621+#590+#592 (s449q, IDs 630–639, v2-340 only if
  emission moves), R = scoping #593+#594+#530+#532 (s449r, IDs 640–649,
  v2-330), S = parser fillers #563+#564+#550 (+#620 bonus) (s449s, IDs
  650–659, v2-335).  Standing rules as rounds 5–6 (no sweeps, no
  review-docs, rebase before final gate; Fable merge review owes ONE sweep
  + io/ leg (Q) + gate-SET for R/S where a refusal/drop stops firing).
- **ROUND 7 COMPLETE (same session): ALL THREE MERGED** (Q `cf49f95` → R
  `f0dda81` → S `46325f9`), every diff read, ~35 own probes vs perl 5.40.3
  per the three reviews, guard files re-run (Q 31 rows, R 72, S 83).
  Generation **v2-355** (S's post-rebase string adopted, artifacts
  regenerated by S on exactly the merged tree — the round-5 K precedent).
  **ONE review fix** (`2d0da65`): `_repair_glob_pattern_cascade` runs to
  FIXPOINT — one derail HIDES the next (two glob statements with no `/`
  between them: the first's runaway Regexp swallows the second's own `/`,
  so a single pass fixed glob one and dropped glob two); guard row 24 in
  readline-ternary-01.t; corpus-diff identical, no gen bump.
- **Batch legs, all on the merged tree**: full sweep ×2 GATE clean, **TOTAL
  18321 → 18325 (+4)** = method.t's four `local *1 = sub {123}` rows
  (#564), verified SERIALLY on main AND a c8a8311 worktree; the parallel
  run's 7 unstable + 10 unverified rows all re-run serially, byte-identical
  to baseline (load noise); pass-baseline method.t row edited by hand
  (`$pvlv_glob->method` verified pre-existing on base, unrecorded).  Drops
  5 = census, child drops 9/6 = the s444 census.  **Gate-SET scan vs
  c8a8311, both populations, 638×2: byte-IDENTICAL** (R's widened detector
  + narrowed exit and S's five widenings move no verdict).  **Companion
  io/+op/+uni/ `--quick`: six REAL movers, every one attributed by
  base-worktree A/B and spliced with cause** — io/open.t 136/25→141/24
  (R #593+#594), io/pipe.t 8/0→18/14 (Q #590: the Broken-pipe abort is now
  perl's false print — ten more rows run), op/method.t 99/26→103/22
  (S #564), op/gv.t 130/61→131/60 + uni/gv.t 55/33→56/32 (S #564's
  declared-subs "static twin" — the constant-subroutine rows; still failing
  at f0dda81, passing at 46325f9); pvbm (12th sighting) + uni/variables
  (TIMEOUT giant) left per precedent.
- **uni/parser.t 26/32 → 18/5 is NOT round 7 — a STALE ROW hiding a ROUND-5
  REGRESSION, task #663**: uni/ had not been in a companion leg since s444,
  and the glob-ELEMENT read `${*$a{SCALAR}}` dies "Can't use an undefined
  value as a symbol reference" where perl reads the slot (5-line ASCII
  repro in the task; the discriminating pair isolates the READ, not the
  assignments).  Bisected across round 5's merge order: good at c42cc8a and
  at M/L/J/I's tips, broken at f95fa97 — **agent K's #463 glob-value range
  owns it** (35 rows of coverage lost).  LESSON: a companion leg that skips
  a directory leaves its rows unrefreshable — rounds touching glob/name
  machinery should include uni/.
- **Fable filings from the review probes**: #660 (slurp-at-EOF first read
  is a DEFINED "" in perl, undef in PCL — pre-existing, plain empty file),
  #661 (fcntl on a never-opened lexical DIES in perl, PCL answers
  undef+EBADF — the #529 "two negative answers" lesson; survey the family
  first), #662 (`f % 3` after a declared sub is perl's hash %3, PCL's
  modulo — outside #550's punct set, pre-existing), #663 (above).  Q's asks
  ruled: refused-write $! = EBADF accepted (errno-fidelity stance);
  `>&`-of-O_RDWR bidirectional watched on the io/ leg (clean); in-memory
  `<` read-only approved (no blessed stance existed — probed identical to
  perl).
