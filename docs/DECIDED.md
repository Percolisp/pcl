...
- **"OK" never meant "the plan was produced" — the SHORTFALL is now an instrument** (I2, `baselines/row-shortfall.tsv`, SHARED by both runners through `tools/lib/PCLShortfall.pm`, keyed `perl-tests/<name>` / `t/<rel>` like the drop census, same contract: rows leave BY EDIT, MORE fails the run).  **459,454 rows over 260 files; 376,788 caused, 86,126 UNEXPLAINED = the audit's queue**, printed every run.  The sweep's oracle is the file's own `1..N` plan, the companion's is real perl's run; the sweep also splits the two halves — of its 12,257, **222 produced no TAP at all** (the PARTIAL class) and **12,035 were SKIPPED** (pack.t 8,997, lc.t 2,577).  A GROWN shortfall joins the #215 serial re-run.  Runbook §4c.
