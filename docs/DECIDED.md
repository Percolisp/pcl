...
- **#1022(a)'s MEASURED COST, left for Fable**: `perl-tests/loopctl.t` OK 63/1 → PARTIAL 40/0 (23 passing rows), `op/loopctl.t` −23, `op/rt119311.t` −3 (six `{ foo(sub { …; last }) }` sites).  The s329 archetype's own cost shape, incurred knowingly; the baselines are NOT edited and the commit is separate so it can be held.  Half (b) (a catch frame per loop ENTRY, measure-first) is what removes it.
