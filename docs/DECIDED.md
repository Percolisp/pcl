...
## s470bt (2026-09-06, Opus) — B6 the IR CONFORMANCE CORPUS (`tools/ir-conform`, 347 cases) and B7 the two target notes as tables

- **"A BACKEND for PCL's IR is done when it answers every case the way real perl does."**  `tools/ir-conform` is `tools/pcl-conform`'s sentence one level up: `ir-conform/cases/NNN-<topic>.pl` (347 programs, 22 topics) + `.expected` (perl 5.40.3's stdout + exit code) + `.ir` (the DATA form, `pl2cl --emit-sexp`) + optional `.rules`.  `ir-conform/README.md` is the backend author's page.
...
- **Path dependence is caught, not blessed**: every candidate was run TWICE under TWO DIFFERENT FILE NAMES and its normalised output compared.  Harvest: 752 probe files under `~/pcl-agent-scratch/{s469,s470}` → 731 distinct bodies → 347 kept (self-contained, deterministic, produces stdout).
...
- **MEASURED**: PCL 289 pass / 0 fail / **58 known** / 0 stale (65 s at `--jobs 2`, load ~5); perl 347/347 (19 s).
...
- **Gate row `Pl/t/ir-conform-01.t` (8 rows, 6.3 s) = the structural invariants over the WHOLE corpus + a 20-case SAMPLE.**  The full corpus is a WHAT-TO-RUN-WHEN entry (after a `cl/` runtime or `Pl/` emission change), not a gate row — wall time is the metric.
