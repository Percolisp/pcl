...
| 3 | **Real lambda lists (fixed *and* optional arity).** When `@_` is only consumed by a leading parameter binding and never otherwise touched (no later `@_`, `shift`, `goto &sub`, `$_[n]`, `wantarray`-on-`@_`), emit real CL params instead of `(&rest %_args)` + `(p-list-= …)`. **Key constraint — a Perl sub is callable with ANY arity, including zero.** `f()`, `f(1)`, `f(1,2,3)` must *all* work whatever the binding lists: missing params read as `undef`, extra args are ignored. So the CL lambda list must be `&optional`/`&rest` and must NEVER use required positionals (a required-param `defun` would arity-error the instant the sub is called with too few args — which is legal Perl). **Three shapes, one mechanism:** ① `my ($a,$b)=@_` (untouched after) → `(&optional ($a (p-undef)) ($b (p-undef)) &rest --ignore--)` — `&optional` `*p-undef*` defaults cover the short call, the swallow-`&rest` covers the long call. ② `my ($a,@rest)=@_` → `(&optional ($a (p-undef)) &rest @rest)` (the slurpy array maps straight to `&rest`). ③ a real signature `sub f($a,$b=5){…}` → `(&optional ($a) ($b 5))` (defaults become lambda-list init-forms). A genuinely fixed list (exact arity, arity-error on mismatch) is only emitted when a signature has no defaults/slurpy AND prototype/strict-arity is in force. **Caller-side caveat (known, deferred):** the lambda-list only helps a call site whose argument *arity is static* — `foo($x,$y)` can compile to a direct `(pl-foo $x $y)`. A call whose args **flatten at runtime** — `foo(1..4)`, `foo(@a)`, `foo(%h)`, `foo(@a,$x)` — does not know its count at compile time, so it must `(apply #'pl-foo (p-flatten-args …))`. `apply` *does* spread correctly into the `&optional`/`&rest` lambda list (so it stays correct), but it re-introduces the flattened list there, so the rest-cons saving is realised only on the static-arity call sites, not the flattening ones. The callee lambda-list is still worth it (most hot calls are static-arity), but the win is call-site-dependent, not universal. | every `(&rest %_args)` + `(p-list-= (vector …) @_)`; signatures still build a `%_args` rest then destructure | BlockAnalyzer proves `@_` untouched after the leading bind; the bind is a single `my (LIST)=@_` / `my $x=shift` chain / a signature; **plus** a call-site check (static arg arity → direct call, else `apply`) | sub codegen + BlockAnalyzer flag + funcall codegen |
