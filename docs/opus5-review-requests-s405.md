...
| s405c | **#277** — the installer (release phase 1) |
| s405d | docs: session log, DECIDED, ir-spec §6.3, the snapshot/baseline edits, this file |
...
most real code uses), **§6** (the installer's shape) and **§7**'s asks.
...
**#348's cloexec half is unblocked** — the remaining blocker for the
`which_perl` switch is #347 alone.  The six rows that still differ are the `$^F`
/ fd-inheritance feature, filed as #359 (§5.1).
...
| measurement | result |
|---|---|
| Gate `tools/prove-core` | **146 files / 5341 rows**, failures exactly the 13 pclxs xs rows |
...
| Full sweep (runtime changed → non-optional) | **GATE clean**, 0 new / 0 fixed, **TOTAL 18517 = baseline**, DROPS census 12 = 12 |
