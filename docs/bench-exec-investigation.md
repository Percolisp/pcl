...
3. **`raw-numeric` / `raw-string` verdicts** — use-proven eager coercion for
   ...
   `(setf $n (%pcl-to-number …))`, real host number in the slot; symmetric
   string verdict via `to-string`, sound for refs because PCL ref identity is
   a stable counter ID, not an address).  Full design incl. the
   boolean-context asymmetry, flag semantics (no-overload corpus scan only),
   and warning-count caveat: `docs/raw-numeric-verdict.md`.  Kills tax 2
   generally (not just for range loops).
