...
| 5 | **s473b** — **ALL FOUR MEMBERS SHIPPED s473b/s477** (gen v2-1030; #1151 the nested `=` writes THROUGH the slot; #1241 = #1150 the READ path vivifies every intermediate via the `p-viv-container` marker + the existing chain walker, closing #1456 and the 7 ir-conform autoviv rows; #1152 an element compound assign evaluates container and KEY once, +0.55 % on its own bench row; #1010 a SLICE vivifies where its CONSUMER aliases it, ONE rule and five consumers incl. the `p-viv-slice` marker.  Sweep 0/0/0 TOTAL 18673, for.t 131 → 130 by EDIT because perl's own RT #2166 TODO row now fails as it does under perl.  Filed #1349–#1353) | the nested-element family: #1241 = #1150 (read vivifies), #1151, #1152, #1010 — 7 rows; bench-gated | v2-880 | a slot |
