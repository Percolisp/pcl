...
| **t5e** | **DONE (s473t5e, 2026-09-13).**  `re/` was **9,836 blessed fail rows over 34 files with CAUSES 0** plus **352,093 rows short over 80 files, 19,409 UNEXPLAINED in 22**.  Now **3,940 fail rows, ALL 3,940 caused**, and the shortfall's UNEXPLAINED half is **0 files / 0 rows**.  THREE FIXES, all three rule-11/rule-12 shaped: **FIX 1** (`[:^class:]` and `[:ascii:]` had no arm in the POSIX table, so a class AGREED with its complement — ONE range table, the negated spelling derived; `reg_posixcc.t` 1544/7646 → **1544/1016**, 6,630 rows); **FIX 2 = #1711** (the transpile server's STRICT `:encoding(utf-8)` substituted the `\x{...}` escape text for perl's non-characters and desynchronised its own char-counted protocol, so `eval "\x{10FFFF}"` died `end of file on string-input-stream` — `:utf8`, the layer `pl2cl`'s own STDOUT already uses; the three `do regexp.t` drivers advance from re_tests line 1859 to line 2030, **+513 rows**); **FIX 3 = #1715** (the INLINE `(?a:…)` group was a cl-ppcre SYNTAX ERROR, not "ignored" as `not-supported.md` claimed; `charset.t` 2776/2776 → **5224/328**, **+2,448 passing rows**).  **The re_tests TIMEOUT question is ANSWERED and it is HANG (#1714)**: `regexp_qr_embed.t` at `--timeout 1200` gives 907 rows against 905 at 90 s, and re_tests lines 906–926 are `.X(.+)+X` at 2^n (PCL n=30 = 64 s, perl 0 s) — no allowance, and do NOT `--quick`-skip them.  Filed #1712 #1713 #1714 #1716 #1717 #1718 **#1719 (a zero-length match does not advance `pos`, so `while ($s =~ /(\w*)/g)` NEVER TERMINATES)** #1720 #1721 #1723 #1724 #1725.  Two STALE attributions corrected: `reg_mesg.t`'s 3,348 are #1590 (`display_rx` is in PERL's test.pl) and the six `fold_grind_*` are XS `Encode` / no-locales, not #1036.  Bars: gate **241 files / 8214 rows** (only the 13 pclxs xs rows), sweep **GATE clean / TOTAL 18675 (+0) / drops 5 = census**, corpus-diff **IDENTICAL over 111** (no `Pl/*.pm` change) → **no generation bump**, ir-conform 321/0/24/0, ir-host-leak 31 = main's.  **Leaves for a later round:** #1713 (1,016 rows, needs an in-class scanner), #1719 (the `//g` zero-width rule — a HANG in ordinary code, the biggest single prize left), #1720 (pat.t's 898), #1590 (reg_mesg.t's 3,348 + the heap ceiling), #1714 (registered, engine). | 34 files, DONE |
