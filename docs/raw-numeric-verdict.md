...
| reference | identity is a **stable counter ID** (see above), so frozen ≡ live for `==`, `eq`, hash-keying; only `use overload` (per-use code) can observe the freeze | **the runtime decline** (s460/#890; the flag/corpus scan is blind to a module's overloads) |
...
conditions below are exactly right about *when* an eager coercion is
unsound; what they were wrong about is the remedy.  Dying assumes the miss
is a compiler bug the author can fix — but the licence is a **textual scan
of the transpiled file** for `use overload`, so it is blind by construction
to an overloaded object that arrives from a MODULE, from a container, or
from a string `eval`, and no scan can ever close that.  The measured cost
was whole programs: `use version (); my $w = version->new('5.30.0'); $] >= $w`
was fatal where perl just runs (#890).

The shipped shape:

- `%pcl-raw-freeze-unsafe-p` answers the two conditions.  On true, the
  coercer stores **`(p-box-init V)`** — a fresh box under box-set's own
  assignment rules, i.e. exactly the value the general-form (boxed)
  compiler would have stored for the same write.  So the raw slot holds the
  *general form's own representation*, and because every consumer of a raw
  slot is polymorphic (`setf`, the `-raw` compound twins, `prog1`, and
  reads, which go to ordinary `p-*` ops), the optimized emission runs
  identically to `PCL_OPT=none` **by construction** rather than by a case
  analysis of consumers.  This is the registry's own contract.
- The decline is **silent and value-correct**, not an announcement: it
  produces the answer perl produces.  It is also *narrow* — it fires only
  for the values that actually carry per-use behavior, where a compile-time
  fix (widening the scan to the modules a file loads) would have
  de-optimized every numeric lexical in any file that touches an
  overload-carrying module.
- **Condition 1 is "the class overloads ANY operator", not `""`/`0+`.**
  Asking only about the two conversion handlers was itself a silent wrong:
  a class overloading `<=>` alone gets every numeric comparison
  autogenerated from it, and a frozen slot compared the ref's stable ID —
  `CmpOnly->new(42) > 10` answered 0 where perl answers 1.  The question
  "does this class overload anything" now has ONE reading,
  `%p-class-overloaded-p`, shared with `overload::Overloaded`
  (`p-overloaded`), and it walks `@ISA`, because perl's own
  `overload::Overloaded` is inheritance-aware (probed 5.40.3).
- Guards: the `#890` block in `Pl/t/raw-verdict-01.t` — including the
  negatives that keep the decline from widening (a plain blessed ref still
  freezes to its stable ID) and a `PCL_OPT=none` equivalence row.
- Cost: the restructure made the check *cheaper*.  A raw operand now settles
  on one inline `p-box-p` test where it used to make two out-of-line
  `p-find-overload` calls plus `%pcl-dualvar-p` — the `collatz` bench row
  (two B-regime writes per iteration) went 0.770 s → 0.486 s, 0.40× perl →
  **0.25×**.

### The original s286 rule, kept for its reasoning
...
> **Superseded s460** — see the section above.  The two conditions stand;
> the remedy is a decline, not a die, because the classifier *cannot* be
> fixed to see a module's overloads and re-boxing is not something the
> program's author can do at the point the die fires.
