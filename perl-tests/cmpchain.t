...
my @cheqop = qw(== != eq ne);
my @nceqop = qw(<=> cmp); # ~~ (smart match) not supported by PCL
my @chrelop = qw(< > <= >= lt gt le ge);
my @ncrelop = qw(isa);

# PCL assumes valid Perl input and does not need to detect or reject invalid Perl.
# These tests verify that chaining non-associative operators (<=> cmp isa) produces
# a compile-time error in Perl.  Since PCL only transpiles functioning Perl code,
# this behaviour is out of scope.
if (0) {
...
} # end if (0) — non-associative tests skipped
